...
|------|----------|------------|
| `layout_pbt::layout_invariants_hold_for_random_scenarios` | Mode switches, action replay, end-state equivalence | Steps 1, 4 |
| `layout_pbt::block_ref_inside_tree_item_has_nonzero_height` | Expand state, tree item layout | Step 4 |
| `layout_pbt::streaming_collection_data_arrival` | MutableVec data delivery, subscribe_inner_collections | Steps 2, 3 |
...
| Layout proptest | `cargo nextest run -p holon-gpui --test layout_pbt` |
...
4. **Step 4 after PBT verification** — run layout_pbt with caches removed. If tests pass, caches are redundant. If not, the failing test shows which edge case still needs them.
