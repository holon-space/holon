...
### Integration Tests
- `layout_pbt::layout_invariants_hold_for_random_scenarios` — mode switches, entity cache preservation
- `layout_pbt::block_ref_inside_tree_item_has_nonzero_height` — expand state
- `layout_pbt::streaming_collection_data_arrival` — props watchers, InterpretFn
...
cargo nextest run -p holon-gpui --test layout_pbt 2>&1 | tee /tmp/layout-pbt.log
...
6. **Checkpoint**: Run all tests (especially layout_pbt mode switch scenarios)
