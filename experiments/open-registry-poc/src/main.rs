//! Open-registry transition design — runnable PoC (unified-architecture
//! revision
...
use crate::core::build_alphabet;
use crate::core::cap;
use crate::core::check_invariants;
use crate::core::discovered_transitions;
...
fn main() {
    ...
    println!(
        "  Toggle+Editor  hosts {:?}",
        full.cap_set().names(&all_caps())
    );
    println!("    -> alphabet: {:?}", names_in_alphabet(&seed, &full));
    println!(
        "  Editor only    hosts {:?}",
        no_toggle.cap_set().names(&all_caps())
    );
    println!(
        "    -> alphabet: {:?}",
        names_in_alphabet(&seed, &no_toggle)
    );
    ...
    for step in 0..8 {
        ...
        transition
            .preconditions(&ref_state)
            .expect("generator emits only applicable transitions");
        println!(
            "  step {step}: {:<14} {transition:?}",
    ...
    println!(
        "  cloned {}-step sequence; clone is byte-identical ✓",
        sequence.len()
    );
    ...
    println!(
        "  deserialized {} steps via typetag; replay reproduced final state ✓",
        replayed.len()
    );
...
mod tests {
    ...
    fn typetag_roundtrip_preserves_behaviour() {
        let original: Vec<Box<dyn Transition>> = vec![
            Box::new(transitions::split::SplitBlock {
                target: 1,
                new_id: 4,
            }),
    ...
    fn cap_gate_drops_unwired_subsystem() {
        ...
        assert!(
            !names.contains(&"ToggleState"),
            "Toggle-gated variant leaked"
        );
    }
