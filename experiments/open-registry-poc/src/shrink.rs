...
//!
//! The active *optional-subsystem set* is **test input proptest can shrink**,
//! so a failing case auto-minimizes to the minimal `(set of subsystems,
//! transition sequence)` that still reproduces. The axes are REAL components
//! (§8.7: never fixtures) — `build_sut` wires a real
//! `ToggleStore`/`EditorComponent` per present `Subsystem` over the always-on
//! `BlockStore`.
//!
//! Bugs are planted as **wrong *reference* data** (the components stay
//! correct), so the differential invariant fires only when its subsystem is
//! wired:
//!   - `ToggleBug` → `inv-toggle-match-ref` fails iff `Toggle` wired
//!     (seed-time).
...
use crate::core::RefState;
use crate::core::Subsystem;
use crate::core::Transition;
use crate::core::check_invariants;
use crate::transitions::split::SplitBlock;
use crate::transitions::toggle::ToggleState;
use crate::transitions::typechar::TypeChar;
...
impl Op {
    fn realize(&self, ref_state: &RefState) -> Box<dyn Transition> {
        match self {
            Op::Split => Box::new(SplitBlock {
                target: 1,
                new_id: ref_state.next_id,
            }),
...
/// The reference, with the plant's wrong data baked in (components stay
/// correct).
...
/// Run `(active, ops)` with cap-gated precondition replay + the behavioral
/// plant. `Err` ⇒ a wired invariant caught the planted reference divergence.
...
/// Drive proptest until the planted bug reproduces, then return the
/// **minimized** `(config, sequence)`. Deterministic RNG ⇒ a stable, assertable
/// result.
...
mod tests {
    ...
    /// Deterministic causal structure (no shrinking) — the robust §8.7
    /// evidence.
    #[test]
    ...
    /// proptest shrinking isolates the causal subsystem set + shortest
    /// sequence.
    #[test]
    fn shrinking_isolates_the_causal_subset() {
        let (cfg, ops) = minimize(Plant::ToggleBug);
        assert!(
            cfg.contains(&Subsystem::Toggle),
            "Toggle is causal; got {cfg:?}"
        );
        assert!(
            ops.is_empty(),
            "toggle bug needs no transition; got {ops:?}"
        );

        let (cfg, ops) = minimize(Plant::EditorBug);
        assert!(
            cfg.contains(&Subsystem::Editor),
            "Editor is causal; got {cfg:?}"
        );
        assert!(
        ...
        assert!(
            ops.is_empty(),
            "blocktree bug needs no transition; got {ops:?}"
        );
    }
}
