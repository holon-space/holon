...
//! - `ToggleStore` (optional axis `Toggle`) → `SutToggleWrite` + `SutToggleRead`
//! - `EditorComponent` (optional axis `Editor`) → `SutEditorWrite` + `SutEditorRead`
//!
...
use crate::core::{
    CapMap, CapProvider, Subsystem, SutBlockRead, SutBlockTreeWrite, SutEditorRead, SutEditorWrite,
    SutToggleRead, SutToggleWrite,
};
...
impl BlockStore {
    pub fn seeded(ids: &[u64]) -> Arc<Self> {
        Arc::new(BlockStore { blocks: Mutex::new(ids.to_vec()) })
    }
}
