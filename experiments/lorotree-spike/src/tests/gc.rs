...
//! After extracting a subtree into a shared tree, the parent tree can be
//! compacted via shallow_snapshot to reclaim space from the extracted content's
//! history.
...
fn gc_parent_tree_after_extraction() {
    ...
    // Replace mount node (simplified: just insert metadata on root indicating
    // mount)
