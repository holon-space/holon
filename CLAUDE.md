...
- Every bug discovered OUTSIDE an automated test (dogfooding, agent exploration, user report) MUST be triaged with the `bug-gap-triage` skill (.claude/skills/bug-gap-triage/SKILL.md) and recorded as ONE new file under docs/Testing/bugfunnel/entries/ before/alongside the fix. Latency above the SLO (p95 interaction→projection-visible < 200ms) counts as such a bug. Totals are derived (`python3 scripts/bugfunnel.py counts`) — never hand-maintained.
