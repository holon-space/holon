...
/// Tests that X25519 Diffie-Hellman works when using keys converted from
/// Ed25519.
...
fn ed25519_to_x25519_dh() {
    ...
    // Compare the scalar bytes to the first 32 bytes of SHA-512(secret_key). We
    // have to clamp and reduce the SHA-512 output because that's what the spec
    // does before using the scalars for anything.
    ...
    // Test the claim made in the comments of SigningKey::to_scalar_bytes, i.e.,
    // that the resulting scalar is a valid private key for the x25519 pubkey
    // represented by `sk.verifying_key().to_montgomery()`
    assert_eq!(
