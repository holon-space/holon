...
mod vectors {
    ...
    // Computes the prehashed or non-prehashed challenge, depending on whether
    // context is given
    ...
    // Tests that verify_strict() rejects small-order pubkeys. We test this by
    // explicitly constructing a pubkey-signature pair that verifies with
    // respect to two distinct messages. This should be accepted by verify(),
    ...
    fn repudiation() {
        ...
        // Now check that the sigs fail under verify_strict. This is because
        // verify_strict rejects small order pubkeys.
    ...
    // verify_prehashed_strict. See comments above for a description of what's
    // happening.
    #[cfg(feature = "digest")]
    #[test]
...
mod integrations {
    ...
    fn ed25519ph_sign_verify() {
        ...
        // ugh… there's no `impl Copy for Sha512`… i hope we can all agree these are the
        // same hashes
    ...
    fn montgomery_and_edwards_conversion() {
        ...
        // Check that modulo sign, to_montgomery().to_edwards() returns the original
        // point
