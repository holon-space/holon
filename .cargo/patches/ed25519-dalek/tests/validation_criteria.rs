...
/// Each variant describes a specific edge case that can occur in an Ed25519
/// signature. Refer to the test vector [README][] for more info.
///
...
/// The test vector struct from [CCTV][]. `sig` may or may not be a valid
/// signature of `msg` with respect to `pubkey`, depending on the verification
/// function's validation criteria. `flags` describes all the edge cases which
/// this test vector falls into.
///
...
/// Tests that the verify() and verify_strict() functions succeed only on test
/// cases whose flags (i.e., edge cases it falls into) are a subset of
/// VERIFY_ALLOWED_EDGECASES and VERIFY_STRICT_ALLOWED_EDGECASES, respectively
#[test]
fn check_validation_criteria() {
    ...
    {
        // If all the verify-permitted flags here are ones we permit, then verify()
        ...
        // If all the verify_strict-permitted flags here are ones we permit, then
        // verify_strict() should succeed. Otherwise, it should not.
...
/// Prints the flags that are consistently permitted by verify() and
/// verify_strict()
#[test]
fn find_validation_criteria() {
    ...
    // Counts the number of times a signature with a re-encoded k and a low-order A
    // verified. This happens with 1/8 probability, assuming the usual
    // verification equation(s).
    ...
    {
        // If verify() was a success, add all the associated flags to verify-permitted
        // set
        let success = pubkey.verify(&msg, &sig).is_ok();
