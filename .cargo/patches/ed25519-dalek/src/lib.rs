...
//! SIGNATURE_LENGTH}; # let mut csprng = OsRng.unwrap_err();
//! # let signing_key: SigningKey = SigningKey::generate(&mut csprng);
//! # let message: &[u8] = b"This is a test of the tsunami alert system.";
//! # let signature: Signature = signing_key.sign(message);
//!
//! let verifying_key_bytes: [u8; PUBLIC_KEY_LENGTH] =
//! signing_key.verifying_key().to_bytes(); let secret_key_bytes: [u8;
//! SECRET_KEY_LENGTH] = signing_key.to_bytes(); let signing_key_bytes:    [u8;
//! KEYPAIR_LENGTH]    = signing_key.to_keypair_bytes(); let signature_bytes:
//! [u8; SIGNATURE_LENGTH]  = signature.to_bytes(); # }
//! ```
//! 
...
//! VerifyingKey::from_bytes(&verifying_key_bytes)?; let signing_key: SigningKey
//! = SigningKey::from_bytes(&signing_key_bytes); let signature: Signature =
...
//! deserialize(&encoded_verifying_key).unwrap(); let decoded_signature:
//! Signature = deserialize(&encoded_signature).unwrap();
//!
//! # assert_eq!(verifying_key, decoded_verifying_key);
//! # assert_eq!(signature, decoded_signature);
//! #
//! let verified: bool = decoded_verifying_key.verify(&message,
//! &decoded_signature).is_ok();
//!
//! assert!(verified);
//! # }
//! ```
...
#[cfg(feature = "digest")]
pub use ::signature::DigestVerifier;
#[cfg(feature = "digest")]
pub use curve25519_dalek::digest::Digest;
pub use ed25519::Signature;
#[cfg(feature = "pkcs8")]
pub use ed25519::pkcs8;
pub use ed25519::signature::Signer;
pub use ed25519::signature::Verifier;
#[cfg(feature = "digest")]
pub use sha2::Sha512;
