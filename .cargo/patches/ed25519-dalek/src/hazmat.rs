...
//!
//! If you are an end user / non-expert in cryptography, **do not use any of
//! these functions**. Failure to use them correctly can lead to catastrophic
//! failures including **full private key recovery.**
...
// Some conversion methods for `ExpandedSecretKey`. The signing methods are
// defined in `signing.rs`, since we need them even when `not(feature =
// "hazmat")`
impl ExpandedSecretKey {
    /// Construct an `ExpandedSecretKey` from an array of 64 bytes. In the spec,
    /// the bytes are the output of a SHA-512 hash. This clamps the first 32
    /// bytes and uses it as a scalar, and uses the second 32 bytes as a
    /// domain separator for hashing.
    ...
    /// error value is an `SignatureError` describing the error that
    /// occurred, namely that the given slice's length is not 64.
...
/// Compute an ordinary Ed25519 signature over the given message. `CtxDigest` is
/// the digest used to calculate the pseudorandomness needed for signing.
/// According to the Ed25519 spec, `CtxDigest = Sha512`.
///
/// # ⚠️  Cryptographically Unsafe
///
/// Do NOT use this function unless you absolutely must. Using the wrong values
/// in `ExpandedSecretKey` can leak your signing key. See
...
/// Compute a signature over the given prehashed message, the Ed25519ph
/// algorithm defined in [RFC8032 §5.1][rfc8032]. `MsgDigest` is the digest
/// function used to hash the signed message. `CtxDigest` is the digest function
/// used to calculate the pseudorandomness needed for signing. According to the
/// Ed25519 spec, `MsgDigest = CtxDigest = Sha512`.
///
/// # ⚠️  Cryptographically Unsafe
//
/// Do NOT use this function unless you absolutely must. Using the wrong values
...
/// * `prehashed_message` is an instantiated hash digest with 512-bits of output
///   which has had the message to be signed previously fed into its state.
/// * `verifying_key` is a [`VerifyingKey`] which corresponds to this secret
///   key.
/// * `context` is an optional context string, up to 255 bytes inclusive, which
///   may be used to provide additional domain separation.  If not set, this
///   will default to an empty string.
///
/// `scalar` and `hash_prefix` are usually selected such that `scalar ||
...
/// Compute an ordinary Ed25519 signature, with the message contents provided
/// incrementally by updating a digest instance.
///
/// The `msg_update` closure provides the message content, updating a hasher
/// argument. It will be called twice. This closure MUST leave its hasher in the
/// same state (i.e., must hash the same values) after both calls. Otherwise it
/// will produce an invalid signature.
///
/// `CtxDigest` is the digest used to calculate the pseudorandomness needed for
/// signing. According to the Ed25519 spec, `CtxDigest = Sha512`.
///
/// # ⚠️  Cryptographically Unsafe
///
/// Do NOT use this function unless you absolutely must. Using the wrong values
...
/// The ordinary non-batched Ed25519 verification check, rejecting non-canonical
/// R values.`CtxDigest` is the digest used to calculate the pseudorandomness
/// needed for signing. According to the Ed25519 spec, `CtxDigest = Sha512`.
...
/// The batched Ed25519 verification check, rejecting non-canonical R values.
/// `MsgDigest` is the digest used to hash the signed message. `CtxDigest` is
/// the digest used to calculate the pseudorandomness needed for signing.
...
mod test {
    ...
    // Pick distinct, non-spec 512-bit hash functions for message and sig-context
    // hashing
    type CtxDigest = blake2::Blake2b512;
    type MsgDigest = sha3::Sha3_512;

    impl ExpandedSecretKey {
        // Make a random expanded secret key for testing purposes. This is NOT how you
        // generate expanded secret keys IRL. They're the hash of a seed.
    ...
    // Check that raw_sign_prehashed and raw_verify_prehashed work when distinct,
    // non-spec MsgDigest and CtxDigest are used
    #[cfg(feature = "digest")]
    #[test]
