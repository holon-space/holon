...
impl VerifyingKey {
    ...
    ///
    /// Verifies the point is valid under [ZIP-215] rules. RFC 8032 / NIST point
    /// validation criteria are currently unsupported (see
    /// [dalek-cryptography/curve25519-dalek#626]).
    ///
    /// # Example
    ///
    /// ```
    ...
    /// # Returns
    ///
    /// A `Result` whose okay value is an EdDSA `VerifyingKey` or whose error
    ...
    /// has low order.
    ///
    /// A weak public key can be used to generate a signature that's valid for
    /// almost every message. [`Self::verify_strict`] denies weak keys, but
    /// if you want to check for this property before verification, then use
    /// this method.
    pub fn is_weak(&self) -> bool {
        self.point.is_small_order()
    }

    /// The ordinary non-batched Ed25519 verification check, rejecting
    /// non-canonical R values. (see [`Self::RCompute`]). `CtxDigest` is the
    /// digest used to calculate the pseudorandomness needed for signing.
    ...
    /// The prehashed non-batched Ed25519 verification check, rejecting
    /// non-canonical R values. (see [`Self::recompute_R`]). `CtxDigest` is
    /// the digest used to calculate the pseudorandomness needed for
    /// signing. `MsgDigest` is the digest used to hash the signed
    ...
    /// signature must fail if the scalar `s` is not properly reduced mod
    /// $\ell$:
    ///
    ...
    ///
    /// Useful for cases where the whole message is not available all at once,
    /// allowing the internal signature state to be updated incrementally
    /// and verified at the end. In some cases, this will reduce the need
    ...
    /// Verify a `signature` on a `prehashed_message` using the Ed25519ph
    /// algorithm, using strict signature checking as defined by
    /// [`Self::verify_strict`].
    ///
    /// # Inputs
    ///
    ...
    /// keys. The output of this function is a valid X25519 public key whose
    /// secret key is `sk.to_scalar_bytes()`, where `sk` is a valid signing
    /// key for this `VerifyingKey`.
    ///
    /// # Note
    ///
    /// We do NOT recommend this usage of a signing/verifying key. Signing keys
    /// are usually long-term keys, while keys used for key exchange should
    /// rather be ephemeral. If you can help it, use a separate key for
    /// encryption.
    ///
...
///
/// For pre-hashed variants a `h` with the context already included can be
/// provided. Note that this returns the compressed form of R and the caller
/// does a byte comparison. This means that all our verification functions do
/// not accept non-canonically encoded R values. See the validation criteria
/// blog post for more details:     https://hdevalence.ca/blog/2020-10-04-its-25519am
...
where
    CtxDigest: Digest<OutputSize = U64>,
{
    /// If `prehash_ctx.is_some()`, this does the prehashed variant of the
    /// computation using its contents.
...
/// Equivalent to [`VerifyingKey::verify_prehashed`] with `context` set to
/// [`None`].
#[cfg(feature = "digest")]
