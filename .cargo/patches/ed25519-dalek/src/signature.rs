...
///
/// **Unsafe**: This version of `check_scalar` permits signature malleability.
/// See README.
...
fn check_scalar(bytes: [u8; 32]) -> Result<Scalar, SignatureError> {
    ...
    // You cannot do arithmetic with scalars construct with Scalar::from_bits. We
    // only use this scalar for
    // EdwardsPoint::vartime_double_scalar_mul_basepoint, which is an accepted
    // usecase.
...
impl InternalSignature {
    ...
    /// ```
    ///
    /// However, this check is hacky and insufficient to check that the scalar
    /// is fully reduced `mod \ell = 2^252 +
    /// 27742317777372353535851937790883648493` as it leaves us with a
    /// guanteed bound of 253 bits.  This means that there are
    /// `2^253 - 2^252 + 2774231777737235353585193779088364849311` remaining
    /// scalars which could cause malleabilllity.
    ///
