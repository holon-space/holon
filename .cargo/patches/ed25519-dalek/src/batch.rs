...
/// An implementation of `rand_core::RngCore` which does nothing. This is
/// necessary because merlin demands an `Rng` as input to
/// `TranscriptRngBuilder::finalize()`. Using this with `finalize()`
...
impl rand_core::RngCore for ZeroRng {
    ...
    /// A no-op function which leaves the destination bytes for randomness
    /// unchanged.
    ///
...
///
/// The nonces for batch signature verification are derived purely from the
/// inputs to this function themselves.
///
...
///
/// For ed25519 batch verification we include the following as scalars in the
/// protocol transcript:
///
...
///
/// The latter prevents a malleability attack wherein an adversary, without
/// access to the signing key(s), can take any valid signature, `(s,R)`, and
/// swap `s` with `s' = -z1`.  This doesn't constitute a signature forgery,
/// merely a vulnerability, as the resulting signature will not pass single
...
pub fn verify_batch(
    ...
    // H(R || A || M) for each (R, A, M) triplet, we will feed _that_ into our
    // transcript rather than each R, A, M individually. Since R and A are
    // fixed-length, this modification is secure so long as SHA-512
    // is collision-resistant.
    // It suffices to take `verifying_keys[i].as_bytes()` even though a
    // `VerifyingKey` has two fields, and `as_bytes()` only returns the bytes of
    // the first. This is because of an invariant guaranteed by `VerifyingKey`:
    // the second field is always the (unique) decompression of the first. Thus,
    // the serialized first field is a unique representation of the entire
    // `VerifyingKey`.
    ...
    // Update transcript with the hashes above. This covers verifying_keys,
    // messages, and the R half of signatures
    for hram in hrams.iter() {
        transcript.append_message(b"hram", hram);
    }
    // Update transcript with the rest of the data. This covers the s half of the
    // signatures
    for sig in signatures {
        transcript.append_message(b"sig.s", sig.s_bytes());
    }

    // All function inputs have now been hashed into the transcript. Finalize it and
    // use it as randomness for the batch verification.
    ...
    // Compute (-∑ z[i]s[i] (mod l)) B + ∑ z[i]R[i] + ∑ (z[i]H(R||A||M)[i] (mod l))
    // A[i] = 0
