...
use curve25519_dalek::digest::array::typenum::U64;
use curve25519_dalek::edwards::CompressedEdwardsY;
use curve25519_dalek::edwards::EdwardsPoint;
use curve25519_dalek::scalar::Scalar;
#[cfg(feature = "pkcs8")]
use ed25519::pkcs8;
use ed25519::signature::KeypairRef;
use ed25519::signature::MultipartSigner;
use ed25519::signature::MultipartVerifier;
...
use crate::Signature;
use crate::constants::KEYPAIR_LENGTH;
use crate::constants::SECRET_KEY_LENGTH;
#[cfg(feature = "digest")]
use crate::context::Context;
use crate::errors::InternalError;
use crate::errors::SignatureError;
use crate::hazmat::ExpandedSecretKey;
...
impl SigningKey {
    ...
    /// Construct a [`SigningKey`] from the bytes of a `VerifyingKey` and
    /// `SecretKey`.
    ///
    /// # Inputs
    ///
    ...
    /// # Returns
    ///
    /// A `Result` whose okay value is an EdDSA [`SigningKey`] or whose error
    ...
    /// # Note
    ///
    /// The RFC only permits SHA-512 to be used for prehashing, i.e., `MsgDigest
    /// = Sha512`. This function technically works, and is probably safe to
    /// use, with any secure hash function with 512-bit digests, but
    /// anything outside of SHA-512 is NOT specification-compliant. We expose
    ...
    ///
    /// let sig: Signature = signing_key.sign_prehashed(prehashed,
    /// Some(context))?; #
    /// # Ok(sig)
    /// # }
    /// # fn main() {
    ...
    /// Verify a `signature` on a `prehashed_message` using the Ed25519ph
    /// algorithm.
    ///
    /// # Inputs
    ///
    ...
    ///   which may be used to provide additional domain separation.  If not
    ///   set, this will default to an empty string.
    /// * `signature` is a purported Ed25519ph [`Signature`] on the
    ///   `prehashed_message`.
    ///
    /// # Returns
    ///
    ...
    /// Some(context))?;
    ///
    /// // The sha2::Sha512 struct doesn't implement Copy, so we'll have to
    /// create a new one: let mut prehashed_again: Sha512 =
    /// Sha512::default(); prehashed_again.update(message);
    ///
    /// let verified =
    /// signing_key.verifying_key().verify_prehashed(prehashed_again,
    /// Some(context), &sig);
    ///
    ...
    /// Strictly verify a signature on a message with this signing key's public
    /// key.
    ///
    ...
    /// Convert this signing key into a byte representation of an unreduced,
    /// unclamped Curve25519 scalar. This is NOT the same thing as
    /// `self.to_scalar().to_bytes()`, since `to_scalar()` performs a
    /// clamping step, which changes the value of the resulting scalar.
    ///
    /// This can be used for performing X25519 Diffie-Hellman using Ed25519
    /// keys. The bytes output by this function are a valid corresponding [`StaticSecret`](https://docs.rs/x25519-dalek/2.0.0/x25519_dalek/struct.StaticSecret.html#impl-From%3C%5Bu8;+32%5D%3E-for-StaticSecret)
    /// for the X25519 public key given by
    /// `self.verifying_key().to_montgomery()`.
    ///
    /// # Note
    ///
    /// We do NOT recommend using a signing/verifying key for encryption.
    /// Signing keys are usually long-term keys, while keys used for key
    /// exchange should rather be ephemeral. If you can help it, use a
    /// separate key for encryption.
    ///
    ...
    pub fn to_scalar_bytes(&self) -> [u8; 32] {
        ...
        // where the two outputs are both 32 bytes. scalar_bytes is what we return. Its
        // clamped and reduced form is what we use for signing (see impl
        // ExpandedSecretKey)
        let mut buf = [0u8; 32];
    ...
    /// Convert this signing key into a Curve25519 scalar. This is computed by
    /// clamping and reducing the output of [`Self::to_scalar_bytes`].
    ///
    /// This can be used anywhere where a Curve25519 scalar is used as a private
    /// key, e.g., in [`crypto_box`](https://docs.rs/crypto_box/0.9.1/crypto_box/struct.SecretKey.html#impl-From%3CScalar%3E-for-SecretKey).
    ///
    /// # Note
    ///
    /// We do NOT recommend using a signing/verifying key for encryption.
    ...
    pub fn to_scalar(&self) -> Scalar {
        // Per the spec, the ed25519 secret key sk is expanded to
        //     (scalar_bytes, hash_prefix) = SHA-512(sk)
        // where the two outputs are both 32 bytes. To use for signing, scalar_bytes
        // must be clamped and reduced (see ExpandedSecretKey::from_bytes). We
        // return the clamped and reduced form.
...
/// # Note
///
/// The RFC only permits SHA-512 to be used for prehashing. This function
/// technically works, and is probably safe to use, with any secure hash
/// function with 512-bit digests, but anything outside of SHA-512 is NOT
/// specification-compliant. We expose [`crate::Sha512`] for user convenience.
#[cfg(feature = "digest")]
...
/// `SHA512(sk)`, clamps the first 32 bytes and uses it as a scalar, and uses
...
//
// Signing functions. These are pub(crate) so that the `hazmat` module can use
// them
//

impl ExpandedSecretKey {
    /// The plain, non-prehashed, signing function for Ed25519. `CtxDigest` is
    /// the digest used to calculate the pseudorandomness needed for
    /// signing. According to the spec, `CtxDigest = Sha512`, and `self` is
    /// derived via the method defined in `impl From<&SigningKey> for
    /// ExpandedSecretKey`.
    ///
    /// This definition is loose in its parameters so that end-users of the
    /// `hazmat` module can change how the `ExpandedSecretKey` is calculated
    /// and which hash function to use.
    #[allow(non_snake_case)]
    #[allow(clippy::unwrap_used)]
    #[inline(always)]
    ...
    /// Sign a message provided in parts. The `msg_update` closure will be
    /// called twice to hash the message parts. This closure MUST leave its
    /// hasher in the same state (i.e., must hash the same values) after
    /// both calls. Otherwise it will produce an invalid signature.
    #[allow(non_snake_case)]
    #[inline(always)]
    ...
    /// The prehashed signing function for Ed25519 (i.e., Ed25519ph).
    /// `CtxDigest` is the digest function used to calculate the
    /// pseudorandomness needed for signing. `MsgDigest` is the
    /// digest function used to hash the signed message. According to the spec,
    /// `MsgDigest = CtxDigest = Sha512`, and `self` is derived via the
    /// method defined in `impl From<&SigningKey> for ExpandedSecretKey`.
    ///
    /// This definition is loose in its parameters so that end-users of the
    /// `hazmat` module can change how the `ExpandedSecretKey` is calculated
    /// and which `CtxDigest` function to use.
    #[cfg(feature = "digest")]
    #[allow(non_snake_case)]
    #[inline(always)]
