...
impl CGContextRef {
    ...
    // `CGContextSetFontSmoothingStyle` is an unpublished CoreGraphics SPI. Any
    // reference to it (even from a never-called wrapper) is retained in a
    // `staticlib` bitcode CGU and dragged into the final app binary that Xcode
    // force-loads with dead-stripping disabled, tripping App Store static
    // analysis (ITMS-90338 "non-public API _CGContextSetFontSmoothingStyle").
    // Gate it off on iOS so no iOS binary can carry the symbol. macOS keeps it.
