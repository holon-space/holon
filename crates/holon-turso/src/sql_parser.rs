//! SQL parsing, dependency extraction, and SQL-level transformations using
//! sqlparser.
//!
...
//!
//! All post-compilation SQL transformations (entity_name injection,
//! _change_origin, JSON aggregation for UNIONs) are implemented as
//! `SqlTransformer` trait objects. They are registered in DI and applied
...
/// Convenience: parse SQL string, inject entity_name, return modified SQL
/// string. Returns the original string unchanged if parsing fails.
...
pub fn inject_entity_name(stmts: &mut [Statement]) {
    for stmt in stmts.iter_mut() {
        if let Statement::Query(query) = stmt {
            ...
            // Skip recursive CTE bodies — UNION ALL branches must have matching column
            // counts
...
fn get_primary_table_name(select: &Select, cte_names: &HashSet<String>) -> Option<String> {
    // Try dominant qualifier first: if all qualified columns reference the same
    // alias, resolve that alias to a real table name.
...
/// If all qualified columns in the SELECT projection reference the same table
/// alias, resolve that alias to the real table name by looking through FROM +
/// JOINs.
...
/// Extract the table qualifier from a compound identifier (e.g., `_v4."id"` →
/// `_v4`).
...
/// Extract the dominant qualifier from SELECT projection (if all qualified
/// columns reference the same alias). Returns None if there are mixed or no
/// qualifiers.
...
/// Like `resolve_table_factor` but returns the alias when present (for column
/// qualification).
...
/// Injects `table._change_origin` column reference into SELECT projections
/// (priority 20).
///
...
/// For UNION queries, wraps each branch in a CTE with `SELECT json_object(*) AS
/// data` (priority 30).
///
/// This ensures heterogeneous UNION branches (with different column sets) are
/// normalized into a single `data` JSON column before being combined.
...
/// For UNION queries, wrap each branch in a CTE with `SELECT json_object(*) AS
/// data`.
...
/// Recursively collect all leaf branches and operators from a nested
/// SetOperation tree.
...
mod tests {
    ...
    fn test_extract_join() {
        let refs = extract_table_refs(&parse(
            "SELECT * FROM users u JOIN orders o ON u.id = o.user_id LEFT JOIN items i ON o.id = \
             i.order_id",
        ));
        assert_eq!(
            refs,
    ...
    fn test_extract_cte() {
        let refs = extract_table_refs(&parse(
            "WITH active AS (SELECT * FROM users WHERE active = 1) SELECT * FROM active JOIN \
             orders ON active.id = orders.user_id",
        ));
        assert!(refs.contains(&Resource::schema("users")));
    ...
    fn test_extract_nested_cte() {
        let refs = extract_table_refs(&parse(
            ...
            SELECT * FROM grandchildren JOIN file ON grandchildren.doc_id = file.id",
        ));
        assert!(refs.contains(&Resource::schema("block")));
        assert!(refs.contains(&Resource::schema("file")));
        assert!(
    ...
    fn test_extract_recursive_cte_in_subquery() {
        ...
        let sql = "\
            WITH RECURSIVE _vl1 AS (SELECT _v0.id AS node_id, 0 AS depth, CAST(_v0.id AS TEXT) AS \
                   visited FROM block AS _v0 WHERE _v0.id = 'test-root' UNION ALL SELECT _fk.id, \
                   _vl1.depth + 1, _vl1.visited || ',' || CAST(_fk.id AS TEXT) FROM _vl1 JOIN \
                   block _fk ON _fk.parent_id = _vl1.node_id WHERE _vl1.depth < 10) SELECT \
                   _v2.id, _v2.content FROM block AS _v0 JOIN block AS _v2 ON 1 = 1 WHERE _v0.id \
                   = 'test-root' AND _v2.id IN (SELECT node_id FROM _vl1 WHERE depth >= 1 AND \
                   depth <= 10)";
    ...
    fn test_inject_union() {
        ...
        let sql = sql_to_string(&stmts);
        assert!(sql.contains("'block' AS entity_name"), "got: {sql}");
        assert!(sql.contains("'file' AS entity_name"), "got: {sql}");
    }

    #[test]
    fn test_inject_cte() {
        let mut stmts = parse(
            "WITH active AS (SELECT * FROM block WHERE active = 1) SELECT * FROM active JOIN file \
             ON active.doc_id = file.id",
        );
        inject_entity_name(&mut stmts);
        let sql = sql_to_string(&stmts);
        // CTE body injects 'block' AS entity_name
        assert!(sql.contains("'block' AS entity_name"), "got: {sql}");
        // Outer SELECT * FROM CTE: wildcard already includes entity_name, no
        // extra column added
    }
    ...
    fn test_inject_cte_explicit_columns_passes_through_entity_name() {
        let mut stmts = parse(
            "WITH children AS (SELECT * FROM block WHERE parent_id = 'x') SELECT id, content FROM \
             children",
        );
        inject_entity_name(&mut stmts);
        let sql = sql_to_string(&stmts);
    ...
    fn test_inject_join_uses_primary_table() {
        let mut stmts = parse("SELECT b.id, f.name FROM block b JOIN file f ON b.doc_id = f.id");
        inject_entity_name(&mut stmts);
        let sql = sql_to_string(&stmts);
        assert!(sql.contains("'block' AS entity_name"), "got: {sql}");
        assert!(
            !sql.contains("'file' AS entity_name"),
    ...
    fn test_inject_gql_style_qualified_columns_from_join() {
        ...
        let mut stmts = parse(
            "SELECT _v4.\"id\" AS \"id\", _v4.\"content\" AS \"content\" FROM current_focus AS \
    ...
    fn test_inject_nested_union() {
        let mut stmts = parse(
            "SELECT id FROM block UNION ALL SELECT id FROM file UNION ALL SELECT id FROM tags",
        );
        inject_entity_name(&mut stmts);
        let sql = sql_to_string(&stmts);
        assert!(sql.contains("'block' AS entity_name"), "got: {sql}");
    ...
    fn test_json_agg_preserves_existing_ctes() {
        let mut stmts = parse(
            "WITH active AS (SELECT * FROM projects WHERE active = 1) SELECT id FROM active UNION \
             ALL SELECT id FROM tasks",
        );
        inject_json_aggregation(&mut stmts);
        let sql = sql_to_string(&stmts);
        let lower = sql.to_lowercase();
    ...
    fn test_apply_sql_transforms_returns_original_on_parse_error() {
        let transformers: Vec<Box<dyn SqlTransformer>> = vec![];
