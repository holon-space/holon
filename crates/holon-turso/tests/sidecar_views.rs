...
const SESSION_LAST_MESSAGE_SQL: &str = "SELECT 'cc-session:' || session_id AS session_id, \
       MAX(timestamp) AS last_ts, \
       substr(MAX(timestamp || '|' || role), 26) AS last_role \
     FROM cc_message GROUP BY session_id";

const SESSION_STATUS_SQL: &str = "SELECT session_id, last_ts, last_role, \
