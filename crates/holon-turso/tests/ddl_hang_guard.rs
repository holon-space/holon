...
async fn watch_ddl_on_chained_matview_never_hangs() {
    ...
    handle
        .execute_ddl(
            "CREATE TABLE items (id TEXT PRIMARY KEY, parent_id TEXT NOT NULL, content TEXT \
             DEFAULT '')",
        )
        .await
        .expect("create items");
    ...
    handle
        .execute_ddl(
            "CREATE TABLE navigation_cursor (region TEXT PRIMARY KEY, history_id INTEGER \
             REFERENCES navigation_history(id))",
        )
        .await
    ...
    handle
        .execute_ddl(
            "CREATE MATERIALIZED VIEW current_focus AS SELECT nc.region, nh.block_id, nh.id AS \
             hid FROM navigation_cursor nc JOIN navigation_history nh ON nc.history_id = nh.id",
        )
        .await
    ...
    let chained_sql = "SELECT cf.region, cf.block_id, i.id AS root_id FROM current_focus AS cf \
    ...
    assert!(
        outcome.is_ok(),
        "watch DDL on a chained matview HUNG past {:?} — the actor freeze guard did not fire. \
         This is the app-freeze bug.",
        ceiling
    );
