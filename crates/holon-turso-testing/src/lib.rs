...
//!
//! Owns the pure-Turso correspondence ARMS, co-located out of the central table:
//! - The two Turso arms (`block_raw`, `matview`) of the SHARED [`NonSeedBlocks`]
//!   observable (whose struct + `ref_non_seed_blocks` stay on the pbt-core floor;
//!   the `/loro` arm is contributed by `holon-loro-testing`).
//! - The whole `block_content` / `block_parent` / `advice_matviews` observables
//!   (every arm Turso-owned): observable struct + `impl Observable` + `ref_project`
//!   move here entirely.
//!
...
//! `Ref*` / `Sut*` capabilities generically through the composed `CapMap`. A guard
//! integration test (`tests/no_ref_state_dep.rs`) fails the build if this crate
//! ever reaches for the central `ReferenceState` monolith.
//!
...
/// `holon-turso`'s contribution to the ONE composed PBT: its six storage-pipeline
/// correspondence arms. `cap_installers` is empty — the central `compose_sut_*`
/// still assembles the `CapMap` (incl. the `SqlProjectionComponent`); `generators`
/// is empty because no transitions have co-located here.
...
/// Static, boot-free footprint — the ladder-floor enumeration for `holon-turso`.
/// Held in lockstep with [`pbt_contribution`] by the anti-rot test below. Order
/// matches `correspondences::wire_all()` (per-observable, per-store).
...
mod tests {
    ...
    /// live contribution's invariant ids, in order.
    #[test]
