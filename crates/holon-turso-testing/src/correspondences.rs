//! The Turso storage-pipeline correspondence arms, co-located out of the
//! central table (co-location Phase 2) now that the correspondence framework +
//! shared `NonSeedBlocks` observable + `block_compare` leaves live on the
//! pbt-core floor.
//!
//! Each `pub fn <observable>()` builds one [`Correspondence`]: one reference
//! projection + N SUT store projections. The crate's [`pbt_contribution`]
//! splices `<observable>().wire()` into the composed catalog — adding a store
//! or observable is an entry here, nothing else. Extraction/comparison
//! strategies are named `fn`s in this module (greppable wiring).
//!
//! ## What lives here
...
//!   `holon-loro-testing`).
//! - `block_content()`, `block_parent()`, `advice_matviews()` — whole
//!   observables whose every arm is Turso-owned, so their observable struct +
//!   `impl Observable` + `ref_project` move here entirely (the `Observable`
//!   trait is the foreign trait; the struct is local — the orphan rule is
//!   satisfied).
//!
...
use holon_pbt_core::observables::is_synthetic_ref_id;
...
// + `ref_non_seed_blocks`). This crate contributes its two Turso
// storage-pipeline arms — `block_raw` (write-side source of truth) and
// `matview` (CDC mirror). The `/loro` arm is contributed by
// `holon-loro-testing`; the `/org` arm stays a hand-written central invariant
...
/// blocks — which are legitimately absent from the ref model — from the
/// spurious set. The exclusion is confined to THIS direction by construction:
/// the missing side takes no filter, so it is physically impossible to weaken
/// the data-loss direction the same way. The rule-block exclusion itself is NOT
/// implemented here — today every caller passes `|_| true`.
...
/// snapshot → `/block_raw`). Synthetic ref ids (`block::split-N`) are remapped
/// to UUIDs production-side, so the SUT has no row at the synthetic id —
/// skipped on both projection paths (content equality on stable ids only; the
/// wider PBT reconciles synthetic-id mapping).
