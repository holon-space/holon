//! Minimal reproducer for Turso IVM "dirty pages should be empty for read txn"
//! bug
//!
//! **Bug**: During IVM processing of a write that triggers multiple
//! materialized view updates (especially recursive CTE + JOIN matviews), the
//! pager panics with:   "dirty pages should be empty for read txn"
//! at pager.rs:4699 inside `rollback()`.
//!
//! **Secondary corruption**: After the panic, the BTree index becomes
//! inconsistent:   "Index points to non-existent table row"
//! at incremental/persistence.rs:152.
//!
//! **Key insight**: The bug occurs on a SINGLE connection when IVM processes a
//! write that cascades through multiple materialized views. The pager's
//! internal read sub-operations (used by IVM to compute deltas) find dirty
//! pages from the ongoing write transaction. This is NOT a cross-connection
//! race.
//!
...
async fn main() -> anyhow::Result<()> {
    ...
    for i in 0..5 {
        conn.execute(
            &format!(
                "CREATE MATERIALIZED VIEW watch_view_{} AS SELECT id, content FROM blocks WHERE \
                 content_type = 'text'",
                i
            ),
            (),
    ...
    for chain in 0..20 {
        ...
        if let Err(e) = conn
            .execute(
                &format!(
                    "INSERT INTO blocks (id, parent_id, content) VALUES ('{}', 'doc:test.org', \
                     'root-{}')",
                    root_id, chain
                ),
                (),
        ...
        if let Err(e) = conn
            .execute(
                &format!(
                    "INSERT INTO events (id, event_type, aggregate_type, aggregate_id, origin, \
                     payload, created_at) VALUES ('evt-{}', 'block.created', 'block', \
                     'chain-{}-0', 'org', '{{}}', {})",
                    chain,
                    chain,
                    chain * 1000 + 1
    ...
    if !bug_detected.load(Ordering::SeqCst) {
        ...
        for round in 0..20 {
            ...
            if let Err(e) = conn
                .execute(
                    &format!(
                        "CREATE MATERIALIZED VIEW IF NOT EXISTS {} AS SELECT id, parent_id, \
                         content FROM blocks WHERE content LIKE 'child-%'",
                        view_name
                    ),
                    (),
                )
