...
//!
//! Schema (per H1):
//! - `task(id, status, gate, priority, effort)` with composite index on `(gate,
...
async fn populate(conn: &turso::Connection, scenario: Scenario) -> Result<()> {
    ...
    for i in 0..N_TASKS {
        ...
        conn.execute(
            &format!(
                "INSERT INTO task (id, status, gate, priority, effort) VALUES ('{id}', \
                 '{status}', {gate_sql}, {priority}, {effort})"
            ),
            (),
        )
        .await?;
            ...
            for i in 0..N_TASKS {
                ...
                for _ in 0..n_blockers {
                    ...
                    let _ = conn
                        .execute(
                            &format!(
                                "INSERT OR IGNORE INTO task_blocks (from_id, to_id) VALUES \
                                 ('{from}', '{to}')"
                            ),
                            (),
                        )
        ...
        Scenario::HotBlockers => {
            ...
            for i in 0..N_TASKS {
                ...
                let _ = conn
                    .execute(
                        &format!(
                            "INSERT OR IGNORE INTO task_blocks (from_id, to_id) VALUES ('{from}', \
                             '{to}')"
                        ),
                        (),
                    )
...
async fn bench_query(conn: &turso::Connection, label: &str, sql: &str) -> Result<Stats> {
    ...
    println!(
        "  {label}  p50={:6.2}ms  p99={:6.2}ms  p99.9={:6.2}ms  min={:5.2}  max={:6.2}  \
         mean={:6.2}",
...
async fn report_data_shape(conn: &turso::Connection) -> Result<(usize, usize)> {
    ...
    let mut q = conn
        .query(
            "SELECT COUNT(*) FROM task t WHERE t.status = 'TODO' AND t.gate = 'G1' AND NOT EXISTS \
             (SELECT 1 FROM task_blocks bb JOIN task b ON b.id = bb.to_id WHERE bb.from_id = t.id \
             AND b.status != 'DONE')",
            (),
        )
        .await?;
