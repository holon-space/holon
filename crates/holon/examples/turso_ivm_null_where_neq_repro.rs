...
//! != ''` returns 4 rows via plain SQL but 50 rows from its materialized view
//! (46 extra rows all have `name IS NULL`). The matview was created at DB
//! startup, rows were inserted incrementally, and the IVM delta computation
//! treated `NULL != ''` as true instead of NULL/false.
//!
...
/// Test: UPDATE a row's name from NULL to a real value — should appear in
/// matview.
...
/// Test: UPDATE a row's name from a value to NULL — should disappear from
/// matview.
...
async fn test_stale_matview_after_reopen() -> anyhow::Result<bool> {
    ...
    // Phase 2: reopen and check — do stale NULLs appear?
    {
        ...
        let plain = count(&conn, "SELECT COUNT(*) FROM items WHERE name != ''").await?;
        println!(
            "  Phase 2 (after reopen): matview has {matview} rows (expected: 2), plain SQL: \
             {plain}, NULL leaked: {null_leaked}"
        );
...
async fn test_production_like_schema() -> anyhow::Result<bool> {
    ...
    for (id, name) in [
        ...
        conn.execute(
            &format!(
                "INSERT INTO block (id, parent_id, content, name, created_at, updated_at) VALUES \
                 ('{id}', 'sentinel:no_parent', '', '{name}', 1000, 1000)"
            ),
            (),
        )
        .await?;
    ...
    println!(
        "  Total blocks: {total}, Plain SQL: {plain}, Matview: {matview}, NULL leaked: \
         {null_leaked}"
    );
