...
//! - `task_blocks(from_id, to_id, PK(from_id, to_id), FK CASCADE on both)` —
//!   junction. Cascade deletes mean a deleted task automatically removes its
//!   incident edges.
...
//!   `blocker_status != 'DONE'`. Pure relational JOIN + WHERE — no GROUP BY, no
//!   recursion.
//!
//! "Is task X unblocked?" is then `NOT EXISTS (SELECT 1 FROM
//! task_blocking_edges WHERE task_id = X)`. Cheap query against an indexed
//! matview.
//!
...
//!                            cascade should remove those edges and matview
//! rows. P6 DELETE blocker task  — same FK-cascade path; verifies cascade also
