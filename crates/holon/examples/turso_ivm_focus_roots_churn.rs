...
//!
//! Production symptom: `gpui_ui_pbt` `[inv-editable-text-has-draggable] CDC not
//! quiescent — spurious events: [("region:main", 9..11)]`
//! (`crates/holon-integration-tests/src/ test_environment.rs:1333`). The Holon
//! test framework's `setup_region_watch` subscribes to a `watch_view` shaped
//! like   `SELECT fr.root_id AS id, b.content, b.parent_id
...
async fn main() -> anyhow::Result<()> {
    ...
    for i in 1..=4 {
        conn.execute(
            &format!(
                "INSERT INTO block (id, parent_id, content) VALUES ('a-{i}', 'doc:aaa', 'Doc A \
                 block {i}')"
            ),
            (),
        )
        ...
        conn.execute(
            &format!(
                "INSERT INTO block (id, parent_id, content) VALUES ('b-{i}', 'doc:bbb', 'Doc B \
                 block {i}')"
            ),
            (),
        )
    ...
    conn.execute(
        "INSERT INTO block (id, parent_id, content) VALUES ('doc:aaa', 'root', 'Doc A')",
        (),
    )
    .await?;
    conn.execute(
        "INSERT INTO block (id, parent_id, content) VALUES ('doc:bbb', 'root', 'Doc B')",
        (),
    )
    .await?;
    ...
    let check = |label: &str, evs: &[CdcEvent]| -> bool {
        if evs.len() > max_expected_batches {
            println!(
                "!!! BUG: {label} emitted {} batches for two navigations (expected ≤ \
                 {max_expected_batches}) — IVM is firing CDC callbacks on transactions that don't \
                 change the matview's output",
                evs.len(),
            );
            true
        } else {
    ...
    if fr.len() == rmv.len() && fr.len() > 1 {
        ...
        let max_skew_ms = pairs
            ...
            .unwrap_or(0);
        println!(
            "\n[diagnostic] focus_roots and region_main_view fire in lockstep ({} matched \
             batches, max skew {} ms) — confirms shared upstream churn",
            pairs.len(),
            max_skew_ms,
        );
    }
    ...
    println!(
        "\n[diagnostic] {} of {} CDC batches contain zero items — IVM is notifying for upstream \
         writes that don't change matview output",
        empty_batches,
        recorded.len(),
    );
    ...
    //   [inv-editable-text-has-draggable] CDC not quiescent — spurious events:
    // [("region:main", 4)]
    //
    ...
    conn.execute(
        &format!(
            "INSERT OR REPLACE INTO navigation_cursor (region, history_id) VALUES \
             ('left_sidebar', {preroll_history_id})"
        ),
        (),
    )
    .await?;
    ...
    conn.execute(
        &format!(
            "DELETE FROM navigation_history WHERE region = 'left_sidebar' AND id > \
             {preroll_history_id}"
        ),
        (),
    )
    ...
    if main_view_items > 0 {
        println!(
            "\n!!! BUG: region_main_view received {main_view_items} item(s) for a \
             left_sidebar-only navigation — IVM is not applying the WHERE fr.region = 'main' \
             filter to the propagated delta"
        );
        phase2_bug = true;
    }
    if left_view_items == 0 {
        println!(
            "\n!!! WARN: region_left_sidebar_view saw zero items — the test navigation may not \
             have actually fired (sanity check failed)"
        );
        phase2_bug = true;
    }
    ...
    //
    // gpui_ui_pbt symptom: ClickBlock(LeftSidebar, ...) fires
    // inv-editable-text-has-draggable on `region:main` with 4 items. The click
    // handler at `frontends/gpui/src/render/builders/render_entity.rs:54`
    // dispatches `editor_focus(region="main", block_id=...)` (region is
    // hard-coded to "main" — see TODO in that file). `editor_focus` performs a
    //
    ...
    println!(
        "[test] INSERT OR REPLACE INTO editor_cursor (region='main', block_id='b-1', \
         cursor_offset=0)"
    );
    conn.execute(
        "INSERT OR REPLACE INTO editor_cursor (region, block_id, cursor_offset) VALUES ('main', \
         'b-1', 0)",
        (),
    )
    .await?;
    ...
    println!(
        "[test] INSERT OR REPLACE INTO editor_cursor (region='main', block_id='b-2', \
         cursor_offset=0)"
    );
    conn.execute(
        "INSERT OR REPLACE INTO editor_cursor (region, block_id, cursor_offset) VALUES ('main', \
    ...
    if main_view_items_p3 > 0 || left_view_items_p3 > 0 || main_panel_items_p3 > 0 {
        println!(
            "\n!!! BUG: editor_cursor writes triggered {} item(s) on matviews that don't depend \
             on editor_cursor — IVM is firing CDC callbacks on relations whose schema doesn't \
             include the written table",
    ...
    println!(
        "\nPASS: phase 1 emitted ≤ {max_expected_batches} batches per matview, phase 2 saw zero \
         cross-region leakage, phase 3 saw zero unrelated-table churn."
    );
    Ok(())
}
