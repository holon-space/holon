...
//!
//! The error manifests as "Invalid join commit state" during the IVM commit
//! phase.
//!
//! Run with: cargo run --example turso_ivm_recursive_cte_join_repro
//!
//! Expected: Panic with "Invalid join commit state" or similar JoinOperator
//! error
...
async fn main() -> anyhow::Result<()> {
    ...
    let db = turso::Builder::new_local(db_path)
        .experimental_materialized_views(true) // Enable experimental materialized views
        .build()
        .await?;
    let conn = db.connect()?;
    ...
    conn.execute(
        r#"INSERT INTO blocks (id, parent_id, content, content_type)
           VALUES ('root-1', 'doc:test.org', 'Root content', 'text')"#,
        (),
    )
    .await?;
    ...
    // The combination with existing current_focus JOIN view can cause "Invalid join
    // commit state"
