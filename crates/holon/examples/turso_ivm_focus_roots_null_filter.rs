...
async fn test_matview_state_filters_null() -> anyhow::Result<bool> {
    ...
    .await?;
    conn.execute(
        "INSERT INTO navigation_history (region, block_id, timestamp) VALUES ('right', 'block:c', \
         1004)",
        (),
    )
    .await?;
    ...
    println!(
        "  navigation_history: {total_history} rows (5 inserted: 2 home, 3 pinned)\n  \
         ...
         leaked: {null_leaked} rows (expected: 0)"
    );
...
async fn test_update_block_id_to_null() -> anyhow::Result<bool> {
    ...
    .await?;
    println!(
        "  After UPDATE:  focus_roots has {after} rows (expected: 0), NULL leaked: {null_leaked}"
    );
...
async fn test_cdc_events_filter_null() -> anyhow::Result<bool> {
    ...
    conn.execute(
        "UPDATE navigation_history SET block_id = 'block:b', timestamp = 1002 WHERE block_id IS \
         NULL",
        (),
    )
    .await?;
    ...
    println!(
        "  Final matview state: {final_state} rows (expected 1: 'block:b')\n  CDC events with \
         NULL/missing root_id: {null_events} (expected 0 if WHERE filters CDC)"
    );
