...
async fn test_close_single_open_row() -> anyhow::Result<bool> {
    ...
    conn.execute(
        "INSERT INTO navigation_history (region, block_id, timestamp) VALUES ('main', \
         'block:journals', 1000)",
        (),
    )
    .await?;
    ...
    conn.execute(
        "UPDATE navigation_history SET closed_at = datetime('now') WHERE region = 'main' AND \
         closed_at IS NULL",
        (),
    )
    .await?;
    ...
    println!(
        "  After close:  base table open rows = {base_open} (expected: 0), focus_roots = {after} \
         (expected: 0)"
    );
