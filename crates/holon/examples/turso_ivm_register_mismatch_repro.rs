...
//! inserting into the base table triggers IVM incremental maintenance that
//! panics:
//!
//!   assertion `left == right` failed: Mismatch in number of registers! Got 30,
//! expected 26
...
//! 3. `focus_roots` matview: UNION ALL of two JOINs on block (also depends on
//!    block)
//! 4. `events_view_block` matview: filtered SELECT on events table
//! 5. Events inserted alongside block inserts (both matviews updated in same
//!    commit)
...
async fn main() -> anyhow::Result<()> {
    ...
    // In production, the operations table DDL fires right when blocks are being
    // synced
    conn.execute(
    ...
    // Updating block-1 (root) forces IVM to re-evaluate the entire recursive
    // expansion
    conn.execute("BEGIN", ()).await?;
    for i in 1..=10 {
