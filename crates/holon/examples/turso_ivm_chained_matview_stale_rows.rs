//! Reproducer for Turso IVM: chained matview retains stale rows after upstream
//! UPDATE
//!
...
//!   navigation_cursor (table) → current_focus (matview, JOIN) → focus_roots
//! (matview, JOIN+UNION)   items table is also joined into focus_roots
//!
...
async fn main() -> anyhow::Result<()> {
    ...
    for i in 1..=8 {
        conn.execute(
            &format!(
                "INSERT INTO items (id, parent_id, content) VALUES ('a-root-{i}', 'doc:aaa', 'Doc \
                 A root {i}')"
            ),
            (),
        )
    ...
    for i in 1..=4 {
        conn.execute(
            &format!(
                "INSERT INTO items (id, parent_id, content) VALUES ('a-child-{i}', 'a-root-1', \
                 'Doc A child {i}')"
            ),
            (),
        )
    ...
    for i in 1..=5 {
        conn.execute(
            &format!(
                "INSERT INTO items (id, parent_id, content) VALUES ('b-root-{i}', 'doc:bbb', 'Doc \
                 B root {i}')"
            ),
            (),
        )
    ...
    for i in 1..=3 {
        conn.execute(
            &format!(
                "INSERT INTO items (id, parent_id, content) VALUES ('b-child-{i}', 'b-root-1', \
                 'Doc B child {i}')"
            ),
            (),
        )
    ...
    for i in 1..=3 {
        conn.execute(
            &format!(
                "INSERT INTO items (id, parent_id, content) VALUES ('c-root-{i}', 'doc:ccc', 'Doc \
                 C root {i}')"
            ),
            (),
        )
    ...
    for (nav_idx, (doc_id, expected_roots)) in docs.iter().enumerate() {
        ...
        conn.execute(
            &format!(
                "INSERT OR REPLACE INTO navigation_cursor (region, history_id) VALUES ('main', \
                 {history_id})"
            ),
            (),
        )
        ...
        conn.execute(
            &format!(
                "INSERT OR REPLACE INTO items (id, parent_id, content) VALUES \
                 ('ephemeral-{history_id}', '{doc_id}', 'Concurrent mutation {history_id}')"
            ),
            (),
        )
        .await?;
        ...
        let stale_count = count_query(
            &conn,
            &format!(
                "SELECT count(*) as cnt FROM focus_roots WHERE region = 'main' AND block_id != \
                 '{doc_id}'"
            ),
        )
        .await?;
        ...
        if stale_count > 0 {
            ...
            print_query(
                ...
                "SELECT region, block_id, root_id FROM focus_roots WHERE region = 'main' ORDER BY \
                 root_id",
            )
            .await?;
