...
//! filter, even though:
//!   - the `focus_roots` matview's STATE correctly excludes NULL block_id
//!     rows on `aff40a84` (verified in turso_ivm_focus_roots_null_filter),
//!   - Turso's raw `set_change_callback` on `focus_roots` does NOT fire
//!     for excluded rows (verified there too).
//!
...
//!
//! - H1: Inner matview correctly suppresses CDC; outer matview also
//!   suppresses (no events on either for NULL block_id rows). Then the
//!   bug is in holon's broadcast translation layer, not Turso. Production
//!   GQL via JOIN drops NULL anyway.
//!
//! - H2: Inner suppresses but outer leaks. Upstream Turso bug, file at
//!   `bigdata/turso/bugs/`.
//!
//! - H3: Outer leaks regardless of inner state (always fires per inner
//!   change). Different upstream bug shape.
//!
...
async fn main() -> anyhow::Result<()> {
    ...
    conn.execute(
        "UPDATE navigation_history SET block_id = NULL WHERE region = 'main' AND block_id = 'block:a'",
        (),
    )
    .await?;
    ...
    .await?;
    conn.execute(
        "UPDATE navigation_history SET block_id = 'block:c' WHERE region = 'main' AND block_id IS NULL AND timestamp = 1000",
        (),
    )
    .await?;
