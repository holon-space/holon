//! Reproducer for Turso IVM "expected a positive weight, found -1" on app
//! restart.
//!
...
//!   3. Second "session": reopen same DB, matview already exists (IF NOT
//!      EXISTS), INSERT OR REPLACE all rows (simulates org sync re-importing
//!      same blocks)
//!   4. Query matview → "expected a positive weight, found -1"
//!
//! This differs from the original reproducer which did everything in one
//! session. The bug may specifically involve IVM state persisted to disk
//! between sessions.
//!
