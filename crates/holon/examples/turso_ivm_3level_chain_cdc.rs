...
//!   Level 2 (matview): focus_roots = cursor JOIN history JOIN block (inlined
//! current_focus)   Level 3 (matview): watch_view = focus_roots JOIN block
//! (recursive CTE)
//!
...
async fn main() -> anyhow::Result<()> {
    ...
    for i in 1..=5 {
        conn.execute(
            &format!(
                "INSERT INTO block (id, parent_id, content, content_type) VALUES ('a-{i}', \
                 'doc:aaa', 'Doc A block {i}', 'text')"
            ),
            (),
        )
        .await?;
    ...
    for i in 1..=3 {
        conn.execute(
            &format!(
                "INSERT INTO block (id, parent_id, content, content_type) VALUES ('b-{i}', \
                 'doc:bbb', 'Doc B block {i}', 'text')"
            ),
            (),
        )
        .await?;
    ...
    // NOTE: watch_view is created LATER (after first navigation) to match
    // production. In production, query_and_watch lazily creates the matview
    // when the UI first renders.
    ...
    // Create blocks_with_paths (recursive CTE, creates IVM pressure like
    // production)
    conn.execute(
    ...
    for suffix in [
        ...
        conn.execute(
            &format!(
                "CREATE MATERIALIZED VIEW watch_view_{suffix} AS SELECT id, content, \
                 content_type, parent_id FROM block WHERE id = 'dummy-{suffix}' OR parent_id = \
                 'dummy-{suffix}'"
            ),
            (),
        )
    ...
    // NOW create the main watch_view — this matches production where
    // query_and_watch lazily creates the matview AFTER focus_roots already has
    // data and many other matviews exist.
    ...
    if watch_view_stale {
        println!(
            "\n!!! BUG: watch_view matview ({wv_count} rows) disagrees with raw SQL re-evaluation \
             ({raw_wv_count} rows)"
        );
    }
    if watch_view_no_cdc && !focus_roots_stale {
        println!(
            "\n!!! BUG: watch_view received 0 CDC events even though focus_roots updated \
             ({fr_cdc} CDC events). The matview-on-matview chain dropped CDC."
        );
    }
    ...
    assert!(
        wv_cdc > 0,
        "watch_view should receive CDC events when focus_roots changes, got 0. focus_roots got \
         {fr_cdc} CDC events."
    );
