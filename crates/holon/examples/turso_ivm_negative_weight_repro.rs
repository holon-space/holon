...
//!
//! **Bug**: Any row modification (UPDATE, INSERT OR REPLACE, ON CONFLICT DO
//! UPDATE) to a table with a recursive CTE materialized view corrupts IVM
//! internal weights.
//!
//! **Minimal trigger**: A self-referencing table with a recursive CTE matview,
//! 8 root nodes with chains of 20 children each, then INSERT OR REPLACE all
//! rows. No CDC, no other matviews, no OR conditions needed.
//!
//! **Confirmed** via:
//! - This Rust example (100% repro)
//! - Pure SQL via holon-live MCP (100% repro, even with 2 rows when other
//!   matviews exist)
