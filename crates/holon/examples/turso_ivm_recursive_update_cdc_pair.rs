...
//! ## Expected — either of:
//!
//! - **(A) ideal** — one `Update` event: ```text [matview CDC] descendants_view
//!   changes=1 [0] Update id='child_1' ```
...
//! - Holon-side handoff: `HANDOFF_TURSO_RECURSIVE_CTE_UPDATE_CDC.md`
    ...
    match kinds.as_slice() {
        ...
        ["Delete", "Insert"] => println!(
            "OK-ish: matview emitted Delete-then-Insert pair (Debezium-style).\nDownstream \
             coalescers can fold this to Update."
        ),
        ["Insert", "Delete"] => println!(
            "BUG: matview emitted Insert-then-Delete pair on the same key.\nOrder-aware \
             coalescers will treat this as a transient no-op\nand DROP the user-visible content \
             change."
        ),
        [] => println!(
