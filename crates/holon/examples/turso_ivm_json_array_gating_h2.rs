...
//!   - "Edge" mutations are UPDATEs on a single column, not INSERT/DELETE on a
//!     junction table.
//!
...
//! P3 UPDATE blocker→DONE   — set B.status='DONE' (matview row A→B should
//! drop). P4 UPDATE blocked_by=[]  — clear A.blocked_by (matview row A→C should
//! drop). P5 DELETE blocked task   — restore A→C, then DELETE A (matview should
//! drop). P6 DELETE blocker task   — recreate A→C, then DELETE C (dangling JSON
//! ref).
//!
...
async fn test_raw_query_fallback() -> anyhow::Result<bool> {
    ...
    conn.execute(
        "INSERT INTO task (id, status, blocked_by) VALUES ('A', 'TODO', '[\"B\",\"C\"]'), ('B', \
         'DONE', NULL), ('C', 'TODO', NULL)",
        (),
    )
    .await?;

    // Raw gating query — the form a non-IVM read path would use.
    let raw_sql = "SELECT t.id, j.value, b.status FROM task t, json_each(COALESCE(t.blocked_by, \
                   '[]')) j JOIN task b ON b.id = j.value WHERE b.status != 'DONE' ORDER BY t.id, \
                   j.value";
