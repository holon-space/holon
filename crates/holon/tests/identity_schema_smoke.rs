...
//!
//! Verifies that `IdentitySchemaModule::ensure_schema` creates
//! `canonical_entity`, `entity_alias`, and `proposal_queue` tables with the
//! expected shape, and that a basic insert + select round-trips correctly.
//! Operations (merge_entities, propose_merge, accept_proposal, reject_proposal)
//! land in a follow-up step; this test pins the schema surface so future
//! operation work can rely on it.
...
async fn identity_tables_round_trip_basic_inserts() {
    ...
    handle
        .execute(
            "INSERT INTO canonical_entity (id, kind, primary_label, created_at) VALUES \
             ('canon:1', 'person', 'Sarah Chen', 1)",
            vec![],
        )
        .await
    ...
    handle
        .execute(
            "INSERT INTO proposal_queue (id, kind, evidence_json, status, created_at) VALUES (1, \
             'merge', '{\"a\":\"x\",\"b\":\"y\"}', 'pending', 1)",
            vec![],
        )
        .await
...
async fn entity_alias_enforces_system_foreign_id_primary_key() {
    ...
    handle
        .execute(
            "INSERT INTO canonical_entity (id, kind, primary_label, created_at) VALUES \
             ('canon:1', 'person', 'A', 1), ('canon:2', 'person', 'B', 1)",
            vec![],
        )
        .await
