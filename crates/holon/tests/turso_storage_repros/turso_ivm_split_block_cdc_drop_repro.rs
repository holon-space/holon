...
//!   regression. INSERT + UPDATE on different rows; only the Created event
//!   fires.
//! - `split_block_update_then_insert` — order doesn't matter.
//! - `split_block_insert_sleep_update` — even with 300ms between writes so the
//!   IVM has its own tick, the second UPDATE produces 0 events.
//! - `recursive_matview_handles_update_alone` — *just* an UPDATE on a recursive
//!   matview, no INSERT in sight: still 0 events. So the bug is **UPDATE
//!   through a recursive CTE matview** in general, not the INSERT + UPDATE
//!   interaction. The split_block scenario is just the most user-visible
//!   incarnation.
//! - `recursive_matview_delete_then_insert_workaround` — DELETE + INSERT for
//!   the same id **does** fire two CDC events (Deleted then Created). After
//!   `coalesce_row_changes`, downstream sees an Updated. Viable
//!   application-level workaround.
//! - `split_block_nonrecursive_matview` — control: a plain `WHERE parent_id =
//!   'doc'` matview emits both events correctly. So the bug is specific to
//!   `WITH RECURSIVE`, not matviews in general.
//!
//! Run with:
...
async fn split_block_emits_both_insert_and_update_events() {
    ...
    handle
        .execute(
            "INSERT INTO block (id, content, parent_id, sort_key) VALUES ('child_1', 'one two \
             three', 'doc', 'B0')",
            vec![],
        )
        .await
    ...
    handle
        .execute(
            "INSERT INTO block (id, content, parent_id, sort_key) VALUES ('child_2', 'tail', \
             'doc', 'C0')",
            vec![],
        )
        .await
    ...
    handle
        .execute(
            "INSERT INTO block (id, content, parent_id, sort_key) VALUES ('child_1_split', 'two \
             three', 'doc', 'B5')",
            vec![],
        )
        .await
    ...
    let rows = handle
        .query(
            "SELECT id, content FROM descendants_view WHERE id IN ('child_1', 'child_1_split') \
             ORDER BY id",
            HashMap::new(),
        )
        .await
...
async fn split_block_update_then_insert() {
    ...
    handle
        .execute_ddl(
            "CREATE TABLE block (id TEXT PRIMARY KEY, content TEXT DEFAULT '', content_type TEXT \
             DEFAULT 'text', parent_id TEXT DEFAULT '', sort_key TEXT DEFAULT '', properties TEXT \
             DEFAULT '{}')",
        )
        .await
        .unwrap();
...
async fn recursive_matview_delete_then_insert_workaround() {
    ...
    handle
        .execute(
            "INSERT INTO block (id, content, parent_id, sort_key) VALUES ('child_1', 'one', \
             'doc', 'B0')",
            vec![],
        )
        .await
