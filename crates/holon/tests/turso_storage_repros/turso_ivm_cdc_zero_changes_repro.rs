//! Minimal reproducer: Turso IVM CDC doesn't deliver changes after UPDATE on
//! base table
//!
//! Steps:
...
async fn matview_cdc_after_update_simple() {
    ...
    assert!(
        matview_events > 0,
        "BUG: UPDATE block SET content='updated' affected 1 row, but matview CDC delivered 0 \
         events. The matview query returns '{}'. Turso IVM fires the CDC callback but with an \
         empty changeset.",
        mv_content
    );
}
...
async fn matview_cdc_after_update_full_schema() {
    ...
    for i in 0..5 {
        handle
            .execute(
                &format!(
                    "INSERT INTO block (id, parent_id, document_id, content) VALUES ('block:b{}', \
                     'doc:root', 'doc:d1', 'content {}')",
                    i, i
                ),
                vec![],
            )
    ...
    handle
        .execute_ddl(
            "CREATE MATERIALIZED VIEW IF NOT EXISTS mv_full AS SELECT id, content, content_type, \
             source_language, parent_id FROM block",
        )
        .await
        .unwrap();
    ...
    assert!(
        matview_events > 0,
        "BUG: UPDATE affected 1 row but matview CDC delivered 0 events. Matview query returns \
         '{}' (expected 'CHANGED').",
        mv_content
    );
}
