...
//!   cargo test -p holon turso_matview_test::test_create_matview_normal_async
//! -- --nocapture   cargo test -p holon
//! turso_matview_test::test_create_matview_block_in_place -- --nocapture
//!
//! Run with timeout to prevent hang:
//!   timeout 30 cargo test -p holon
//! turso_matview_test::test_create_matview_block_in_place -- --nocapture
...
async fn test_create_matview_normal_async() {
    ...
    db.execute_ddl(
        "CREATE MATERIALIZED VIEW IF NOT EXISTS events_view_block AS SELECT * FROM events WHERE \
         status = 'confirmed'",
    )
    .await
    .unwrap();
...
async fn test_create_matview_block_in_place() {
    ...
    tokio::task::block_in_place(|| {
        tokio::runtime::Handle::current().block_on(async {
            ...
            db.execute_ddl(
                "CREATE MATERIALIZED VIEW IF NOT EXISTS events_view_block AS SELECT * FROM events \
...
async fn test_create_matview_separate_block_in_place() {
    ...
    // Second block_in_place: Create view (simulates CacheEventSubscriber::start ->
    // subscribe)
    {
        let backend_clone = Arc::clone(&backend);
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                ...
                db.execute_ddl(
                    "CREATE MATERIALIZED VIEW IF NOT EXISTS events_view_block AS SELECT * FROM \
                     events WHERE status = 'confirmed'",
                )
                .await
                .unwrap();
...
/// Documents a known Turso IVM limitation: `WHERE 1=1` is rejected because the
/// IVM filter parser only accepts `column op value` or `column op column`.
/// Production code works around this (see turso_event_bus.rs:364-392).
#[ignore = "Known Turso IVM limitation: WHERE 1=1 not supported in materialized views"]
#[tokio::test(flavor = "multi_thread")]
    ...
    // subscribe)
    {
        ...
        tokio::task::block_in_place(|| {
            tokio::runtime::Handle::current().block_on(async {
                ...
                db.execute_ddl(
                    "CREATE MATERIALIZED VIEW IF NOT EXISTS events_view_block AS SELECT * FROM \
                     events WHERE 1=1 AND status IN ('confirmed') AND aggregate_type IN ('block')",
                )
                .await
                .unwrap();
...
/// This simulates the DI scenario where tasks are spawned before subscribe() is
/// called
...
async fn test_create_matview_with_spawned_tasks() {
    ...
    // Now try to create materialized view in block_in_place (simulating
    // CacheEventSubscriber::start)
    tokio::task::block_in_place(|| {
        tokio::runtime::Handle::current().block_on(async {
            let backend_guard = backend.read().await;
            let db = backend_guard.handle();
        ...
        tokio::runtime::Handle::current().block_on(async {
            ...
            tokio::task::block_in_place(|| {
                tokio::runtime::Handle::current().block_on(async {
                    ...
                    db.execute_ddl(
                        "CREATE MATERIALIZED VIEW IF NOT EXISTS events_view_block AS SELECT * \
