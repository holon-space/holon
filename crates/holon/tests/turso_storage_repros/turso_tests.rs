...
//
// - `cdc_tests` - Basic CDC insert/update/delete tracking Now covered by PBT
//   with CDC operations and verification
//
// - `incremental_view_maintenance_tests` - Materialized view operations Now
//   covered by PBT with CreateMaterializedView transitions
//
// - `turso_materialized_view_tests` - More materialized view tests Now covered
//   by PBT with random operation sequences on views
//
// - `turso_cdc_with_materialized_views_tests` - CDC integration with views Now
//   covered by PBT which tests CDC alongside view operations
//
// - `view_change_stream_tests` - View change notifications Now covered by PBT
//   with CreateViewStream transitions
//
// - `filter_building_tests` - All filter types (Eq, In, And, Or, IsNull,
//   IsNotNull) Now covered by PBT through random query generation
//
// The PBT suite runs 30 test cases with random sequences of 1-50 operations
// each, providing much more comprehensive coverage than the individual unit
// tests.
//
// ============================================================================
...
// - `value_conversion_tests` - Value type conversion edge cases with property
//   tests
...
mod view_cdc_tests {
    ...
    async fn test_view_cdc_filtered_view_with_trigger() {
        ...
        let schema = TypeDefinition::new(
            ...
            vec![
                FieldSchema::new("id", "TEXT").primary_key().indexed(),
                FieldSchema::new("category", "TEXT").indexed(),
        ...
        backend
            .handle()
            .execute_ddl(
                "CREATE VIEW high_priority_bugs AS SELECT * FROM view_filter_base WHERE category \
                 = 'bugs' AND priority >= 3",
            )
            .await
            .unwrap();
...
mod concurrent_ddl_dml_tests {
    ...
    async fn test_concurrent_ddl_and_dml_via_schema_registry() {
        ...
        let schema = TypeDefinition::new(
            ...
            vec![
                FieldSchema::new("id", "TEXT").primary_key().indexed(),
                FieldSchema::new("parent_id", "TEXT").nullable().indexed(),
        ...
        for i in 0..10 {
            ...
            dml_tasks.push(tokio::spawn(async move {
                let sql = format!(
                    "INSERT INTO base_concurrent (id, parent_id, content) VALUES ('item-{}', \
                     'root', 'content-{}')",
                    i, i
                );
                h.execute(&sql, vec![]).await
    ...
    async fn test_concurrent_updates_on_same_row() {
        ...
        let schema = TypeDefinition::new(
            ...
            vec![
                FieldSchema::new("id", "TEXT").primary_key().indexed(),
                FieldSchema::new("counter", "INTEGER"),
            ],
        );
        backend.create_entity(&schema).await.unwrap();
    ...
    async fn test_materialized_view_after_concurrent_writes() {
        ...
        for i in 0..20 {
            ...
            tasks.push(tokio::spawn(async move {
                let sql = format!(
                    "INSERT INTO mv_base (id, parent_id, content) VALUES ('item-{}', '{}', \
                     'data-{}')",
                    i, parent_owned, i
                );
                h.execute(&sql, vec![]).await
