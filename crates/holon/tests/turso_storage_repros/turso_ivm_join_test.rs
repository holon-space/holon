...
//!
//! Run with:
//!   cargo test -p holon turso_ivm_join_test::test_matview_with_join --
//! --nocapture
//!
//! The panic occurs at:
...
/// Run with: cargo test -p holon
/// turso_ivm_join_test::test_join_matview_insert_panics -- --nocapture
#[tokio::test]
...
///
/// Run with: cargo test -p holon
/// turso_ivm_join_test::test_navigation_schema_panics -- --nocapture
#[tokio::test]
...
/// REPRODUCER: Watch query with JOIN panics on base table insert (with
/// transaction)
///
...
///
/// Run with: cargo test -p holon
/// turso_ivm_join_test::test_watch_query_join_insert_in_transaction --
/// --nocapture
#[tokio::test]
...
///
/// Run with: cargo test -p holon
/// turso_ivm_join_test::test_watch_query_join_insert_panics -- --nocapture
#[tokio::test]
...
///
/// Run with: cargo test -p holon
/// turso_ivm_join_test::test_full_holon_schema_panic -- --nocapture
#[tokio::test]
async fn test_full_holon_schema_panic() {
    ...
    db.execute_ddl(
        ...
        SELECT blocks.id, blocks.parent_id, blocks.content, blocks.content_type, blocks.id AS \
         sort_key
        FROM blocks
...
///
/// The bug is: when a matview (watch_view) JOINs with another matview
/// (current_focus), and that other matview also has a JOIN internally,
/// inserting into the base table panics.
///
/// Scenario:
/// 1. current_focus = MATVIEW with JOIN (navigation_cursor JOIN
///    navigation_history)
/// 2. watch_view = MATVIEW with JOIN (blocks JOIN current_focus)
/// 3. INSERT into blocks -> PANIC!
///
/// Run with: cargo test -p holon
/// turso_ivm_join_test::test_nested_matview_joins_panic -- --nocapture
#[tokio::test]
...
///
/// Run with: cargo test -p holon
/// turso_ivm_join_test::test_multiple_matviews_with_join -- --nocapture
#[tokio::test]
