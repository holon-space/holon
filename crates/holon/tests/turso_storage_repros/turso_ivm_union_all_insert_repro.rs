//! Minimal reproducer: Turso IVM CDC missing for chained UNION ALL matview
//! after INSERT
//!
//! Scenario (mirrors production `navigation.sql`):
//!   1. Tables: navigation_history, navigation_cursor, block
//!   2. Matview chain: current_focus (joins cursor+history) → focus_roots
//!      (joins current_focus+block)
...
/// Chained case: focus_roots depends on current_focus matview — CDC may be
/// missing
#[tokio::test]
async fn union_all_matview_cdc_after_insert_chained() {
    ...
    handle
        .execute(
            "INSERT INTO navigation_history (id, region, block_id) VALUES (1, 'left_sidebar', \
             'b1')",
            vec![],
        )
        .await
    ...
    assert!(
        ...
        "BUG: INSERT INTO block with parent_id matching the focus target should trigger CDC for \
         the chained focus_roots matview, but 0 events were delivered. The matview query \
         correctly returns {:?}, proving IVM updated the data but didn't fire CDC. Simple \
         (non-chained) case works fine.",
        after_ids
    );
}

/// Double-chained case: a watch matview on top of focus_roots
/// (matview-on-matview-on-matview). This matches what `query_and_watch()` does:
/// it creates a matview like `SELECT ... FROM focus_roots fr JOIN block b ON
...
async fn watch_matview_on_focus_roots_cdc_after_insert() {
    ...
    // Create the "watch" matview on top of focus_roots (what query_and_watch
    // creates)
    handle
        .execute_ddl(
    ...
    assert!(
        watch_events > 0,
        "BUG: INSERT INTO block should propagate through the matview chain (block → focus_roots → \
         mv_region_watch) and trigger CDC for the watch matview. Got 0 events. focus_roots got {} \
         events. The watch query correctly returns {:?}.",
        focus_roots_events,
        after_ids
    );
}
