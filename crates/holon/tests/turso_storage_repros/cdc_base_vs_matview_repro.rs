...
//!
//! - Direct `INSERT`/`UPDATE`/`DELETE` against a base table emits **zero** CDC
//!   batches with `relation_name == <table>` on the broadcast channel.
...
/// Counts of (Created, Updated, Deleted) for a given relation across the
/// events.
...
async fn setup() -> (TempBackend, RawCdcRx) {
    ...
    handle
        .execute_ddl(
            "CREATE MATERIALIZED VIEW block_mirror AS SELECT id, content, content_type, \
             parent_id, sort_key, properties FROM block",
        )
        .await
        .unwrap();
...
async fn base_table_writes_only_emit_via_matview() {
    ...
    be.handle
        .execute(
            "INSERT INTO block (id, content, parent_id, sort_key) VALUES ('blk-1', 'hello', '', \
             'A0')",
            vec![],
        )
        .await
...
async fn matview_created_event_carries_full_row() {
    ...
    be.handle
        .execute(
            "INSERT INTO block (id, content, parent_id, sort_key) VALUES ('blk-2', 'lorem ipsum', \
             'doc-1', 'B0')",
            vec![],
        )
        .await
...
async fn transactional_batch_emits_only_via_matview() {
    ...
    be.handle
        .transaction(vec![
            (
                "INSERT INTO block (id, content, parent_id, sort_key) VALUES ('tx-a', 'A', '', \
                 'A0')"
                    .into(),
                vec![],
            ),
            (
                "INSERT INTO block (id, content, parent_id, sort_key) VALUES ('tx-b', 'B', '', \
                 'B0')"
                    .into(),
                vec![],
            ),
            (
                "INSERT INTO block (id, content, parent_id, sort_key) VALUES ('tx-c', 'C', '', \
                 'C0')"
                    .into(),
                vec![],
            ),
