//! Stateful property-based tests comparing two DocumentRepository
//! implementations
//!
...
mod stateful_tests {
            ...
            match transition {
                BlockTransition::WatchChanges { watcher_id } => {
                    ...
                    let stream = match state._runtime.block_on(async {
                        ...
                        Ok(s) => s,
                        Err(_) => panic!(
                            "Timed out creating watcher stream. Likely synchronous replay prefill \
                             into a bounded channel in MemoryBackend::watch_changes_since."
                        ),
                    };
                ...
                _ => {
                    state
                        ...
                        .expect(
                            "Reference backend transition should succeed (preconditions validated \
                             it)",
                        );
                    ...
                    for watcher_id in watcher_ids {
                            ...
                            && let Some(descriptor) = state.watchers.get(&watcher_id)
                        {
                            ...
                            let stream = match state._runtime.block_on(async {
                                ...
                                Ok(s) => s,
                                Err(_) => panic!(
                                    "Timed out rehydrating watcher {}. Likely bounded replay \
                                     prefill deadlock.",
                                    watcher_id
                                ),
                            };
                        ...
                        if let Some(stream) = state.live_streams.get_mut(&watcher_id) {
                            ...
                            loop {
                                if event_count >= MAX_EVENTS_PER_WATCHER {
                                    panic!(
                                        "Watcher {} received {} events without timeout - possible \
                                         infinite loop",
    ...
    impl StateMachineTest for BlockTreeTest<LoroBackend> {
            ...
            match &sut_transition {
                BlockTransition::WatchChanges { watcher_id } => {
                    ...
                    let mut stream = runtime.block_on(async {
                        ...
                        .await
                        .expect(
                            "Timed out creating SUT watcher stream. Likely bounded channel replay \
                             deadlock - watch_changes_since should spawn a task for replay \
        ...
        fn check_invariants(
            ...
            // Compare reference backend (MemoryBackend) with system under test
            // (LoroBackend)
            ...
            for (watcher_id, ref_descriptor) in &ref_state.watchers {
                ...
                assert_eq!(
                    ref_changes.len(),
                    sut_changes.len(),
                    "Reference and SUT should receive same number of change \
                     notifications.\nReference: {} changes\nSUT: {} changes",
                    ref_changes.len(),
                    sut_changes.len()
                );
                ...
                for ref_change in ref_changes.iter() {
                    let matched = match ref_change {
                        Change::Created {
                            ...
                            // Find matching SUT change by content and parent_id (after ID
                            // translation)
...
mod custom_properties_round_trip {
    ...
    async fn later_property_update_merges_with_earlier() {
        ...
        assert_eq!(
            p.get("effort"),
            Some(&Value::String("7yzXz".into())),
            "effort lost after a later update_block_properties call that did not mention effort. \
             Full props: {p:?}"
        );
    }
    ...
    async fn snapshot_blocks_from_doc_preserves_custom_properties() {
        ...
        assert_eq!(
            p.get("effort"),
            Some(&Value::String("7yzXz".into())),
            "effort missing from snapshot_blocks_from_doc output — LoroSyncController outbound \
             diff would emit an UPDATE with stripped properties. Full props: {p:?}"
        );
        assert_eq!(p.get("task_state"), Some(&Value::String("WAITING".into())));
    }
