...
//! feed re-extracts its `[[...]]` links into `block_link`; a removal drops them.

use std::collections::HashMap;
use std::sync::Arc;
use std::time::{Duration, Instant};

use holon::storage::turso::{DbHandle, TursoBackend};
use holon::sync::link_event_subscriber::LinkEventSubscriber;
use holon::sync::live_data::LiveData;
use holon_api::block::Block;
use holon_api::{Change, EntityUri};
...
/// Poll `block_link` until `source`'s targets equal `expected` (order-insensitive),
/// or panic after `timeout`. The subscriber processes the feed on a spawned task,
/// so the write is eventually-consistent.
