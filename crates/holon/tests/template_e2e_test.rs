...
const TEMPLATES_ORG: &str =
    include_str!("../../../assets/default/Templates.org");
...
fn instantiate_params(
    ...
    params.insert(
        "target_parent".into(),
        Value::String(target_parent.into()),
    );
...
async fn query_by_id(
    engine: &BackendEngine,
    id: &str,
) -> Option<holon_api::StorageEntity> {
    let rows = engine
        .db_handle()
        .query(
            &format!(
...
async fn plan_my_day_instantiates_with_no_variables_and_preserves_marks() {
    ...
    engine
        .execute_operation(
            ...
            "create",
            {
                ...
                p.insert(
                    "id".into(),
                    Value::String("block:__templates__".into()),
                );
                p.insert(
                    "content".into(),
    ...
    let template_root_ids =
        seed_templates(&engine, "__templates__").await;
    ...
    let context_key = format!("pmd-{}", std::time::SystemTime::now()
    ...
    assert_eq!(root_props["instance_of"].as_str(), Some(plan_my_day_id.as_str()));
    assert!(
...
async fn new_month_substitutes_date_and_is_idempotent() {
    ...
    let root_props = props_of(&root);
    assert_eq!(root_props["instance_of"].as_str(), Some(new_month_id.as_str()));
    ...
    let tasks_children =
        query_children(&engine, &str_field(&tasks_heading, "id")).await;
    assert!(
        !tasks_children.is_empty(),
        "Tasks must have TODO children"
    );
    let first_task = &tasks_children[0];
    let ft_props = props_of(first_task);
    assert_eq!(
        ft_props.get("task_state")
            .and_then(|v| v.as_str()),
        Some("TODO"),
    ...
    let buchungen_children =
        query_children(&engine, &str_field(&buchungen, "id")).await;
    ...
    let lex_children =
        query_children(&engine, &str_field(&lex_office, "id")).await;
