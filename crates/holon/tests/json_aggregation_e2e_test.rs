...
/// Strip render directives from PRQL before compilation.
///
/// Removes:
/// - Top-level `render (...)` lines
/// - `derive { ... = (render ...) }` lines (inline render expressions)
fn strip_render_directives(prql: &str) -> String {
    prql.lines()
        .filter(|line| {
            let trimmed = line.trim();
            !trimmed.starts_with("render ")
                && !trimmed.starts_with("render(")
                && !(trimmed.starts_with("derive") && trimmed.contains("(render "))
        })
        .collect::<Vec<_>>()
        .join("\n")
}
...
async fn setup_test_schema(engine: &Arc<BackendEngine>) -> Result<()> {
    ...
    engine
        ...
        .execute(
            r#"
        CREATE TABLE IF NOT EXISTS todoist_project (
            id TEXT PRIMARY KEY,
            name TEXT NOT NULL,
            parent_id TEXT,
            color TEXT,
    ...
    engine
        ...
        .execute(
            r#"
        CREATE TABLE IF NOT EXISTS todoist_task (
            id TEXT PRIMARY KEY,
...
async fn insert_test_data(engine: &Arc<BackendEngine>) -> Result<()> {
    ...
    engine
        .db_handle()
        .execute(
            "INSERT INTO todoist_project (id, name, parent_id, color, is_favorite, is_archived) \
             VALUES ('proj-1', 'Project 1', NULL, 'red', 0, 0)",
            vec![],
        )
        .await?;
    engine
        ...
        .execute(
            "INSERT INTO todoist_project (id, name, parent_id, color, is_favorite, is_archived) \
             VALUES ('proj-2', 'Project 2', 'proj-1', 'blue', 1, 0)",
            vec![],
        )
        .await?;
    ...
    engine
        .db_handle()
        .execute(
            "INSERT INTO todoist_task (id, content, parent_id, project_id, priority, completed, \
             is_deleted) VALUES ('task-1', 'Task 1', NULL, 'proj-1', 2, 0, 0)",
            vec![],
        )
        .await?;
    engine
        ...
        .execute(
            "INSERT INTO todoist_task (id, content, parent_id, project_id, priority, completed, \
             is_deleted) VALUES ('task-2', 'Task 2', 'task-1', 'proj-1', 1, 1, 0)",
            vec![],
        )
        .await?;
...
append (
    ...
    for row in &results {
        ...
        assert!(
            entity_name == "todoist_project" || entity_name == "todoist_task",
...
/// Test the JsonAggregationTransformer automatically injects json_object for
/// UNION queries Uses simplified `select { this.* }` instead of explicit column
/// lists
#[tokio::test]
async fn test_json_aggregation_transformer_auto_injection() -> Result<()> {
    ...
    engine
        .db_handle()
        .execute(
            "INSERT INTO projects (id, name, description) VALUES ('proj-1', 'Project A', \
             'Description A')",
            vec![],
        )
        .await?;
...
/// Test that JsonAggregationTransformer includes DERIVED columns (not just base
/// table columns) This test uses SIMPLIFIED select statements - the
/// ColumnPreservationTransformer converts them to `select { this.* }`
/// automatically
#[tokio::test]
...
async fn test_json_aggregation_includes_all_base_columns() -> Result<()> {
    ...
    // To include base columns in the json_object, they must be explicitly
    // referenced.
    //
...
append (
    ...
    // The SQL should use json_object (JsonAggregationTransformer converts to
    // json_object)
    assert!(
        ...
        Err(e) => {
            ...
            for i in 1..=10 {
                let single_sql = format!(
                    "SELECT id, title, json_object('id', id, 'title', title, 'content', content) \
                     as data FROM test_special WHERE id = 'row-{}'",
                    i
                );
                match engine
