...
/// Global widget name registry. Set once at startup by the frontend via
/// `register_widget_names()`. Callers of `parse_render_dsl()` that can't
/// thread names explicitly use `registered_widget_names()` to read this.
static WIDGET_NAMES: OnceLock<Vec<String>> = OnceLock::new();

/// Register the authoritative list of widget names for the render DSL parser.
///
/// Called once at startup by the frontend module, passing the auto-generated
...
/// Get the registered widget names.
///
/// Panics if `register_widget_names()` has not been called. This is
/// intentional — callers must wire up registration at startup. Silent
/// fallbacks mask wiring bugs.
pub fn registered_widget_names() -> &'static [String] {
    WIDGET_NAMES
        .get()
        .expect("register_widget_names() must be called before any render DSL parsing")
}

/// Parse a render DSL string into a RenderExpr.
pub fn parse_render_dsl(source: &str) -> Result<RenderExpr> {
    parse_render_dsl_with_engine(source, &create_render_engine())
}
...
pub fn parse_render_dsl_with_names(source: &str, names: &[&str]) -> Result<RenderExpr> {
    parse_render_dsl_with_engine(source, &create_render_engine_with_names(names))
}
...
///
/// Widget names come from `registered_widget_names()` (set at startup by the
/// frontend via `register_widget_names()`). Falls back to an empty set if
/// called before registration — tests should call `register_widget_names()`
/// or use `create_render_engine_with_names()` directly.
