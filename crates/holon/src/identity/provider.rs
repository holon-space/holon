...
//! (`restore_canonical_after_merge`, `delete_proposal`,
//! `revert_proposal_status`) are listed in `operations()` so the dispatcher can
//! route inverse executions; they are not intended as user-facing surfaces.
//!
...
impl IdentityProvider {
    ...
    async fn merge_entities(
        ...
        // 2. Verify canonical_b exists (otherwise the UPDATE silently rewrites aliases
        //    to a phantom canonical_id).
        ...
        let alias_rows = self
            .db_handle
            .query(
                "SELECT system, foreign_id, confidence FROM entity_alias WHERE canonical_id = $id",
                params.clone(),
            )
            .await
    ...
    async fn restore_canonical_after_merge(
        ...
        self.db_handle
            .query(
                "INSERT INTO canonical_entity (id, kind, primary_label, created_at) VALUES ($id, \
                 $kind, $primary_label, $created_at)",
                insert_params,
            )
            .await
        ...
        for alias in &aliases {
            ...
            self.db_handle
                .query(
                    "UPDATE entity_alias SET canonical_id = $id, confidence = $confidence WHERE \
                     system = $system AND foreign_id = $foreign_id",
                    alias_params,
                )
                .await
    ...
    async fn propose_merge(&self, params: &StorageEntity) -> Result<OperationResult> {
        ...
        self.db_handle
            .query(
                "INSERT INTO proposal_queue (id, kind, evidence_json, status, created_at) VALUES \
                 ($id, $kind, $evidence_json, 'pending', $created_at)",
                insert,
            )
            .await
    ...
    async fn delete_proposal(&self, params: &StorageEntity) -> Result<OperationResult> {
        ...
        let rows = self
            .db_handle
            .query(
                "SELECT kind, evidence_json, status, created_at FROM proposal_queue WHERE id = $id",
                sel.clone(),
            )
            .await
    ...
    async fn restore_proposal(&self, params: &StorageEntity) -> Result<OperationResult> {
        ...
        self.db_handle
            .query(
                "INSERT INTO proposal_queue (id, kind, evidence_json, status, created_at) VALUES \
                 ($id, $kind, $evidence_json, $status, $created_at)",
                insert,
            )
            .await
