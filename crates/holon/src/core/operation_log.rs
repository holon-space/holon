...
impl OperationLogStore {
    ...
    pub async fn initialize_schema(&self) -> Result<()> {
        let td = OperationLogEntry::type_definition();
        let create_table_sql = td.to_create_table_sql();
        let index_sqls = td.to_index_sql();
...
impl OperationLogOperations for OperationLogStore {
    async fn log_operation(&self, operation: Operation, inverse: UndoAction) -> Result<i64> {
        ...
        let id = id_result
            ...
            .and_then(|v| v.as_i64())
            .ok_or("Failed to get inserted operation ID from RETURNING clause")?;

        // Amortize trim: only check every 10th operation to avoid per-op COUNT(*).
        if id % 10 == 0 {
