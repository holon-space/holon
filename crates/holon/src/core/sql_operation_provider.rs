...
impl SqlOperationProvider {
    ...
    async fn prepare_update(&self, params: &StorageEntity) -> Option<PreparedOp> {
        let id = params
            .get("id")
            .and_then(|v| v.as_string())
        ...
        if !extra_props.is_empty() {
            ...
            let mut existing: serde_json::Map<String, serde_json::Value> = self
                .db_handle
                .query(&select_props_sql, HashMap::new())
                .await
                .ok()
                .and_then(|rows| rows.into_iter().next())
                .and_then(|row| row.get("properties").cloned())
                .and_then(|v| match v {
                    Value::String(s) => serde_json::from_str(&s).ok(),
                    Value::Object(m) => {
                        Some(m.into_iter().map(|(k, v)| (k, value_to_json(&v))).collect())
                    }
                    _ => None,
                })
                .unwrap_or_default();
            ...
            let merged_json =
                serde_json::to_string(&existing).expect("merged properties must serialize");
        ...
        // Don't include events in the PreparedOp — the event needs the full row
        // which is only available AFTER the SQL UPDATE runs. The caller
        // (execute_operation for "update") publishes the event after execution.
        let _routing_doc_uri = self.find_document_uri(&id).await;
