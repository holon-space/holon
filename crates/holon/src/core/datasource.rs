...
pub trait OperationProvider: Send + Sync {
    ...
    ///
    /// Semantically equivalent to calling `execute_operation` for each entry,
    /// but implementations may wrap the batch in a single transaction for
    /// atomicity and to reduce CDC/IVM churn.
    ///
    ...
    /// specific `EventOrigin`. Used by sync controllers that need the events
    /// to carry their identity so the inverse direction can skip echoes.
    ///
    /// Default implementation ignores the origin and delegates to
    /// `execute_batch`. Implementations that publish events should override
    /// this to thread `origin` into event construction.
