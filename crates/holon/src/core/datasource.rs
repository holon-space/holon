...
/// Provider of entity schemas for GQL graph registration.
///
/// Implementations contribute `EntitySchema`s that are collected via DI
/// (`get_all_trait::<dyn EntitySchemaProvider>()`) and registered in the
/// `GraphSchemaRegistry` to build the GQL `GraphSchema`.
///
/// Compile-time entities (Block, Document, TodoistTask, etc.) implement this
/// via a simple provider struct. Runtime entities (MCP) use the dynamic
/// registration path on `BackendEngine` instead.
/// flutter_rust_bridge:ignore
pub trait EntitySchemaProvider: Send + Sync {
