<<<<<<< conflict 1 of 1
+++++++ rlkumzum 65e4d4e8 (rebase destination)
            ...
            tokio::spawn(async move {
                ...
                while let Some(result) = stream.next().await {
                    match result {
                            ...
                            for change in changes {
                                match change {
                                    Change::Created { data, .. } | Change::Updated { data, .. } => {
                                        ...
                                        {
                                            tracing::debug!("Error ingesting change: {}", e);
                                        }
                                    }
                                    Change::Deleted { id, .. } => {
                                        let db_guard = db.write().await;
                                        if let Err(e) = db_guard.delete(&table, &id).await {
                                            tracing::debug!("Error ingesting delete: {}", e);
                                        }
                                    }
                        ...
                        Err(e) => {
                            tracing::debug!("Error in change stream: {:?}", e);
            ...
            loop {
                match rx.recv().await {
                    ...
                    Err(broadcast::error::RecvError::Lagged(n)) => {
                        tracing::debug!("Stream lagged by {} messages, triggering resync", n);
                            ...
                            for change in &changes_clone {
                                match change {
                                    ...
                                    Change::Deleted { id, .. } => {
                                        let db_guard = db.write().await;
                                        if let Err(e) = db_guard.delete(&table, id).await {
                                            tracing::debug!("Error deleting from cache: {}", e);
                                        }
                                    }
                                    Change::FieldsChanged {
                                        entity_id, fields, ..
...
------- tznoqmkw efae88b1 "test(pbt): TestPlatform sim PBT — fix signal delivery, settle, input fidelity, leak-on-drop" (parents of rebased revision)
//! QueryableCache as transparent proxy for stream-based external system integration
//!
//! QueryableCache wraps a CrudOperations and provides:
//! - Read operations from local cache (fast)
...
+++++++ omzrrnyy ae9a4a94 "review: apply 60+ adversarially-verified fixes from the 199-agent review fleet" (rebased revision)
------- tznoqmkw efae88b1 "test(pbt): TestPlatform sim PBT — fix signal delivery, settle, input fidelity, leak-on-drop" (parents of rebased revision)
//! QueryableCache as transparent proxy for stream-based external system integration
//!
//! QueryableCache wraps a CrudOperations and provides:
...
+++++++ rrlyvlkz 03b51ebe (rebased revision)
