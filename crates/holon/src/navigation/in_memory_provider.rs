...
//! local-only by design (see the
//! `loro_paths_do_not_reference_navigation_tables` regression test in `mod.rs`)
//! — it is never replicated. Holding it in memory for a Loro session, and
//! letting it live for the lifetime of the process (it need not be reset across
//! a restart-free session boundary), matches that intent.
//!
...
use super::provider::ENTITY_NAME;
use super::provider::navigation_operation_descriptors;
