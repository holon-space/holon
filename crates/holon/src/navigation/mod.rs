...
//!
//! This module provides navigation operations that work through Turso's IVM
//! (Incremental View Maintenance). Navigation state is stored in database
//! tables, and queries JOIN against `current_focus` view to automatically
...
//! ## Schema Management
//!
//! Navigation schema is managed by `NavigationSchemaModule` via the
//! `SchemaRegistry`. See holon-turso `schema_modules.rs` for the schema
//! definition.
...
pub use provider::navigation_operation_descriptors;
    ...
    fn loro_paths_do_not_reference_navigation_tables() {
        ...
        for (file, src) in sources {
            for needle in FORBIDDEN {
                assert!(
                    !src.contains(needle),
                    "Loro source `{file}` references local-only nav table `{needle}` — navigation \
                     state is per-device by design (pinned tabs, back/forward history). If you \
                     intended to replicate, add the table to the proper replicated-tables list \
                     and remove it from this test's FORBIDDEN list after a deliberate decision."
                );
            }
        }
    }
