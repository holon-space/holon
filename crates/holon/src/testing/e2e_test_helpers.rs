...
///
/// Wraps [`HolonService`] and adds test-only construction helpers
/// (in-memory databases, custom providers) and stream assertion utilities.
...
impl E2ETestContext {
    ...
    /// Access the underlying `HolonService`.
    pub fn service(&self) -> &HolonService {
        &self.service
    }

    /// Compile and execute a query, returning data rows.
    pub async fn query(
        &self,
        source: String,
    ...
    /// The first batch in the stream contains the initial query results.
    pub async fn query_and_watch(
        ...
        self.service
            .query_and_watch(&source, language, params, None)
            .await
    }

    /// Execute an operation on the specified entity.
    pub async fn execute_op(
        ...
        self.service
            .execute_operation(&entity_name.into(), op_name, params)
            .await
            .map(|_| ())
    }
