...
//!         db_handle.execute_ddl(BLOCK_HIERARCHY_SQL).await
//!     }
//! }
//! ```
...
pub trait SchemaModule: Send + Sync {
    ...
    ///
    /// The db_handle is provided with write access for DDL operations.
    async fn ensure_schema(&self, db_handle: &DbHandle) -> Result<()>;
...
/// registry.initialize_all(&db_handle, vec![]).await?;
/// ```
...
impl SchemaRegistry {
    ...
    ///
    /// # Arguments
    /// * `db_handle` - Database handle for executing DDL and resource registration
...
mod tests {
    ...
    impl SchemaModule for TestModule {
        ...
        async fn ensure_schema(&self, _db_handle: &DbHandle) -> Result<()> {
            Ok(())
        }
    }
