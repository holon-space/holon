...
impl TursoBackend {
    ...
    pub fn new(
        ...
        tracing::trace!("[TursoBackend] set_change_callback: enabled");
    ...
    pub async fn execute_via_actor(&self, sql: &str, params: Vec<turso::Value>) -> Result<()> {
        trace_sql_positional("execute_via_actor", sql, &params);
    ...
    pub async fn execute_sql(
        ...
        trace_sql_params("execute_sql", sql, &params);
    ...
    async fn process_actor_command(
        ...
        _cdc_broadcast: &broadcast::Sender<BatchWithMetadata<RowChange>>,
    ) -> bool {
        match cmd {
            DbCommand::Query {
                sql,
            ...
            DbCommand::SubscribeCdc { relation, response } => {
                let rx = _cdc_broadcast.subscribe();
