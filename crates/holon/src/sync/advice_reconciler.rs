...
//! discovered by the profile-pattern scan ([`holon_advice::GET_ADVICE_RULES_SQL`]).
//! When a rule block appears / changes / is deleted, the engine must synthesize,
//! diff, or tear down the rule's `advice_rule_{slug}` matview.
//!
//! **DDL never runs inline in the CDC delivery path.** Running `CREATE/DROP
//! MATERIALIZED VIEW` from inside the task that drains the CDC stream deadlocks the
//! Turso `DatabaseActor` (it starves waiting on the very actor the DDL call blocks on
//! — the `turso_storage_pbt` deadlock precedent). So this splits into two tasks:
//!
//! 1. **Drainer** — reads the discovery matview's CDC stream and does only cheap work:
//!    convert each `Change` into a [`RuleEvent`] and forward it over an mpsc channel.
//!    Never touches the DB.
//! 2. **Reconciler** — owns the `DbHandle`, receives events sequentially, parses the
//!    YAML, decides via the pure [`AdviceReconcilerState`], and runs the DROP/reconcile
//!    DDL one event at a time. Records the outcome in the [`AdviceRuleStatusHandle`].
//!
//! Initial rows are pushed through the same channel *before* the stream drainer starts,
//! so the reconciler sees them first (ordering).

use holon_advice::{
    AdviceReconcilerState, AdviceRuleStatus, AdviceRuleStatusHandle, ReconcilePlan, RuleEvent,
    StatusOutcome, parse_advice_rule, reconcile_advice_rule,
};
use holon_api::StorageEntity;
use holon_api::streaming::{ActorAbortGuard, Change};
use tokio::sync::mpsc;
...
/// (mirrors `LinkEventSubscriberHandle` / the profile watcher staying alive by being
/// held on the engine).
pub struct AdviceReconcilerHandle {
    _aborts: ActorAbortGuard,
}

/// Distil a CDC `Change` over the `(id, content)` discovery view into a [`RuleEvent`].
///
/// `Created`/`Updated` carry the row (id + content); `Deleted` carries only the id —
/// which is exactly what [`RuleEvent::Deleted`] needs. `FieldsChanged` is not emitted
/// by this matview shape; if one ever arrives it is surfaced loudly, not silently
/// dropped.
...
/// Execute one plan against the live DB, recording status. DDL failures are surfaced
/// (status + `tracing::error`), never swallowed.
async fn apply_plan(db_handle: &DbHandle, status: &AdviceRuleStatusHandle, plan: ReconcilePlan) {
    // Drops first (rename / teardown), so a rename can't briefly have two live views.
    ...
    match plan.status {
        ...
        StatusOutcome::Set { block_id, status: intended } => {
...
/// Discover advice-rule blocks and keep their matviews reconciled with live edits.
///
/// Returns a handle that must be held (on the engine) for the reconciler to keep
/// running. Watches [`holon_advice::GET_ADVICE_RULES_SQL`] via `matview_manager`
/// exactly like the profile resolver watches `PROFILE_SQL`.
pub async fn spawn_advice_reconciler(
    ...
    // Feed initial rows through the SAME channel first (ordering before the stream).
