...
use crate::sync::event_bus::{
    AggregateType, Consumer, Event, EventBus, EventFilter, EventKind, EventStatus,
};
...
impl CacheEventSubscriber {
    ...
    async fn start_block_subscription(&self, event_bus: Arc<dyn EventBus>) -> Result<()> {
        ...
        let mut event_stream = event_bus.subscribe(filter, Consumer::CACHE).await?;
    ...
    /// Flush buffered block changes — mark events as processed.
    ///
    /// The block cache and SqlOperationProvider share the same Turso
    /// connection, so block data is already committed before events are
    /// published. Re-applying here would trigger a redundant IVM CDC
    /// event (same row, different `_change_origin`) that causes spurious
    ...
    async fn flush_block_batch(
        ...
        let _batch = std::mem::take(changes);
        ...
        // Inherit the flush caller's span so the fire-and-forget UPDATE shows
        // up as a child of `flush_block_batch` in the trace, not as an
        // orphan task.
        let parent = tracing::Span::current();
        tokio::spawn(
            async move {
                if let Err(e) = event_bus_clone
                    .mark_processed_batch(&ids, Consumer::CACHE)
                    .await
                {
                    tracing::warn!(
                        "[CacheEventSubscriber] Failed to mark {} events as processed: {}",
                        ids.len(),
                        e
                    );
                }
    ...
    where
        T: holon_api::IntoEntity
            + holon_api::TryFromEntity
            + serde::Serialize
            ...
            + 'static,
    {
        ...
        } else if let Err(e) = event_bus.mark_processed_batch(&ids, Consumer::CACHE).await {
            tracing::warn!(
                "[CacheEventSubscriber] Failed to mark {} events as processed: {}",
                ids.len(),
                e
    ...
    fn event_to_block_change(event: &Event) -> Result<Change<Block>> {
        Self::event_to_entity_change::<Block>(event)
    }
}
