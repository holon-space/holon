...
pub struct LoroUiWatcher {
    source: Arc<dyn BlockQuerySource>,
    /// Advice-rule status surface (ADR 0022). Empty in a no-Turso session (no advice
    /// reconciler synthesizes matviews there), but wired for parity with the Turso
    /// watcher so a rule block's error would render in place if one is ever recorded.
    advice_status: holon_advice::AdviceRuleStatusHandle,
}
...
impl LoroUiWatcher {
    ...
    /// Wire a shared advice-rule status handle (the reader side of ADR 0022's surface).
...
impl crate::api::ui_watcher::UiWatcher for LoroUiWatcher {
    async fn watch_ui(self: Arc<Self>, block_id: EntityUri) -> Result<WatchHandle> {
        loro_watch_ui(Arc::clone(&self.source), block_id, self.advice_status.clone()).await
    }
}
    ...
    loop {
        ...
        let (expr, ordered) = match advice_status.get(block_id.as_str()) {
            Some(status) if !status.is_active() => {
                (error_render_expr(&format!("advice rule: {status}")), Vec::new())
            }
...
mod tests {
    ...
    async fn loro_watch_ui_emits_source_render_then_children_data() {
        ...
        let mut handle = loro_watch_ui(source, root.id.clone(), holon_advice::AdviceRuleStatusHandle::new()).await.unwrap();
