...
impl Lifecycle for MemoryBackend {
    async fn create_new(doc_id: String) -> Result<Self, ApiError> {
        let doc_uri = EntityUri::doc(&doc_id);
        ...
        let first_block_id_str = Self::generate_block_id(&mut state);
        let first_block_id = EntityUri::parse(&first_block_id_str).unwrap();
        let first_block = Block::new_text(
            first_block_id,
            doc_uri.clone(),
            EntityUri::no_parent(),
            String::new(),
        );
        state.blocks.insert(first_block_id_str.clone(), first_block);
...
impl CoreOperations for MemoryBackend {
    ...
    async fn get_all_blocks(
        ...
        for parent_key in state.children_by_parent.keys() {
            if EntityUri::from_raw(parent_key).is_doc() {
    ...
    async fn create_block(
        ...
        if !state.blocks.contains_key(&parent_id_str) {
            return Err(ApiError::BlockNotFound { id: parent_id_str });
        }

        // Create block using flattened structure
        let block = Block::from_block_content(block_id, parent_id, EntityUri::no_parent(), content);
