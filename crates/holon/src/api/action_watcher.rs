...
use crate::api::rule_status::{RuleStatus, RuleStatusHandle};
...
fn start_pair(
    ...
    if matches!(action_language, Some(SourceLanguage::LegacyAction)) {
        tracing::warn!(
            "[action_watcher] {action_id} uses the retired 'action' language — \
             not executing; rename the source block to holon_rule"
        );
