...
pub const PBT_UNSUPPORTED_REASON: &str = "Property-based testing is currently available only on native targets because the tokio \
...
pub fn generate_crud_transitions(
    ...
    // When there are no blocks at all, create root-level blocks
    if all_ids.is_empty() {
        let sentinel = EntityUri::no_parent().to_string();
        let create_root = "[a-z]{1,10}"
            .prop_map(move |content| BlockTransition::CreateBlock {
                parent_id: sentinel.clone(),
                content,
            })
            .boxed();
        return create_root;
    ...
    // When we have no non-root blocks, only allow create operations
