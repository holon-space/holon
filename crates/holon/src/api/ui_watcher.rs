...
async fn render_and_forward(
    ...
    // Advice-rule status surface (ADR 0022 "rule blocks render their own status"): a
    // rule block whose id carries a NON-Active status has its render replaced by the
    // error surface, so parse errors and async DDL failures are visible in place (v1
    // replaces, like the existing render-failure path; over-cap banner comes later).
