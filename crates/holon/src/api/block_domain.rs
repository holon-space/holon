...
use holon_api::{ActionSpec, EntityUri, QueryLanguage, Value, WidgetSpec};
...
impl<'a> BlockDomain<'a> {
    ...
    pub async fn render_block(
        ...
        let block_info = self.load_block_with_query_source(block_id).await?;

        let query_source = block_info
            .get("query_source")
            .and_then(|v| v.as_string())
            .ok_or_else(|| anyhow::anyhow!("Block '{}' has no query source child", block_id))?
            .to_string();
        ...
        let query_language: QueryLanguage = block_info
            ...
            .transpose()
            .map_err(|e| anyhow::anyhow!("Block '{}' has invalid query_language: {}", block_id, e))?
            .unwrap_or(QueryLanguage::HolonPrql);

        let parent_id = block_info
            .get("parent_id")
            .and_then(|v| v.as_string())
        ...
        let mut context =
            QueryContext::for_block_with_path(EntityUri::from_raw(block_id), parent_id, block_path);

        if let Some(variant) = &preferred_variant {
            context.profile_context = Some(crate::entity_profile::ProfileContext {
                preferred_variant: Some(variant.clone()),
                view_width: None,
            });
        } else {
            context.profile_context = Some(crate::entity_profile::ProfileContext::default());
        }
    ...
    #[tracing::instrument(skip(self))]
    async fn load_block_with_query_source(&self, block_id: &str) -> Result<HashMap<String, Value>> {
    ...
    pub async fn rank_tasks(&self) -> Result<crate::petri::RankResult> {
        ...
        let blocks: Vec<holon_api::block::Block> = rows
            .into_iter()
            .filter_map(|row| {
                let id = holon_api::EntityUri::from_raw(row.get("id")?.as_string()?);
                let parent_id = holon_api::EntityUri::from_raw(row.get("parent_id")?.as_string()?);
                let content = row
                    .get("content")
                    .and_then(|v| v.as_string())
                    .unwrap_or("")
                    .to_string();
                let content_type: holon_api::ContentType = row
                    .get("content_type")
                    .and_then(|v| v.as_string())
                    .unwrap_or("text")
                    .parse()
                    .expect("Invalid content_type in database");
                let source_language: Option<holon_api::SourceLanguage> = row
                    .get("source_language")
                ...
                let document_id = row
                    ...
                    .unwrap_or_else(|| holon_api::EntityUri::no_parent());

                Some(holon_api::block::Block {
                    id,
                    parent_id,
                    document_id,
                    content,
