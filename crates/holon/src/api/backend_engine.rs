...
pub struct BackendEngine {
    ...
    /// action watcher (deprecation / parse / compile / exec outcomes), read by the
    /// render path and MCP so a broken or deprecated rule surfaces its error in
    /// place. Empty until action watchers run.
    ...
    /// Keeps the clock scheduler's ticking task alive (ADR 0024 P5, time-as-data).
    /// `None` until installed in `create_initialized_engine`; the boot guard there
    /// fails loud if it stays `None`.
...
impl BackendEngine {
    ...
    /// Install the clock scheduler (ADR 0024 P5): hold its keep-alive handle so the
    /// day-rollover ticking task survives. Called once during engine initialization.
