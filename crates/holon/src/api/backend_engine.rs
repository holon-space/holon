...
impl BackendEngine {
    ...
    /// Register an entity schema at runtime (e.g., from MCP integration).
    ///
    /// Adds the schema to the persistent registry and rebuilds the cached
    /// `GraphSchema` so subsequent GQL queries can reference the new entity.
    pub fn register_entity_schema(&self, schema: holon_api::EntitySchema) {
    ...
    pub async fn query_and_watch(
        ...
        self.inject_computed_fields(&mut widget_spec);

        Ok((widget_spec, change_stream))
    }

    /// Enrich initial query rows: flatten properties JSON into top-level keys
    /// and inject computed fields from entity profiles.
    ///
    /// This mirrors what `enrich_row()` does in `ui_watcher.rs` for CDC updates,
    /// ensuring `col("task_state")` etc. work on both initial render and CDC.
    fn inject_computed_fields(&self, widget_spec: &mut WidgetSpec) {
        for row in &mut widget_spec.data {
            Self::flatten_properties(row);
            let (_profile, computed) = self.profile_resolver.resolve_with_computed(row);
    ...
    /// Promote keys from the `properties` JSON object to top-level row keys.
    /// Existing top-level keys take precedence (are not overwritten).
    fn flatten_properties(row: &mut HashMap<String, Value>) {
        if let Some(Value::Object(props)) = row.get("properties") {
            let promoted: Vec<(String, Value)> = props
                .iter()
                .filter(|(k, _)| !row.contains_key(k.as_str()))
