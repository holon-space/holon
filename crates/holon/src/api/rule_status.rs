//! Reactive-rule (ADR 0024) compilation/runtime status — the action-side twin of
//! advice's [`holon_advice::AdviceRuleStatus`] (ADR 0022 "rule blocks render their
//! own status — fail loud, visibly").
//!
//! A rule block's parse/compile/exec outcome is carried here, keyed by the rule
//! (action) block's id, written by the [`action_watcher`](crate::api::action_watcher)
//! and read by the render path so a broken — or deprecated — rule surfaces its
//! error IN PLACE rather than silently no-op'ing. In particular the retired
//! `action` language records [`RuleStatus::DeprecatedLanguage`] so it can never
//! become silently inert (ADR 0024 WP3 / A5).
...
pub enum RuleStatus {
    ...
    ParseError(String),
    /// The trigger query did not compile to SQL. The message is the typed error.
    CompileError(String),
    /// The rule fired but its effect execution failed. The message is the error.
    ExecError(String),
    /// The rule uses the retired `action` language. It does NOT execute; it must
    /// be renamed to `holon_rule`. Surfaced loud, never silently inert (WP3 / A5).
    DeprecatedLanguage,
}
