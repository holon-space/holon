...
use super::registration::register_core_services_no_turso;
...
///
/// Call this during initialization, AFTER schema modules have been initialized
/// via DI but BEFORE file watching or data sync starts.
///
...
///
/// This is the sync portion of DI setup — suitable for use in
/// `Module::configure()`. Schema initialization happens lazily via `DbReady<*>`
/// providers.
///
/// `storage` selects the substrate: `Turso` opens a `TursoBackend` and
/// registers the full Turso core; `LoroMemory` skips Turso entirely (no
/// connection, no schema/matview registration) and registers only the
/// Turso-free core — the caller's `setup_fn` then registers the Loro adapter +
/// its `BlockQuerySource`.
...
mod no_turso_tests {
    ...
    /// resolve an `Arc<dyn BlockQuerySource>` — proving the Turso-less wiring
    /// is expressible (ADR 0004 Phase 9). No `TursoBackend` is opened; the
