...
pub async fn preload_startup_views(
    ...
    // Compile PRQL startup queries to SQL before preloading as materialized views
...
/// Open the database, create the DI container with core services, initialize
/// schemas, and build the ServiceProvider.
async fn build_di_container<F>(db_path: PathBuf, setup_fn: F) -> Result<ferrous_di::ServiceProvider>
where
...
where
    F: FnOnce(&mut ServiceCollection) -> Result<()>,
{
    let provider = build_di_container(db_path, setup_fn).await?;
