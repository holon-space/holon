...
use super::lifecycle::create_backend_engine;
use super::lifecycle::create_backend_engine_with_extras;
use crate::api::backend_engine::BackendEngine;
use crate::storage::turso::TursoBackend;

/// Create a BackendEngine for testing using dependency injection
///
/// This sets up a complete DI container with all core services, using an
/// in-memory database. This ensures tests use the same setup as production
/// code.
///
/// # Example
/// ```rust
...
///
/// This allows tests to register additional providers before creating the
/// engine. The providers will be collected by OperationModule and included in
/// the OperationDispatcher.
///
/// # Arguments
/// * `db_path` - Path to the database file (use ":memory:" for in-memory)
...
///
/// This module makes it easy to register operation providers and syncable
/// providers for testing. It supports both direct provider instances and
/// factory-based providers that receive the backend at creation time.
///
...
impl TestProviderModule {
    ...
    /// Add an operation provider directly (for providers that don't need the
    /// backend)
...
impl Module for TestProviderModule {
    fn configure(&self, injector: &Injector) -> std::result::Result<(), fluxdi::Error> {
        ...
        // These receive the backend at creation time, matching how production code
        // works
        for factory in self.operation_provider_factories.lock().unwrap().drain(..) {
            // Wrap the factory in a Mutex so we can move it into the closure and call it
            // once
            ...
            injector.provide_into_set::<dyn OperationProvider>(Provider::root(move |resolver| {
                // Get the backend via TursoBackendProvider trait (registered in
