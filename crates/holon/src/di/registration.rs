...
/// (ENVIRONMENT is the top BugFunnel escape category — every embedder must seed).
async fn assert_clock_seeded(db_handle: &DbHandle) {
    ...
    assert!(
        epoch_day > 0,
        "[DI] boot guard: clock day row still holds the 1970 placeholder \
         (epoch_day={epoch_day}) — the clock scheduler did not seed the real date"
    );
}
...
async fn create_initialized_engine(
    ...
    // Clock scheduler (ADR 0024 P5, time-as-data): seed the `clock` day row from the
    // injected wall clock and re-fire temporal-guard matviews on day-rollover. Every
...
/// Rule-head blocks, keyed by `parent_id`, backing the `rule_sibling(parent_id)`
/// Rhai lookup used by the `is_program` computed field (ADR 0024 WP3 clause b):
/// "does my parent have a rule-head child?" → I am the trigger sibling of a rule.
/// Both the current `holon_rule` language and the retired `action` language count
/// (the latter still needs its trigger hidden while it surfaces its deprecation).
/// A plain filtered read of `block_raw` — no self-join, no chained matview.
fn rule_head_blocks_sql() -> String {
    format!(
        "SELECT id, parent_id, source_language FROM {table} \
         WHERE content_type = 'source' \
         AND (source_language = 'holon_rule' OR source_language = 'action')",
        table = crate::storage::BLOCK_READ_TABLE,
    )
}
