...
use holon_api::{EntityName, row_id};
...
pub fn parse_entity_profile(yaml_content: &str) -> Result<EntityProfile> {
    ...
    let default = Arc::new(StoredProfile {
        name: "default".to_string(),
        render: parse_render_text(&raw.default.render)?,
    });
    ...
    for raw_variant in &raw.variants {
        ...
        let profile = Arc::new(StoredProfile {
            ...
            render: parse_render_text(&raw_variant.render)?,
        });
...
impl EntityProfile {
    ...
    pub fn resolve(
        ...
        self.resolve_with_computed(row, live_entities).0
    }
    ...
    pub fn resolve_with_computed(
        ...
        row: &HashMap<String, holon_api::Value>,
        live_entities: &LiveEntities,
    ) -> (Arc<StoredProfile>, HashMap<String, holon_api::Value>) {
    ...
    fn resolve_from_scope(&self, engine: &RhaiEngine, scope: &mut Scope<'_>) -> Arc<StoredProfile> {
...
impl ProfileResolver {
    pub fn new(
        ...
        let fallback = Arc::new(StoredProfile {
        ...
        let initial_cache = Arc::new(Self::build_cache_from_source(&source, &ui_info));
        ...
        tokio::spawn(async move {
            ...
            while version_rx.changed().await.is_ok() {
                let new_cache = Arc::new(Self::build_cache_from_source(&bg_source, &ui_info));
...
impl ProfileResolving for ProfileResolver {
    ...
    fn resolve_with_computed(
        ...
        // Look up by entity_name, then by ID scheme if entity_name is a view/matview alias.
        let profile_result = cache.profiles.get(&entity_name);

        if let Some(entity_profile) = profile_result {
            let (stored, computed) = entity_profile.resolve_with_computed(row, &self.live_entities);
            return (self.materialize(&stored, row), computed);
        } else {
            return (self.materialize(&self.fallback, row), HashMap::new());
        }
    }
...
"#,
        ...
        let resolved = profile.resolve(&row, &LiveEntities::new());
...
"#,
        ...
        let (profile_result, computed) = profile.resolve_with_computed(&row, &LiveEntities::new());
