...
use crate::snapshot::VISIBLE_LEAF_TYPES;
...
pub fn assert_all_nonzero_except(snap: &BoundsSnapshot, fixture_label: &str, allow_zero: &[&str]) {
    ...
    if !zeros.is_empty() {
        let mut msg = format!(
            "layout invariant violated (nonzero): {} widget(s) have zero width or height in \
...
pub fn assert_containment(snap: &BoundsSnapshot, fixture_label: &str, allow_overflow: &[&str]) {
    ...
    for (id, info) in &snap.entries {
        ...
        if !child_rect.inside(parent_rect) {
            violations.push(format!(
                "  {id:24} {:>6.1}×{:<6.1} @ ({:>6.1},{:>6.1}) escapes parent {parent_id} \
                 {:>6.1}×{:<6.1} @ ({:>6.1},{:>6.1})",
                info.width,
                info.height,
                info.x,
    ...
    if !violations.is_empty() {
        let mut msg = format!(
            "layout invariant violated (containment): {} widget(s) escape their parent in fixture \
             `{fixture_label}`\n",
            violations.len()
        );
        for v in &violations {
...
pub fn assert_no_sibling_overlap(
        ...
        for i in 0..visible.len() {
            for j in (i + 1)..visible.len() {
                ...
                if Rect::of(a_info).overlaps(Rect::of(b_info)) {
                    violations.push(format!(
                        "  siblings of {parent_id}: {a_id} ({:>6.1},{:>6.1} {:>6.1}×{:<6.1}) \
                         overlaps {b_id} ({:>6.1},{:>6.1} {:>6.1}×{:<6.1})",
                        a_info.x,
                        a_info.y,
                        a_info.width,
                        a_info.height,
    ...
    if !violations.is_empty() {
        let mut msg = format!(
            "layout invariant violated (sibling overlap): {} pair(s) overlap in fixture \
             `{fixture_label}`\n",
            violations.len()
        );
        for v in &violations {
...
pub fn assert_content_fidelity(snap: &BoundsSnapshot, fixture_label: &str) {
    ...
    for (shell_id, shell_info) in &shells {
        ...
        if total_descendants > 0 && visible_leaves == 0 {
            violations.push(format!(
                "  {shell_id:24} {:>6.1}×{:<6.1} @ ({:>6.1},{:>6.1}) has {total_descendants} \
                 descendant(s) but 0 visible leaf widgets",
    ...
    if !violations.is_empty() {
        let mut msg = format!(
            "layout invariant violated (content fidelity): {} reactive_shell(s) laid out without \
             visible content in fixture `{fixture_label}`\n",
            violations.len()
        );
        for v in &violations {
            msg.push_str(v);
