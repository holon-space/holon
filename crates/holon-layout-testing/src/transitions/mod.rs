...
//!
//! 1. Implement [`crate::sut::Clickable`] + [`crate::sut::LiveBlockSink`] on
//!    the test session.
//! 2. Implement [`crate::sut::LayoutRefState`] on the reference-state type (or
//!    a generation-context struct for the layout PBT).
//! 3. At the dispatch site: wrap with `LayoutSut::new(&mut session)` /
//!    `LayoutRef::new(&ref_state)` and call `transition.apply_to_sut(...)` /
//!    `weighted_generator(...)`.
