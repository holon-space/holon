...
//! the reference oracle and can be toggled as bisection nodes. From it we derive
//! *everything*: the transition alphabet (via `Wiring`), the invariant
//! `Subsystem` selection (`subsystems()` mapping, in the consumer crate that owns
//! `Subsystem`), the reference fragments, and the runner (`needs_real_window`).
//!
...
//! ergonomic [`ComponentSet::of`] takes a closed [`Component`] enum so a literal
//! list can't smuggle in an ill-typed member.

use std::collections::BTreeSet;

use crate::wiring::{Actor, StorageAdapter, SyncAdapter, Wiring, WiringError};
...
/// `[Loro, ViewModel, UI]` literal (three different types): every element is one
/// `Component`, so `ComponentSet::of` can lower a homogeneous array.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub enum Component {
...
impl ComponentSet {
    ...
    /// Build from a flat list of [`Component`]s (ADR 0009 §0). Storage/sync/actor
    /// members lower into the `Wiring`; projection members into `projections`.
    /// Accepts anything that converts into `Component`, so callers can write
    ...
    /// Wide + real UI window — the GPUI runner. `Wiring::full()` (which includes
    /// `Actor::UI`) + both projections. Subsystems = all 9.
    ...
    /// Wide, cheap, headless — the CI-default. `Wiring::full()` minus `Actor::UI`
    ...
    /// Turso-only (no Loro) wide slice — `Wiring::sql_only()` + both projections.
    /// Carries both projections so `subsystems()` reproduces the pre-ADR
    /// `headless_wide` selection (the `*_sql_only` blessed slices). The
    /// `Actor::UI` in `sql_only`'s manifest is normalised away headless by the
    /// runner (it derives UI from a real window, not the manifest), so this is
    /// the one-line form for the headless Turso-only slices.
    ...
    /// projections that make `subsystems()` reproduce today's headless selection
    /// (ADR 0009 migration step 1a). Used as the behaviour-preservation anchors.
    ...
    /// invert the lattice direction (ADR 0009 §3 `UpwardMinimal`). Bounded by the
    /// observed maximal (`ceiling`) set so the walk stays within the lattice
    /// between a candidate and the full reproducing set, and pruned to valid nodes.
