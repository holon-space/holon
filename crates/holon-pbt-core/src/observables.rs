...
/// Whether `id` is a ref-side SYNTHETIC placeholder the SUT replaces with a real
/// UUID. Only split suffixes are: the reference `split_block` transition mints
/// `block::split-N` (note the double colon) and the SUT later binds it to a
/// freshly-minted UUID. Bulk ids (`block:bulk-N-i`) are NOT synthetic — they are
/// written deterministically on BOTH sides, so they appear verbatim in
...
/// central crate); substring sniffs like `contains(":split-")` false-positive on
/// legitimately named blocks (e.g. `block:split-target-block`).
