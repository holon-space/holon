...
pub enum Localization {
    ...
    /// Smallest reproducing set reachable by *adding* components back toward the
    /// ceiling: it reproduces, and no valid parent (within the ceiling) does. A
    /// present-when-component-*absent* bug (a handler missing for the disabled
    /// case), which inverts the lattice direction.
...
/// until no valid child reproduces. Returns the locally-minimal reproducing set.
///
...
/// the empty-projection floor of the ceiling's wiring (an absent-component bug).
/// Returns [`Localization::NotReproduced`] if neither end reproduces.
pub fn bisect(
    ceiling: ComponentSet,
    floor: ComponentSet,
    mut reproduces: impl FnMut(&ComponentSet) -> bool,
