...
pub trait SutUnschemedIdDispatch {
    ...
    /// UNSCHEMED form (`my-block`, not `block:my-block`) through the production
    /// operation dispatcher, and assert the boundary refuses it by naming the
    /// parameter and the offending value.
    ///
    /// `bare_id` names no block on purpose: the refusal is a property of the id
    /// form alone, so the assertion must also tell it apart from the
    /// "block does not exist" answer a reachable id would produce.
...
/// frontends subscribe to, not a re-derivation of it. A bus that stopped being
/// raised on therefore shows up as a missing disclosure rather than a quiet
/// pass.
#[holon_macros::capmap_adapter]
pub trait SutConditions {
    async fn conditions_now(&self) -> Vec<RaisedCondition>;
}

/// True when `subject` names exactly `expected` at a PATH-COMPONENT boundary.
///
/// The model pins a file NAME ([`RefConditions::expected_conditions`]); the
/// raise site's subject is normally an absolute path inside the run's temp
/// vault. The comparison must therefore be IDENTITY, not a raw string suffix:
/// under `ends_with`, a DIFFERENT file called `a-keystone-recipe.cook`
/// satisfies an expectation for `keystone-recipe.cook`, so the pinned file's
/// refusal goes undisclosed and the invariant reports a clean run — the silent
/// failure this capability exists to prevent. See bugfunnel entry
/// `2026-09-15-conditions-invariant-matches-subject-by-raw-suffix`.
///
/// A subject that is a bare name (no path separator) still matches whole, which
/// is what "the whole subject where it is a name" means. An expectation that
/// itself contains a separator can never be satisfied — a LOUD red, never a
/// silent pass.
pub fn subject_matches_expected(subject: &str, expected: &str) -> bool {
    std::path::Path::new(subject)
        .file_name()
        .is_some_and(|name| name == std::ffi::OsStr::new(expected))
}

/// What the reference model expects to be disclosed, and which kinds it claims
/// authority over.
#[holon_macros::capmap_adapter] // sync trait → no async-trait
