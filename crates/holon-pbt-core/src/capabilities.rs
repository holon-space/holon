...
pub trait RefPeers {
    ...
    /// `ApplyMutation` generator excludes these from the primary's mutable-block
    /// candidate set (a block a peer is mid-editing is not a safe primary target).
    /// Wide PBT reads `peer.modified_stable_ids`; a peer-less slice returns empty.
...
pub trait RefApplyMutationMut {
    /// Apply a non-peer (`UI`/`External`) content `mutation` and perform all the
    /// encapsulated reference bookkeeping described on the trait. Undo-snapshot
    /// (UI only) and focus-clear-on-delete stay OUTSIDE this method — the generic
    /// `apply_to_ref` drives them through [`RefBlockTreeMut::push_undo_snapshot`]
