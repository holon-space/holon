...
//! lifted to `holon-pbt-core` (co-location Phase 1a follow-on) so every companion
//! `*-testing` crate can compare a store's block snapshot against the reference
//! without depending on `holon-integration-tests`.
//!
//! Block equivalence in Holon is *defined by* the org round-trip: two blocks are
//! "equal" when their org-normalized forms match. [`normalize_block`] hand-replicates
//! that round-trip (trim trailing whitespace, strip internal/null/empty properties,
//! unify the document root) using only `holon-api` — so the floor stays org-crate-free.
//!
//! The `inv-blocks-match-ref` composite checks the SAME comparison against every store
//! that holds blocks (Loro, Org, `block_raw`, the `block` matview): each store normalises
//! to a `Vec<holon_api::Block>` and this module compares it to the reference's snapshot.
//! Two facets, both derived from the normalised `Vec<Block>`:
//!
//! - **fields** — [`normalize_block`] + sort-by-id + `==`. `normalize_block` zeroes
//!   timestamps and strips internal/null/empty properties, so this compares content,
//!   content_type, parent, properties, tags, requires, task_state, source_language —
//!   everything *except* sibling order.
//! - **order** — per-parent sibling order under the renderer's canonical sort
//!   (source/image before text, then `sequence`, then id).
//!
//! Bodies stay dumb: they pick a store snapshot, a readiness gate, and which facets
//! apply, then call [`compare_blocks`]. The per-store `RunMode` and CDC-lag → `Skipped`
//! decision live in the body/runner, not here.
...
/// `holon-api`). This is the inlined equivalent of `holon_org_format::OrgBlockExt::sequence`
/// — kept here so block comparison never drags an org crate onto the pbt-core floor. The
/// `sequence` property is the canonical intra-parent ordering key; absent → `0`.
...
/// sort (source/image first, then `sequence`, then id), only comparing when both
/// sides hold the same id set and skipping all-source sibling groups. Returns the
/// first divergent parent as an `Err`.
