...
//!
//! North-star (memory `north-star.md`): ONE env-selected composed PBT, assembled
//! from per-crate contributions; slices are scaffolding to delete. This module is
//! the assembly seam that makes "a crate changed → activate that crate's PBT" a
//! physical, enumerable signal.
//!
...
//!   wiring-ladder's floor and offline map verification read. Kept honest against
//!   the live contribution by a per-crate `footprint_matches_contribution()`
//!   anti-rot test (parse-don't-validate).
//!
...
//! [`CapMap`] only, so whether the reference model is today's central monolith or
//! a future per-subsystem `RefLoro` split lives entirely behind the cap traits —
//! this seam does not depend on it.

use crate::component_set::ComponentSet;
use crate::composition::{CapInvariant, CapMap};
...
impl CrateId {
    /// The cargo crate name that owns this contribution — the reverse-dep key the
    /// ladder floor inverts (`holon-X changed → run holon-X-testing`).
...
/// providers into the composed [`CapMap`], gated by the active [`ComponentSet`].
///
/// The central assembler still owns precedence/ordering (§8 risk 2: `CapMap` is
/// insert-only + fail-loud on duplicate); an installer only registers the caps a
/// present component supplies. Empty on the Phase-0 central contributor — the
...
/// [`PbtFootprint::transition_kinds`]); the strategy-producing method lands when
/// the transition enum co-locates (Phase 4), because the concrete `Transition`
/// type it must produce is not nameable in `holon-pbt-core`.
...
impl PbtContribution {
    ...
    /// counterpart the anti-rot test compares its static [`PbtFootprint`] against.
