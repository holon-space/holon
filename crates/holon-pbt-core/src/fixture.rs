...
//! - **Validate that a fixture is still in the strategy's domain.**
//!   Proptest's maintainers cite this as why they reject value persistence;
//!   we sidestep it by treating preconditions as gates inside `replay` and
//!   reporting *which* steps no longer apply, so a stale fixture degrades
//!   into a clear diagnostic instead of a silent pass.
...
/// - `check_invariants(&ref, &sut) -> async Result<(), InvariantFailure>`:
///   runs every invariant the slice cares about. The closure form sidesteps
///   the dyn-trait restriction on `async fn` in traits; each slice already has
///   a `check_invariants` method emitted by `declare_pbt_slice!`, so wiring
///   is one line.
///
/// Returns `Vec<StepOutcome>` so the caller decides whether a
/// `SkippedPreconditions` outcome is soft (drift) or hard (regression-now-stale).
