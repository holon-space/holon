...
pub trait BlockQuery {
    ...
    /// [`children_ordered`](Self::children_ordered); implementors with a cheaper
    /// recursive query may override it.
...
pub struct BlockSnapshot {
    /// id → block (order-independent; `HashMap` equality ignores insertion order).
    by_id: HashMap<EntityUri, Block>,
    /// parent id → child ids in canonical sibling order (per-parent order is the
    /// meaningful order and *is* compared).
...
impl BlockSnapshot {
    ...
    /// Iterate all captured blocks (unordered — for diagnostics / set comparison).
