...
//! are the common error surface. The path `#crate_path::storage::types::StorageEntity`
//! is also hard-coded in the `operations_trait` macro, so this layout is load-bearing.

use holon_api::Value;
use std::collections::HashMap;
use thiserror::Error;

/// StorageEntity type alias for `HashMap<Arc<str>, Value>`. // ALLOW(compatibility): macro-emitted callers expect this exact path.
