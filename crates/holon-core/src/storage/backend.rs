...
pub trait StorageBackend: Send + Sync {
    ...
    /// Insert a new row. Uses interior mutability - implementations should be
    /// thread-safe.
    async fn insert(&self, schema: &TypeDefinition, data: StorageEntity) -> Result<()>;

    /// Update an existing row. Uses interior mutability - implementations
    /// should be thread-safe.
    async fn update(&self, schema: &TypeDefinition, id: &str, data: StorageEntity) -> Result<()>;

    /// Delete a row. Uses interior mutability - implementations should be
    /// thread-safe.
