//! I3-0 acceptance: the `person_profile.display_name` computed field — read
//! from the REAL shipped yaml, not a copied string — must parse into the typed
//! `Computation` subset and lower to SQL, so it is served from a planted
//! matview column (seat A) rather than a per-row Rhai call (seat B).
...
fn display_name_source() -> String {
    ...
    doc["computed"]["display_name"]
        .as_str()
        .expect("computed.display_name must be a string")
        .to_string()
}
...
fn display_name_lowers_to_sql_and_plants_to_seat_a() {
    let src = display_name_source();
    let comp = expr_parser::parse(&src).unwrap_or_else(|e| panic!("must parse `{src}`: {e}"));
    let frag = comp
        .compile_sql()
        .unwrap_or_else(|e| panic!("compile_sql must succeed for `{src}`: {e}"));
    println!("display_name SQL: {}", frag.sql);

    let plan = DerivedFieldPlan::plan(vec![DerivedField::new("display_name", comp)]);
    assert_eq!(
