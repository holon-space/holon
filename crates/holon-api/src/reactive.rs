    ...
    Inner: Stream<Item = U> + Send + Unpin + 'static,
{
    ...
    tokio::spawn(async move {
        tokio::pin!(source);
        let mut current_forwarder: Option<tokio::task::JoinHandle<()>> = None;

        while let Some(outer_item) = source.next().await {
            if let Some(handle) = current_forwarder.take() {
                handle.abort();
            }

            let inner = f(outer_item);
            let tx = tx.clone();
            current_forwarder = Some(tokio::spawn(async move {
                tokio::pin!(inner);
...
pub struct UiState {
    render_expr: crate::render_types::RenderExpr,
...
pub enum UiEventResult {
    /// Structure changed — frontend should re-render entirely with the new WidgetSpec.
    StructureChanged(crate::widget_spec::WidgetSpec),
...
impl UiState {
    pub fn new(initial: crate::widget_spec::WidgetSpec) -> Self {
        Self {
            render_expr: initial.render_expr,
            data: CdcAccumulator::from_rows(initial.data),
            generation: 0,
        }
    }
    ...
    pub fn to_widget_spec(&self) -> crate::widget_spec::WidgetSpec {
        crate::widget_spec::WidgetSpec {
            render_expr: self.render_expr.clone(),
    ...
    pub fn apply_event(&mut self, event: crate::UiEvent) -> UiEventResult {
        match event {
            crate::UiEvent::Structure {
                ...
                self.render_expr = widget_spec.render_expr.clone();
                self.data = CdcAccumulator::from_rows(widget_spec.data.clone());
                self.generation = generation;
                UiEventResult::StructureChanged(widget_spec)
            }
            ...
            crate::UiEvent::CollectionUpdate { .. } => UiEventResult::NoChange,
        }
    }
}
