...
//! - **WASM note**: tokio channels would need replacing for WASM targets; the
//!   diff types and operator signatures are runtime-agnostic.
...
/// spawn-based operators (`scan_state`, `switch_map`, `combine_latest`,
/// `coalesce`).
...
pub trait ReactiveStreamExt: Stream + Sized {
    /// Accumulate state from a stream of events (like Redux `reduce` / Rx
    /// `scan`).
    ///
...
///
/// Replaces the duplicated match-on-Change logic across frontend `cdc.rs`
/// files. Maintains a `HashMap<String, V>` keyed by entity ID and applies diffs
/// incrementally.
...
pub enum UiEventResult {
    /// Structure changed — frontend should re-render entirely with the new
    /// RenderExpr.
