...
pub trait QueryEngine: Send + Sync {
    ...
    /// return its current rows. Unlike [`Self::watch_query`], this sets up **no**
    /// materialized view and **no** CDC stream.
    ///
    ...
    /// shape MUST NOT be handed to `watch_query`, which matview-izes any SQL and
    /// whose Turso IVM cannot maintain an anti-join FUSED WITH an aggregate/GROUP BY
    /// (see holon-advice `probe_ivm_shape_findings`). NOTE: a PLAIN anti-join in a
    /// non-aggregating outer view IS incrementally maintained (proven by
    /// `probe_outer_antijoin_is_incrementally_maintained`), so watching such a read
    /// is fine — the advice weaver does exactly that.
    /// See `holon_frontend::advice_weaver`.
    ///
    /// Default impl fails loud (returns `Err`) rather than silently returning an
    /// empty set — a `QueryEngine` that has not wired one-shot execution should
    /// surface that, not fake success. The Turso `BackendEngine` overrides it.
