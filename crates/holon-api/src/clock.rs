...
/// DI seam for the wall-clock authority the `ClockScheduler` ticks on. Production
/// boot registers nothing and the engine factory falls back to [`SystemClock`];
/// a test wiring registers this newtype holding a controllable [`TestClock`] so
/// the keystone `AdvanceDay` transition advances time through the real scheduler
/// path (never a raw `clock`-relation write). A wrapper newtype (not a bare
/// `Arc<dyn Clock>`) so fluxdi keys on one stable type both sides name.
#[derive(Clone)]
...
/// Deterministic clock for tests. Cheap to clone; clones share the same instant.
#[derive(Debug, Clone)]
