...
//!
//! # Deferred: reactive render consumption
//!
//! The frontend watches the fixed `block:root-layout` and derives its 3-column
//! render through profile resolution (`BlockDomain::render_entity`) on the
//! Turso arm and `loro_ui_watcher::derive_render_expr` on the no-Turso arm.
//! Making the *live* render follow the active-perspective pointer means
//! teaching those two render-derivation sites to resolve panels from the
//! pointed-to perspective instead of the root-layout's own children. That is a
//! cross-arm change tracked as the remaining C8 work;
//! [`resolve_active_perspective`] is the function those sites will call. See
...
pub struct PerspectiveSpec {
    ...
    /// Entity-profile override block this perspective activates, if any.
    pub profile_override: Option<EntityUri>,
    /// Concealment parameters.
    ...
    pub fn parse(perspective_id: &EntityUri, blocks: &[Block]) -> anyhow::Result<Self> {
        ...
        let profile_override = match block.get_property_str(P_PROFILE) {
            Some(raw) => Some(EntityUri::parse(&raw).map_err(|e| {
                anyhow::anyhow!(
                    "perspective {perspective_id}: {P_PROFILE} {raw:?} is not a valid id: {e}"
                )
            })?),
            None => None,
        };
...
/// Resolve and parse the currently-active perspective. This is the function the
/// reactive render-derivation sites will call to pick panels once the render
/// seam consumes perspectives (see module docs).
