...
/// The per-row identity key carried by [`ReactiveRowProvider::keyed_rows_signal_vec`]:
/// the entity plus which display occurrence of it this row is. The
/// `ReactiveRowSet` store stays `EntityUri`-keyed — this widened key lives only
/// in the driver-local diffing layer (ADR 0016 §3).
