...
pub trait ReactiveRowProvider: Send + Sync {
    ...
    ///
    /// A watched query's `ORDER BY` is stripped from the matview body (Turso
    /// accepts it there only as column ordinals), so the rendered collection
    /// is the only place left that can honour it. A collection with no explicit
