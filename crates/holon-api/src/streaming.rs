...
/// state or only new changes. flutter_rust_bridge:non_opaque
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub enum StreamPosition {
    /// Start from the beginning: first stream all current blocks as Created
    /// events, then stream subsequent changes
    Beginning,
    /// Start from a specific version: stream only changes that occurred after
    /// this version
    Version(Vec<u8>),
}
...
/// Stored in `_change_origin` column to propagate trace context across threads
/// (CDC callbacks). flutter_rust_bridge:non_opaque
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub enum ChangeOrigin {
    /// Change initiated by this client
    Local {
        /// Span ID (16 hex chars) linking this change to the originating
        /// operation
...
impl ChangeOrigin {
    /// Create Local origin with trace context extracted from current
    /// OpenTelemetry span
    ///
    ...
    /// Create Remote origin with trace context extracted from current
    /// OpenTelemetry span
    ///
    /// flutter_rust_bridge:ignore
...
///
/// Note: This generic type is not directly exposed to FRB. Use concrete type
/// aliases like `BlockChange` or `MapChange`. flutter_rust_bridge:ignore
...
impl<T> Change<T> {
    /// Transform the data type inside Created/Updated variants.
    ///
    /// `Deleted` and `FieldsChanged` pass through unchanged (they don't carry
    /// `T`).
...
/// Type alias for Change<DataRow>
///
/// Used for streaming query result changes.
/// flutter_rust_bridge:non_opaque
pub type MapChange = Change<crate::widget_spec::DataRow>;

/// Type alias for Change<EnrichedRow>
///
/// Used in the reactive pipeline where data has been through enrichment.
pub type EnrichedChange = Change<crate::widget_spec::EnrichedRow>;
...
///
/// This allows you to add metadata to any type without modifying the original
/// type. Implements Deref/DerefMut for ergonomic access to the inner type.
/// flutter_rust_bridge:non_opaque
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub struct BatchMetadata {
    ...
    /// Subscribers track the highest `seq` they've consumed so a test or
    /// driver can wait until `consumed_seq >= cdc_emitted_watermark()`
    /// instead of sleeping a fixed timeout. `0` means "unset" — the seq
    /// is filled in by `TursoBackend`'s change callback before broadcast,
...
impl BatchTraceContext {
    ...
    pub fn to_span_context(&self) -> Option<opentelemetry::trace::SpanContext> {
        ...
        let trace_id = TraceId::from_hex(&self.trace_id).ok()?; // ALLOW(ok): invalid hex is non-fatal
        let span_id = SpanId::from_hex(&self.span_id).ok()?; // ALLOW(ok): invalid hex is non-fatal
        let trace_flags = TraceFlags::new(self.trace_flags);
        let trace_state = self
            .trace_state
            .as_ref()
            .and_then(|s| TraceState::from_str(s).ok()) // ALLOW(ok): invalid trace state is non-fatal
            .unwrap_or_default();
    ...
    /// Alias for [`span_id`](Self::span_id). Some callers historically read
    /// this name.
...
///
/// This trait provides race-free state sync by streaming the current document
/// state followed by all subsequent changes. Backends that support real-time
/// updates implement this trait.
///
/// # Architecture
///
...
pub trait ChangeNotifications<T>: Send + Sync {
    ...
    ///
    /// Returns a Stream that emits batched document changes. Behavior depends
    /// on the `position` parameter:
    /// - `StreamPosition::Beginning`: First emits all current blocks as
    ///   `Change::Created` events in a batch, then continues streaming
    ///   subsequent changes (initial sync mode)
    /// - `StreamPosition::Version(v)`: Streams only changes that occurred after
    ///   version `v` (incremental sync mode)
    ///
    /// # Arguments
    ///
    /// * `position` - Stream position to start from (beginning or specific
    ///   version)
    ///
    /// # Returns
    ///
    /// A Stream that yields `Result<Vec<Change<T>>, ApiError>` items. Each item
    /// is a batch of changes. The stream continues until either:
    ...
    /// # Resource Management
    ///
    /// The stream automatically unsubscribes and releases resources when
    /// dropped. No explicit cleanup method needed.
    ...
    ///
    /// Returns the version vector representing the current state of the
    /// document. This can be used to track document evolution over time.
    ///
    /// # Returns
    ///
...
/// Tagged enum for the unified `watch_ui` stream.
///
/// A single long-lived stream carries both structural changes (new RenderExpr)
/// and incremental data updates (CDC batches). The `generation` field ties
/// Data events to the Structure event that created the underlying matview;
/// stale generations are discarded by the frontend.
///
/// flutter_rust_bridge:non_opaque
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum UiEvent {
    /// Emitted on initial render, structural changes (query/render source
    /// edited, block appears/disappears), and variant switches.
    ///
    /// `candidates` contains all matching render variants. The frontend picks
    /// the active one based on local UI state (focus, view mode). On render
    /// failure, `candidates` contains a single error variant.
    ///
    /// `render_expr` is the first candidate's render expression (backward
    /// compat).
    Structure {
        render_expr: RenderExpr,
        /// All matching variant candidates for frontend-side selection.
        #[serde(default, skip_serializing_if = "Vec::is_empty")]
        candidates: Vec<RenderVariant>,
        generation: u64,
    },
    /// CDC deltas for the active query.
    /// `generation` matches the Structure that created the matview.
    Data {
...
/// Command channel for controlling a UiWatcher.
///
/// flutter_rust_bridge:non_opaque
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum WatcherCommand {
    /// Switch to a different entity profile variant.
    /// Re-renders with the new variant but keeps the data matview alive
    /// (same generation) if the underlying query didn't change.
    SetVariant(String),
}

/// Drop guard that aborts a set of background tasks when the owner drops.
///
/// Used by `WatchHandle` to deterministically tear down the actor pipeline
/// behind `watch_ui` instead of relying on each actor's source stream to
/// eventually complete (which can take arbitrarily long if the source is
/// engine-scoped). PBT non-determinism on a shared runtime came from this
/// drain gap — actors would still be processing while the next case ran.
///
/// On wasm the field is empty: `wasm_bindgen_futures::spawn_local` does not
/// expose abort handles, so the wasm build keeps the historical fire-and-
/// forget behaviour. PBT runs on native only, so this is sufficient.
///
/// flutter_rust_bridge:ignore
#[derive(Default)]
pub struct ActorAbortGuard {
...
/// Owns both ends of a `watch_ui` connection: the event receiver and the
/// command sender. Keeping them bundled prevents the accidental-drop bug
/// where dropping the sender kills the UiWatcher's run loop.
///
/// Also owns an `ActorAbortGuard` so that dropping the handle deterministically
/// cancels every background task spawned by `watch_ui`, instead of leaving them
/// parked on engine-scoped streams until the engine itself drops.
///
/// flutter_rust_bridge:ignore
pub struct WatchHandle {
    output_rx: mpsc::Receiver<UiEvent>,
...
impl WatchHandle {
    pub fn new(
        ...
        Self {
            output_rx,
            command_tx,
            _aborts: ActorAbortGuard::new(),
        }
    }

    /// Construct with an explicit abort guard. Used by `watch_ui` to attach
    /// the JoinHandles for the actor pipeline so they are aborted when the
    /// handle is dropped.
    pub fn with_aborts(
        output_rx: mpsc::Receiver<UiEvent>,
        command_tx: mpsc::Sender<WatcherCommand>,
        aborts: ActorAbortGuard,
    ) -> Self {
        Self {
            output_rx,
            command_tx,
    ...
    /// Receive the next UiEvent. Returns `None` when the stream closes.
    pub async fn recv(&mut self) -> Option<UiEvent> {
        self.output_rx.recv().await
    }

    /// Non-blocking receive. Returns `Err` if the channel is empty or closed.
    pub fn try_recv(&mut self) -> Result<UiEvent, mpsc::error::TryRecvError> {
        self.output_rx.try_recv()
    }

    /// Switch to a different entity profile variant.
    pub async fn set_variant(&self, variant: String) -> anyhow::Result<()> {
        self.command_tx
            .send(WatcherCommand::SetVariant(variant))
            .await
            .map_err(|_| anyhow::anyhow!("Watch stream closed — cannot set variant"))
    }

    /// Decompose into the raw receiver, command sender, and abort guard.
    ///
    /// Use this when the receiver must be forwarded elsewhere (e.g. Flutter's
    /// StreamSink) while the sender needs separate lifetime management.
    /// The caller is responsible for keeping all three alive — dropping the
    /// returned `ActorAbortGuard` will cancel the background actor pipeline
    ...
    pub fn into_parts(
        ...
        (self.output_rx, self.command_tx, self._aborts)
    }
}
...
mod mutation_gap_tests {
    ...
    fn change_origin_trace_surface() {
        let local = ChangeOrigin::local_with_trace(Some(TRACE.to_string()), Some(SPAN.to_string()));
        let remote =
