...
/// Opaque handle for requesting more items from a lazy collection.
/// Re-exported from holon-frontend for use in UiEvent.
///
/// flutter_rust_bridge:non_opaque
...
///
/// A single long-lived stream carries both structural changes (new WidgetSpec)
...
pub enum UiEvent {
    ...
    /// On render failure, `widget_spec` contains an error RenderExpr.
    Structure {
        widget_spec: crate::widget_spec::WidgetSpec,
        generation: u64,
    },
    ...
    /// Response to an `expand_range()` request for lazy collection items.
    /// Carries a window of materialized ViewModels for the given collection.
    CollectionUpdate {
        collection_id: CollectionId,
        /// The offset of the first item in this batch.
        offset: usize,
        /// Serialized ViewModel items (serialized as JSON for FFI).
        /// In Rust, callers deserialize to Vec<ViewModel>.
        items_json: String,
    },
}
