...
impl ContentType {
    ...
    /// blocks** (the source loop precedes the image loop in `process_headlines`).
    /// So the post-round-trip kind order is `Source < Image < Text`.
    ///
...
pub enum SourceLanguage {
    ...
    /// rule card — "legacy 'action' language; rename to holon_rule" — instead of
    /// silently becoming an inert unknown source. See ADR 0024 WP3.
    LegacyAction,
    Other(String),
}
...
impl SourceLanguage {
    ...
    /// `action` language. Both mark a block as program (routed to the rule card).
