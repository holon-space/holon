...
//!
//! Sibling order is "an ordered list of children per parent" — never a per-block
//! key. The only ways to change the tree are [`BlockMutation::Create`],
//! [`BlockMutation::Move`], and [`BlockMutation::MoveAfter`]. Each carries an
//! `after: Option<EntityUri>` positioning intent (`None` = first child).
//!
...
/// `after` precondition: not a self-reference, and (when `Some`) a current child
/// of `parent`.
fn check_after(
    after: Option<&EntityUri>,
    subject: &EntityUri,
