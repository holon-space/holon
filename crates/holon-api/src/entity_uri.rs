...
///
/// Newtype around `fluent_uri::Uri<String>` — every entity ID is a valid RFC
/// 3986 URI.
///
/// Common schemes:
...
impl EntityUri {
    ...
    pub fn new(scheme: &str, path: &str) -> Self {
        ...
        debug_assert!(
            ...
            "EntityUri::new({scheme:?}, {path:?}): path already begins with a scheme prefix — \
             double-scheme bug. Use EntityUri::from_raw / parse for an already-schemed string, or \
             pass the bare id."
        );
    ...
    // -- File constructors --
    // File URIs represent on-disk org files (e.g. `file:index.org`,
    // `file:projects/todo.org`). They are transient identifiers used during
    ...
    /// there on, pass the `EntityUri` through; do not turn it back into a
    /// string and re-`from_raw` it deeper in. Re-parsing internal data is
    /// the smell that lets scheme-fragility (bare `x` vs `block:x`) leak
    /// between layers — it's flagged by archlint `entity_uri_from_raw`.
