...
//! deliberate later step (D slice 2), not something a caller can do by accident.
//!
//! **Consolidator is pinned at session start** (Risk #4, decided): who owns
//! order/merge is resolved once into a [`SessionCapabilities`] and never changes
//! for the life of the session. Adding or removing Loro mid-session requires a
//! full re-sync, not a live handoff — so [`SessionCapabilities`] is immutable
//! after construction.
...
/// The two real runtime sync configurations. Sealed: not an open N-peer lattice.
