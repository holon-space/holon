...
//!
//! - [`interaction_dispatched`] — called at the operation-dispatch entry
//!   point (`holon-frontend` `dispatch_intent{,_sync}`) with the op name and
//!   the target entity id. Starts the clock.
//! - [`rows_delivered`] — called from `LiveData::subscribe` (the shared
//!   engine mirror that BOTH the default SQL path and the CRDT projection
//!   path feed through CDC) with the ids touched by an applied batch. The
//!   first batch whose ids contain a pending target (as row id or as
//!   `parent_id` of a created/updated row — covers create/split ops whose
//!   new row is parented under the target) closes the clock and emits:
//!
...
//! - Ops without an `id` param, and deletes whose CDC `Deleted.id` is a
//!   rowid rather than the entity id, are not correlated (no event).
