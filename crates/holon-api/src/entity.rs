...
// =============================================================================
// HasSchema trait - For entity introspection (compile-time schema)
// =============================================================================

/// Trait for types that have a compile-time database schema.
///
/// Supertrait of `IntoEntity + TryFromEntity` for backward compatibility.
/// Implemented by `#[derive(Entity)]` to provide schema introspection
/// and conversion to/from `DynamicEntity`.
/// flutter_rust_bridge:ignore
pub trait HasSchema: IntoEntity + TryFromEntity {
...
pub struct EntitySchema {
    ...
    /// GQL node label (e.g., "Block"). None means this entity is not exposed as a GQL node.
    #[serde(default)]
...
pub struct EntityFieldSchema {
    ...
    pub required: bool,
    pub indexed: bool,
    /// GQL edge name for Reference fields (e.g., "CHILD_OF"). None for non-relationship fields.
    #[serde(default)]
...
impl FieldType {
    ...
    pub fn from_sql_type(sql: &str) -> Self {
        match sql.to_uppercase().as_str() {
            "INTEGER" => FieldType::Integer,
            "REAL" => FieldType::String, // no Float variant; approximate as String
