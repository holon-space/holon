...
pub fn sorted_rows(rows: &[DataRow], sort_key: Option<&str>) -> Vec<DataRow> {
...
impl OutlineTree {
    pub fn from_rows(rows: &[DataRow], parent_id_col: &str, sort_col: &str) -> Self {
...
pub fn has_drawer_rows(rows: &[DataRow]) -> bool {
    rows.iter().any(|row| {
        row.get("collapse_to")
