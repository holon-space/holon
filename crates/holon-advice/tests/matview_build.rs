...
//! tag-overlap rows are correct. Suppression + recency are applied at READ time
//! (the in-matview forms are silently miscompiled by current Turso IVM — see the
//! `probe_*` tests, which document the finding that drove that split). Mirrors the
//! harness in `holon-turso/tests/sidecar_views.rs`.
...
async fn probe_multi_junction_fanout_fix_shapes() {
    ...
    holon_turso::matview_manager::reconcile_named_view(&handle, "probe_fanout_current", &single_view_shape(""))
        .await
    ...
    assert_eq!(tags, vec!["t1", "t2", "t3"], "tags survive (1 requires × 1 sup)");
    assert_eq!(
        req,
        vec!["b2", "b2", "b2"],
    ...
    for (name, jt, src, tgt) in [
        ...
        ("probe_requires_agg", "block_requires", "block_id", "required_id"),
        ("probe_sup_agg", "advice_suppressed", "anchor_id", "lesson_id"),
    ] {
        holon_turso::matview_manager::reconcile_named_view(
            &handle,
            name,
    ...
    let assert_chained = |label: &'static str,
        ...
        async move {
            ...
            assert_eq!(n1.max(n2).max(n3), 1, "[{label}] {id}: exactly one matview row");
    ...
    handle
        ...
        .await
        .unwrap();
    assert_chained("+r2", "b1", &["t1", "t2", "t3", "t4"], &["b2", "r2"], &["x1"]).await;

    // Incremental: delete a tag.
    handle
        .execute("DELETE FROM block_tags WHERE block_id = 'b1' AND tag = 't1'", vec![])
        .await
        .unwrap();
    ...
    handle
        .transaction(vec![
            ("DELETE FROM block_tags WHERE block_id = 'b1'".into(), vec![]),
            ("INSERT INTO block_tags (block_id, tag) VALUES ('b1', 'u1')".into(), vec![]),
            ("INSERT INTO block_tags (block_id, tag) VALUES ('b1', 'u2')".into(), vec![]),
        ])
        .await
    ...
    let (req, _) = json_col(&handle, "probe_fanout_current", "b1", "requires").await;
    let (sup, _) = json_col(&handle, "probe_fanout_current", "b1", "advice_suppressed").await;
    assert_eq!(tags, vec!["u1", "u1", "u2", "u2"], "incremental fan-out: tags × |requires|");
    assert_eq!(req, vec!["b2", "b2", "r2", "r2"], "incremental fan-out: requires × |tags|");
    assert_eq!(sup, vec!["x1", "x1", "x1", "x1"], "incremental fan-out: sup × |tags×requires|");
    let (tags, _) = json_col(&handle, "probe_fanout_distinct", "b1", "tags").await;
    let (req, _) = json_col(&handle, "probe_fanout_distinct", "b1", "requires").await;
    let (sup, _) = json_col(&handle, "probe_fanout_distinct", "b1", "advice_suppressed").await;
