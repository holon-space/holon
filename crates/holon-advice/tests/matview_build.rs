...
use holon_advice::BUNDLED_LESSONS_FOR_TASKS_YAML;
...
/// NOT strengthen the PBT oracle — `updated_at` remains unobservable to the
/// reference, so the reference still cannot pin recency order; lesson_id merely
/// stops the renderer flickering between equal-ranked rows.
fn read_query(anchor: &str, k: u8) -> String {
    format!(
        "SELECT v.lesson_id FROM advice_rule_lessons_for_tasks v LEFT JOIN advice_suppressed s ON \
         s.anchor_id = v.anchor_id AND s.lesson_id = v.lesson_id JOIN block_raw c ON c.id = \
    ...
    let present = handle
        .query(
            "SELECT name FROM sqlite_master WHERE type='view' AND \
             name='advice_rule_lessons_for_tasks'",
            HashMap::new(),
        )
        .await
...
async fn probe_ivm_shape_findings() {
    ...
    let created = holon_turso::matview_manager::reconcile_named_view(
        &handle,
        "probe_overlap",
        "SELECT atg.block_id AS anchor_id, ctg.block_id AS lesson_id, COUNT(*) AS n FROM \
         block_tags atg JOIN block_tags ctg ON ctg.tag = atg.tag AND ctg.block_id <> atg.block_id \
    ...
    let rows = handle
        .query(
            "SELECT lesson_id, n FROM probe_overlap WHERE anchor_id='t1' ORDER BY n DESC, \
             lesson_id",
            HashMap::new(),
        )
        .await
    ...
    holon_turso::matview_manager::reconcile_named_view(
        ...
        "SELECT atg.block_id AS anchor_id, ctg.block_id AS lesson_id, COUNT(*) AS n FROM \
         block_tags atg JOIN block_tags ctg ON ctg.tag = atg.tag AND ctg.block_id <> atg.block_id \
         LEFT JOIN advice_suppressed s ON s.anchor_id = atg.block_id AND s.lesson_id = \
         ctg.block_id WHERE s.lesson_id IS NULL GROUP BY atg.block_id, ctg.block_id",
    )
    .await
...
/// proxy-trigger watches (and the id-less-junction-row enrichment panic with
/// it).
///
...
async fn probe_outer_antijoin_is_incrementally_maintained() {
    ...
    let sql = "SELECT v.anchor_id AS anchor_id, v.lesson_id AS id, v.shared_tag_count AS \
               shared_tag_count FROM advice_rule_lessons_for_tasks v LEFT JOIN advice_suppressed \
               s ON s.anchor_id = v.anchor_id AND s.lesson_id = v.lesson_id JOIN block_raw c ON \
               c.id = v.lesson_id WHERE s.lesson_id IS NULL";
    let created =
...
async fn probe_multi_junction_fanout_fix_shapes() {
    ...
    let single_view_shape = |distinct: &str| {
        format!(
            "SELECT b.id, COALESCE(json_group_array({distinct}bt.tag) FILTER (WHERE bt.tag IS NOT \
             NULL), '[]') AS tags, COALESCE(json_group_array({distinct}br.required_id) FILTER \
             (WHERE br.required_id IS NOT NULL), '[]') AS requires, \
             COALESCE(json_group_array({distinct}asup.lesson_id) FILTER (WHERE asup.lesson_id IS \
             NOT NULL), '[]') AS advice_suppressed FROM block_raw b LEFT OUTER JOIN block_tags bt \
             ON bt.block_id = b.id LEFT OUTER JOIN block_requires br ON br.block_id = b.id LEFT \
             OUTER JOIN advice_suppressed asup ON asup.anchor_id = b.id GROUP BY b.id"
        )
    };
    ...
    holon_turso::matview_manager::reconcile_named_view(
        &handle,
        "probe_block_chained",
        "SELECT b.id, COALESCE(ta.vals, '[]') AS tags, COALESCE(ra.vals, '[]') AS requires, \
         COALESCE(sa.vals, '[]') AS advice_suppressed FROM block_raw b LEFT OUTER JOIN \
         probe_tags_agg ta ON ta.source_id = b.id LEFT OUTER JOIN probe_requires_agg ra ON \
         ra.source_id = b.id LEFT OUTER JOIN probe_sup_agg sa ON sa.source_id = b.id",
    )
    .await
    ...
    let changed = holon_turso::matview_manager::reconcile_named_view(
        ...
        "probe_block_chained",
        "SELECT b.id, COALESCE(ta.vals, '[]') AS tags, COALESCE(ra.vals, '[]') AS requires, \
         COALESCE(sa.vals, '[]') AS advice_suppressed, b.updated_at FROM block_raw b LEFT OUTER \
         JOIN probe_tags_agg ta ON ta.source_id = b.id LEFT OUTER JOIN probe_requires_agg ra ON \
         ra.source_id = b.id LEFT OUTER JOIN probe_sup_agg sa ON sa.source_id = b.id",
    )
    .await;
    assert!(
