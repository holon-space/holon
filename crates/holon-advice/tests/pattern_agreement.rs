...
//! the SAME binding set — on a seeded journal fixture and across generated worlds.
//!
...
use holon_api::Value;
use holon_api::pattern::{
    BuiltinRef, CmpOp, CurrentSchema, FieldRef, Guard, InMemoryWorld, Operand, PathPattern,
    PathSegment, Pattern, Subject, WorldBlock,
};
...
/// A world of up to 6 blocks (parents point only at earlier blocks, so no cycles)
/// plus a clock value. Deliberately dense in `Journals` / date names so the
/// journal inhibitor flips both ways.
