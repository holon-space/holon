...
//! discovered by the exact `content_type = 'source'` scan that
//! `get_profiles.sql` uses for entity profiles. This module owns the SQL asset
//! and the parse of a discovered block into a typed rule (or its typed error —
//! carried, not dropped).
...
/// A discovered rule keyed by its block id, carrying either the parsed rule or
/// the typed parse error. The error is returned (not logged-and-dropped) so the
/// caller can render it on the rule block (ADR 0022 "rule blocks render their
/// own status").
