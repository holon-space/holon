...
//! 'holon_advice_rule_yaml'`) parsed at the boundary into a closed, typed
//! representation. Illegal states are refused at parse — never carried forward
//! as strings to be re-validated. The typed `AnchorSelector` lowers to a SQL
//! `WHERE` fragment (injection-safe by refusal, not escaping), which keeps the
//! anchor set IVM-computable.
...
pub struct AdviceRule {
    ...
    pub name: RuleSlug,
    /// SQL-lowerable typed anchor predicate (which blocks get advice woven
    /// under them).
    ...
    #[serde(default)]
    n: Option<BoundedN>,
    /// Explicit activation flag — gates matview DDL reconcile (ADR 0022
    /// sub-decision 1).
    pub active: bool,
    /// RESERVED versioned raw-query escape hatch (ADR 0022 sub-decision 3). The
    /// schema reserves it day one; v1 parse REFUSES any rule that sets it. Kept
    /// as a field (never read) so `deny_unknown_fields` routes a set value
    /// into `Reserved`'s always-failing deserializer rather than an
    /// "unknown field".
    #[serde(default)]
    #[allow(dead_code)]
...
/// A stable rule slug. Constrained to a SQL-identifier-safe charset because it
/// is interpolated directly into the matview name (`advice_rule_{slug}`) — an
/// identifier position that cannot be parameterized, so it is validated, not
/// escaped.
...
/// Top-K bound: `1..=10`. A larger K is refused at parse — not clamped, not a
/// bare `usize`.
...
/// Retrieval-width bound: `1..=50` (ADR 0023). The recall-oriented `LIMIT N`
/// the matview read pulls before the app-layer reranker selects the final
/// top-K. A larger N is refused at parse — not clamped, not a bare `usize`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// NOT a scripting expression — that is what keeps the anchor set
/// IVM-computable rather than a per-commit full scan (ADR 0022).
///
/// Authored as a single-key YAML map (`has_tag: task`, `entity: note`,
/// `prop_eq: {key, value}`, `and: [...]`). serde_yaml renders *derived*
/// externally tagged enums as `!tag` YAML tags, which is hostile to
/// hand-authored config, so this is deserialized via a single-key wire struct
/// with an exactly-one-key check.
    ...
    fn deserialize<D: serde::Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
        ...
        if set != 1 {
            return Err(serde::de::Error::custom(format!(
                "an anchor selector needs exactly one of `entity` / `has_tag` / `prop_eq` / `and` \
                 (found {set})"
            )));
        }
...
/// Closed scoring template. v1 has a single variant; new relevance ideas ship
/// as new variants (data-compatible additions), never as raw user query text.
///
...
pub struct TagOverlapRecencySpec {
    /// Which blocks are eligible candidates (typed, SQL-lowerable like the
    /// anchor).
    pub source: AnchorSelector,
}

/// The RESERVED raw-query escape hatch (ADR 0022 sub-decision 3). Any attempt
/// to deserialize a value into this type fails loudly. `raw_query: null` (or
/// absent) is treated as unset and allowed.
#[derive(Debug, Clone, PartialEq)]
struct Reserved;
...
impl<'de> Deserialize<'de> for Reserved {
    fn deserialize<D: serde::Deserializer<'de>>(_: D) -> Result<Self, D::Error> {
        Err(serde::de::Error::custom(
            "the `raw_query` field is reserved and not yet supported (ADR 0022 sub-decision 3): \
             raw PRQL/SQL scoring will open up via this field in a later version; v1 refuses it \
...
/// deserialize a value into this type fails loudly. `rerank: null` (or absent)
...
impl<'de> Deserialize<'de> for RerankReserved {
    fn deserialize<D: serde::Deserializer<'de>>(_: D) -> Result<Self, D::Error> {
        Err(serde::de::Error::custom(
            "the `rerank` field is reserved and not yet supported (ADR 0023 stage 2): it will \
             name a reranker model PROFILE (endpoint/key/consent are device-local prefs, never \
             synced); reranking lands in Increment H; v1 refuses it rather than silently ignoring \
             it",
        ))
    }
}
...
/// status — never logged-and-dropped (ADR 0022 "rule blocks render their own
/// status").
#[derive(Debug, Clone, Error, PartialEq)]
pub enum AdviceRuleParseError {
    #[error(
        "invalid advice-rule slug {slug:?}: must be non-empty and only [a-z0-9_] (it is \
         interpolated into a matview identifier)"
    )]
    Slug { slug: String },
    ...
    #[error(
        "advice-rule k={k} exceeds retrieval width n={n}: cannot display more rows than are \
         retrieved (ADR 0023 — k is the final top-K, n the recall-oriented LIMIT N feeding the \
         reranker)"
    )]
...
/// Parse a rule block's YAML content into a typed `AdviceRule`, or a typed
/// error.
///
/// Refused at this boundary: unknown fields (`deny_unknown_fields`), a slug
/// outside `[a-z0-9_]`, `k`/`n` over their caps, `k > n`, and any use of the
/// reserved `raw_query` / `rerank` fields.
pub fn parse_advice_rule(yaml_content: &str) -> Result<AdviceRule, AdviceRuleParseError> {
    let rule = serde_yaml::from_str::<AdviceRule>(yaml_content).map_err(|e| {
        AdviceRuleParseError::Yaml {
            message: e.to_string(),
        }
    })?;
