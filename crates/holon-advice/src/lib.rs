...
//! discovered by the entity-profile scan pattern, parsed at the boundary into a
//! closed typed model, and compiled by the engine into ONE anchor-denormalized
//! materialized view per rule (`advice_rule_{slug}`) via `reconcile_named_view`.
//!
...
pub use discovery::{
    ADVICE_RULE_SOURCE_LANGUAGE, DiscoveredRule, GET_ADVICE_RULES_SQL, is_advice_rule_block,
    parse_discovered_rule,
};
pub use lowering::LoweringError;
pub use reconcile_plan::{AdviceReconcilerState, ReconcilePlan, RuleEvent, StatusOutcome};
pub use rule::{
    AdviceRule, AdviceRuleParseError, AnchorSelector, BoundedK, BoundedN, PropEqSpec, RuleSlug,
    ScoringTemplate, TagOverlapRecencySpec, parse_advice_rule,
};
pub use status::{AdviceRuleStatus, AdviceRuleStatusHandle};
pub use synthesis::{
    AdviceSynthesisError, ReconcileOutcome, SynthesizedMatview, reconcile_advice_rule,
    synthesize_matview,
};

/// The bundled lessons-for-tasks rule (ADR 0022 v1 cut). Shipped here as a crate
/// asset for the DDL snapshot test; final vault seeding (as an org block under
/// `assets/default/`) is wired in Increment F Step 6 — see the report's open items.
...
mod tests {
    ...
    /// Snapshot of the synthesized DDL for the bundled rule. If this changes, the
    /// IVM shape changed — update deliberately and re-check against a live matview.
    #[test]
    fn bundled_rule_ddl_snapshot() {
        ...
        let expected = "SELECT\n    \
            ...
            FROM block_tags atg\n\
            JOIN block_tags ctg ON ctg.tag = atg.tag AND ctg.block_id <> atg.block_id\n\
            JOIN block_tags ht0 ON ht0.block_id = atg.block_id AND ht0.tag = 'task'\n\
            JOIN block_tags ht1 ON ht1.block_id = ctg.block_id AND ht1.tag = 'lesson'\n\
