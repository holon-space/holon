...
//! inner JOIN onto `block_tags` (proven IVM-correct — Spike-2 probes); `block_type`
//! / `properties` predicates need a 1:1 JOIN onto `block_raw` used ONLY in `WHERE`
//! (a `block_raw` column in `GROUP BY`/`SELECT` corrupts the IVM aggregate — see
//! the probe tests). The selector is lowered relative to an *id expression* (e.g.
//! `atg.block_id`) rather than a fixed alias, because the synthesized matview
//! identifies anchors/candidates by the `block_tags` overlap columns, not a
//! `block_raw` alias.
//!
//! Injection safety is by REFUSAL, not escaping (parse-don't-validate): string
//! values are validated against a conservative charset and refused if they carry
//! anything exotic, then wrapped in single quotes.

use thiserror::Error;

use crate::rule::{AnchorSelector, PropEqSpec};

/// SQL fragments a selector contributes to the synthesized matview.
#[derive(Debug, Clone, Default, PartialEq)]
pub struct SqlFragments {
    /// Full `JOIN …` clauses (each a self-contained IVM-safe inner join onto `block_tags`).
    pub joins: Vec<String>,
    /// `WHERE` conjuncts (plain column/scalar predicates — no subqueries).
    pub predicates: Vec<String>,
    /// True if any predicate reads `block_raw` columns → the synthesizer must add a
    /// 1:1 `JOIN block_raw {raw_alias} ON {raw_alias}.id = {id_expr}` (WHERE-only use).
    pub needs_block_raw: bool,
}

/// A typed lowering error, carried so a broken rule fails loud on its own block.
#[derive(Debug, Error, PartialEq)]
pub enum LoweringError {
    #[error(
        "advice-rule string value {value:?} for {field} contains characters that \
         are refused at the SQL boundary (allowed: letters, digits, space, and \
         `_ - . / :`); refusing rather than escaping"
    )]
    UnsafeValue { field: &'static str, value: String },

    #[error(
        "advice-rule property key {key:?} is not a safe JSON key (allowed: \
         letters, digits, `_`); refusing rather than escaping"
    )]
...
impl AnchorSelector {
    ...
    ///
    /// - `id_expr`: the block-id expression this selector filters (e.g. `atg.block_id`).
    /// - `raw_alias`: the `block_raw` alias to use if a predicate needs `block_raw`
    ///   columns; the synthesizer joins it 1:1 iff `needs_block_raw` is set.
    /// - `join_seq`: mints unique `block_tags` join aliases across the whole query.
    pub fn lower(
        &self,
        id_expr: &str,
        raw_alias: &str,
