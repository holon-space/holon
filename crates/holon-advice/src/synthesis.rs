...
//!
//! Each active rule compiles to EXACTLY ONE anchor-denormalized materialized view
//! `advice_rule_{slug}` carrying an `anchor_id` output column, read at render time
//! with `WHERE anchor_id = ?`. This is the Spike-2 shape: one matview per *rule*,
//! denormalized over all anchors — never one matview per anchor (the N-matview
//! cliff). The suppression anti-join (`LEFT JOIN advice_suppressed … IS NULL`) is
//! implied by every rule and never configurable (ADR 0021).
...
use crate::rule::{AdviceRule, ScoringTemplate, TagOverlapRecencySpec};

/// The synthesized matview for a rule: a stable name + the `SELECT` body that
/// `reconcile_named_view` wraps in `CREATE MATERIALIZED VIEW {name} AS {select}`.
...
///   <> atg.block_id` excludes self → one row per (anchor, candidate) shared tag.
/// - `COUNT(*)` = shared-tag count (block_tags PK `(block_id, tag)` makes each
///   (atg, ctg) pairing unique per shared tag).
/// - anchor/candidate predicates lower to `block_tags` joins (`has_tag`) or a 1:1
///   `block_raw` join used ONLY in `WHERE` (`entity`/`prop_eq`).
///
/// DELIBERATELY NOT in this matview (Spike-2 findings — the naive in-matview forms
/// are silently miscompiled by current Turso IVM, see probe tests, resolving
/// ADR 0021's open stage-5 fork toward read-time application):
/// - **Suppression** (`LEFT JOIN advice_suppressed … IS NULL`): an in-matview
///   anti-join is IGNORED by IVM. Applied at READ time instead.
/// - **Recency** (`updated_at`): a `block_raw` column in `GROUP BY`/`SELECT`
///   corrupts the IVM aggregate. The `updated_at` tiebreak is applied at READ time
///   (join `block` for `updated_at`).
...
///   `ORDER BY shared_tag_count DESC, updated_at DESC LIMIT k`. Keeping the matview
///   `ORDER BY`-free also keeps `reconcile_named_view` idempotent across boots.
...
pub enum ReconcileOutcome {
    /// Active rule: matview was created or its definition changed and was recreated.
    Created,
...
/// Reconcile a rule's matview through `matview_manager`'s named/diffed/torn-down
/// path — never the content-addressed `watch_view_{hash}` path (which would mint
/// one matview per anchor: the N-matview cliff, ADR 0022).
///
/// - `active` rule → synthesize + `reconcile_named_view` (unchanged view skipped;
///   changed view recreated).
/// - inactive rule → DROP the matview if present (ADR 0022 sub-decision 1: flipping
///   `ACTIVE` off tears the view down).
