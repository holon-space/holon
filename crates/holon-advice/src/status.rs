//! Advice-rule compilation/runtime status (ADR 0022 "rule blocks render their own
//! status — fail loud, visibly").
//!
//! A rule block's synthesis outcome is carried here, keyed by the rule block's id,
//! written by the engine's advice reconciler and read by the UI watchers so a broken
//! rule renders its error IN PLACE rather than silently no-op'ing. Synchronous parse
//! errors and asynchronous DDL failures alike surface through this one map.
...
///
/// Only [`AdviceRuleStatus::Active`] renders normally; every other variant replaces
/// the block's render with its message (the fail-loud-visibly surface).
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum AdviceRuleStatus {
    /// Rule parsed and its matview reconciled (or was torn down because the rule is
    /// inactive) — nothing is wrong. Renders normally (no banner in v1).
    ...
    ParseError(String),
    /// The rule parsed but its matview synthesis / reconcile failed (an asynchronous
    /// DDL failure at reconcile time — never a silent no-op, ADR 0022).
    DdlError(String),
    /// A second rule block claimed a slug already owned by a live different block.
    /// First-owner-wins: the newcomer is refused (no reconcile), and this status
    /// names the winning owner block.
...
/// Shared, cheaply-clonable handle to the advice-rule status map (block id → status).
///
/// The engine's reconciler task is the sole writer; the UI watchers are readers.
