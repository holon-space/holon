...
//!    delimiter to be followed by whitespace / punctuation / end. So a mark on
//!    a substring that's word-adjacent to the surrounding text (e.g. Bold over
//!    chars 0..1 of "abc") renders to `*a*bc` and the parser sees that as
//!    literal text, not Bold. Real frontends apply marks to whole words /
//!    selections, so this isn't a UX issue, but it's a parser/renderer
//!    asymmetry callers should know about.
//!
//! 2. **Nested marks ending at the same position**: `*foo=bar=*` — Verbatim
//!    ending immediately before Bold's close — sometimes fails to re-parse
//!    cleanly. Rare in practice (renderer naturally avoids this when emitting
//!    from clean mark sets) but a known orgize edge case. The 27 unit tests in
//!    `inline_marks.rs::tests` cover the common nested patterns explicitly.
//!
//! # What's NOT tested (deliberate)
//!
//! - **Crossing marks**: `render_inline_marks` documents these as lossy (Org
//!   cannot represent `*A /B* C/`); a `tracing::warn!` is emitted.
//! - **Link marks**: `EntityRef` carries arbitrary URLs / block IDs; generating
//!   valid ones is a separate concern. Unit tests cover Link.
//! - **Sub / Super**: `_{…}` and `^{…}` have specific peeling logic; covered by
//!   unit tests.

use holon_api::InlineMark;
use holon_api::MarkSpan;
use holon_org_format::extract_inline_marks;
use holon_org_format::render_inline_marks;
