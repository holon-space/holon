...
struct SectionContent {
    ...
    // ALLOW(fallback): orgize misclassifies SCHEDULED as PARAGRAPH when properties drawer precedes planning
    /// SCHEDULED extracted from paragraph text (fallback when orgize misclassifies)
    scheduled_fallback: Option<String>,
    // ALLOW(fallback): orgize misclassifies DEADLINE as PARAGRAPH when properties drawer precedes planning
...
fn extract_section_content(headline: &Headline) -> SectionContent {
    ...
    // ALLOW(fallback): regex-extract SCHEDULED/DEADLINE from PARAGRAPH text orgize misclassifies
...
";
    ...
    fn test_parser_no_marks_keeps_content_byte_identical() {
        ...
        // ALLOW(compatibility): describes a parser invariant for legacy headings, not a versioning shim
