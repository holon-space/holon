...
- **vs naive org:** TOON saves a meaningful **~12–13%** on the drawer-heavy,
  DONE-task-dominated motivating file, but only **~2%** on the
...
# Generic tabular: JSON vs TOON on query results

The block-forest verdict above is *not* the end of the story. TOON's tabular
arrays win exactly on the shape a block forest lacks — **wide, uniform,
value-heavy rows** — and that shape is precisely what the `holon` MCP
`execute_query` / `execute_raw_sql` tools return: a JSON array of row objects
that repeats every column name once per row. So the codec was generalized
(`src/table.rs`) and wired into those tools as the default output format
(`format: "json"` is the explicit opt-out).

Reproduce (fully synthetic — no vault content):

```
cargo run -p holon-toon --example measure --features measure -- synthetic
```

We measure the **rows payload** both ways: a JSON array of row objects
(`[{...}, ...]`, matching what the MCP tool emits) vs a single TOON tabular
array. Same tokenizer (`o200k_base`); ratios are the finding.

## Results

| shape | rows | json | toon | TOON saves | round-trip |
|---|--:|--:|--:|--:|:--:|
| wide uniform (40×8) | 40 | 1773 | 1030 | **41.9%** | ok |
| narrow (60×2) | 60 | 663 | 488 | **26.4%** | ok |
| with JSON column (20×3) | 20 | 588 | 595 | −1.2% | ok |
| tiny (2×3) | 2 | 32 | 30 | 6.2% | ok |

## Reading the numbers

- **Wide uniform tables win big (~42%).** Every column name (`id`, `state`,
  `priority`, `effort`, `assignee`, `done`, `progress`, `title`) is written once
  in the header instead of on all 40 rows. The more columns × rows, the larger
  the win — the exact opposite economics of the id-dominated block forest.
- **Narrow tables still win (~26%).** Even two columns repeated across 60 rows
  is enough repeated-key tax for the header to pay off.
- **A nested-JSON column erodes the win (−1.2% here).** The JSON string must be
  quoted and its inner `"`/`,`/`:` escaped, which re-adds per-row bytes; when a
  large opaque JSON blob dominates each row, JSON-in-cell can even lose slightly.
  TOON is worth requesting when the *scalar* columns dominate.
- **Tiny results barely move (~6%).** With 2 rows the header is nearly the whole
  payload, so there is little repetition to dedup — JSON is fine here.

## Recommendation

TOON is the **default** MCP query output (Martin's ruling — the common shape is
wide/uniform, where it saves 25–40%+ of the payload); `format: "json"` is the
explicit opt-out for small or heterogeneous results. This is the inverse
of the block-forest verdict, and it is why the prototype was generalized rather
than deleted: TOON is the wrong container for a narrow, deep, id-dominated
forest and the right one for a wide, uniform table.

## Caveats / threats to validity (generic section)

- Synthetic shapes, chosen to bracket the real cases; absolute counts are
  illustrative, ratios are the finding.
- The comparison is the *rows payload* only. JSON's real MCP response also
  carries a small `{"row_count":N,"duration_ms":…}` envelope TOON omits (the
  header's `[N]` already states the count), so TOON's real-world edge is a hair
  larger than the table shows.
- o200k ≠ Claude's tokenizer (as above).
