...
pub trait DocumentManager: Send + Sync {
    ...
    async fn name_chain(&self, doc_id: &str) -> Result<Vec<String>> {
        ...
        loop {
            ...
            if doc.id == EntityUri::doc_root().to_string()
                || EntityUri::from_raw(&doc.parent_id).is_sentinel()
            {
                break;
            }

            chain.push(doc.name.clone());
            current_id = doc.parent_id.to_string();
        }
    ...
    async fn find_by_name_chain(&self, chain: &[&str]) -> Result<Option<Document>> {
        ...
        let mut current_parent_id = EntityUri::doc_root().to_string();
        ...
        for segment in chain {
            ...
            {
                Some(doc) => {
                    current_parent_id = doc.id.to_string();
    ...
    async fn get_or_create_by_name_chain(&self, chain: &[&str]) -> Result<Document> {
        ...
        for segment in chain {
            ...
            {
                Some(existing) => {
                    current_parent_id = existing.id.to_string();
                    current_doc = Some(existing);
                }
                None => {
                    let new_doc = Document::new(
                        EntityUri::doc_random().to_string(),
                        current_parent_id.clone(),
                        segment.to_string(),
                    );
                    let created = self.create(new_doc).await?;
                    current_parent_id = created.id.to_string();
