...
impl BlockReader for CacheBlockReader {
    async fn get_blocks(&self, doc_id: &str) -> anyhow::Result<Vec<Block>> {
        ...
        frontier.insert(doc_id.to_string());

        loop {
            let mut next_frontier = HashSet::new();
            let mut found_any = false;
            // Scan all blocks, collect those whose parent_id is in the frontier.
            // This is O(levels * total_blocks) but avoids N+1 SQL queries.
            // For typical document sizes (tens to hundreds of blocks) this is fast.
            for block in &all_blocks {
                if frontier.contains(block.parent_id.as_str())
                    && !result.iter().any(|b: &Block| b.id == block.id)
                {
                    next_frontier.insert(block.id.as_str().to_string());
    ...
    async fn iter_documents_with_blocks(&self) -> Vec<(String, Vec<Block>)> {
        use holon::core::datasource::DataSource;
        ...
        let mut children_of: HashMap<String, Vec<&Block>> = HashMap::new();
        for block in &all_blocks {
            children_of
        ...
        let doc_uris: Vec<String> = children_of
            .keys()
            .filter(|pid| {
                holon_api::EntityUri::parse(pid)
                    .map(|u| u.is_doc())
                    .unwrap_or(false)
            })
            .cloned()
        ...
        for doc_uri in doc_uris {
            ...
            let mut queue = vec![doc_uri.clone()];
            while let Some(pid) = queue.pop() {
                if let Some(children) = children_of.get(&pid) {
                    for block in children {
                        queue.push(block.id.as_str().to_string());
    ...
    async fn find_foreign_blocks(
        ...
        Ok(all_blocks
            .iter()
            .filter(|b| id_set.contains(b.id.as_str()) && b.parent_id.as_str() != expected_doc_uri)
            .map(|b| (b.id.as_str().to_string(), b.parent_id.as_str().to_string()))
            .collect())
    }
}

/// DocumentManager backed by DocumentOperations.
pub struct DocumentManagerAdapter {
    doc_ops: Arc<DocumentOperations>,
}

impl DocumentManagerAdapter {
    pub fn new(doc_ops: Arc<DocumentOperations>) -> Self {
        Self { doc_ops }
    }
}

#[async_trait::async_trait]
impl DocumentManager for DocumentManagerAdapter {
    async fn find_by_parent_and_name(
        ...
        self.doc_ops
            .find_by_parent_and_name(parent_id, name)
            .await
            .map_err(|e| anyhow::anyhow!("{}", e))
    }
    ...
    async fn get_by_id(&self, id: &str) -> anyhow::Result<Option<Document>> {
        self.doc_ops
            .get_by_id(id)
            .await
            .map_err(|e| anyhow::anyhow!("{}", e))
...
impl crate::org_sync_controller::AliasRegistrar for LoroAliasRegistrar {
    async fn register_alias(&self, doc_id: &str, path: &Path) {
        let store = self.doc_store.read().await;
        store.register_alias(doc_id, path).await;
    }

    async fn resolve_alias_to_path(&self, doc_id: &str) -> Option<PathBuf> {
        let store = self.doc_store.read().await;
        store.resolve_alias_to_path(doc_id).await
    }
}
        ...
        services.add_trait_factory::<dyn OperationProvider, _>(
            ...
            move |resolver| {
                ...
                // on_file_changed and on_block_changed — no locks needed.
                {
                    ...
                    tokio::spawn(async move {
                        let doc_manager: Arc<dyn DocumentManager> =
                            Arc::new(DocumentManagerAdapter::new(doc_ops_clone));
                                ...
                                loop {
                                    tokio::select! {
                                        ...
                                        Some(event) = tokio_stream::StreamExt::next(&mut event_rx) => {
                                            ...
                                            if doc_ids.is_empty() {
                                                // block.deleted / block.fields_changed: can't determine doc_id,
                                                // re-render all tracked files
...
fn extract_doc_ids_from_event(event: &holon::sync::event_bus::Event) -> Vec<String> {
    ...
    match event.event_kind {
            ...
            if let Some(data) = event.payload.get("data") {
                if let Some(parent_id) = data.get("parent_id").and_then(|v| v.as_str()) {
                    if let Ok(uri) = holon_api::EntityUri::parse(parent_id) {
                        if uri.is_doc() {
                            doc_ids.insert(uri.as_str().to_string());
                        }
                    }
                }
            }
        ...
        EventKind::FieldsChanged => {
            // can't determine doc_id from fields_changed events
        }
    }
