...
impl OrgSyncController {
    ...
    pub async fn initialize(&mut self) {
        ...
        for (doc_id, blocks) in documents {
            ...
            let rendered = OrgRenderer::render_blocks(&blocks, &file_path, &doc_id);
    ...
    async fn resolve_doc_to_path(&self, doc_id: &str) -> Option<PathBuf> {
    ...
    pub async fn on_file_changed(&mut self, path: &Path) -> Result<()> {
        ...
        let document = self
            ...
            .await?;
        let document_uri = document.id.to_string();

        // Register UUID → file path alias (if Loro is available)
        if let Some(ref registrar) = self.alias_registrar {
            registrar.register_alias(document.id.as_str(), path).await;
        }

        let file_id = generate_file_id(path, &self.root_dir).to_string();

        // Parse old state (from last_projection) and new state (from disk)
        let old_blocks = if last.is_empty() {
            HashMap::new()
        } else {
            match parse_org_file(path, last, "root", 0, &self.root_dir) {
                Ok(result) => result
                    .blocks
                    .into_iter()
        ...
        let new_parse = parse_org_file(path, &disk_content, "root", 0, &self.root_dir)?;
        let new_blocks_vec = new_parse.blocks;
        let new_blocks: HashMap<String, Block> = new_blocks_vec
            .iter()
            .map(|b| (b.id.to_string(), b.clone()))
            .collect();

        // Check for duplicate block IDs owned by other documents
        let new_block_ids: Vec<String> = new_blocks_vec
            .iter()
            .filter(|b| !old_blocks.contains_key(b.id.as_str()))
        ...
        for block in &new_blocks_vec {
            if !old_blocks.contains_key(block.id.as_str()) {
                let parent_id = if block.parent_id.as_raw_str() == file_id {
        ...
        for (id, new_block) in &new_blocks {
            if let Some(old_block) = old_blocks.get(id) {
                if blocks_differ(old_block, new_block) {
                    let parent_id = if new_block.parent_id.as_raw_str() == file_id {
                        document_uri.to_string()
                    } else {
                        new_block.parent_id.to_string()
                    };
        ...
        // Re-project from block reader (may have merged concurrent changes)
        let rendered = self
            .render_file_by_doc_id(document.id.as_str(), path)
            .await?;
        assert!(
    ...
    pub async fn on_block_changed(&mut self, doc_id: &str) -> Result<()> {
        let path = self.doc_id_to_path(doc_id).await;

        let path = match path {
            Some(p) => p,
            None => {
                debug!(
                    "[OrgSyncController] No path for doc_id {}, skipping",
                    doc_id
                );
                return Ok(());
            }
    ...
    pub async fn re_render_all_tracked(&mut self) -> Result<()> {
        ...
        for path in paths {
            ...
            let rendered = self.render_file_by_doc_id(doc.id.as_str(), &path).await?;
    ...
    async fn render_file_by_doc_id(&self, doc_id: &str, path: &Path) -> Result<String> {
        let blocks = self.block_reader.get_blocks(doc_id).await?;
        let rendered = OrgRenderer::render_blocks(&blocks, path, doc_id);
        assert!(
    ...
    async fn doc_id_to_path(&self, doc_id: &str) -> Option<PathBuf> {
