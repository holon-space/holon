...
/// - `doc:doc.org` for files in the root
/// - `doc:projects/todo.org` for nested files
...
pub fn parse_org_file(
    ...
    let parent_uri =
        EntityUri::parse(parent_dir_id).unwrap_or_else(|_| EntityUri::doc(parent_dir_id));
    let mut document = Document::new(file_id.to_string(), parent_uri.to_string(), file_name);
...
mod tests {
    ...
    fn test_generate_ids() {
        ...
        assert_ne!(id1, id2);
        assert!(id1.is_doc());
        // Should be relative paths with doc: scheme
        assert_eq!(id1.as_str(), "doc:file1.org");
        assert_eq!(id2.as_str(), "doc:file2.org");
