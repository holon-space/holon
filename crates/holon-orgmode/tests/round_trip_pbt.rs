...
fn document_strategy() -> impl Strategy<Value = Document> {
    (
        ...
        .prop_map(|(title, todo_keywords)| {
            ...
            let mut doc = Document::new(id, EntityUri::doc_root(), "test.org".to_string());
...
fn corpus_document_from_index_org() -> CompleteDocument {
    ...
    let parse_result = parse_org_file(&file_path, &org_text, &EntityUri::doc_root(), 0, &root)
...
fn build_org_text(doc: &Document, blocks: &[Block]) -> String {
    ...
    let rendered = OrgRenderer::render_blocks(blocks, &file_path, &doc.id);
...
fn parse_org(org_text: &str) -> Result<holon_orgmode::parser::ParseResult, String> {
    ...
    let root = PathBuf::from("/test");
    parse_org_file(&path, org_text, &EntityUri::doc_root(), 0, &root).map_err(|e| e.to_string())
}
...
proptest! {
    ...
    fn test_render_order_independent(mut complete_doc in complete_document_strategy(), seed in any::<u64>()) {
        ...
        let blocks = complete_doc.all_blocks();
        let file_path = PathBuf::from("/test/test.org");
        let render_canonical = OrgRenderer::render_blocks(&blocks, &file_path, &complete_doc.document.id);
        ...
        let render_shuffled = OrgRenderer::render_blocks(&shuffled, &file_path, &complete_doc.document.id);
