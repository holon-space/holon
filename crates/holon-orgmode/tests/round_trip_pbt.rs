...
impl NormalizedDocument {
    fn from_doc_and_blocks(doc: &Block, blocks: &[Block]) -> Self {
        ...
        NormalizedDocument {
            title: doc.file_title().map(|t| t.trim().to_string()),
...
impl HeadlineSpec {
    ...
    fn to_block(&self, parent_id: &EntityUri, sequence: &mut i64) -> Vec<Block> {
...
impl CompleteDocument {
    fn all_blocks(&self) -> Vec<Block> {
        ...
        for headline in &self.root_headlines {
            blocks.extend(headline.to_block(&doc_id, &mut sequence));
        }
