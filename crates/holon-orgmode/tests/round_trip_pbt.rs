...
//!
//! Generators + comparison shapes (`HeadlineSpec`, `NormalizedBlock`, ...) live
...
// ============================================================================
// Org-flavored document strategy (uses OrgDocumentExt for `#+TITLE:` /
// `#+TODO:` headers, which are org-specific concerns).
// ============================================================================

fn document_strategy() -> impl Strategy<Value = Block> {
    (
...
impl CompleteDocument {
    fn all_blocks(&self) -> Vec<Block> {
        build_blocks(&self.document.id, &self.root_headlines)
    }
}

/// Walk the document, collect every TODO/DONE keyword used by any headline,
/// and either configure `#+TODO:` accordingly or clear it (when every keyword
/// is in the parser defaults). Org-specific.
fn ensure_todo_keywords_configured(cd: &mut CompleteDocument) {
    ...
    for headline in &cd.root_headlines {
    ...
    if all_within_defaults {
        cd.document.set_todo_keywords(None);
    } else {
        let states: Vec<TaskState> = all_todos
            ...
            .collect();
        cd.document.set_todo_keywords(Some(states));
    }
}

fn complete_document_strategy() -> impl Strategy<Value = CompleteDocument> {
    prop_oneof![
        // ~80% random documents (with org-specific TODO-keyword propagation applied)
        4 => document_strategy().prop_flat_map(|doc| {
            let doc_clone = doc.clone();
            root_headlines_strategy().prop_map(move |root_headlines| {
                let mut cd = CompleteDocument {
                    document: doc_clone.clone(),
                    root_headlines,
                };
                ensure_todo_keywords_configured(&mut cd);
                cd
            })
        }),
...
fn corpus_document_from_index_org() -> CompleteDocument {
    ...
    let parse_result = parse_org_file(&file_path, &org_text, &EntityUri::no_parent(), &root)
    ...
    let root_headlines = blocks
        .iter()
        .filter(|b| {
            (b.parent_id.is_sentinel() || b.parent_id.is_no_parent())
...
fn parse_org(org_text: &str) -> Result<holon_orgmode::parser::ParseResult, String> {
    ...
    let root = PathBuf::from("/test");
    parse_org_file(&path, org_text, &EntityUri::no_parent(), &root).map_err(|e| e.to_string())
}
...
proptest! {
    ...
    fn test_round_trip(mut complete_doc in complete_document_strategy()) {
        ...
        let expected = NormalizedDocument::from_blocks(complete_doc.document.file_title(), &blocks);
        let actual = NormalizedDocument::from_blocks(parse_result.document.file_title(), &parse_result.blocks);
        ...
        // Root-level blocks have parent_id = document block ID.
        // The strategy and parser create different document block IDs,
        // so compare root status rather than exact parent_id for root blocks.
        let expected_doc_id = &complete_doc.document.id;
        let actual_doc_id = &parse_result.document.id;
        ...
        for expected_block in &expected.blocks {
            ...
            let expected_is_root = expected_block.parent_id == *expected_doc_id;
            let actual_is_root = actual_block.parent_id == *actual_doc_id;
            ...
            if expected_block.content_type == ContentType::Source {
                prop_assert_eq!(
                    &expected_block.source_content,
                    &actual_block.source_content,
                    "Source content must match for source block '{}'",
                    expected_block.id
                );
    ...
    ///
    /// The full document PBTs above have unrelated pre-existing failures
    /// around source-block fence escaping and sibling ordering; this PBT
    /// isolates the tag invariant by building a minimal headline (no
    /// source blocks, no children, no body), so a failure here is
    /// unambiguously a tag-handling regression.
    ///
    /// Generated tags come from `valid_tag()`, which biases toward the
    /// spec-conformant `[A-Za-z0-9_]+` shapes but also exercises hyphens.
    /// Against upstream orgize this PBT fails as soon as a hyphenated
    /// tag is sampled (orgize follows org-element.el strictly and
    /// silently drops the tag block). Against `holon-space/orgize` the
    /// `ParseConfig::is_tag_char` predicate accepts `-`, so the PBT
    /// passes for both shapes.
    #[test]
    fn test_tags_round_trip(
        title in valid_title(),
        tags in prop::collection::vec(valid_tag(), 1..=5),
    ) {
        // Dedupe — org tag rendering deduplicates implicitly via the colon
        // separator, but the parser preserves order; a duplicate would make
        // expected/actual diverge for reasons unrelated to the round-trip.
        let mut seen = std::collections::HashSet::new();
        let unique_tags: Vec<String> = tags
            .into_iter()
            .filter(|t| seen.insert(t.clone()))
            .collect();
        prop_assume!(!unique_tags.is_empty());

        let block_id = EntityUri::block(&Uuid::new_v4().to_string());
        let doc_id = EntityUri::block(&format!("test-{}", Uuid::new_v4()));
        let mut doc = Block::new_text(doc_id.clone(), EntityUri::no_parent(), "test.org");
        doc.set_page(true);

        let mut block = Block::new_text(block_id.clone(), doc_id.clone(), &title);
        block.set_level(1);
        block.set_sequence(0);
        block.set_tags(holon_api::Tags::from(unique_tags.clone()));

        let blocks = vec![block];
        let org_text = build_org_text(&doc, &blocks);
        ...
        let parsed_block = parse_result
            .blocks
            .iter()
            .find(|b| b.id == block_id)
            .ok_or_else(|| {
                TestCaseError::fail(format!(
                    "block {block_id} missing after round-trip\n\norg:\n{org_text}"
                ))
            })?;

        let expected: BTreeSet<String> = unique_tags.iter().cloned().collect();
        let actual: BTreeSet<String> = parsed_block.tags().to_set();
        prop_assert_eq!(
            &expected,
            &actual,
            "tags must round-trip exactly\n\norg:\n{}",
            org_text,
        );

        // Tag block must not leak into the title.
        let parsed_title = parsed_block.org_title().to_string();
        for tag in &unique_tags {
            prop_assert!(
                !parsed_title.contains(&format!(":{tag}:"))
                    && !parsed_title.contains(&format!(":{tag} "))
                    && !parsed_title.ends_with(&format!(":{tag}")),
                "tag `{tag}` leaked into title `{parsed_title}`\n\norg:\n{org_text}",
            );
        }
    }
...
fn reassign_parser_sequences(blocks: &mut [Block]) {
    ...
    let mut roots: Vec<usize> = ids
        .iter()
        .enumerate()
        .filter(|(_, (_, pid, _, _, _))| pid.is_no_parent() || pid.is_no_parent())
        .map(|(i, _)| i)
        .collect();
    roots.sort_by(|&a, &b| {
        ids[a]
...
// `assert_normalized_docs_equal` lives in `holon_block_roundtrip_testing` —
// imported at the top of the file alongside the other shared utilities.
...
proptest! {
    ...
    fn test_in_memory_mutation(
        ...
        let expected = NormalizedDocument::from_blocks(baseline.document.file_title(), &expected_blocks);
        ...
        let actual = NormalizedDocument::from_blocks(actual_result.document.file_title(), &actual_result.blocks);
