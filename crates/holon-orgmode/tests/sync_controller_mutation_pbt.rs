...
impl TestFixture {
    fn new(temp_dir: &std::path::Path) -> Self {
        ...
        let doc_id = EntityUri::doc_random().to_string();
    ...
    fn seed_blocks(&self, blocks: &[Block]) {
        self.store.seed_blocks(&self.doc_id, blocks.to_vec());
    }

    fn get_stored_blocks(&self) -> Vec<Block> {
        self.store.get_all_blocks(&self.doc_id)
    }
}
...
fn generate_baseline_blocks(doc_id: &str, variant: u8) -> Vec<Block> {
    let doc_uri = EntityUri::from_raw(doc_id);
...
fn stabilize_blocks(blocks: &[Block], doc_id: &str, root_dir: &std::path::Path) -> Vec<Block> {
    let file_path = root_dir.join("test.org");
    let org_text = OrgRenderer::render_blocks(blocks, &file_path, doc_id);
    let parse_result = parse_org_file(&file_path, &org_text, ROOT_ID, 0, root_dir)
