...
mod fast_path_loro_presence_tests {
    ...
    /// reboot: `persist_file_hash` captures into a shared map, `load_file_hashes`
    /// serves it back — so a second controller's `initialize()` arms the fast
    /// path with the hash the first boot stamped (no hand-computed hash, no
    /// coupling to the renderer version or consolidator tag).
    ...
    async fn fast_path_reingests_when_loro_tree_empty() {
        ...
        assert!(
            mutations > 0,
            "fast path skipped ingest despite an empty Loro tree — SQL and Loro \
             would stay silently diverged (the WP-D / I2 regression)"
        );
    }

    /// Control: SQL hash matches AND the Loro tree holds the doc root → the fast
    /// path is still allowed to skip (no cold-boot perf regression).
    #[tokio::test]
