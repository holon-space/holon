...
use crate::peer_ops::{peer_alive_blocks, peer_create_block};
...
/// `multi_peer::get_alive_stable_ids`'s HashSet idiom): two CRDT containers that
/// share a stable id are one logical block, collapsed by the SQL `block_raw` PK
/// on projection.
