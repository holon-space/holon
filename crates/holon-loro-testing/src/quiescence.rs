...
//!
//! Waits until the `LoroSyncController` reports every change in both projected
//! documents as landed in the sink. Used by [`crate::LoroSut`]'s
...
pub async fn wait_for_loro_quiescence_on(
    ...
    loop {
        ...
        if handle
            .is_settled()
            .await
            .expect("wait_for_loro_quiescence: settle predicate failed")
        {
        ...
        if tokio::time::Instant::now() >= deadline {
            ...
            let msg = format!(
                "loro sync never quiesced within {timeout:?} after {attempts} attempts: \
                 the projection still owes the sink a change (watermarks behind the docs' heads, \
                 or pending facts)"
            );
