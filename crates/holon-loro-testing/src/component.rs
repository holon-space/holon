...
//!   `blocks_match`, `no_orphan`, `block_content`, `block_parent`) runs over
//!   the Loro realization *for free*, the §6 payoff: a new component lights up
//!   the whole catalog without touching it.
//! - [`SutLoroLog`] — the Loro-specific read surface (`loro_children_of`
//!   exposes the tree's authoritative fractional-index sibling order) that
//!   unlocks the `inv-loro-children-match-ref` cross-check against the
...
/// store this read cap observes (the F2 committed-content keystone). `Arc`
/// keeps the sharing uniform with the memory slice; `LoroBackend::clone`
/// already shares the CRDT doc, but `Arc` makes the single-store contract
/// explicit.
pub struct LoroBackendComponent {
    ...
    /// backend whose doc a controller is projecting into SQL (full mode).
...
impl LoroBackendComponent {
    ...
    /// `loro_had_errors` reports the controller's real error counter — the
...
impl SutLoroLog for LoroBackendComponent {
    ...
    /// counter — the production teeth for `inv-loro-no-errors`. A standalone
    /// CRDT (pure-Loro slice) has no controller, so `sync_handle` is `None`
    /// and the honest answer is `false` (§5.1 hidden capability: an honest
    ...
    /// in the tree (e.g. the virtual root): the body then skips it, so
    /// root-level order is not asserted here, matching the E2ESut
    /// realization.
