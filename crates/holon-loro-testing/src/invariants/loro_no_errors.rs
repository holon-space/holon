...
//! whenever a Loro store is wired. Asserts the LoroSyncController logged no error
//! since startup. Inert-but-honest in the pure-Loro slice (a standalone CRDT has
//! no sync controller, so `loro_had_errors` is structurally `false`); its real
