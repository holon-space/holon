...
//! `NonSeedBlocks` correspondence, co-located here (Phase 1a follow-on) now that
//! the correspondence framework + observable + `block_compare` leaf live on the
//! pbt-core floor.
//!
...
//! `run_selected` runs all invariants order-independently, so this per-crate arm
//! is exactly equivalent to holding it in the central multi-store table.
...
use holon_pbt_core::capabilities::{RefBackend, SutLoroLog};
use holon_pbt_core::composition::{CapId, CapInvariant, CapMap, Needs};
use holon_pbt_core::correspondence::{
    Converge, Correspondence, Extraction, NamedCompare, StoreProjection,
};
