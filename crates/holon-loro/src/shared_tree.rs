//! Shared tree management: extract subtrees for collaboration via
//! fork-and-prune.
//!
//! The fork-and-prune algorithm:
//! 1. Reparent the subtree root to tree root (CRITICAL: must happen before
//!    parent deletion)
...
//!
//! In the original (personal) tree, the extracted subtree is replaced with a
//! mount node that references the shared tree's ID.
...
pub enum HistoryRetention {
    /// Keep all history (including operations on now-deleted non-subtree
    /// nodes). Largest output but preserves full undo history of the
    /// subtree.
    Full,
    ...
    /// Smallest output. Use when history doesn't matter (e.g., one-time
    /// sharing).
    None,
}
...
pub fn extract_subtree(
    ...
    // Step 3: Collect ALL root-level nodes and their descendants, delete
    // non-subtree ones. We iterate roots to find all top-level trees, then walk
    // each one.
...
pub struct MountInfo {
    /// Unique identifier for the shared tree (used to look up the shared
    /// LoroDoc)
...
/// Remove a mount node and optionally re-integrate the shared subtree into the
/// personal tree. If `reintegrate_doc` is Some, the shared tree's contents are
/// imported back. If None, the mount node is simply deleted (unshare without
/// keeping content).
///
...
mod tests {
    ...
    ///
    /// Natural proof: an anonymous `LoroDoc` created with NO key material of
    /// any kind can import the exported snapshot and read the block content
    /// back in plaintext. If `share_subtree` encrypted the payload this
    /// import-and-read would be impossible without a key. There is no key
    /// parameter anywhere on the share path — that absence is the point.
    #[test]
