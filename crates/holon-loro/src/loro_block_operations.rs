...
mod advice_dismiss_tests {
    ...
    async fn dismiss_appends_then_is_idempotent() {
        ...
        ops.execute_operation(&entity, "dismiss_advice", dismiss_params(&anchor, "block:l1"))
            .await
    ...
    async fn dismiss_second_lesson_preserves_first() {
        ...
        ops.execute_operation(&entity, "dismiss_advice", dismiss_params(&anchor, "block:l2"))
            .await
            .expect("dismiss l2");
        let got = read_suppressed(&ops, &anchor).await;
        assert!(got.contains(&"block:l1".to_string()), "l1 preserved: {got:?}");
        assert!(got.contains(&"block:l2".to_string()), "l2 appended: {got:?}");
        assert_eq!(got.len(), 2);
    }
    ...
    async fn dismiss_missing_anchor_fails_loud() {
        ...
        let entity = EntityName::new("block");
        let err = ops
            .execute_operation(&entity, "dismiss_advice", dismiss_params("block:ghost", "block:l1"))
            .await
