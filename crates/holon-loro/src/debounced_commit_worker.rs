...
//!
//! 1. `LoroDoc::subscribe_root` → a `Notify` waker, optionally filtered by a
//!    caller-supplied predicate on `DiffEvent` (e.g. "local-origin commits
//!    only").
//! 2. Task loop: wait on notify, sleep for the debounce window, drain any
//!    notifications that piled up during the sleep (so a typing burst coalesces
//!    into a single work call), run the async work callback.
//!
...
/// - `F`: the work factory — called each time the debounce fires. Must return a
///   fresh `Future` (so it can be called repeatedly). Using a factory rather
///   than a `FnMut() -> Future` lets the caller close over whatever `Arc` state
///   it needs and produce fresh futures without &mut bookkeeping.
/// - `Fut`: the future type the factory returns. Must be `Send` + the `Result`
///   it resolves to is printed via `{:#}` on error.
///
