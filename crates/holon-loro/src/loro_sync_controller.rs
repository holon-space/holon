...
//! committing thread) extracts the commit's dirty facts (`extract_pending_changes`
//! — a pure function of the event, no `doc` access, no checkout) into a shared
//! queue and wakes the controller. Each wake drives one outbound reconcile via
//! [`LoroProjection`].
//!
//! ## Diff strategy — event-driven, `O(changed)`
//!
//! Steady state is the ONLY steady-state path: drain the pending-facts queue and
//! read just the changed nodes from the CURRENT tree (`incremental_block_changes`),
//! diffing against the in-memory `live` snapshot (the last-emitted state). Cost is
//! proportional to what changed, not the tree size, and nothing checks the shared
//! live doc out — eliminating the torn-walk race behind the flaky `SplitBlock …
//! Block not found`.
//!
...
//! `Frontiers` watermark advance only AFTER the sink write succeeds, so a failed
//! (rolled-back) batch never advances the base ahead of the sink — the next pass
//! reseeds and retries rather than silently dropping the change.
...
pub struct LoroProjection {
    ...
    /// reseed on cold boot (and any unsettled/unarmed reseed pass); steady-state
    /// edits mutate only the changed keys. Persistence is not needed: on restart
    /// the snapshot is rebuilt from the loaded `.loro` (the authority) and
    /// reconciled once against the SQL sink.
...
impl LoroProjection {
    ...
    /// only the changed nodes (`O(changed)`); the diff "before" is the in-memory
    /// `live` snapshot. The full walk runs only to (re)seed `live` — cold boot
    /// (diffing against the SQL sink), reseed-on-unsettled, and the
    /// unarmed/oversized-batch bootstrap. Diffing against a stable base means
    /// re-projecting an unchanged snapshot emits zero ops regardless of any
    /// frontier position.
    pub async fn project(&self) -> Result<()> {
        ...
        if !after_settled {
            ...
            if withheld > 0 {
                tracing::warn!(
                    "[LoroProjection] withholding {} orphan create(s) whose parent \
                     is absent from the torn snapshot (snapshot_settled=false)",
                    withheld,
                );
            }
        }
