        ...
        match doc.import(&bytes) {
            ...
            Err(e) => {
                ...
                match std::fs::rename(&path, &quarantine) {
                    ...
                    Err(rename_err) => warn!(
                        "[snapshot_store] corrupt snapshot {} could not be renamed to {}: \
                         {rename_err}; decode error was: {e}",
                        path.display(),
                        quarantine.display()
                    ),
                }
...
mod tests {
    ...
    fn save_does_not_leave_tmp_file() {
        ...
        let tmps: Vec<_> = std::fs::read_dir(dir.path().join("shares"))
            .unwrap()
            .filter_map(|e| e.ok()) // ALLOW(filter_map_ok) ALLOW(ok): test fixture — readdir entry failure isn't part of
