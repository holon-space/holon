...
/// Read/move-path parent resolution: a missing parent is a generic anyhow error.
...
/// Incremental block changes from a batch of `PendingChange` facts (drained from
/// the `subscribe_root` event stream), cost proportional to the number of
/// changed nodes, NOT the total tree size. This is the O(changed) replacement
/// for the full-document walk + full-map diff on every commit. It reads only the
/// CURRENT tree state for the named nodes — it never calls `doc.diff`, which
/// checked the shared live doc out and raced concurrent readers.
///
...
/// no re-entrant lock, and crucially no checkout. `project()` drains a batch and
/// reads the CURRENT tree state for the named nodes. This is the O(changed)
...
/// Extract the dirty facts from a `subscribe_root` DiffEvent. A pure function of
/// the event (no `doc` access), safe to call inside the subscribe callback.
///
...
/// projection stopped calling `doc.diff`, NOTHING checks the global live doc out
/// (all other `fork()`/`fork_at()` are on separate/shared docs), so a Checkout
/// firing here is an invariant breach — drop it and warn.
...
mod diff_checkout_race_tests {
    ...
    //! the global doc from a spawned worker while split/create ops read the same
    //! doc → the just-created node was transiently absent → `Block not found`.
    use super::*;
    use crate::LoroDocument;
    use std::sync::Arc;
    ...
    /// MECHANISM PROOF (diagnostic, `#[ignore]`d in CI because it asserts a race
    /// *occurs* and is therefore timing-dependent): hammering `doc.diff` across
    /// an interval whose `from` predates the child checks the shared live doc
    /// back to before the child on every iteration; a concurrent resolve then
    /// intermittently fails with `Block not found`. Run manually with
    /// `--ignored` to observe the keystone flake mechanism deterministically.
    ...
    /// FIX-APPROACH GATE (stable): the event-driven projection reads the CURRENT
    /// tree state (`snapshot_blocks_from_doc_settled` / per-node reads) instead
    /// of `doc.diff`. A current-state read takes no checkout, so hammering it
    /// concurrently with a resolve must NEVER produce a spurious `Block not
    /// found`. Green on this proves the fix approach eliminates the race; it
    /// would be RED if the projection still checked out. Stable (asserts absence
    /// of a race, which holds deterministically for a checkout-free reader).
    #[tokio::test(flavor = "multi_thread", worker_threads = 4)]
