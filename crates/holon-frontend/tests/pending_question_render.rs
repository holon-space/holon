...
//!
//! The profile is read out of `assets/integrations/claude-history.yaml` rather
...
impl BuilderServices for QuestionViewServices {
    /// QuestionViewServices is a test double with no backend to await: it
    ...
    > {
        ...
        Box::pin(std::future::ready(Ok(holon_core::Delivery::Unproven {
            detail: "QuestionViewServices: test double, no delivery to prove".to_string(),
        })))
    }
