...
pub struct ViewModel {
    ...
    /// occurrence ever touching the `EntityUri`. `#[serde(skip)]`: the coordinate
    /// is in-memory only — it never rides the serialized wire form, keeping the
    /// ViewModel bytes identical to before this widening.
    #[serde(skip)]
