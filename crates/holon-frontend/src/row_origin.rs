...
/// - **rooted tree** (focus root + descendants, e.g. the main panel's
///   `MATCH (fr:focus_root) … RETURN d`): exactly one row is the forest root
///   (its stated parent is not itself displayed) → new items parent to that
///   root, i.e. `fr.root_id`, the focused block.
///
...
/// - a structurally impossible rowset (several disjoint roots) → `panic!`, since
///   a hidden wrong-parent is worse than a loud failure.
pub fn resolve_creation_parent(
    ...
    match roots.as_slice() {
        [single] => {
            ...
            // ALLOW(entity_uri_from_raw): forest-root id read back from a rendered row (render-pipeline/row boundary)
        ...
        many => {
            ...
            panic!(
                "creation slot: rowset has {} disjoint root rows (container={container}); \
                 cannot resolve a single query focus root — malformed query or render spec",
                many.len()
            )
        }
    }
