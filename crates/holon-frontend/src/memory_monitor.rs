...
// TODO: Why is this in holon-frontend? Memory-monitoring is not
// frontend-specific.
                ...
                loop {
                    ...
                    if let Some(prev) = prev_mb {
                        ...
                        if delta > GROWTH_ALERT_MB {
                            tracing::error!(
                                "[MemoryMonitor] ALERT: RSS {current_mb:.1}MB (+{delta:.1}MB in \
                                 {INTERVAL_SECS}s, +{since_baseline:.1}MB since start)"
                            );
                        } else if delta > GROWTH_WARN_MB {
                            tracing::warn!(
                                "[MemoryMonitor] RSS {current_mb:.1}MB (+{delta:.1}MB in \
                                 {INTERVAL_SECS}s, +{since_baseline:.1}MB since start)"
                            );
                        } else {
                            tracing::info!(
                                "[MemoryMonitor] RSS {current_mb:.1}MB (delta {delta:+.1}MB, \
                                 +{since_baseline:.1}MB since start)"
                            );
                        }
                    } else {
                    ...
                    if current_mb > rss_abort_mb {
                        tracing::error!(
                            "[MemoryMonitor] RSS {current_mb:.1}MB exceeds abort threshold \
                             {rss_abort_mb:.0}MB — aborting to flush dhat-heap.json"
                        );
                        // SIGINT ourselves so the ctrlc handler in heap_profile::start() flushes
                        // dhat
                        unsafe {
