...
//! so the woven rows are visible on the pure/snapshot/MCP render path, not only
//! on the streaming (GPUI) driver path.
//!
...
//! - reactively in a live session by [`spawn_session_weaver`], which watches the
//!   rule set + suppression table and recomputes on each change.
//!
//! ## The one-shot-only constraint (IVM, load-bearing)
//!
//! The canonical read ([`advice_read_all_sql`]) is an anti-join + `ORDER BY`
//! over the synthesized `advice_rule_{slug}` matview. That shape MUST only ever
//! run as a **one-shot** [`holon_api::QueryEngine::execute_query`] — NEVER
//! `watch_query`, which matview-izes any SQL and whose Turso IVM silently
//! miscompiles this shape. Only the rule-set and suppression TRIGGER watches are
//! plain base-table / matview SELECTs safe to watch; their contents are ignored
//! (each batch just re-fires a one-shot recompute).
...
use holon_api::{
    EntityUri, Occurrence, OccurrenceId, QueryLanguage, RowKey, Value,
};
...
/// with the lesson content joined in. Mirrors the pinned per-anchor `read_query`
/// in `crates/holon-advice/tests/matview_build.rs` — read-time suppression
/// anti-join (Turso IVM can't do it inside the matview), score+recency ordering,
/// and the `lesson_id` UI-stability tiebreak — but grouped by anchor (the
/// per-anchor top-K truncation is applied in Rust after this ordered read).
///
/// **One-shot ONLY** — see the module doc. `view` is `rule.name.matview_name()`.
/// Ordered by `anchor_id` first so [`recompute_sidecar`] can group contiguously.
fn advice_read_all_sql(view: &str) -> String {
    format!(
        "SELECT v.anchor_id, v.lesson_id, c.content \
         FROM {view} v \
...
/// The plain-SELECT suppression trigger (safe to watch: a base-table projection,
/// no anti-join/aggregate/order). Its CONTENT is ignored — the one-shot read
/// re-applies the suppression anti-join; this watch only re-fires recompute so a
/// dismiss is reflected immediately.
const SUPPRESSION_WATCH_SQL: &str = "SELECT anchor_id, lesson_id FROM advice_suppressed";

/// PURE CORE (unit-tested): build the woven advice suffix rows for a collection.
///
/// `per_anchor` is `(anchor, ranked_lessons)` where `ranked_lessons` is the
/// canonical read output for that anchor in SCORE order (rank 0 = best), already
/// suppression-filtered and capped at `k`. `content_of` resolves a lesson's LIVE
/// row (its canonical cell) for content — content is taken from the live row,
/// NEVER snapshotted out of the read. When it returns `None` (lesson not yet
/// materialized) the row still renders (empty content), refreshed next recompute.
///
/// Column / key contract of each synthesized row:
/// - `id`        = canonical lesson id → edits/click route to canonical (ADR 0015 rule 3)
...
/// - `anchor_id` = anchor id → threaded so `dismiss_advice` binds anchor + lesson
/// - key = `(lesson_uri, Occurrence::Placed(OccurrenceId::for_placement(lesson, anchor)))`
pub fn synthesize_advice_rows(
    ...
    for (anchor, lessons) in per_anchor {
        for (rank, lesson) in lessons.iter().enumerate() {
            ...
            let mut row: DataRow = content_of(lesson)
                .map(|r| (*r).clone())
                .unwrap_or_default();
...
/// read ONCE, group by anchor, apply the per-anchor top-K, and replace the whole
/// sidecar. No active rule → clear the sidecar (teardown: rule deleted /
...
pub async fn recompute_sidecar(query_engine: &dyn holon_api::QueryEngine, sidecar: &AdviceSidecar) {
    ...
    for r in &rows {
        ...
        if kept {
            ...
            crow.insert(
                "content".into(),
                r.get("content").cloned().unwrap_or(Value::String(String::new())),
            );
    ...
    for (anchor, lessons) in &per_anchor {
        let synthesized =
            synthesize_advice_rows(&[(anchor.clone(), lessons.clone())], &content_of);
        map.insert(
            anchor.clone(),
...
/// blocks via the plain `GET_ADVICE_RULES_SQL` (a one-shot; safe SELECT over the
/// block matview), parses each with the typed boundary parser, and keeps the
/// only `Ok` + `active` rule.
...
/// Spawn the session-level reactive weaver: an initial [`recompute_sidecar`] then
/// a recompute on every rule-set / suppression change (both plain SELECTs safe to
/// watch). Runs for as long as the session lives; the task holds only the query
/// engine + the shared sidecar (no engine back-reference), so it is spawned
/// lazily from `advice_children` without a self-referential Arc.
pub fn spawn_session_weaver(
    ...
    runtime.spawn(async move {
        ...
        let rules_trigger = watch_trigger(
            query_engine.clone(),
            holon_advice::GET_ADVICE_RULES_SQL,
            notify.clone(),
        );
        let supp_trigger =
            watch_trigger(query_engine.clone(), SUPPRESSION_WATCH_SQL, notify.clone());
        ...
        futures::future::join3(rules_trigger, supp_trigger, recompute_loop).await;
    });
}

/// Establish a watch as a bare recompute TRIGGER: drain the stream, notifying on
/// every batch, ignoring contents. On establish failure the trigger idles
/// (disclosed at debug) rather than faking success.
fn watch_trigger(
    query_engine: Arc<dyn holon_api::QueryEngine>,
    sql: &'static str,
    ...
    Box::pin(async move {
        match query_engine
            .watch_query(sql, QueryLanguage::HolonSql, HashMap::new(), None)
            .await
        {
            Ok(mut stream) => {
            ...
            Err(e) => {
                tracing::debug!(
                    "advice weaver: trigger watch not established ({e:#}); \
                     advice will not react to this source"
                );
...
mod tests {
    ...
    fn content_row(id: &str, content: &str) -> Arc<DataRow> {
        ...
        row.insert("parent_id".into(), Value::String("block:real-parent".into()));
    ...
    fn read_all_sql_names_rule_view_and_orders_by_anchor_first() {
        ...
        assert!(sql.contains("s.lesson_id IS NULL"), "read-time suppression anti-join");
        assert!(
            sql.contains("ORDER BY v.anchor_id, v.shared_tag_count DESC, c.updated_at DESC, v.lesson_id"),
