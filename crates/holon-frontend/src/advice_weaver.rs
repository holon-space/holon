...
//!
//! The canonical read ([`advice_read_all_sql`]) — an anti-join + `ORDER BY` over
//! the synthesized `advice_rule_{slug}` matview, with per-anchor top-K applied in
//! Rust — runs as a **one-shot** [`holon_api::QueryEngine::execute_query`] for the
//! actual data (ordering + top-K are read-time / Rust-side, not maintained by IVM).
//!
//! To know WHEN to recompute, [`spawn_session_weaver`] watches (via `watch_query`)
//! two things: the rule set (`GET_ADVICE_RULES_SQL`) and the active rule's
//! canonical read as an entity-shaped matview ([`advice_watch_sql`], `lesson_id AS
//! id`). The latter transitively depends on both the rule matview AND
//! `advice_suppressed`; Turso IVM DOES incrementally maintain the suppression
//! anti-join in that non-aggregating outer view (proven by holon-advice
...
//! reactive TRIGGER — its content is not consumed for data (the one-shot read is
//! the source of truth); rows carry `id` only so the enrichment boundary
...
use holon_api::{EntityUri, Occurrence, OccurrenceId, QueryLanguage, RowKey, Value};
...
/// `lesson_id AS id` (so every row enriches cleanly — no id-less junction rows at
/// the `row_id` boundary) and WITHOUT `ORDER BY` / top-K (both are read-time /
/// Rust-side; a watched multiset is unordered anyway). Watched purely as a
/// reactive trigger: its transitive deps (the rule matview + `advice_suppressed`)
/// mean one watch fires on every advice-relevant change. The suppression anti-join
/// IS incrementally maintained in this non-aggregating outer view — see holon-advice
/// `probe_outer_antijoin_is_incrementally_maintained` (contrast the aggregate form,
/// `probe_ivm_shape_findings` FINDING 2, which IVM cannot maintain).
fn advice_watch_sql(view: &str) -> String {
    format!(
        "SELECT v.lesson_id AS id, v.anchor_id, c.content \
         FROM {view} v \
         LEFT JOIN advice_suppressed s ON s.anchor_id = v.anchor_id AND s.lesson_id = v.lesson_id \
         JOIN block_raw c ON c.id = v.lesson_id \
         WHERE s.lesson_id IS NULL"
...
/// a recompute on every advice-relevant change. Runs for as long as the session
/// lives; holds only the query engine + the shared sidecar (no engine
...
///
/// Both streams are entity-shaped (rows carry `id`), so enrichment never sees an
/// id-less junction row. The first batch of each is the initial snapshot (already
/// reflected by the preceding recompute) and is skipped.
    ...
    runtime.spawn(async move {
        ...
        loop {
                ...
                .await
            {
                ...
                Err(e) => {
                    tracing::debug!(
                        "advice weaver: rule-set watch not established ({e:#}); advice will not react"
                    );
                    return;
                }
            };
            ...
            let mut data_stream = match discover_active_rule(query_engine.as_ref()).await {
                    ...
                    .await
                {
                    ...
                    Err(e) => {
                        tracing::debug!(
                            "advice weaver: data watch not established ({e:#}); advice will \
                             not react to suppression/scoring until the rule set changes"
                        );
                        None
                    }
                },
