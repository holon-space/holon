    ...
    fn list(#[default = 4.0] gap: f32, children: Collection) {
        ...
        match children {
            CollectionData::Streaming { item_template, data_source, sort_key, rules } => {
                ...
                ViewModel::streaming_collection("list", item_template, data_source, gap, sort_key, __parent_space, None, virtual_child, None, rules)
            }
            CollectionData::Static { mut items } => {
                ...
                ViewModel::static_collection("list", items, gap)
            }
        }
    }
}
