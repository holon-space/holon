...
//!
//! A single `PopupMenu` handles all inline popup overlays (slash commands,
//! wiki-links, mentions, etc.). Each popup type is a `PopupProvider` that
//! supplies candidates reactively via futures-signals and handles item
//! selection.
...
/// - Async: `filter.map_future(|f|
///   tokio_handle.spawn(query(f))).to_signal_vec()`
...
impl PopupMenu {
    ...
    ///
    /// Returns the signal that the frontend should watch (via `cx.spawn` +
    /// `for_each`) to update `self.items` and trigger re-renders.
    pub fn activate(
        &mut self,
        ...
        // Convert SignalVec → Signal<Vec<T>> so the frontend can snapshot on each
        // change
    ...
    /// Synchronously set items (for tests or sync providers that don't need
    /// signals).
