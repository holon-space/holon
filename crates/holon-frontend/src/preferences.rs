...
pub fn preferences_render_expr(defs: &[PreferenceDef]) -> holon_api::render_types::RenderExpr {
    ...
    let col_args: Vec<Arg> = section_exprs
        .into_iter()
        .map(|s| Arg {
            name: None,
            value: s,
...
mod tests {
    ...
    fn preferences_to_rows_uses_defaults() {
        ...
        let rows = preferences_to_rows(&defs, &empty);
    ...
    fn preferences_to_rows_uses_overrides() {
        ...
        let rows = preferences_to_rows(&defs, &overrides);
    ...
    fn preferences_render_expr_structure() {
        ...
        // Top-level is col() with section children
        match &expr {
