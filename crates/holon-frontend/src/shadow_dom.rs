...
use crate::display_node::{DisplayNode, NodeKind};
...
/// A flattened, indexed view of a DisplayNode tree that supports
/// efficient parent traversal and input bubbling.
///
/// Built once from a rendered DisplayNode tree. Frontends construct a
...
impl ShadowDom {
    /// Build a ShadowDom from a rendered DisplayNode tree.
    pub fn from_display_tree(root: &DisplayNode, key_map: KeyMap) -> Self {
...
/// Flatten a DisplayNode tree into a vec of ShadowNodes with parent indices.
fn flatten_recursive(
    ...
    if let NodeKind::Collection { widget, items } = &node.kind {
        let navigator = build_navigator(widget, items);
...
fn build_navigator(widget: &str, items: &[DisplayNode]) -> Option<Box<dyn CollectionNavigator>> {
...
fn collect_direct_entity_ids(items: &[DisplayNode]) -> Vec<String> {
    items
        .iter()
...
fn collect_tree_structure(
    ...
    for item in items {
        match &item.kind {
            NodeKind::Layout { widget, children } if widget == "tree_item" => {
                ...
                if let Some(first) = children.first() {
                    if let Some(id) = first.entity_id() {
                        ...
                        if children.len() > 1 {
                            collect_tree_structure(&children[1..], Some(id), dfs_order, parent_map);
                        }
                    }
                }
            }
...
mod tests {
    ...
    fn make_element(id: &str) -> DisplayNode {
        DisplayNode::element(
            "block",
    ...
    fn bubble_navigate_in_list() {
        let tree = DisplayNode::collection(
            "list",
    ...
    fn bubble_key_chord_to_operation() {
        ...
        let tree = DisplayNode::layout("row", vec![block]);
    ...
    fn unhandled_chord_returns_none() {
        let tree = DisplayNode::collection("list", vec![make_element("a")]);
        let dom = ShadowDom::from_display_tree(&tree, KeyMap::new());
