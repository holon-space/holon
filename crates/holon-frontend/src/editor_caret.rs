...
//! - production [`crate::headless_editor_mirror::HeadlessEditorMirror`] uses
//!   the position primitives (`move_left`/`move_right`/`clamp_boundary`) and
//!   the byte→codepoint conversions ([`byte_to_codepoint`]/[`codepoint_len`])
//!   it needs to build Loro's codepoint-indexed `TextOp`s;
...
/// Delete up to `n` chars before `caret` (codepoint-wise, via [`move_left`])
/// and return the new caret. The string-owning analog of production's repeated
/// mid-line backspace.
...
mod tests {
    ...
    mod prop {
        ...
        /// 4-byte emoji — exercises every char-width the caret math must
        /// handle.
