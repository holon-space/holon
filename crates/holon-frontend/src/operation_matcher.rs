...
/// `param_mappings.from`), only operations that USE those params are
/// considered. This prevents e.g. `delete` from matching during drag-drop just
/// because it only needs `id`.
...
/// Match an operation against `ctx_params`, always returning a
/// `MatchedOperation`. Unlike `try_match`, this never returns `None` —
/// an op with zero resolvable params still produces a `MatchedOperation`
/// with all params in `missing_params`. Used by `present_op` to decide
/// between direct dispatch and param-collection popup.
pub fn try_match_from_context(
    op: &OperationDescriptor,
    ...
    let mut missing = Vec::new();
    for param in &op.required_params {
        if let Some(value) = resolve_param(&param.name, op, ctx_params) {
            resolved.insert(param.name.clone(), value);
        } else {
            missing.push(param.clone());
        }
