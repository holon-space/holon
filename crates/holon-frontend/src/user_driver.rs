...
pub trait UserDriver: Send + Sync {
    ...
    async fn insert_text(&self, text: &str) -> Result<()> {
        let _ = text;
        anyhow::bail!(
            "insert_text is unimplemented for this UserDriver. The soft-keyboard \
             insertText: path needs a real-window driver (GpuiUserDriver / McpUserDriver). \
             Was a soft-keyboard-input transition generated for a headless run?"
        )
    }
...
impl ReactiveEngineDriver {
    ...
    /// "live_block still pending" gap in `shadow_builders::tree`. So there is no
    /// rendered virtual node to read here. Rather than re-derive from the editor
    /// caret (`engine.focused_block()`, ADR 0010 — WRONG, diverges from the nav
    /// root under `SplitBlock`/`JoinBlock`/`FocusEditableText`), this reuses the
    /// exact production parent-resolution (`resolve_creation_parent`) on the
        ...
        .with_context(|| {
            format!(
                "creation-slot commit produced no create intent for slot {slot_id} content \
                 {content:?} — the slot id did not parse as a CreationPlaceholder, or the \
                 content was empty"
            )
        })?;
