...
//! # What this is NOT
//!
//! ALLOW(frontend-storage-backend): doc comment explaining the boundary, not an
//! import
//! - **Not a Loro cursor.** Cursors are framework-coupled
//!   (`loro::cursor::Cursor` needs the `loro` crate). The GPUI integration
//!   layer owns the cursor handles and re-resolves them to scalar positions
//!   after every backend mutation, then pushes the resolved values into this
//!   struct via `set_resolved`. That keeps `holon-frontend` Loro-free.
