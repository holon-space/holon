...
//!
//! Design notes:
//! - The AST is `Clone + Debug + PartialEq` (no closures), so failure
//!   messages can show the full expression and the dump can serialize it.
//! - Arithmetic uses Rust's `Add`/`Sub`/`Mul`/`Div` operator overloads so
//!   widget code reads like an equation: `Expr::children(Y) + (Expr::ChildCount - 1.0) * 4.0 + 16.0`.
//! - `Free` propagates through ops: any `Free` in an arithmetic chain makes
//!   the whole expression `Free` (= "no constraint"). That matches the
//!   semantics "I have nothing to claim about this dimension".
//! - The interpreter does not recurse through nested wrapper widgets
//!   automatically. If a wrapper says `min_h = Expr::Child("foo", Y)` and
//!   that child's expectation is itself relational, the registry-side
//!   `EvalCtx` is responsible for resolving — typically by storing observed
//!   bounds, since the observed value is what the wrapper actually wraps.
