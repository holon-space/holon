...
pub use render_context::{BlockRenderCache, RenderContext};
...
impl<T> FrontendSession<T> {
    ...
    const DEFAULT_DOC_URI: &'static str = "doc:__default__";
    ...
    async fn seed_default_layout(engine: &BackendEngine) -> Result<()> {
        ...
        let root = Path::new("");
        let parse_result =
            holon_orgmode::parse_org_file(path, content, Self::DEFAULT_DOC_URI, 0, root)?;
        ...
        for block in &parse_result.blocks {
            let parent_id = if block.parent_id.as_str() == file_doc_uri {
                Self::DEFAULT_DOC_URI.to_string()
            } else {
                block.parent_id.to_string()
            };
            let params =
                holon_orgmode::build_block_params(block, &parent_id, Self::DEFAULT_DOC_URI);
