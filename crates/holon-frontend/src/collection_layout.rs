...
//!    `LayoutShape` via this registry and picks the right driver (`tree_driver`
//!    for `Hierarchical`, `flat_driver` for `Flat`).
//! 3. **Platform renderer** — registered in each frontend's `builders` module
//!    as a render fn keyed by name (e.g.
//!    `frontends/gpui/src/render/builders/board.rs`).
//!
