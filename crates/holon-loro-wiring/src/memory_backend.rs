...
use holon_loro::DEFAULT_EVENT_RING_CAPACITY;
use holon_loro::EventRing;
use holon_loro::deliver_to_subscribers;
use tokio::sync::mpsc;
...
struct MemoryState {
    ...
    /// Soft-deleted block IDs (blocks remain in `blocks` but are treated as
    /// deleted)
...
impl MemoryBackend {
    ...
    /// This ensures the same sequence of operations always generates the same
    /// IDs, which is crucial for property-based testing with proptest where
    /// states are cloned.
