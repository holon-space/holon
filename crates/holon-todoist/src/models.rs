...
impl TodoistTask {
    ...
    pub fn to_api_response(&self) -> TodoistTaskApiResponse {
        TodoistTaskApiResponse {
            id: self.id.clone(),
            ...
            checked: Some(self.completed),
            priority: Some(self.priority),
            due: self.due_date.as_ref().map(|d| TodoistDue {
                date: d.clone(),
                timezone: None,
                string: d.clone(),
                is_recurring: false,
            }),
            labels: self.labels.as_ref().map(|l| vec![l.clone()]),
            added_at: self.created_at.clone(),
    ...
    fn due_date(&self) -> Option<chrono::DateTime<chrono::Utc>> {
        self.due_date.as_ref().and_then(|d| {
            chrono::DateTime::parse_from_rfc3339(d)
                .ok() // ALLOW(ok): boundary parse from API
