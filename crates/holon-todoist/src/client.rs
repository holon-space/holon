...
impl TodoistClient {
                ...
                Ok(response) => {
                    ...
                    if Self::is_retryable_status(status) && attempt < self.max_retries {
                        ...
                        let retry_after_ms = response
                            .headers()
                            .get("retry-after")
                            .and_then(|v| v.to_str().ok()) // ALLOW(ok): HTTP header parse
                            .and_then(|s| s.parse::<u64>().ok()) // ALLOW(ok): non-critical retry-after
                            .map(|secs| secs * 1000);
    ...
    fn inject_otel_headers(headers: HeaderMap) -> HeaderMap {
        #[cfg(not(target_arch = "wasm32"))]
        {
    ...
    async fn execute_command(&self, command: SyncCommand) -> Result<CommandResponse> {
        ...
        let headers = Self::inject_otel_headers(self.default_headers.clone());
