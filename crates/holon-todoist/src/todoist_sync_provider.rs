...
use holon_api::{BatchMetadata, EntityName, SyncTokenUpdate, WithMetadata};
use std::sync::Arc;
    ...
    async fn sync(&self, _position: StreamPosition) -> Result<StreamPosition> {
        ...
        // the span is a proper child of the current span, inheriting OTel context
        {
            ...
            let token_str = match &current_position {
                ...
                // ALLOW(ok): non-UTF8 means unknown position
            ...
            let task_metadata = BatchMetadata {
                relation_name: "todoist_task".to_string(),
            ...
            let project_metadata = BatchMetadata {
                relation_name: "todoist_project".to_string(),
                trace_context,
...
/// Handles both updates and deletions.
fn compute_project_changes(
    response: &serde_json::Value,
) -> anyhow::Result<Vec<Change<TodoistProject>>> {
    // Capture trace context ONCE for all changes in this batch
    let origin = ChangeOrigin::remote_with_current_span();

    // Extract projects array from response
    let projects_array = response
        .get("projects")
        .and_then(|p| p.as_array())
        .context("[compute_project_changes] No projects array in response")?;

    projects_array
        .iter()
        .map(|project_json| {
            let api_project =
                serde_json::from_value::<TodoistProjectApiResponse>(project_json.clone())
                    .context("[compute_project_changes] Failed to parse project")?;
