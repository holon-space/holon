...
use crate::models::{
    CreateTaskRequest, SyncResponse, TodoistTask, TodoistTaskApiResponse, UpdateTaskRequest,
};
...
impl TodoistApiClient for TodoistFakeClient {
    async fn sync_items(&self, _sync_token: Option<&str>) -> ApiResult<SyncResponse> {
        ...
        let items: Vec<TodoistTaskApiResponse> =
            tasks.values().map(|task| task.to_api_response()).collect();
    ...
    async fn create_task(
        ...
        let response = task.to_api_response();
