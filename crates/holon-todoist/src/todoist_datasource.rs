...
impl TaskOperations<TodoistTask> for TodoistTaskOperations {
    ...
    async fn set_title(&self, _: &str, _: &str) -> Result<OperationResult> {
    ...
    // Handled by MCP update_tasks — trait impl required for type bounds only
    async fn set_priority(&self, _: &str, _: i64) -> Result<OperationResult> {
...
impl CrudOperations<TodoistProject> for TodoistProjectDataSource {
    async fn set_field(&self, _: &str, field: &str, value: Value) -> Result<OperationResult> {
        match field {
            "name" => {
...
mod tests {
    ...
    impl holon::core::datasource::DataSource<TodoistTask> for EmptyDataSource {
        ...
        async fn get_by_id(&self, _: &str) -> holon::core::datasource::Result<Option<TodoistTask>> {
            Ok(None)
        }
    }
