...
/// Entity schema provider for Todoist entities (TodoistTask, TodoistProject).
struct TodoistEntitySchemaProvider;

impl EntitySchemaProvider for TodoistEntitySchemaProvider {
...
impl ServiceModule for TodoistModule {
    fn register_services(self, services: &mut ServiceCollection) -> DiResult<()> {
        ...
        // Register Todoist entity schemas for GQL graph
        services.add_trait_factory::<dyn EntitySchemaProvider, _>(Lifetime::Singleton, |_| {
            Arc::new(TodoistEntitySchemaProvider) as Arc<dyn EntitySchemaProvider>
        });
