...
//! de-leak Stage 6).
//!
//! The Turso frontend ([`add_frontend`](crate::wiring::FrontendInjectorExt::add_frontend))
//! builds its [`FrontendSession`] from a `BackendEngine`. A no-engine session
//! instead renders structural blocks from an `Arc<dyn BlockQuerySource>` already
//! registered in the container (e.g. by `register_loro_block_query_source`).
//!
//! This registers the matching `FrontendSession` + `ReactiveEngine` providers so
//! the storage backend is a **DI choice**, not a branch in the caller: both the
//! Turso and the block-query containers expose `FrontendSession` and
...
use holon_frontend::reactive::{
    make_interpret_fn, BuilderServicesSlot, RenderInterpreterInjectorExt,
};
...
/// `FrontendSession` factory that builds [`FrontendSession::from_block_query_source`]
/// instead of resolving a `BackendEngine`.
