...
fn fresh_db_boot_reaches_seed_without_history_tables_panic() {
    ...
    rt.clone().block_on(async {
        ...
        // Build the engine through the SAME lazy-DI entry the gpui frontend
        // uses (`create_backend_engine_with_extras`), configuring the minimal
        // SqlOnly module set that `seed_default_layout` needs:
        ...
        // In this SqlOnly config `BlockOrdering::create_in_tree` returns false,
        // so each seed block routes through `engine.execute_operation("block",
        // "create", ..)` — the history-recording path that fired the regression.
        // Resolving `dyn BlockOrdering` in the `extra_resolve` closure exercises
        // the same graph the FrontendSession factory walks.
        let (engine, ordering) = holon::di::create_backend_engine_with_extras(
            db_path.clone(),
            |injector| {
