...
/// Order-of-magnitude regression bound, not the interaction SLO. The overlay
/// drops any response a newer keystroke has overtaken, so a search this slow
/// renders permanently empty — the "no matches for every query" escape the
/// JOIN caused (6-10 s, measured on the shipped fix's predecessor). A 1-2
/// character query costs ~1-5 s from per-row materialisation depending on
/// machine load (OPEN entry
/// `a-broad-search-query-costs-a-second-at-vault-scale`, measured 0.8-5.3 s
/// across quiet and loaded runs of THIS test) — well over the
/// 200 ms interaction SLO but nowhere near the JOIN's cost. A budget between
/// those two numbers would flap on ordinary shared-machine contention, so this
/// bound catches only the JOIN-shaped regression and leaves the SLO to the OPEN
/// entry, whose fix will tighten this test rather than the other way round.
const JOIN_REGRESSION_BOUND: Duration = Duration::from_secs(6);
...
fn quick_open_search_at_vault_scale() {
    ...
    rt.clone().block_on(async {
        ...
        for q in ["S", "Su", "Sup", "Supp", "Suppe"] {
            ...
            assert!(
                elapsed < JOIN_REGRESSION_BOUND,
                "quick_open_search({q:?}) took {elapsed:?}, over the {JOIN_REGRESSION_BOUND:?} \
                 join-regression bound — a search this slow is overtaken by the next keystroke and \
