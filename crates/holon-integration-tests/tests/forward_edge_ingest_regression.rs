//! Regression for the dogfood 2026-07-10 forward-edge ingest abort (P0 follow-up).
//!
//! Minimal failing corpus: a SINGLE org file whose block `flow-mode-shell` carries
//! a `:REQUIRES:`/`:BLOCKED-BY:` edge to `orient-daily-view` — a sibling parsed
//! LATER in the same file (a legal forward reference). Before the fix, the
//! `block_requires.required_id` FK rejected `flow-mode-shell`'s create at COMMIT
//! (target not yet inserted), the whole file ingest aborted, and every block from
//! `flow-mode-shell` onward (plus its parent subtree) was silently dropped — the
//! error was even MISATTRIBUTED as "parent block not found: ba5ad62d" although the
//! parent existed. The fix drops the soft TARGET FK; dangling targets are a normal
//! representable state consumed by an INNER-JOIN matview.
//!
        ...
        let present: HashSet<String> = rows
            ...
            .filter_map(|r| {
                r.get("id").and_then(|v| v.as_string()).map(|s| {
                    s.strip_prefix("block:").unwrap_or(s).to_string()
                })
            })
            .collect();
