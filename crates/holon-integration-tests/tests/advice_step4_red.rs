...
use holon_api::{EdgeFieldUpdate, EntityName, EntityUri, Region, Tags, Value};
use holon_integration_tests::pbt::composed::harness::ComposedSut;
use holon_integration_tests::pbt::composed::wide_e2e::{WideE2E, WideE2EMachine, wide_e2e_ref};
use holon_integration_tests::pbt::reference_state::ReferenceState;
use holon_integration_tests::pbt::transitions::{
    E2ETransition, NavigateFocus, SetEdgeField, WriteOrgFile,
};
...
fn check_green_with_advice_ran(sut: &Sut, ref_state: &ReferenceState, phase: &str) {
    ...
    for id in [
        ...
        assert!(
            ...
            "[{phase}] {id} must have RUN (not vacuously deselected); \
             ran: {:?}, deselected: {:?}",
            report.ran_ids(),
...
fn advice_step6_synthesis_weave_and_dismiss_green() {
    ...
    assert_eq!(exp.k, 2, "k=2 keeps truncation live (d is dropped pre-dismiss)");
