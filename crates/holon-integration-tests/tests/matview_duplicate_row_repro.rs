...
async fn dump_rows(env: &TestEnvironment, block: &str) -> String {
    let rows = env
        .query(
            &format!("SELECT id, tags, requires, advice_suppressed FROM block WHERE id = '{block}'"),
            QueryLanguage::HolonSql,
        )
        .await
        .expect("dump query");
...
async fn assert_no_dupes(env: &TestEnvironment, scenario: &str) {
    ...
    if !dupes.is_empty() {
        ...
        panic!(
            "[{scenario}] `block` matview has DUPLICATE rows: {dupes:?}\n{dump}"
        );
    }
}
...
fn matview_duplicate_after_tags_then_requires() {
    ...
    rt.clone().block_on(async move {
        ...
        eprintln!("[tags_then_requires] after tags:\n{}", dump_rows(&env, BLK_A).await);
        set_requires(&env, BLK_A, &[BLK_B]).await;
        eprintln!("[tags_then_requires] after requires:\n{}", dump_rows(&env, BLK_A).await);
...
fn matview_duplicate_after_requires_then_tags() {
    ...
    rt.clone().block_on(async move {
        ...
        set_requires(&env, BLK_A, &[BLK_B]).await;
        set_tags(&env, BLK_A, &["proj"]).await;
        eprintln!("[requires_then_tags] final:\n{}", dump_rows(&env, BLK_A).await);
...
async fn parsed_edge_col(env: &TestEnvironment, col: &str) -> Vec<String> {
    ...
    assert_eq!(rows.len(), 1, "expected exactly one `block` row for {BLK_A}");
    let raw = rows[0]
        .get(col)
        .and_then(|v| v.as_string())
...
fn matview_duplicate_after_tag_then_delete_all_tags() {
    ...
    rt.clone().block_on(async move {
        ...
        eprintln!("[tag_then_delete] after tags {{Page}}:\n{}", dump_rows(&env, BLK_A).await);
        set_tags(&env, BLK_A, &[]).await;
        eprintln!("[tag_then_delete] after tags {{}}:\n{}", dump_rows(&env, BLK_A).await);
...
fn matview_duplicate_after_two_tags_then_delete_all() {
    ...
    rt.clone().block_on(async move {
        ...
        eprintln!("[two_tags_then_delete] after tags {{Page,x}}:\n{}", dump_rows(&env, BLK_A).await);
        set_tags(&env, BLK_A, &[]).await;
        eprintln!("[two_tags_then_delete] after tags {{}}:\n{}", dump_rows(&env, BLK_A).await);
...
fn matview_duplicate_after_tag_then_clear_tags_and_requires() {
    ...
    rt.clone().block_on(async move {
        ...
        set_tags(&env, BLK_A, &["Page"]).await;
        set_requires(&env, BLK_A, &[BLK_B]).await;
        eprintln!("[tag_then_clear_both] after tags+requires:\n{}", dump_rows(&env, BLK_A).await);
        set_tags(&env, BLK_A, &[]).await;
        set_requires(&env, BLK_A, &[]).await;
        eprintln!("[tag_then_clear_both] after clear both:\n{}", dump_rows(&env, BLK_A).await);
