...
//! authority doc — see `pbt::op_write_cap::EdgeFieldWriter`), so the write
//! flows Loro → `project()` → SQL exactly as production does.
//!
...
/// the production `EdgeFieldWriter` (and `LoroBlockOperations::get_backend`)
/// use, so the edge writes flow Loro → project() → SQL as in prod.
...
/// PAGE-TAG REPRO variant: two tags {Page, x} then REPLACE with {} (delete
/// both).
#[test]
