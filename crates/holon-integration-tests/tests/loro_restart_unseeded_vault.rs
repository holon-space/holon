...
//!    document order),
//! 4. splitting a phase-1 block through the real engine then flows through
//!    Loro (node count grows by exactly the new half) — never the
//!    poisoned-tree "Block not found" / placeholder-root path. If the
//!    re-seed ever declines (guarded), the assertions flip to the disclosed
//!    SQL-owned branch instead of failing blind.
...
async fn run_test(runtime: Arc<tokio::runtime::Runtime>) {
    ...
    env.execute_operation("block", "split_block", split_params)
        .await
        .expect(
            "phase 2: split_block on a phase-1 block must succeed \
             (guarded SQL route if unseeded; Loro route if the rescan seeded it)",
        );
