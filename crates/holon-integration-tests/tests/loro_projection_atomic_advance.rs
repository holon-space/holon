...
//!   5. RE-EMITS the change on the next successful pass — never silently drops it.
//!
...
use holon::sync::{LoroDocumentStore, LoroProjection, SinkReader};
use holon_api::block::Block;
use holon_api::{EntityName, OperationDescriptor, SnapshotBlock, StorageEntity, Value};
use holon_core::{
    OperationProvider, OperationResult, OriginTaggedWrites, Result as DatasourceResult,
};
use holon_loro::{CONTENT_RAW, CONTENT_TYPE, PendingChange, STABLE_ID, TREE_NAME};
use loro::{Frontiers, TreeID, TreeParentId};
...
struct ToggleFailSink {
    /// When `true`, `execute_batch_with_origin` returns `Err` (batch rolled back).
    fail: AtomicBool,
    /// Every `execute_batch_with_origin` invocation increments this — successful
    /// AND failed — so a test can prove an apply was ATTEMPTED on failure and
    /// re-ATTEMPTED on the following success. (The spec's phrasing only calls out
    /// the failure increment; counting every call is a superset that satisfies
    /// both the "apply attempted" and "incremented again" assertions.)
