...
//!
//! Each transition:
//! 1. Has already been applied to the reference `GroupState<()>` by the state
//!    machine harness.
//! 2. Is forwarded to the `StubSut`, which mirrors the reference state's peer 0
//!    Loro doc into the SUT's primary doc. The controller's `subscribe_root`
//!    fires and reconciles the diff into the stub `OperationProvider`'s
...
use holon_integration_tests::pbt::loro_sync::LoroSyncReference;
use holon_integration_tests::pbt::loro_sync::LoroSyncSut;
use holon_integration_tests::pbt::loro_sync::StubSut;
use holon_integration_tests::pbt::loro_sync::check_all_invariants;
