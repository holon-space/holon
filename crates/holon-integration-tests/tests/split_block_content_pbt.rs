...
//! now rides on `ComposedSut<WideE2E>` via
//! `split_block_content_composed_gherkin` (the per-tick
//! `inv-block-content/sql`, selected by the wide config and run every tick by
//! the per-draw non-vacuity floor, catches mis-routing). The pure-parse outline
//! check is SUT-agnostic and stays.
//!
...
//!
//! @pbt kind harness
//! @pbt covers split-content-routing — SplitBlock content routing, driven through ComposedSut<WideE2E>
//! @pbt overlaps general_e2e_composed_pbt — CONVERGENCE CANDIDATE: already on the wide machine; convert to a stored regression seed once catalog covers the teeth
...
/// E3 MIGRATION PROOF (2026-06-25): the gherkin replay over
/// `ComposedSut<WideE2E>` (born-booted `full_headless` CapMap) instead of
/// `E2ESut`. The feature is re-authored onto the wide seed (no `Given org file`
/// / `app is started` ceremony — born-booted), focuses + splits `c1`; the
/// split-routing regression is caught by the per-tick composed catalog
/// (`inv-block-content/sql`, selected by the wide config + run every tick by
/// the per-draw floor → non-vacuous), and the assert vocabulary runs
