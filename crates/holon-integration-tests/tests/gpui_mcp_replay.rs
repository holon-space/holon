...
use holon_integration_tests::{McpUserDriver, UserDriver};
...
#[tokio::test]
#[ignore = "needs a live Holon app serving MCP on 127.0.0.1:$MCP_SERVER_PORT (default 8521); run with --ignored"]
async fn mcp_user_driver_smoke() -> Result<()> {
    ...
    // 3. Assert the DRIVER round-trip: after a short settle for the async
    //    input pipeline, the add-block editor is present in the main-panel
    //    `describe_ui` tree.
    //
