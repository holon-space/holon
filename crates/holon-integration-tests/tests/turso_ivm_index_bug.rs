...
//! 1. JoinOperator BTree cursor corruption (see
//!    HANDOFF_TURSO_IVM_JOIN_PANIC.md)
//! 2. Dirty pages pager bug (see examples/turso_ivm_dirty_pages_repro.rs)
//!
//! Both cause BTree index inconsistency during IVM cascades through chained
//! matviews.
//!
//! The PBT (general_e2e_pbt_sql_only) catches this non-deterministically via
//! stored regression seeds. This standalone test attempts to reproduce the
//! exact conditions but the bug is timing-dependent and may not trigger every
//! run.
//!
...
/// 3. Write 3 additional org files (5+3+3 headings = 11 blocks + sources +
///    defaults = ~28 total)
/// 4. Start app with todoist fake (concurrent DDL)
/// 5. set_field on render source block → "Index points to non-existent table
///    row"
///
/// Note: This test passes most runs. The bug is timing-dependent — when it
/// triggers, the Turso pager's internal read sub-operations (used by IVM to
/// compute deltas) find dirty pages from the ongoing write transaction,
/// corrupting the BTree index.
#[test]
...
/// Stress test: runs startup + mutation pattern in a loop to maximize race
/// window.
///
...
fn turso_ivm_index_bug_stress() {
    ...
    eprintln!(
        "\n[stress summary] {rounds} rounds: {ivm_errors} IVM index errors, {pager_panics} pager \
         panics"
    );
