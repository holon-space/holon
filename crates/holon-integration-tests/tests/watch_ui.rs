...
//! - Error recovery: render failures → error RenderExpr → fix → valid RenderExpr
...
fn test_keybinding_join_on_operations() {
    ...
    runtime.clone().block_on(async {
        use holon_api::input_types::{Key, KeyChord};
        ...
        assert!(
            has_keybinding,
            "No operation on the ViewModel has a keybinding set. \
             The keybinding join in with_operations() is not working. Ops: {all_ops:?}"
        );
        ...
        match &action {
            ...
            _ => {
                panic!(
                    "Keychord Cmd+Enter did NOT match on block:test-block. \
                     Action: {action:?}, Entities: {entity_ids:?}, Ops: {all_ops:?}"
                );
            }
        }
    });
