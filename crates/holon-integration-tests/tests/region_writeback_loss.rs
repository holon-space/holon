...
// DB state. This is defense-in-depth independent of any single root cause:
// force a partial ingest (a block whose id is OWNED by another document,
// re-parented across docs during the scan — the deterministic partial-ingest
// stand-in from `boot_scan_bad_file_survives`) and assert the on-disk file is
// byte-unchanged.
...
fn partial_ingest_does_not_rewrite_the_file() {
    ...
    rt.clone().block_on(async {
        ...
        assert_eq!(
            on_disk, BAD_FILE,
            "quarantined (partially-ingested) file was rewritten — data loss. Every :ID: must \
             remain on disk. Got:\n{on_disk}"
        );
    });
}
