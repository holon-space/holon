...
//!    transitions into deterministic `SkippedByGating` no-ops that are
//!    **never** applied to the reference model — so the engine's applied
//!    sequence equals exactly the node's applicable subsequence, and (since
//!    reference `apply` is pure) the resulting reference state matches a
//!    `Strict` replay of that subsequence. Verified with [`NullStepper`] (no
//!    SUT), so it is fast and deterministic and can guard every CI run.
//!
...
use holon_integration_tests::pbt::bisect_driver::bisect_capture;
use holon_integration_tests::pbt::bisect_driver::ceiling_by_name;
use holon_integration_tests::pbt::bisect_driver::load_capture;
use holon_integration_tests::pbt::bisect_driver::reproduces_under;
use holon_integration_tests::pbt::bisect_driver::reproduction_signature;
use holon_integration_tests::pbt::fresh_reference_state;
use holon_integration_tests::pbt::stepper::NullStepper;
use holon_integration_tests::pbt::stepper::ReplayMode;
use holon_integration_tests::pbt::stepper::StepOutcome;
use holon_integration_tests::pbt::stepper::run_sequence;
use holon_integration_tests::pbt::transitions::E2ETransition;
use holon_integration_tests::pbt::transitions::PressKey;
use holon_pbt_core::StorageAdapter;
use holon_pbt_core::component_set::ComponentSet;
use holon_pbt_core::component_set::Projection;
...
/// needle. `Some(needle)` pins an exact failure; `None` falls back to the
/// shared default ([`reproduction_signature`] — the cross-layer `trouble begins
/// at:` divergence marker, or the `HOLON_BISECT_SIGNATURE` override), so this
/// is exactly `reproduces_under`. Either way a panic *without* the signature is
/// a replay-infidelity abort, not a reproduction. The panic hook is muted so
        ...
        Err(payload) => {
            let msg = payload
                ...
                .unwrap_or_else(|| {
                    ...
                    panic!(
                        "reproduces_with_signature: panic payload is neither String nor &str \
                         (type_id {:?}) — cannot match signature {needle:?}",
                        payload.type_id()
                    )
                });
            msg.contains(&needle)
...
/// `Applied` or `SkippedByGating`. Since reference `apply` is pure, an
/// identical applied sequence implies an identical resulting reference state.
fn assert_skip_gated_is_ref_invariant(ceiling: &ComponentSet, transitions: &[E2ETransition]) {
    ...
    for node in nodes {
        ...
        assert_eq!(
            canon(run.applied()),
            canon(&applicable),
            "node {node:?}: the SkipGated applied sequence must equal the node's applicable \
             subsequence (a gated step must never reach `apply`)",
        );
    }
}
...
fn editor_transition_skips_purely_under_storeless_node() {
    ...
    assert!(
        ...
        "a SkippedByGating step must never reach `apply` (so it cannot change the reference state)",
    );
}
...
/// structurally available under Turso-only wiring (where the on-blur
/// `set_field` path persists it), not only under Loro — so the editor path is
/// bisectable across the storage axis. (Headless Turso-only slices stay
/// unaffected: the transition's `preconditions` gate on `has_editor_buffer()` —
/// the editor capability — so without a wired editor buffer the transition
/// deselects.)
#[test]
...
/// `HOLON_BISECT_CAPTURE` points at a capture file. Optional
/// `HOLON_BISECT_CEILING` names the ceiling preset the capture was generated
/// under (the capture itself does not record wiring — ADR §4 item 2); defaults
/// to `full_headless`.
///
...
fn bisect_capture_from_env() {
    ...
    let capture_path = match (
        ...
        (Err(_), Err(_)) => {
            eprintln!(
                "[bisect] neither HOLON_BISECT_CAPTURE nor HOLON_BISECT_SLICE set — skipping \
                 SUT-backed bisection (set one to localize a capture)",
            );
            return;
        }
    };
    ...
    let (ceiling, ceiling_name) = match std::env::var("HOLON_BISECT_CEILING") {
        ...
        Err(_) => match &fixture.environment.wiring {
            ...
            Some(w) => panic!(
                "[bisect] capture wiring {w:?} matches no ceiling preset — set \
                 HOLON_BISECT_CEILING explicitly"
            ),
    ...
    if std::env::var("HOLON_BISECT_PROBE").is_ok() {
        ...
        eprintln!(
            "[bisect] probe {capture_path} ({} transitions) under {ceiling_name}: reproduces = \
             {reproduced}",
            transitions.len(),
        );
        return;
...
/// transition at a time, keep every drop that still reproduces the failure
/// under `ceiling`, iterate to a fixpoint. The result is a locally-minimal
/// failing subsequence — fewer transitions to read than the raw capture. Writes
/// the minimized fixture next to the input as `<capture>.min.json`.
///
/// Uses the same cheap, in-process oracle as the bisector, so minimization is
/// just a delta-debug over the sequence axis (orthogonal to the
/// component-lattice axis the bisector minimizes).
///
    ...
    if !repro(&seq) {
        eprintln!(
            "[minimize] capture does NOT reproduce under {ceiling_name} ({} transitions) with \
             ...
             (only the real-window gpui runner reproduces it); the headless component oracle \
             never hits its signature.",
            seq.len(),
        );
        return;
    }
