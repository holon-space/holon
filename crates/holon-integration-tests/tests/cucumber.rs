...
use holon_api::{EntityUri, SourceLanguage, Value};
...
pub struct HolonWorld {
    ...
    /// Cached render expression from initial_widget (legacy field)
    last_render_expr: Option<holon_api::RenderExpr>,
    /// Cached data rows from initial_widget (legacy field)
    last_data_rows: Vec<holon_api::widget_spec::DataRow>,
...
impl HolonWorld {
    fn new() -> Self {
        ...
        Self {
            ...
            runtime,
            last_error: None,
            last_render_expr: None,
            last_data_rows: Vec::new(),
...
async fn block_exists_in_doc(
    ...
    let runtime = world.runtime.clone();
    runtime.block_on(async {
        let doc_uri = EntityUri::file(&file_name).to_string();
        world
            .ctx()
...
async fn create_source_block(
    ...
    let source_lang: SourceLanguage = language.parse().expect("Invalid source language in step");
    let runtime = world.runtime.clone();
    runtime.block_on(async {
        if let Err(e) = world
            .ctx()
            .create_source_block(&block_id, &parent_id, source_lang, &content)
            .await
        {
            world.last_error = Some(e.to_string());
        }
...
async fn call_initial_widget(world: &mut HolonWorld) {
    ...
    runtime.block_on(async {
        match world.ctx().initial_widget().await {
            Ok(render_expr) => {
                world.last_render_expr = Some(render_expr);
                world.last_data_rows = Vec::new();
                world.last_error = None;
            }
            Err(e) => {
                world.last_error = Some(e.to_string());
                world.last_render_expr = None;
                world.last_data_rows = Vec::new();
            }
        }
    });
...
async fn initial_widget_row_count(world: &mut HolonWorld, expected: i32) {
    ...
    assert!(
        world.last_render_expr.is_some(),
        "No render expression - call initial_widget first"
    );
    let actual = world.last_data_rows.len();
    assert_eq!(
        actual,
        expected as usize,
...
async fn open_app(world: &mut HolonWorld) {
    ...
    runtime.block_on(async {
        ...
        // Get root render expression with stream
        let result = world.ctx().initial_widget_with_stream().await;
        match result {
            Ok((_root_render_expr, stream)) => {
                // Collect widget states from root and all nested panels
                // Data now comes from the stream, not bundled in the return value.
                // Start with empty root state; CDC events will populate it.
                let mut widget_states = Vec::new();
                widget_states.push(WidgetStateModel::from_data(&[]));

                // Store all widget states for combined text extraction
                world.widget_states = widget_states;
                world.last_render_expr = Some(_root_render_expr);
                world.last_data_rows = Vec::new();
                // Use empty root state for legacy widget_state field
                world.widget_state = Some(WidgetStateModel::from_data(&[]));
