...
use holon_integration_tests::pbt::composed::live_mcp::{
    LiveMcpE2E, WideE2ELiveMcpMachine, capture_live_cap_set,
};
...
/// headless keystone above is the only PBT a plain `cargo test` runs. Hand-rolled
/// (not `prop_state_machine!`) because that macro cannot express a runtime skip.
///
/// `max_shrink_iters: 0` — NEVER shrink through resets (the server's reset budget
/// is 20/process). To reproduce a live-found failure: replay its persisted
/// regression seed in-proc (`HOLON_PBT_FORCE_FULL=1 cargo test general_e2e_composed_pbt`,
/// the headless twin, which shares the alphabet) to shrink it, then re-verify the
/// shrunk seed live once against a fresh app.
#[test]
