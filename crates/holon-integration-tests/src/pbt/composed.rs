...
//! into a `CapInvariant` (its [`invariants`]`::*::wire()`) lives **here, once**,
//! and *every* composed slice runs the **same** [`composed_invariant_catalog`]
//! through `run_selected` against *its own* component `CapMap`. Selection then
//! lights up exactly the subset whose `Needs` that slice's caps satisfy.
//!
...
//!
//! ## Layout
//! - [`catalog`] — `composed_invariant_catalog()`: one `wire()` line per invariant.
//! - [`invariants`] — one file per invariant: its `Needs`+bridge `wire()` and a
//!   `#[cfg(test)]` triad (positive / negative-containment / catch) driven by the
//!   hand-crafted doubles below (no real backend needed). *Adding an invariant =
//!   one new file here + one `catalog.rs` line.*
//! - `fixtures` (test-only) — slice-agnostic SUT/Ref doubles + a `fixtures::*`
//!   prelude. These are hand-crafted `Vec<Block>`/editor states, not tied to any
//!   real component, so the catch tests run without Turso/Loro/MemoryBackend.
...
/// ([`span_metrics::ComposedSpanMetrics`]). `otel-testing`-gated like the budget
/// body itself (the `pbt` feature always enables `otel-testing`); consumed by the
/// `wide_e2e` slice (registers it) and the `harness` (drives its lifecycle).
...
/// the lib built without `cfg(test)`). Backs the swap slice in [`wide_e2e`].
...
/// by the faithful convergence harness). Available under `pbt` (not just `cfg(test)`)
/// so the composed `harness` it backs can be driven from `tests/` integration tests.
#[cfg(any(test, feature = "pbt"))]
...
/// THE SWAP slice (`WideE2E`) + its helpers — the composed `general_e2e` machinery,
/// relocated here from `frontend_slice/structural_pbt.rs`'s `cfg(test)` mod and
/// `pbt`-gated so the `tests/` integration test `general_e2e_composed_pbt` can drive
