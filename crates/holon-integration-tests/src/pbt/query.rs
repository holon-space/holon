...
pub enum QuerySource {
    ...
    /// The production seeded left-sidebar watch from `assets/default/index.org`:
    /// every page block except the `__default__` seed page. SQL-only —
    /// `SELECT b.* FROM block b JOIN block_tags bt ON bt.block_id = b.id
    /// WHERE bt.tag = 'Page' AND b.id != 'block:__default__'`. No PRQL/GQL
    /// surface (the sidebar is authored in holon_sql).
    PageBlocks,
}
...
impl TestQuery {
    ...
    /// read-path, `block_raw` for the lag-classifier truth-path). Reproduces the
    /// production `index.org` left-sidebar query, projecting `self.columns`
    /// qualified with the `b.` alias (both tables carry `id`, so the JOIN needs
    /// disambiguation). Single source for both [`to_sql`](Self::to_sql) and
