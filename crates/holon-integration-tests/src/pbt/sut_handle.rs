...
/// `CreateDocument` synthetic→uuid reconcile live in the
/// `block_tree_post_action` seam. `self.deref()` forces the `TestEnvironment`
/// inherent methods — a bare `self.simulate_restart`/`create_document` would
/// re-dispatch to these trait methods and recurse (the "trait shadows Deref
/// inherent" gotcha).
...
impl crate::pbt::local_caps::SutAppLifecycle for E2ESut {
        ...
        match (expects_valid_index, render_result) {
            ...
            (true, Err(e)) => {
                ...
                if err_str.contains("ScalarSubquery") || err_str.contains("materialized view") {
                    tracing::trace!(
                        "[apply] render_entity('{}') failed due to known Turso IVM limitation \
                         (GQL): {}",
                        root_id,
                        e
                    );
    ...
    async fn concurrent_schema_init(&self) {
        ...
        // Each creates a materialized view, which should work fine when done one at a
        // time
        for i in 0..3 {
            let prql = format!(
            ...
            match engine.query_and_watch(sql, HashMap::new(), None).await {
                ...
                Err(e) => {
                    ...
                    if error_str.contains("database is locked") {
                        panic!(
                            "DATABASE LOCK BUG: Sequential query_and_watch {} failed with \
                             'database is locked' after {:?}!\nThis indicates the \
                             ensure_navigation_schema() is still being called multiple \
                             times.\nError: {}",
                            i, elapsed, error_str
                        );
                    }
                    // Other errors (like "Database schema changed") might occur
                    // due to other concurrent activity and
                    // are not necessarily the double-init bug
                }
            }
        }
        ...
        for i in 0..2 {
            ...
            match engine.execute_query(sql, HashMap::new(), None).await {
                ...
                Err(e) => {
                    ...
                    if error_str.contains("database is locked") {
                        panic!(
                            "DATABASE LOCK BUG: Sequential simple query {} failed with 'database \
                             is locked' after {:?}!\nError: {}",
                            i, elapsed, error_str
                        );
                    }
...
/// impl. The `ref_state`-dependent post-actions (e.g. the `PressKey`
/// Enter-split reconcile) live in `block_tree_post_action`, so these are pure
/// `&self` driver dispatches.
#[async_trait::async_trait(?Send)]
impl SutBlockInteract for E2ESut {
    ...
    async fn press_key(&self, chord: &holon_api::KeyChord) {
        ...
        // decomposition): the harness seam owns `ref_state`, letting this
        // action be a pure `&self` keystroke send.
    }
    ...
    /// shared `holon_layout_testing` bodies through `SutClickAdapter`. Region
    /// is unknown at this layer (the shared variant only carries an
    /// `element_id`), so it is inferred from the id prefix.
    async fn click_at_element(&self, element_id: &str) {
        ...
        let element_uri = holon_api::EntityUri::parse(target).unwrap_or_else(|e| {
            panic!(
                "[LayoutPBT::click_at_element] {element_id:?} (target {target:?}) is not an \
                 EntityUri: {e}"
            )
        });
        driver
...
/// `SutFocusWrite` for `E2ESut` — the fine-grained focus write cap the
/// decomposed `NavigateFocus` transition now binds (SutHandle decomposition
/// increment 1). `&self`, so the native macro dispatch (`sut: &mut E2ESut`)
/// drives it via auto-reborrow. Both methods forward to the existing production
/// helpers (`SutHandle::apply_navigate_focus`, now `&self`; the shared
...
/// production focus walker (`advance_focus`) move focus; the
/// `ref_state`-derived prediction the old `SutHandle` method logged carried no
/// behaviour, so it is dropped (the `ref_state`-off-the-cap principle).
#[async_trait::async_trait(?Send)]
    ...
    async fn apply_navigate_focus(&self, region: CapRegion, id: &holon_api::EntityUri) {
        ...
        loop {
            ...
            driver
                ...
                .unwrap_or_else(|e| {
                    panic!(
                        "[NavigateFocus] click_entity failed for sidebar entry {resolved_id}: \
                         {e:#}"
                    )
                });
            ...
            if Instant::now() >= deadline {
                let page_rows = self
                    .engine()
                    .execute_query(
                        "SELECT b.id FROM block b JOIN block_tags bt ON bt.block_id = b.id WHERE \
                         bt.tag = 'Page'"
                            .to_string(),
                        HashMap::new(),
                        None,
...
/// `NavigateHome` transition binds (SutHandle decomposition increment 2).
/// Forwards to the existing production helper
/// (`SutHandle::apply_navigate_home`, now `&self`), so native behaviour is
/// unchanged. `&self`, so the native macro dispatch (`sut: &mut E2ESut`) drives
/// it via auto-reborrow. Back/forward are deferred to E4.
#[async_trait::async_trait(?Send)]
...
/// `SetupWatch` transition binds (SutHandle decomposition increment 3).
/// Forwards to the existing production helper (`TestEnvironment::setup_watch`,
/// reached via `Deref`, now `&self` after the watch state was made
/// interior-mutable), so native behaviour is unchanged. `&self`, so the native
/// macro dispatch (`sut: &mut E2ESut`) drives it via auto-reborrow. The
/// transition compiles `TestQuery → (source, lang)` at the boundary, so this
/// takes the compiled form.
#[async_trait::async_trait(?Send)]
...
/// `Deref`-reached `TestEnvironment` inherent method — calling
/// `self.switch_view` would re-dispatch to this trait method and recurse (the
/// "trait shadows Deref inherent" gotcha).
#[async_trait::async_trait(?Send)]
...
/// Undo/redo cap (`UndoLastMutation` / `Redo` transitions). Pure `&self`
/// actions over the engine undo stack; the `ref_state`-dependent
/// block-convergence settle lives in `block_tree_post_action` (decomposition
/// #1b).
#[async_trait::async_trait(?Send)]
...
impl crate::pbt::local_caps::SutMutate for E2ESut {
    async fn toggle_state(
        ...
        assert!(
            click_count > 0,
            "[ToggleState] click_count=0 ({current_state:?} == {new_state:?}) — generator should \
             exclude no-op transitions"
        );
...
/// The seam-relocated mutations (`ApplyMutation`/`BulkExternalAdd`). `E2ESut`
/// provides `SutSeamMutate` because its `block_tree_post_action` seam runs the
/// real, `ref_state`-dependent dispatch; the cap actions themselves are `&self`
/// no-ops that the seam stands behind. (The composed
/// `HeadlessFrontendComponent` deliberately does NOT provide this cap — no seam
/// yet — so these transitions auto-narrow out there.)
#[async_trait::async_trait(?Send)]
impl crate::pbt::local_caps::SutSeamMutate for E2ESut {
    async fn bulk_external_add(&self, _: &holon_api::EntityUri, _: &[holon_api::block::Block]) {
        // Relocated to `block_tree_post_action`'s `BulkExternalAdd` arm: the
        // body serializes the FULL document from `ref_state`
        // (`resolve_ref_blocks`), so it runs in the harness seam that
        // owns `ref_state`. The action is a `&self` no-op;
        // `apply_bulk_external_add_to_sut` runs from the seam.
    }

    async fn apply_mutation(&self, _: crate::pbt::types::MutationEvent) {
        // Relocated to `block_tree_post_action`'s `ApplyMutation` arm: the
        // dispatch needs `ref_state` and the LoroPeer path drives the
        // `&mut self` `apply_peer_*` caps, so it runs in the harness
        // seam. The action is a `&self` no-op;
        // `apply_apply_mutation_to_sut` runs from the seam.
    }
}
