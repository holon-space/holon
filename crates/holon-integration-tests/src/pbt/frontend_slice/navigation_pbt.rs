//! SutHandle decomposition — increments 1 + 2: drive a **composed `CapMap`**
//! through the production `NavigateFocus` (→ `SutFocusWrite`) and
//! `NavigateHome` (→ `SutNavHistoryWrite`) transitions, decomposed off
//! `SutHandle` onto fine-grained caps, checked by the shared composed-invariant
//! catalog's focus invariants against a real `ReferenceState` focus oracle.
//!
...
//! `SutFocusWrite` + `SutNavHistoryWrite` caps on a real headless
//! `FrontendSession`, so each transition runs its real `apply_to_sut` (the
//! production `navigation.focus` / `navigation.go_home` ops) against the
//! windowless engine, and the `current_focus` / `focus_roots` matviews it
//! writes are compared to the oracle via `inv-navigation-focus` /
//! `inv-focus-roots`.
//!
//! Scope = `{ NavigateFocus, NavigateHome }` — the first composed
//! `StateMachineTest` exercising *multiple* decomposed transitions in one
//! heterogeneous loop. `NavigateHome` lowers to `navigation.go_home`
//! (`set_focus(None)` + clear the region's open pins), so it *clears* focus and
//! roots — exercising a settle path `NavigateFocus` (set) never did.
//! `navigate_forward`/`back` (not mirrored headless,
//! see `maybe_mirror_navigation_focus`) are deferred to the windowed component
//! (E4).
//!
...
//! (the focus invariants read `navigation_history`/`open_pins`, not
//! `block_state`).
...
use crate::pbt::transitions::UnpinBlock;

/// The two pinned-id page docs the SUT boots (`#+ID: <bare>` → `block:` scheme
/// at the parser boundary), so the SUT doc-block ids match the reference
/// verbatim — no doc-id remapping for this slice. (Verified by the Step-0
/// probe: `block_raw` carries `block:ref-doc-0` / `block:ref-doc-1`.)
const DOC0_ID: &str = "block:ref-doc-0";
const DOC1_ID: &str = "block:ref-doc-1";

/// Endgame risk probe (compile-only) — the **scaled-up** union-of-bounds
/// keystone question: can ONE concrete SUT type satisfy the union of all
/// transition caps at once (the doc's named gating risk, the miniature of the
/// ~50-transition endgame)? As E3 Phase A ported more caps onto
/// `HeadlessFrontendComponent`, this scaled from the original 3 → 4 → now
/// **14** caps the single type satisfies with no wrapper: the focus/nav drive
/// caps (`SutFocusWrite`, `SutNavHistoryWrite`, `SutNavHistoryDrive`), the
/// A1/A2a drive caps (`SutViewControl`, `SutMcpEmit`, `SutHistoryWrite`), the
/// watch-register drive cap, and the render/projection/org read oracle caps.
/// The bound holds trivially — the union endgame stays tractable at this scale.
/// (`SutBlockTreeWrite` is provided by a *separate* `OpDispatchWriter`
...
/// `current_focus(main)` / `focus_roots(main)` carry `block:journals` before
/// any navigation (verified by the failing first-check: the SUT starts here).
const JOURNALS_ID: &str = "block:journals";

/// The seeded focus oracle: a started `ReferenceState` whose initial navigation
/// focus is `block:journals`, matching the SUT's boot focus (the headless
/// session starts focused on journals, exactly as production `StartApp` does).
/// Without this the very first `check_invariants` sees the SUT's boot
/// `current_focus` (journals) as a ghost the ref never navigated to. Each
/// generated `NavigateFocus::apply_to_ref` then advances the
/// `navigation_history` / `open_pins` the focus invariants read.
fn navigation_ref() -> ReferenceState {
    ...
    // boot history. `build_started_ref` seeds a `c1` focus (for editor
    // preconditions unused by this nav slice), so without this the ref carries
    // a `[None, c1, journals]` history (cursor 2) — a deeper back-stack than
    // the SUT's journals-only boot. A `NavigateBack` would then walk the ref
    // through `c1`/`None` entries the SUT never had, diverging
    // `current_focus`/`focus_roots` at the boundary. With the back-stack
    // trimmed, `can_go_back` is false at the seed (so back-nav only fires after
    // a `NavigateFocus` builds real history) and every back lands on a focus the
    // SUT also visited.
    let history = state
        .ui
        .tab
    ...
    history.cursor = 0;
    // Align the history-id counter with the SUT boot too (same `c1` root cause):
    // the headless session assigns journals `navigation_history.id = 1`
    // (AUTOINCREMENT) at boot, so the next pin gets id 2. `build_started_ref`'s
    // `c1` seed bumped the ref's `next_history_id` an extra time (journals got
    // id 2, next is 3) — a lockstep `PinBlock` would then predict id 3 while
    // the SUT assigns 2, breaking `UnpinBlock` (`close(3)` targets nothing).
    // Reset the journals open-pin to id 1 and `next_history_id` to 2 so the ids
    // mirror the boot. (Verified by `headless_unpin_block_probe` +
    // `unpin_block_lockstep_stays_green`.)
...
/// production `navigation.go_back`/`go_forward` ops headlessly
/// (`SutNavHistoryDrive`); parity is
/// proven by `headless_back_forward_focus_parity_probe`. Two seed/ref bugs had
/// to be fixed before they were safe in the generated alphabet: (1) the boot
/// nav-history-depth mismatch ([`navigation_ref`] trims its phantom `c1`/`None`
/// back-stack to journals-only at cursor 0), and (2) **`navigate_home`'s
/// missing idempotency guard** — `go_home` was pushing a `None` history entry
/// on EVERY call, so `NavigateHome`×N → N phantom home entries that
/// `NavigateBack` walked back through, while production `focus(None)` is
/// idempotent when already home (`headless_go_home_idempotency_probe`:
...
/// `UnpinBlock` (drawn from `open_pins[RightSidebar]`, never the Main focus
/// pin, so `close(history_id)` only ever removes a sidebar pin, not the current
/// focus row).
...
/// Run the catalog against a **`RefFocus`-only** ref `CapMap`, so only the
/// focus invariants select. Mirrors `run_with_seeded_ref`'s off-thread drop of
/// the `Arc<ReferenceState>` (it owns a tokio `Runtime`; dropping it on the
/// async executor panics).
...
/// alphabet (`NavTransition`/`NavMachine`), a real headless `FrontendSession`
/// seed, and a **focus-only** check: it overrides
/// [`run_report`](ComposedSlice::run_report) to run the catalog against a
/// `RefFocus`-only ref `CapMap` (only the two focus invariants select) over the
/// raw oracle. The navigation transitions mint no blocks, so the harness's
/// generic reconcile is a clean no-op (the default `align_ids`).
...
impl ComposedSlice for FrontendNavigation {
    ...
    /// Focus-only check: a `RefFocus`-only ref `CapMap` over the raw oracle, so
    /// only the focus invariants select (the slice's payoff). No
    /// reconcile/scaffold needed.
    async fn run_report(
        caps: &CapMap,
        _: &IdResolver,
...
mod teeth {
    ...
    /// Teeth: navigating focus on the SUT but NOT the oracle (oracle frozen at
    /// the journals-focused seed) makes the SUT's `current_focus` /
    /// `focus_roots` matviews hold `ref-doc-1` while the oracle still holds
    /// journals — both focus invariants MUST `Fail` (not `Skipped`). Proves
    /// the op actually moved the matviews and the invariants have teeth.
    /// The `focus_roots` mirror==matview wiring (`live_focus_root_rows`
    /// reads the matview) guarantees a real `Fail`, not a CDC-lag `Skipped`
    /// (V4).
    #[test]
    ...
    /// Positive (also the increment-2 H4 settle probe): `go_home` on BOTH the
    /// SUT and the oracle in lockstep, from the `block:journals`-focused
    /// seed, keeps the focus invariants green. This proves the production
    /// `navigation.go_home` focus-*clear* settles to a stable empty
    /// `current_focus`/`focus_roots` headlessly — the settle path INC 1's
    /// focus-*set* never exercised — matching the ref's cleared state.
    ...
    /// Teeth: `go_home` on the SUT but NOT the oracle. The `navigation_ref()`
    /// seed boots focused on `block:journals` WITH journals as an open-pin
    /// focus root, so the SUT's `go_home` clears its `current_focus` (→
    /// None) AND its `focus_roots` (→ empty) while the frozen oracle still
    /// holds journals — so BOTH focus invariants MUST `Fail` (not
    /// `Skipped`). This also deterministically dispatches `NavigateHome`
    /// through the real `apply_to_sut` path (the H5 coverage guarantee,
    /// robust to nextest's per-test process isolation where a cross-test
    /// counter can't observe the randomized run).
    #[test]
    ...
    /// Positive (C1 PinBlock increment): `focus_pin(RightSidebar, ref-block-0)`
    /// on BOTH the SUT and the oracle in lockstep keeps the focus
    /// invariants green. The production `navigation.focus_pin` op,
    ...
    /// `open_pins` stays empty — so `inv-focus-roots` MUST `Fail` (not
    /// `Skipped`), proving the `focus_pin` op actually moved the matview
    /// and the invariant has teeth. `inv-navigation-focus` stays green
    /// (pins don't touch `current_focus`), so this asserts the focus-roots
    ...
    fn sut_only_pin_block_is_caught_by_focus_roots() {
        ...
        assert!(
            ...
            "SUT-only PinBlock must trip inv-focus-roots with a Fail; failures were {failures:?}, \
             ran={:?}",
            report.ran_ids()
        );
    }
    ...
    /// journals(boot)→d0→d1 in lockstep, then `go_back` then `go_forward` —
    /// each on BOTH the SUT and the oracle — keeps the focus invariants
    /// green. Proves the production `navigation.go_back`/`go_forward` ops,
    /// dispatched headlessly by `SutNavHistoryDrive`, move
    /// `current_focus(main)` to track the ref's `navigation_history.cursor`
    /// (the historically-doubted parity, proven by
    ...
    /// Teeth: build history to d1 in lockstep, then `go_back` on the SUT but
    /// NOT the oracle — the SUT's `current_focus(main)` moves back to d0
    /// while the frozen oracle still holds d1, so `inv-navigation-focus`
    /// MUST `Fail` (not `Skipped`). Proves the `go_back` op actually moved
    /// the matview and the invariant bites.
    #[test]
    fn sut_only_navigate_back_is_caught_by_focus() {
        ...
        assert!(
            ...
            "SUT-only NavigateBack must trip inv-navigation-focus with a Fail; failures were \
             {failures:?}, ran={:?}",
            report.ran_ids()
        );
    }

    /// The `history_id` the ref assigned the right-sidebar pin of
    /// [`PINNABLE_ID`].
    ...
    /// Teeth: pin lockstep, then `UnpinBlock` on the SUT but NOT the oracle —
    /// the SUT clears `focus_roots(right_sidebar)` while the oracle still
    /// holds the pin, so `inv-focus-roots` MUST `Fail`. Proves
    /// `close(history_id)` actually removed the matview row (and that the
    /// id targeted the right one).
    #[test]
