...
use holon_pbt_core::capabilities::{
    CapRegion, SutAdviceMatview, SutBackend, SutBlockTreeWrite, SutClockAdvance,
    SutEditorMirrorRead, SutEditorMirrorWrite, SutErrorLog, SutFocus, SutFocusWrite,
    SutHistoryWrite, SutMcpEmit, SutNavHistoryDrive, SutNavHistoryWrite, SutOrgRead, SutOrgRender,
    SutQueryResults, SutRenderer, SutSqlProjection, SutViewControl, SutViewSelection, SutWatch,
    SutWatchRegister, WatchRow, WidgetSnapshot,
};
...
use holon_pbt_core::capabilities::{SutAppLifecycle, SutBlockCreate, SutMutate, SutSeamMutate};
use holon_pbt_core::types::CycleTarget;
use holon_pbt_core::types::{Mutation, MutationEvent};
...
impl HeadlessFrontendComponent {
    ...
    /// Like [`Self::new_with_loro`] but injects a controllable [`TestClock`] into
    /// the engine's DI as `InjectedClock`, so the production `ClockScheduler`
    /// ticks on it. The keystone `AdvanceDay` transition (ADR 0024 §6) then
    /// advances this clock through `SutClockAdvance`, driving day-rollover
    /// re-fires of the journal rule down the real reactive path.
...
/// materialized views from `sqlite_master` (same `sql_query` plumbing every other
/// SUT SQL read uses) and reads each one's full row set. Pre-step-6 there is no
/// such matview, so this returns empty (observed-absent).
#[async_trait::async_trait(?Send)]
...
/// injected fake clock and re-run the scheduler's OWN reconcile so a day-rollover
/// CDC re-fires the journal rule — the prod-faithful path (WP1), never a raw
/// `clock`-relation UPDATE. `days == 0` re-ticks the same day (idempotence probe:
/// `reconcile_clock` sees `Unchanged`, no CDC, no re-fire). Hosted only on a
/// clock-injected boot; a `None`-clock component would fail loud rather than
/// silently no-op.
#[async_trait::async_trait(?Send)]
...
/// `CreateBlockUnderFocus` transition. Delegates to the PRODUCTION creation-slot
/// commit seam on this component's own `ReactiveEngineDriver`
...
/// `block::create-N` via the harness's generic per-tick id reconcile. Fails loud
/// (never a silent no-op) if no slot renders or the commit yields no intent.
#[async_trait::async_trait(?Send)]
...
/// the same path `create_document` exercises), which is what makes `WriteOrgFile`
/// — and with it the advice-rule minting arm (ADR 0022 step 4) — reachable in
/// the composed keystone at all. The other four fixture ops (`create_directory`,
/// `git_init`, `jj_git_init`, `create_stale_loro`) back transitions that are all
/// gated on `!app_started`; the composed keystone's oracle boots pre-started
/// (`build_started_ref`), so they can never be dispatched here — fail loud if
/// that ever changes (same convention as `start_app` above).
#[async_trait::async_trait(?Send)]
impl holon_pbt_core::capabilities::SutFixtureFs for HeadlessFrontendComponent {
    async fn write_org_file(&self, filename: &str, content: &str) {
        ...
        let doc_id = loop {
            ...
            assert!(
                start.elapsed() < timeout,
                "[SutFixtureFs::write_org_file] timeout waiting for the doc block \
                 (title {stem:?}) to land in block_raw after writing {filename}"
            );
            tokio::time::sleep(Duration::from_millis(50)).await;
        };
        self.settle_block_ids_stable(Duration::from_secs(5)).await;
...
mod tests {
    ...
    /// A fixed local-noon millis for `y-m-d`, timezone-robust (noon UTC lands on
    /// the same civil date from UTC-12..+12), so the boot day is deterministic
    /// regardless of the test host's TZ.
    ...
    /// The journal DAY children under `block:journals`: `(id, name)` pairs whose
    /// `name` parses as a `YYYY-MM-DD` date. Excludes the rule's source blocks and
    /// the `Journal Auto-Create` heading (also children of `block:journals`, but
    /// not date-named).
    ...
    /// Poll until the journal day-block count reaches `expected` (the rule fires
    /// asynchronously off the CDC path), failing loud on timeout with the current
    /// rows so a divergence is legible.
    ...
    /// `block:journals`; the trigger/action pair lives under a `Journal Auto-Create`
    /// heading (same parent, so `action_discovery` joins them) and NOT directly
    /// under journals, so the day-blocks the action creates are the only
    /// date-content children.
