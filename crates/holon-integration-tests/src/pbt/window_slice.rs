...
//! (`docs/Testing/PbtCompositionBacklog.md`). It provides the one cap the
...
//! be driven (pumped) on its owning thread — so it lives in the **test
//! harness** (`frontends/gpui/tests/`), never in a cap. But the *geometry data*
//! it produces is shareable: [`BoundsRegistry`] is `Send + Sync + Clone` (an
//! `Arc<RwLock<…>>`), and reads go through the abstract [`GeometryProvider`]
//! port. So [`GpuiWindowComponent`] holds only a `Box<dyn GeometryProvider>`
//! clone and is an ordinary `Send` component hosting an ordinary `async
//! fn(&self)` [`SutLayout`] cap on a `CapMap` — exactly like the Loro/SQL
//! components. The harness pumps the window to a fixed point (its
//! realization-specific *settle*); the cap then reads the freshly-committed
//! bounds. The cap model is unchanged.
//!
