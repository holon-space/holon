...
pub(super) fn leader_key_for(nav_op: &str) -> &'static str {
    ...
    key.unwrap_or_else(|| {
        panic!(
            "no leader-chord binding for nav_op '{nav_op}' in assets/default/keybindings.yaml — \
             caller passed a name that doesn't appear in the YAML, or the YAML lost that binding"
        )
    })
}
