//! Tree walker for value-fn provider invariants
...
/// rows the provider currently carries — lets
/// inv-value-fn-provider-arg-variance assert "provider for URI A produced rows
/// ≠ provider for URI B".
...
/// `FunctionCall` with the given name. Used by
/// inv-value-fn-provider-arg-variance to pick render expressions that drive
