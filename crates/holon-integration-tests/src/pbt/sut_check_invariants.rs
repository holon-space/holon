...
impl E2ESut {
        ...
        match transition {
            E2ETransition::SplitBlock(_) => {
                ...
                if db_rows.len() != expected_count {
                    ...
                    eprintln!(
                        "[SplitBlock count-mismatch diag] expected={} db_rows={} unique_ids={} \
                         duplicates={:?} missing_from_block_raw={:?} extra_in_block_raw={:?}",
                        expected_count,
                        db_rows.len(),
                        actual_ids.len(),
                        duplicates,
                ...
                assert!(
                    unknown_ids.len() <= 1,
                    "[SplitBlock] expected exactly one freshly-minted block, found {}: {:?} — a \
                     pre-known block must have vanished (count assert passed)",
                    unknown_ids.len(),
                    unknown_ids
                );
                            ...
                            match SutDriver::engine_focused_block(self).await {
                                EngineFocus::Focused(actual) => {
                                    ...
                                    if tokio::time::Instant::now() >= deadline {
                                        panic!(
                                            "[PressKey-Enter] split focus handoff DIVERGED: \
                                             engine focused {actual}, ref expects the new split \
                                             block {expected_id} (after 2s — async op-response \
                                             focus application never converged)"
                                        );
                                    }
                                }
                                EngineFocus::Unfocused => {
                                    if tokio::time::Instant::now() >= deadline {
                                        panic!(
                                            "[PressKey-Enter] split focus handoff LOST: engine \
                                             has no focused block, ref expects the new split \
                                             block {expected_id} (after 2s)"
                                        );
                                    }
                                }
                                ...
                                EngineFocus::NoEngine => {
                                    eprintln!(
                                        "[PressKey-Enter] split focus handoff UNVERIFIED (no \
                                         frontend engine); seeding caret on ref expectation \
                                         {expected_id}"
                                    );
                                    break;
                                }
    ...
    /// Async body of `apply()` — extracted so Flutter (already async) can call
    /// directly without `block_on`.
    ...
    pub async fn apply_transition_async(
        ...
        // `wait_for_consumers` AFTER the inv-editable-text-has-draggable assert, which
        // let SQL writes produced by inbound EventBus consumers (e.g.
        // `LoroSyncController`'s SQL→Loro path triggering an outbound Loro→SQL
        // reconcile) commit *during* the inv-editable-text-has-draggable grace
        // window — looking like spurious churn when they're really just
        // causally-related writes that haven't settled yet.
        //
        ...
        // inv-editable-text-has-draggable: After draining, no more CDC events should
        // arrive.
        {
            use tracing::Instrument;
            async {
