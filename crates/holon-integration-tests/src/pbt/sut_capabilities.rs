...
use holon_pbt_core::capabilities::EdgeFieldUpdate;
...
// `E2ESut` (`SutHandle` bundles every cap). The transition's generator gates on
// a composed config (`cap_set.is_some()`), and `E2ESut` always leaves `cap_set
// == None`, so `SetEdgeField` never generates against `E2ESut` and this impl is
// a compile-required, fail-loud guard rather than a live path. (Kept faithful —
// not a no-op — so it can't silently mask a future wiring that does reach it.)
...
impl E2ESut {
    ...
    /// `loro_sut` is a wiring bug, not a runtime condition. `&self`:
    /// `LoroSut`'s peer mutations are interior (`RefCell` peers), so the
    /// cap is `&self`-hosted.
...
// `SimulateRestart` transitions actually bind); the coarse trait had zero
// callers.
...
// ─── SutLoroTaskState ─────────────────────────────────────────────────
// E3: `impl SutLoroTaskState for E2ESut` was DELETED here. Its only live
// consumer was the standalone `tests/task_state_coherence_pbt.rs` (a
// `component_pbt!` E2ESut-wrapping slice dispatching
// `inv-task-state-storage-coherence`). Per the convergence rule (Design §8.10)
// that standalone test was DELETED — not rewritten as a composed slice —
// because the ONE PBT already covers the cap: `full_headless` hosts
// `SutLoroTaskState` + `SutSqlProjection`, so `general_e2e_composed_pbt` /
// `WideE2E` selects `inv-task-state-storage-coherence` in the wide config and
// runs it every tick (the per-draw non-vacuity floor); the real-SUT lockstep
// teeth lives in `composed/invariants/task_state_storage_coherence.rs`.
// `SutLoroTaskState` is not a `WideProxyCaps` member and the invariant is
// `NATIVE_ONLY_EXCLUDED`, so the native runner never dispatched it over
// `E2ESut`. The trait + composed hosts (`LoroBackendComponent` /
// `FixtureLoroTaskState`) remain; see `E1_RELOCATED_CAP_COVERAGE` in
// `composed/parity.rs`.

// ─── SutSqlProjection: DELETED (E3, 2026-06-25) ───────────────────────
// The headless Turso SQL-projection read surface (`block_row` / `all_block_ids`
// / `sorted_children` / `block_raw_row` / `block_content` /
// `current_focus_rows` / `focus_roots_rows` / `nav_history_open_rows` /
// `block_tag_block_ids` / `block_task_state`) is now hosted by the composed
// `full_headless` CapMap (`SqlProjectionComponent` /
// `HeadlessFrontendComponent`). Its native consumers — `inv-navigation-focus`
// (now selected by the wide config + run every tick by the per-draw floor) and
// `inv-block-content/sql` — run only via the
// composed catalog (`navigation_focus::wire` + `block_content_sql::wire`); the
// standalone `split_block_content_pbt` / `peer_conflict_pbt` slices were
// deleted. See `NATIVE_ONLY_EXCLUDED` + the `SutSqlProjection` row in
// `E1_RELOCATED_CAP_COVERAGE`.

// ─── SutBackend: DELETED (E3, 2026-06-24) ─────────────────────────────
// The headless `block_raw`/`block`-matview read surface (`live_block_snapshot`
// / `block_raw_snapshot` / `live_focus_root_rows`) is now hosted by the
// composed `HeadlessFrontendComponent` / `SqlProjectionComponent`. Its 6
// structural invariants (matview, block_raw, no-orphan, no-parent-cycles,
// source-language, focus-roots) run only via the composed catalog — see
// `NATIVE_ONLY_EXCLUDED`
...
// — was a redundant wrapper delegating straight to
// `local_caps::SutFixtureFs::write_org_file`, which is what the `WriteOrgFile`
// transition binds directly; the coarse trait had zero callers. Trait removed
// from pbt-core.
...
// `inv-value-fn-provider-{identity,arg-variance-13}`, and the dual
// `SutViewSelection + SutLayout` bodies `inv-frontend-no-error-widgets` /
// `inv-frontend-bounds-rendered`) now run only via the composed catalog:
// `full_headless` (HeadlessFrontendComponent) hosts `SutViewSelection`,
// exercised every tick by `general_e2e_composed_pbt`; the windowed dual bodies
// are also covered by `run_windowed_composed_check`. See `NATIVE_ONLY_EXCLUDED`
// + the `SutViewSelection` row in `E1_RELOCATED_CAP_COVERAGE`.
// (`SutLayout`/`SutDriver` are NOT deleted — they remain load-bearing for
// E2ESut's windowed input/transition-apply shell until E5.)

// ─── SutRenderer ──────────────────────────────────────────────────────

impl E2ESut {
    /// Builder services for an *independent* (fresh) headless re-interpret in
    /// the `SutViewSelection` render invariants. The Turso wiring
    /// re-interprets through a fresh `HeadlessBuilderServices` over its
    /// `BackendEngine` (preserving the established independence from the
    /// live reactive state); the no-Turso wiring has no engine, so it
    /// re-interprets through the reactive engine built over `block_query` —
    /// the only builder-services it carries. The `SutViewSelection` callers
    /// (`headless_error_node_count` etc.) populate the reactive engine
    /// before calling here. `HeadlessBuilderServices` is Turso-only by
    /// construction, so the selection keys off the explicit storage backend,
    /// not a capability-presence proxy.
        ...
        match self.ctx.storage() {
            ...
            holon::di::StorageSelector::LoroMemory => {
                self.render.reactive_engine.borrow().clone().expect(
                    "render_builder_services: no-Turso reactive engine must be set (the \
                     SutViewSelection caller watches the root first)",
...
// invariant dispatch; see `NATIVE_ONLY_EXCLUDED` +
// `E1_RELOCATED_CAP_COVERAGE`).
//
...
impl SutLayout for E2ESut {
    ...
    /// Mirrors the `tree_draggable` collection in
    /// `inv-editable-text-has-draggable` (sut.rs:6634–6643): an element
    /// whose widget_type == "draggable" and entity_id == id.
...
impl E2ESut {
    ...
    async fn wait_for_current_focused_editor_window_focus(&self, verb: &str) {
        ...
        loop {
            ...
            if tokio::time::Instant::now() >= deadline {
                panic!(
                    "[{verb}] engine-focused block {focused:?} never held window focus within 2s \
                     — keystroke would land in the wrong editor"
                );
            }
        }
    }
    ...
    async fn wait_for_editor_text_change(&self, before: &Option<String>, verb: &str) {
        ...
        loop {
            ...
            if tokio::time::Instant::now() >= deadline {
                panic!(
                    "[{verb}] keystroke never changed the focused editor's on-screen text within \
                     2s (still {before:?}) — it was consumed by a stale editor or lost"
                );
            }
    ...
    async fn apply_move_cursor(&self, byte_position: usize) {
        ...
        let right_presses = {
            let text = self
                ...
                .unwrap_or_else(|| {
                    panic!(
                        "[MoveCursor] no pre-transition active editor text — cannot convert byte \
                         position {byte_position} to keystrokes"
                    )
                });
            assert!(
...
// `editor_live_text`) is now hosted by the composed `full_headless` editor read
// cap. Its two ref-comparison invariants (`inv-editor-caret/mirror`,
// `inv-editor-text/mirror`) run only via the composed catalog — see
// `NATIVE_ONLY_EXCLUDED` + the `SutEditorMirrorRead` row in
// `E1_RELOCATED_CAP_COVERAGE`.
...
impl SutBlockTreeWrite for E2ESut {
    async fn apply_split_block(&self, block_id: &EntityUri, position: usize) {
            ...
            .await
        {
            ...
            panic!(
                "[SplitBlock] bounds unavailable for {resolved_id}: \
        ...
        if let Some(parent_id) = parent_for_settle {
            self.wait_for_children_settled(&parent_id, Duration::from_secs(5))
                ...
                .unwrap_or_else(|e| {
                    panic!(
                        "[SplitBlock] children of parent {parent_id} not settled before click: \
                         {e:#}"
                    )
                });
        }
...
impl SutDriver for E2ESut {
    ...
    async fn driver_send_key_chord(&self, _: &str) {
        unimplemented!(
            "SutDriver::driver_send_key_chord on E2ESut: requires a focused entity id and a \
             parsed KeyChord; E2ESut::send_key_chord already provides this with explicit args. \
             Bridge in Phase 7 once SutFocus exposes the current focus id."
        )
    }
...
// `frontend_slice` is now its sole host
// (`frontend_slice_org_render_fixed_point_bites`
// drives `inv-org-render-fixed-point` to both arms). The native runner no
// longer dispatches it (see `NATIVE_ONLY_EXCLUDED`).
...
// — the impl was never wired (`unimplemented!()`); no transition/generator ever
// bound it. Trait removed from pbt-core; re-add when a query-content generator
// needs it.
...
// composed host calls it); only this `E2ESut` impl + the native dispatch are
// gone.
