...
//!
//! 2. **Focused page title rendered with the wrong block-profile variant.**
//!    The focused page's title block (the `Main`-region focus root, a level-0
//!    block that the tree overrides to `role == "page_title"`) must render via
//!    the `page_title` variant — a bare `text` h1 node
//!    (`text(col("content"), #{style: "h1"})`). The GPUI app instead renders it
//!    with the `default` variant, whose fingerprint is a `state_toggle` widget
//!    inside a row (`column(row(… state_toggle …), drop_zone())`). The focus-root
//!    id occurs MULTIPLE times in the snapshot (left-sidebar row, main-panel
//!    title row, …), so we check EVERY occurrence and assert none of their
//!    subtrees contains a `state_toggle` — stopping at the pre-order first match
//!    only ever inspected the sidebar row and was vacuously green.
//!    Blocks are flattened siblings in the tree collection (not nested widgets),
//!    so a focus-root node's subtree is that block's own rendering only.
//!
...
//! ## Bound
//!
//! `S: SutRenderer` for the widget tree; `R: RefBlockTree` for the `Main`-region
//! focus roots (`focus_root_ids`), resolved into SUT id space by the runner's
//! `with_resolved_doc_uris` view.

use holon_pbt_core::capabilities::{CapRegion, RefBlockTree, SutRenderer, WidgetSnapshot};
...
fn is_virtual(node: &WidgetSnapshot) -> bool {
    effective_entity_id(node).is_some_and(|id| id.contains(VIRTUAL_MARKER))
}
    ...
    async fn check(&self, ref_: &R, sut: &S) -> InvariantResult {
        ...
        for node in root.walk() {
            ...
            let has_real_row = node
                .children
                .iter()
                .any(|c| effective_entity_id(c).is_some_and(|id| !id.contains(VIRTUAL_MARKER)));
            if !has_real_row {
                continue;
            }
            ...
            for (pos, child) in node.children.iter().enumerate() {
                ...
                if real_after {
                    ...
                    return InvariantResult::Fail(format!(
                        "[inv-viewmodel-tree-virtual-slots] virtual creation slot \
                         '{}' is at position {pos} of {} in container '{container}' \
                         (kind '{}') but real rows follow it — it must come after \
                         every real row. Siblings in order:\n  {siblings:?}",
                ...
                if toggles > 0 {
                    ...
                    return InvariantResult::Fail(format!(
                        "[inv-viewmodel-tree-virtual-slots] focused page title block \
                         '{}' rendered via the `default` variant ({toggles} \
                         state_toggle widget(s) in occurrence kind '{}', subtree \
                         {kinds:?}) instead of the bare-`text` `page_title` variant",
                        focus_root.as_str(),
                        block_node.kind,
                    ));
                }
