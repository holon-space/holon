...
//!   (`SutLayout::rendered_elements`, the `FrontendBounds` layer; empty
//!   headless → `Skipped`) and checks every widget, including the active
//!   editor.
...
//!   skipping the actively-edited block (its uncommitted `InputState` value is
//!   a widget-layer concern, not a ViewModel one).
//!
//! Localisation (the point of splitting): `/widget` ✗ while `/viewmodel` ✓
//! means the ViewModel held the right content but the paint / `InputState`
//! layer is stale; both ✗ points upstream of the renderer (a bad projection
//! into the VM tree). Both compared `entity_id`s and the resolved reference are
//! in SUT id space (`with_resolved_doc_uris` remaps `active_editor.block_id`),
//! so no per-comparison `reverse_map` is needed.
...
use crate::pbt::invariants::text_compare::TEXT_WIDGET_KINDS;
use crate::pbt::invariants::text_compare::TextSample;
use crate::pbt::invariants::text_compare::compare_text_to_ref;
use crate::pbt::invariants::text_compare::format_mismatch;
...
fn fail_message(
    ...
    format!(
        "[{label}] {n} text widget(s) show stale content at the {layer} layer. The shown string \
         diverged from the reference block content — typical after split_block/join_block when \
         the row's data signal fires but the text (editable_text InputState, text col(...) \
         snapshot, or a stale VM prop) skips the update.\n{body}",
        n = mismatches.len(),
        body = mismatches
            .iter()
            .map(format_mismatch)
