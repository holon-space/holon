...
//!   render expr's function-call name, or — when the engine wraps the root in
//!   a `view_mode_switcher` — the wrapper's first child must.
//!
        ...
        match panel_content_kind(panel_node) {
            ...
            Some(actual) if !is_not_ready_kind(actual) => InvariantResult::Fail(format!(
                "[inv-viewmodel-root-matches-render-expr] Main panel content widget '{actual}' \
                 does not match expected render expr '{expected_panel_name}' \
                 (root layout '{root_kind}', main-panel id '{}')",
                main_panel_id.as_str()
            )),
            ...
            Some(actual) => InvariantResult::Skipped(format!(
                "main-panel content widget '{actual}' is not a rendered widget yet \
                 (root layout '{root_kind}', main-panel id '{}')",
                main_panel_id.as_str()
            )),
            ...
            None => InvariantResult::Skipped(format!(
                "main-panel node (id '{}') has no content child yet \
