        ...
        loop {
            if Instant::now() >= deadline {
                return InvariantResult::Fail(format!(
                    "[inv-org-render-fixed-point] render != disk PERSISTED for {budget:?} — \
                     not a transient projection lag but a real echo-loop / oscillation: \
                     the next re_render_all_tracked would keep rewriting the file.\n{last_fail}"
                ));
            }
...
impl InvOrgRenderFixedPoint {
    /// First `(path, disk, rendered)` whose disk bytes differ from the render, if any.
