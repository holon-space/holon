...
//! `Occurrence::Placed` key derived from `(lesson, anchor)` and appends it under
//! the anchor. A single block id decorates MANY nodes in the snapshot
...
//! dedup, we collapse the multi-node decoration to one entry per DISTINCT anchor
//! id and attribute each placed row to its anchor by the one-way
...
use holon_api::{EntityUri, Occurrence, OccurrenceId};
use holon_pbt_core::capabilities::{AdviceExpectation, RefAdvice, SutRenderer};
...
/// `:__virtual:` detection is `RowOrigin`, as in `viewmodel_tree_virtual_slots`).
...
/// - `min(observed scores) >= max score among exp.scored NOT observed`
///   (top-K boundary dominance).
///
...
pub fn check_advice_relation(
    exp: &AdviceExpectation,
    observed: &[String],
) -> Result<(), String> {
    // pairwise distinct
    for (i, a) in observed.iter().enumerate() {
        if observed[i + 1..].contains(a) {
            return Err(format!(
                "[inv-advice-rows-woven] duplicate woven advice row '{a}' (observed order: {observed:?})"
            ));
        }
    }
    ...
    if observed.len() != want {
        return Err(format!(
            "[inv-advice-rows-woven] wove {} advice rows but expected {want} \
             (k={}, {} scored candidates); observed={observed:?}, scored={:?}",
            observed.len(),
            exp.k,
            exp.scored.len(),
            exp.scored,
            ...
            .filter_map(|n| {
                n.props.get("occurrence").map(|suffix| {
                    let lesson = n.entity_id.as_deref().expect(
                        "a placed advice row stamps props[\"occurrence\"] on its \
                         entity_id (lesson) node",
                    );
        ...
        for (anchor_uri, kind) in &anchors {
            ...
            let observed: Vec<String> = placed
                ...
                .filter_map(|(lesson, suffix)| {
                    ...
                    let expected = Occurrence::Placed(OccurrenceId::for_placement(
                        &lesson_uri,
                        anchor_uri,
                    ))
                    .key_suffix();
        ...
        for (lesson, suffix) in &placed {
            ...
            if !matched {
                let anchor_ids: Vec<&str> =
                    anchors.iter().map(|(u, _)| u.as_str()).collect();
                return InvariantResult::Fail(format!(
