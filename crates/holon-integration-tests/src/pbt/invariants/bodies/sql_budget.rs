...
//! `Invariant<CapMap, CapMap>`, and the cap took a concrete `&ReferenceState`
//! the composed ref `CapMap` can't hand back), so the composed slice covers
...
//! `E2ESut` impl + dispatch were deleted (E3); this module now keeps only the
//! two pieces the composed path reuses: the canonical [`InvSqlBudget::ID`] and
//! the [`SqlBudgetReport`] that [`crate::pbt::sut_metrics::MetricsSut`]
//! produces.

use holon_pbt_core::invariant::InvariantId;

/// Canonical id home for `inv-sql-budget`. No longer an [`Invariant`] impl —
/// the composed [`crate::pbt::composed::span_metrics::InvComposedBudget`] is
/// the sole dispatched body and reuses [`Self::ID`].
///
...
mod otel {
    ...
    /// [`crate::pbt::sut_metrics::MetricsSut::sql_budget_report`]; this carries
    /// only what the invariant decides on. Consumed by the composed
