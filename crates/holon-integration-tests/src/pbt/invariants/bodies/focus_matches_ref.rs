...
//! - An editor is open in the reference model (editor focus is the source of
//!   truth while editing; engine focus may lag).
//!
//! The comparison bridges the ref-model's synthetic URI (e.g.
//! `block:ref-doc-0`) to the engine's resolved UUID via
//! `SutDriver::resolve_ref_block_id`.
//!
//! Status: functional.
    ...
    async fn check(&self, ref_: &R, sut: &S) -> InvariantResult {
        ...
        match sut.engine_focused_block().await {
            ...
            EngineFocus::Focused(actual_id) => InvariantResult::Fail(format!(
                "[inv-focus-matches-ref] Global focus mismatch: reference model has {ref_focused} \
                 (resolved: {resolved_ref}), but engine.focused_block() has {actual_id}"
            )),
            EngineFocus::Unfocused => InvariantResult::Fail(format!(
                "[inv-focus-matches-ref] Global focus LOST: reference model has {ref_focused} \
                 (resolved: {resolved_ref}), but engine.focused_block() is None"
            )),
        }
    }
}
