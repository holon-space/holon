    ...
    async fn check(&self, _: &R, sut: &S) -> InvariantResult {
        match sut.live_vs_fresh_tree_diff().await {
            ...
            Some(diffs) => InvariantResult::Fail(format!(
                "[{label}] LIVE tree diverges from FRESH tree! The collection driver's \
                 set_data path produces different props than fresh interpretation. Child \
                 widgets see stale data in the GPUI frontend.\n\nDiffs ({n}):\n{body}",
                label = Self::LABEL,
                n = diffs.len(),
                body = diffs.join("\n"),
            )),
