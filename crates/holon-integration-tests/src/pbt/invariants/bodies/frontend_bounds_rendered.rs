...
//! expected-size-satisfied, no-error-widgets-rendered, vm-y-order-and-contiguity,
//! non-wrapper-content-when-docs, not-visually-empty, vm-data-tracked-as-content.
//! Warn sub-checks (logged, never fail): bounds-registry-not-empty,
//! vm-entities-have-bounds, known-widget-type, element-has-content.
//!
    ...
    S: SutLayout + SutViewModel,
{
    ...
    async fn check(&self, ref_: &R, sut: &S) -> InvariantResult {
        ...
        if has_documents && layout_ready && !has_content_widget {
            ...
            {
                return InvariantResult::Fail(format!(
                    "[{}/not-visually-empty] UI is visually empty: content_fraction={content_fraction:.4} \
                     <= {min_content:.4} (reference has documents, main_focused={main_focused}, \
                     bounds have no content widget)",
                    Self::LABEL,
                ));
            }
        }
        ...
        if !data_entity_ids.is_empty() {
            ...
            if content_match_count == 0 {
                if has_loading || !all_have_some_widget {
                    ...
                    return InvariantResult::Fail(format!(
                        "[{}/vm-data-tracked-as-content] ViewModel has {} data entity id(s) but none \
                         are tracked as content widgets (render_entity/editable_text/selectable): {:?}",
                        Self::LABEL,
                        data_entity_ids.len(),
                        &data_entity_ids[..data_entity_ids.len().min(5)],
                    ));
