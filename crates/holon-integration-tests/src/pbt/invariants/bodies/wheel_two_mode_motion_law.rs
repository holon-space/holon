...
//! @pbt covers wheel-two_mode_motion_law — two-mode motion law — Δ
//! active-footer.top ∈ {0, −δ} after a wheel @pbt slips-if-removed a wheel that
//! mis-routes or moves the pinned footer by   the wrong amount ships a sticky
//! region that jumps or double-scrolls, which   VM-only checks cannot see
//!
...
//! [`holon_frontend::sticky_accordion::check_two_mode_motion_law`] (unit-tested
...
pub struct InvWheelTwoModeMotionLaw;

impl InvWheelTwoModeMotionLaw {
    pub const ID: InvariantId = InvariantId("inv-wheel-two-mode-motion-law");
    const LABEL: &'static str = "inv-wheel-two-mode-motion-law";
}
...
impl<R, S> Invariant<R, S> for InvWheelTwoModeMotionLaw
where
    S: SutLayout,
{
    ...
    async fn check(&self, _: &R, sut: &S) -> InvariantResult {
        ...
        // before/after the last WheelScroll and evaluate sa::check_two_mode_motion_law.
