...
//! - **flicker (vfn13)** — every cache identity from pass 1 reappears in pass 2.
//!
    ...
    S: SutFrontendEmissions,
{
    ...
    async fn check(&self, ref_: &R, sut: &S) -> InvariantResult {
        ...
        if expects_focus_rows && !report.any_nonempty {
            return InvariantResult::Fail(format!(
                "[{}/vfn11] active render_expr mentions focus_chain and the reference has a focused \
                 block, but no streaming provider produced rows",
                Self::LABEL,
            ));
        }
        ...
        if report.flicker_count > 0 {
            return InvariantResult::Fail(format!(
                "[{}/vfn13] provider cache identity flicker: {} id(s) present in pass-1 but missing \
                 in pass-2 — cache wiring regressed",
                Self::LABEL,
