...
//!   `frontend_geometry`, `frontend_visual_state`) that the phased/GPUI harness
//!   installs after StartApp;
...
impl RenderSut {
    ...
    pub(super) async fn set_expand_toggle_gate(
        ...
        assert!(
            gate.is_some(),
            "set_expand_toggle_gate: no expand_toggle node with target_id={target_id} in reactive \
             tree under {root_id}. The fixture grew an expand_toggle render but the engine didn't \
             produce a matching node — likely a shadow_builder or interpret regression. See \
