...
//! into `holon-pbt-core`.
//!
//! `#[holon_macros::capmap_adapter]` references
//! `::holon_pbt_core::composition::*` by absolute path and the trait by bare
//! name, so it expands correctly in this crate too: `impl SutMutate for CapMap`
//! (local trait for foreign type — allowed) and `impl CapName for dyn
//! SutMutate` (foreign trait for local type — allowed).
...
/// `&self` mutation — `HeadlessFrontendComponent` realizes it HEADLESSLY via
/// the production `set_field task_state` op, so any composed config that has it
/// can drive `ToggleState` faithfully.
...
/// (`ApplyMutation`) and bulk external block adds (`BulkExternalAdd`). Split
/// off `SutMutate` (2026-06-21, swap track) because their real,
/// `ref_state`-dependent dispatch lives ENTIRELY in the `E2ESut` harness seam
/// (`block_tree_post_action`): the cap action itself is a pure `&self` no-op,
/// and a no-op cap is a FAKE on any SUT that lacks the seam (the composed
/// `HeadlessFrontendComponent` would silently pass `ApplyMutation`/
/// `BulkExternalAdd` while doing nothing). So `E2ESut` provides this cap (its
/// seam does the work) but the composed frontend does NOT — keeping
/// cap-presence honest, so these transitions AUTO-NARROW out of the composed
/// alphabet (`required_caps` ∌ a provided cap) instead of generating +
/// diverging. The seam rebuild (a real composed dispatch) is what will let the
/// composed path provide it; until then its absence is the truthful signal.
#[holon_macros::capmap_adapter]
...
/// Loro snapshot. Driven by the `WriteOrgFile`, `CreateDirectory`, `GitInit`,
...
pub trait SutAppLifecycle {
    /// Boot the app. `root_id` (the layout render root) and
    /// `expects_valid_index` are `ref_state`-derived but consumed DURING
    /// the action (render warmup / `ensure_reactive_engine`), so the
    /// `StartApp` transition precomputes them at the boundary and passes
    /// them as typed args — the HARD RULE forbids `ref_state` in the
    /// signature, not derived values. The doc-uri reconcile and
    /// the seed-count settle relocate to the `block_tree_post_action`
    /// `StartApp` arm.
    ...
    /// consolidator flipped (Loro ⇄ SQL). The real `holon-app` wiring guard
    /// must reject it with Model.md invariant 10's hard error BEFORE any
    /// higher store opens; the live app and the persisted marker stay
    /// untouched. The assertion lives in the SUT apply (no separate
    /// invariant) — it panics if the flipped boot succeeds (the prod bug
    /// this transition exists to catch) or if the error is not the
    /// invariant-10 epoch error.
