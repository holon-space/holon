...
//! deterministic skips, never panics — and reports whether the run still failed.
//!
...
//! divergence has often been a reference-model fidelity gap, not a prod bug. The
//! bisector localizes *where* the disagreement enters; *who is wrong* is a human
//! step.

use std::panic::{AssertUnwindSafe, catch_unwind};
use std::path::Path;

use holon_pbt_core::bisect::{Localization, bisect};
...
pub fn load_capture_fixture(path: impl AsRef<Path>) -> Fixture<E2ETransition, ()> {
    ...
    if let Some(report) = fixture.environment.mismatch_report(None) {
        eprintln!(
            "[bisect] WARNING: replay env differs from capture env — replay may be unfaithful:\n{report}"
        );
    }
    fixture
}

/// Replay `transitions` against a SUT wired for `set` under [`ReplayMode::SkipGated`]
/// and report whether the **reference and the node's enabled projections diverge**.
/// The capture carries no wiring (ADR 0009 §4 item 2); `set` supplies it.
///
...
/// abort**, not a divergence: a transition whose SUT path is Turso-coupled (e.g.
/// the `probe_block_sql_state` diagnostic, or a settle timeout) crashing on a
/// no-Turso node it was never generated against. Those are logged and counted as
/// non-reproduction, so the downward walk never *descends into* a node where the
/// capture cannot be faithfully replayed and spuriously localizes there (this is
/// exactly the loro-node false positive that made a Turso-generated capture
/// "reproduce" under `loro_vm_fast`). Trade-off: a genuine non-invariant SUT
/// crash also lacks the marker and is not counted — bisection localizes
/// *divergence*, which is its stated purpose (ADR §3).
///
...
pub fn reproduces_under(set: &ComponentSet, transitions: &[E2ETransition]) -> bool {
    ...
    if !is_divergence {
        ...
        eprintln!(
            "[bisect] {:?}+{:?}: replay aborted, NOT a reproduction (inconclusive node) — {first_line}",
...
/// `String`; `expect`/`&str` literals a `&str`).
...
pub fn ceiling_by_name(name: &str) -> ComponentSet {
    match name {
        ...
        other => panic!(
            "[bisect] unknown ceiling preset {other:?} \
             (expected one of: full_gpui, full_headless, loro_vm_fast, sql_only)"
        ),
    }
}
