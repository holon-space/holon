...
/// org order: a not-yet-caught-up Loro (whose projection still owes `block_raw`
/// its reordered `sort_key`) counts as activity and resets the quiet window, so
/// CDC/org only read "quiet" once that write has landed and re-drained. The old
/// order let stage-1 CDC drain before stage-2 ever wrote the sort_key, and every
/// stage broke on `deadline` returning silently — an unconverged SUT looked
/// settled, the direct cause of the org-render sibling-order flake. Returns
/// `true` iff the fixed point was reached within `budget`; `false` = gave up, and
/// the caller MUST decide (the composed settle fails loud on `false`).
