...
pub(super) const BLOCK_MATVIEW_SNAPSHOT_SQL: &str = "SELECT id, parent_id, content, content_type, source_language, properties, tags, requires, advice_suppressed \
     FROM block";
...
/// matview drops it the same way (`schema_modules.rs`: `WHERE b.id != 'sentinel:no_parent'`).
pub(super) const BLOCK_RAW_SNAPSHOT_SQL: &str = "SELECT id, parent_id, content, content_type, source_language, properties \
     FROM block_raw WHERE id != 'sentinel:no_parent'";
