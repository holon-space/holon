...
//!   `text` widget for the block (e.g. a sidebar `text(col(...))` whose widget
//!   is rendered by the *parent's* render expression, not the block's own).
//!   Diagnostic doesn't apply.
//!
...
// ---------------------------------------------------------------------------
//
// When an `apply_X` panic fires (`wait_for_focus_to_match`,
// `wait_for_blocks_synced`, SplitBlock count mismatch, etc.), we want one
// consolidated message that tells you:
//   - what the GPUI render tree looks like *right now* (which entities are
//     mounted, under which widget type),
//   - what `UiState.focused_block` currently reports,
//   - the recent focus-change history per region (`navigation_history` rows).
//
// This eliminates the "I had to read an unrelated invariant's tree dump 200
// lines earlier to reconstruct what failed at the panic site" pattern.
...
pub fn format_render_tree(elements: &[(String, ElementInfo)], label: &str) -> String {
    ...
    fn print_node(
        ...
        let _ = writeln!(
            out,
            "[{label}] {indent}{id}: widget_type={} entity_id={:?} bounds=({:.0},{:.0} \
             {:.0}x{:.0}) has_content={}",
            info.widget_type,
            info.entity_id,
            info.x,
...
pub async fn fetch_recent_nav_history(engine: &BackendEngine, limit: i64) -> Vec<NavHistoryRow> {
    ...
    let sql = "SELECT id, region, block_id, timestamp, closed_at FROM navigation_history ORDER BY \
               id DESC LIMIT $limit";
...
/// One-stop panic-diagnostic dump: focused_block, navigation_history, render
/// tree.
///
