...
//! ([`ComposedSut::check_invariants`](crate::pbt::composed::harness)) can honour
//! it: a matched `warn`/`skip` failure is logged loudly and made non-fatal; a
//! green run under active overrides is a DISCLOSED degraded run, not a clean pass.
//!
...
/// One parsed `pattern:mode` rule. The pattern matches invariant id strings with
/// a single optional `*` wildcard, case-insensitively — the same glob shape as
/// the transition-weight patterns, kept local to avoid coupling the subsystems.
#[derive(Debug, Clone)]
enum IdPattern {
    Exact(String),
    Prefix(String),
...
/// Effective [`ModeOverride`] for `invariant_id` from `HOLON_PBT_INVARIANTS`, or
/// `None` when no rule matches (use the default: a failure is fatal). First-match-wins
