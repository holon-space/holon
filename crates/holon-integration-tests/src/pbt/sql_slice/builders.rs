...
/// (via `EventInfraModule`) and are out of this slice's scope; create /
/// set_field / delete suffice to drive the block-tree and SQL-projection
/// invariants.
///
...
/// (split_block/indent/outdent/move_up/move_down/join_block) — the
/// structural-op path `new_sql_engine` omits. The dispatcher routes by
/// `(entity, op)` and the two providers' op-name sets are disjoint, so they
/// coexist for the `block` entity.
///
/// This is the async **structural write** substrate for composed slices that
/// drive the production block-tree alphabet through `OpDispatchWriter`
...
/// `SutBlockTreeWrite` (§8.11 LL-2 — a storage-only config has no higher
/// driver, so the floor applies structural ops directly to the engine), sharing
/// `resolver` (the synthetic→real id map the composed runner accumulates).
/// `SutSqlProjection` is intentionally NOT registered so the navigation/focus
/// invariants deselect (this slice has no navigation engine; a focused oracle
/// would false-RED against empty focus rows).
