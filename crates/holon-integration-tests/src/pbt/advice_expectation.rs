...
//!   anchor-denormalized `block_tags` self-join: one row per (anchor,
//!   candidate) with `shared_tag_count = COUNT(*)` over shared tags — the FULL
//!   intersection, marker/selector tags included when genuinely shared. No
//!   suppression, no K.
...
use holon_advice::ADVICE_RULE_SOURCE_LANGUAGE;
use holon_advice::AdviceRule;
...
/// the `source_language` comparison in
/// `ReferenceState::rebuild_profile_tracking`.
...
pub fn active_rule(blocks: &BTreeMap<EntityUri, Block>) -> Option<AdviceRule> {
    ...
    assert!(
        active.len() <= 1,
        "v1 keystone generator guarantees at most one active advice rule per run; found {} \
         (liftable narrowing) — harness bug",
        active.len(),
    );
...
/// anchor-suppressed lessons removed, sorted by count DESC, carrying the rule
/// K.
...
/// The un-capped, un-suppressed matview contract: all (anchor, candidate,
/// count) triples of the rule over every anchor block. No suppression filter,
/// no K.
...
mod tests {
    ...
    fn tag_overlap_counts_full_intersection_including_marker_tags() {
        ...
        assert_eq!(
            exp.scored,
            vec![
                ("block:lessonA".to_string(), 2),
                ("block:lessonB".to_string(), 1)
            ],
        );
        assert_eq!(exp.k, 5);
    }
    ...
    fn anchor_excludes_itself_even_when_it_carries_the_candidate_tag() {
        ...
        // the point of the test is `both` does NOT weave itself despite carrying
        // `lesson`.
