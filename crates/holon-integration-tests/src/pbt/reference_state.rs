...
/// on 2026-01-15, timezone-robust). Both the SUT's `TestClock` and the reference
/// model derive the boot day from this, so they agree by construction without a
/// side channel.
pub const KEYSTONE_CLOCK_BOOT_MS: i64 = 1_768_478_400_000; // 2026-01-15T12:00:00Z

/// Reference-side calendar-clock model (ADR 0024 §6). Tracks the model's current
/// day and the set of distinct days the clock has visited, from which it predicts
/// the journal-rule output: exactly one journal block per distinct day.
#[derive(Debug, Clone)]
...
impl ReferenceState {
    ...
    /// The born-equal sibling of [`create_block_under`]: append a new text block
    /// under `parent` using exactly `new_id` (a fresh normal id supplied by the
    /// transition, NOT a minted `create-N` synthetic). The SUT's op-floor create
    /// dispatches the same id, so the reconcile treats it as born-equal (both
    /// sides already hold it — no synthetic→real pairing). Otherwise identical to
