...
pub struct E2ESut {
    ...
    /// Optional Loro validation — reads blocks from LoroTree and compares
    /// against reference. Active only when Loro is enabled. Built once at
    ...
    pub(super) async fn map_unmapped_split_synthetic_ids(
        ...
        for (parent, mut synths) in ref_by_parent {
            let Some(reals) = real_by_parent.get_mut(&parent) else {
                eprintln!(
                    "{label} split-id pairing: {} unmapped synthetic under parent {parent} but no \
                     new real rows — skipping",
                    synths.len()
                );
                continue;
            };
            if synths.len() != reals.len() {
                eprintln!(
                    "{label} split-id pairing ambiguous under parent {parent}: {} unmapped \
                     synthetic vs {} new real — skipping",
                    synths.len(),
                    reals.len()
                );
                continue;
            ...
            for ((synthetic, _), (real_id_str, _)) in synths.into_iter().zip(reals.iter()) {
                // ALLOW(entity_uri_from_raw): real_id_str id field from non_page_block_rows()
                // snapshot row
...
impl E2ESut {
    ...
    /// Set up the mutation driver from the DI-resolved ReactiveEngine. Called
    /// after start_app. Uses the same dispatch path as GPUI
    /// (BuilderServices::dispatch_intent). Also installs the same `Arc<dyn
    /// UserDriver>` into `live_driver()` so PBT generators read observation
    /// verbs from the same medium.
            ...
            (
                ...
                format!(
                    "SELECT id, parent_id, content, content_type, source_language, \
                     ...
                     json_extract(properties, '$.sequence')  AS sequence FROM block_raw WHERE id \
                     = '{escaped}'"
                ),
            ),
            (
                ...
                format!(
                    "SELECT id, parent_id, content, content_type, source_language, \
                     json_extract(properties, '$.task_state') AS task_state, tags FROM block \
                     WHERE id = '{escaped}'"
                ),
            ),
            (
                "siblings_raw",
                format!(
                    "SELECT b.id, b.content_type, json_extract(b.properties, '$.task_state') AS \
                     task_state, json_extract(b.properties, '$.sequence') AS sequence FROM \
                     block_raw b WHERE b.parent_id = (SELECT parent_id FROM block_raw WHERE id = \
                     '{escaped}') ORDER BY sequence"
                ),
            ),
            (
                "focus_roots",
    ...
    pub(super) async fn wait_for_entity_bounds(
        ...
        loop {
            ...
            if tokio::time::Instant::now() >= scroll_deadline {
                ...
                {
                    tracing::debug!(
                        "wait_for_entity_bounds: scroll_to_entity({entity_id:?}) returned Err — \
                         continuing to poll: {e:#}"
                    );
                }
                ...
                if visible(geometry.as_ref(), &render_id, &selectable_id, entity_id) {
                    eprintln!(
                        "[bounds-wait] {entity_id} bounds present immediately post-scroll \
                         (revealed-by-scroll; absent before)"
                    );
                    return Ok(());
                }
            }
                ...
                let frame_diag = if exit_generation == entry_generation {
                    format!(
                        "NO frame committed during the wait (generation stuck at \
                         {entry_generation}) — paint pipeline idle/stalled, not a missing-row bug"
                    )
                } else {
                    format!(
                        "{} frame(s) committed during the wait (generation {entry_generation} → \
                         {exit_generation}) — element truly absent from rendered output",
                        exit_generation - entry_generation
                    )
                };
                anyhow::bail!(
                    "wait_for_entity_bounds: timed out after {timeout:?} waiting for bounds of \
                     entity {entity_id:?} — tried element ids {render_id:?}, {selectable_id:?}, \
                     and entity_id scan; element was never rendered to BoundsRegistry \
                     (post-scroll), or bounds weren't promoted staged → committed since the last \
    ...
    pub(super) async fn wait_for_widget_kind(
        ...
        match result {
            ...
            Err(observed_for_entity) => {
                ...
                .await;
                anyhow::bail!(
                    "wait_for_widget_kind: {entity_id:?} never rendered as one of {accepted:?} \
                     within {timeout:?}; observed widget_types for this entity_id: \
                     {observed_for_entity:?}\n{diag}"
                );
            }
        }
    ...
    pub(super) async fn wait_for_focus_to_match(
        ...
        match result {
            ...
            Err(actual) => {
                ...
                .await;
                anyhow::bail!(
                    "wait_for_focus_to_match: expected={expected_block_id:?} actual={actual:?} \
                     after {timeout:?}\n{diag}"
                );
            }
        }
    ...
    pub(super) async fn wait_for_children_settled(
        ...
        loop {
            ...
            {
                tracing::debug!(
                    "wait_for_children_settled: scroll_to_entity({target:?}) returned Err — \
                     continuing to poll: {e:#}"
                );
            }
        ...
        .await;
        anyhow::bail!(
            "wait_for_children_settled: parent={resolved_parent} expected {} child widget(s) \
             (rendered_text/editable_text), saw {} after {timeout:?} (incl. scroll-to-reveal); \
             unreachable={missing:?}\n{diag}",
    ...
    /// Handles file:→block: (pages), block::split-N→block:uuid (split-created
    /// blocks), and passes through any URI not in the map unchanged.
    ...
    pub(super) async fn wait_for_window_focused_editor(
        ...
        .map_err(|observed| {
            anyhow::anyhow!(
                "wait_for_window_focused_editor: {block_id:?} editable_text never took window \
                 focus within {timeout:?} (observed focused states: {observed:?})"
            )
        })
    }
    ...
    /// Look up the keybinding for an operation name from the reactive engine's
    /// registry.
...
impl E2ESut {
    ...
    /// (used for org-file/external mutation paths where doc URIs are
    /// UUID-keyed).
