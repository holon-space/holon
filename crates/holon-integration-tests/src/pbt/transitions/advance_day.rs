...
//! `reconcile_clock`.
//!
//! Oracle: `apply_to_ref` advances the reference-model day (`RefClockMut`),
//! which records the distinct day visited. The composed
//! `inv-journal-one-per-day` invariant then asserts the projection holds
//! exactly one journal block per distinct day the clock has visited — so N
//! advances spanning D distinct days ⇒ D journal blocks, and a same-day re-tick
//! (`days == 0`) adds nothing (the deterministic id converges to an upsert).
//! Settle is automatic (`S::settle_after_apply` → `converge_projections`), so
//! the reactive rule firing projects through before the invariant runs.
