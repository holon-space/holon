...
pub async fn apply_bulk_external_add_to_sut(
    ...
    // Resolve file-based URI to UUID-based URI (documents map uses UUID keys after
    // StartApp)
            ...
            match task.await {
                ...
                Ok((i, elapsed, prql, Ok(Err(e)))) => {
                    ...
                    if is_concurrency_error(&error_str) {
                        panic!(
                            "FLUTTER STARTUP BUG REPRODUCED: query_and_watch {} failed with \
                             concurrency error after {:?} while bulk data ({} blocks) was being \
                             synced!\nThis is the exact bug that causes Flutter app to get stuck \
                             during startup.\nQuery: {}\nError: {}",
                            i,
                            elapsed,
                            blocks.len(),
                ...
                Ok((i, elapsed, prql, Err(_timeout))) => {
                    ...
                    panic!(
                        "SCHEDULER BUG: query_and_watch {} timed out after {:?}!\n\nRoot cause: \
                         OperationScheduler's mark_available() was never called for 'blocks' \
                         table.\n\nThe materialized view creation is stuck in the scheduler's \
                         pending queue:\n- execute_ddl_with_deps submitted with \
                         requires=[Schema(\"blocks\")]\n- can_execute() returned false (blocks \
                         not in available set)\n- Operation queued in pending, response_rx.await \
                         blocks forever\n\nQuery: {}\n\nFix required:\n1. Call \
                         scheduler_handle.mark_available() for core tables after schema creation \
                         in DI\n2. Ensure MarkAvailable command calls process_pending_queue() to \
                         wake pending ops",
                        i, elapsed, prql
                    );
                }
