...
impl TransitionRef<ReferenceState> for StartApp {
    ...
    fn apply_to_ref(&self, state: &mut ReferenceState) {
        ...
        // Model the bundled default-asset layout the production boot seeds
        // (page shells + `__default__` + parsed `index.org` + `layout_blocks`
        // classification). Extracted so the composed/wide oracle's
        // `build_started_ref` can model the SAME layout the booted SUT carries
...
/// `__default__` seed page + the parsed `assets/default/index.org` layout blocks
/// (fresh only), all seed-classified (`block_documents[id]=no_parent`), and the
/// `layout_blocks.{query_source_ids,render_source_ids,headline_ids}`
/// classification. Extracted from `StartApp::apply_to_ref` (behavior-preserving)
/// so the composed/wide oracle's `build_started_ref` can model the SAME layout
/// the booted `full_headless` SUT carries — making `inv-watch-rows-match-ref` a
/// full-fidelity full-block-set comparison rather than a seed-excluded one. Does
/// NOT touch nav-focus / sidebar-drawer / seed-profile state (those stay with the
/// caller: `StartApp` applies them; the composed oracle aligns nav in
/// `structural_ref_wired`).
