...
use super::components::{InMemEditorComponent, MemoryBackendComponent};
...
/// cap and the `SutBlockTreeWrite` write cap, over one shared `Arc<MemoryBackend>`.
/// Returns the component `Arc` so the driver can keep its synthetic-id counter in
/// lockstep with the oracle (`set_next_split_id`) and seed the shared store. The
/// composed `CapMap` *is* the structural `SutTransitionTarget` — Stage 1a needs
/// only `SutBlockTreeWrite`, so no editor/focus/quiesce component is wired.
...
pub fn memory_wide_with_editor(backend: MemoryBackend) -> (CapMap, Arc<InMemEditorComponent>) {
    // Share one backend `Arc` between the read/structural cap (`MemoryBackendComponent`)
    // and the editor's commit store, so an editor write is observed by the block-content
    // invariants reading the same store.
