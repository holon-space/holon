...
// central `crate::pbt::sut_loro::LoroSut` (composed::builder) resolves unchanged.
...
/// UUID. Single source of truth lifted to the pbt-core floor (co-location
/// Phase 2) so co-located store arms (`holon-turso-testing`) can filter synthetic
/// ids without reaching into this crate; re-exported here for the many central
/// call sites (`test_environment`, `frontend_slice`, `harness`, `builder`).
