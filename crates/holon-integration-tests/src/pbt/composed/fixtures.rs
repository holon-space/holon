...
//! [`FixtureSqlProjection`], …) hand-craft states the real validating APIs
//! can't produce, to drive invariants to *failure*. They don't touch a real
//! backend — the catch tests run without Turso/Loro/MemoryBackend.
//!
//! The **reference** side is no longer faked here: slices and catch tests seed
//! the real [`ReferenceState`](crate::pbt::reference_state::ReferenceState)
//! oracle via [`seed_ref`](crate::pbt::composed::subsystem_seed::seed_ref) /
...
pub use holon_api::Block;
pub use holon_api::BlockContent;
pub use holon_api::ContentType;
...
pub use holon_pbt_core::composition::run_selected;
...
impl SutLoroLog for FixtureLoroLog {
    ...
    /// `loro_children_match_ref` read only the two methods above); honest
    /// `None`.
...
/// A hand-crafted [`SutErrorLog`] SUT — its `error_count` can be set non-zero
/// to inject what a clean frontend session can't, so the `inv-no-errors` catch
/// test runs without a real `FrontendSession`. Same `SutErrorLog` cap,
...
/// `inv-task-state-storage-coherence` needs. The SQL projection's
/// `all_block_ids` is driven by the SQL `task_state` map (the invariant
/// iterates it), and the per-side maps can be set to agree (positive) or
/// diverge (catch).
...
/// (the production [`ComposedSpanMetrics`] needs
/// `note_transition_start`/`freeze_for_check` driven by the harness, which the
/// teeth don't run). Same `ComposedBudget` cap, identical selection path.
///
