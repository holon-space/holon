...
//! carries `SutBlockInteract` + `SutArrowNavigate`, so the input transitions are
//! cap-admitted on this path. Selection still picks exactly the windowed
...
use holon_pbt_core::composition::{RunReport, run_selected};
...
///   `UserDriver` — `SimUserDriver` under TestPlatform, `GpuiUserDriver` under a
///   real window).
/// - `ref_state`: the **live** per-tick reference oracle (not a fresh/seeded one)
///   so the displayed-text / bounds checks compare against what the run expects —
...
pub async fn run_windowed_composed_check(
    ...
    assert!(
        ...
        "[windowed composed check @ {context}] non-vacuity: inv-frontend-bounds-rendered must \
         run on the composition path (ran: {:?}, deselected: {:?})",
        report.ran_ids(),
