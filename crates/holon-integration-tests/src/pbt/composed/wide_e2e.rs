...
/// Loro-only draw has no such corpus and `inv-ingest-totality` deselects there. Mirrors
...
/// Seed the forward-edge ingest-totality corpus into `state` as NON-seed working blocks
/// under a seed `forward-edge-page` — the reference half that makes `inv-ingest-totality`
/// (and the symmetric `inv-blocks-match-ref/*`) expect `fe-parent`/`fe-blocked`/`fe-target`
...
pub async fn boot_and_seed_wide(
    ...
    // Forward-edge ingest-totality corpus (dogfood 2026-07-10 P0): seed `forward-edge-page.org`
    ...
    // `fe-blocked`/`fe-target` diverges the block-id sets and fires `inv-ingest-totality`.
...
pub fn wide_e2e_ref_for(wiring: &Wiring) -> ReferenceState {
    ...
    // Forward-edge ingest-totality corpus (dogfood 2026-07-10 P0): a Turso-ingest-only
    ...
    // draw (no `ViewModel` projection) never carries it and `inv-ingest-totality`
    // deselects there (no `SutSqlProjection`).
