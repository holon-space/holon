...
///
/// `fe-blocked` carries a forward same-file `:REQUIRES: fe-target` edge — the target is
/// a LATER sibling in the SAME file. Before the fix, `fe-blocked`'s create transaction
/// hit the `block_requires.required_id` FK at COMMIT (the target row not yet inserted),
/// aborted the WHOLE file ingest, and silently dropped every block from `fe-blocked`
/// onward (`fe-blocked` + `fe-target`) while `fe-parent` survived. The fix drops that
/// soft target FK (`crates/holon-turso/sql/schema/block_requires.sql`) so all three land.
///
/// This is the environment half of the `inv-blocks-match-ref/{block_raw,matview}`
/// SQL-projection arms (whose enhanced missing-id direction reports a dropped block as
/// loud INGEST DATA LOSS). It is seeded (by
/// [`boot_and_seed_wide`]) ONLY for a frontend draw whose oracle carries the corpus
/// ([`seed_forward_edge_corpus`]) — the Turso ingest path is where the bug lives; a
/// Loro-only draw has no such corpus and those SQL-projection arms deselect there. Mirrors
/// the standalone `forward_edge_ingest_regression` shape, minus its cross-file dangling
/// `now-query-mcp` target: the INNER-JOIN `block_requirement_edges_matview` drops a
/// dangling target (matview `requires=[]`) while the org drawer keeps it, so a dangling
/// target would make the matview and org projections disagree on the compared `requires`
/// field. A forward SAME-FILE ref alone reproduces the COMMIT-time FK abort with all
/// three projections agreeing on `requires=[fe-target]`.
pub const FORWARD_EDGE_ORG: &str = "#+ID: forward-edge-page\n\
    * fe-parent\n:PROPERTIES:\n:ID: fe-parent\n:END:\n\
    * fe-blocked\n:PROPERTIES:\n:ID: fe-blocked\n:REQUIRES: fe-target\n:END:\n\
    * fe-target\n:PROPERTIES:\n:ID: fe-target\n:END:\n";

/// The three non-page ids the forward-edge corpus MUST ingest (in file order). Every one
/// of these must land in the SUT projection — `fe-blocked` (the forward-`:REQUIRES:`
/// block) and `fe-target` are what the pre-fix FK abort dropped.
...
/// under a seed `forward-edge-page` — the reference half that makes the symmetric
/// `inv-blocks-match-ref/*` arms expect `fe-parent`/`fe-blocked`/`fe-target`
/// in the SUT projection. Called by [`wide_e2e_ref_for`] ONLY for a frontend wiring, so a
/// Loro-only draw's oracle never carries the corpus (and the file is never seeded there —
/// [`boot_and_seed_wide`] keys the org seed on the corpus being present in the ref). The
/// page is a seed page (filtered from the block comparison); its children are non-seed and
/// stay out of the scaffold seed (their ids are in [`boot_and_seed_wide`]'s `tree` set), so
/// a dropped `fe-blocked`/`fe-target` diverges the ref/SUT block-id sets and fires
/// `inv-blocks-match-ref/{block_raw,matview}` as INGEST DATA LOSS.
pub fn seed_forward_edge_corpus(state: &mut ReferenceState) {
    ...
    for (i, name) in FORWARD_EDGE_IDS.into_iter().enumerate() {
        ...
        // `fe-blocked` forward-`:REQUIRES:` its later sibling `fe-target` — the edge that
        // the pre-fix `block_requires.required_id` FK rejected at COMMIT. The org drawer,
        // the `block` matview (inner-join keeps the present target), and the Loro authority
        // all project `requires=[fe-target]`, so the field comparison stays consistent.
        if name == "fe-blocked" {
...
pub async fn boot_and_seed_wide(
    ...
    // Forward-edge ingest corpus (dogfood 2026-07-10 P0): seed `forward-edge-page.org`
    // through the REAL FileSyncController ingest ONLY when this draw's oracle carries the
    // corpus (a frontend draw — `wide_e2e_ref_for` inserts it via `seed_forward_edge_corpus`).
    // Keying the file seed on the oracle keeps every non-corpus frontend boot (the teeth,
    // which build their own oracles) untouched: no corpus in the ref ⇒ no file on disk ⇒
    // no `/org` divergence.
    if ref_state
        .domain
        .block_state
    ...
    let ids = fixed_ids();
    let journals = EntityUri::parse("block:journals").expect("journals id");
    // The non-seed working tree kept OUT of the scaffold seed (so it stays compared):
    // `parent`/`c1`/`c2` plus, for a frontend draw that seeded it, the forward-edge corpus
    // children (`fe-parent`/`fe-blocked`/`fe-target`). Listing the corpus ids
    // unconditionally is a no-op for a Loro-only draw (they are in neither `booted` nor
    // `ref_ids` there); for a frontend draw it keeps them non-seed so a dropped
    // `fe-blocked`/`fe-target` diverges the block-id sets and fires
    // `inv-blocks-match-ref/{block_raw,matview}` as INGEST DATA LOSS.
...
pub fn wide_e2e_ref_for(wiring: &Wiring) -> ReferenceState {
    ...
    // regression, so seed it ONLY for a frontend draw. `boot_and_seed_wide` keys the
    // `forward-edge-page.org` seed on this corpus being present in the oracle, so a Loro-only
    // draw (no `ViewModel` projection) never carries it and the
    // `inv-blocks-match-ref/{block_raw,matview}` arms deselect there (no `SutSqlProjection`).
...
mod tests {
    use super::*;
    use crate::pbt::transitions::{AddPeer, MergeFromPeer, PeerEdit, SyncWithPeer};
