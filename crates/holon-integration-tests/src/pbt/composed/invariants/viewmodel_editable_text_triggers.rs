...
//! (no reference): a SUT-internal contract on the rendered tree. Selected by any
//! slice with a renderer — the frontend slice's real headless `ReactiveEngine`.
//! This is the last `SutRenderer` native consumer; porting it completes
//! `SutRenderer`'s composed coverage.
