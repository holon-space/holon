...
//! rendered tree are a subset of the data rows. `Needs SutRenderer +
//! RefViewSelection + RefLayout`. The ref side is the production
//! `ReferenceState`; selection ANDs the SUT and ref cap sets, so it only fires
//! where a real renderer slice is wired (the frontend slice).
