...
//! transition + frozen oracle); ignores the reference structurally (the budget's
//! `expected_sql` is computed inside the host from the oracle it was handed at
//! `freeze_for_check`). Only a slice that registers a span-metrics provider (the
//! `wide_e2e` slice's `ComposedSpanMetrics` over the production full-headless
//! CapMap) selects it; storage-only / pure slices deselect it (disclosed, not
//! faked). The budget is OFF by default (`HOLON_PERF_BUDGET`), so a clean run is
//! `Ok` / unenforced violations are `Skipped`; the catch test injects an enforced
//! violation through the fixture.

use holon_pbt_core::RunMode;
use holon_pbt_core::composition::{BridgedInvariant, CapId, CapInvariant, Needs};

use crate::pbt::composed::span_metrics::{ComposedBudget, InvComposedBudget};
...
mod tests {
    ...
    /// Positive: a budget surface reporting clean ⇒ selected (a `ComposedBudget` is
    /// wired) and passing.
    #[tokio::test]
