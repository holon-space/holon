...
//!
//! `Strict`. The body internally `Skip`s when the frontend root is still
//! loading (no `frontend_root_vm`), and downgrades its document-gated content
//! checks when the reference has no user documents — so over a minimal oracle
//! it runs its strict geometry checks (expected-size, no-error-widgets, VM
//! y-order / contiguity) over the window's real bounds.
