...
//! whenever an error-log surface is wired (the composed
//! `HeadlessFrontendComponent` hosts it over the production `FrontendSession`'s
//! publish-error tracker). Asserts no app-level error was logged since startup.
//! Storage-only / pure slices don't wire `SutErrorLog`, so it deselects there
//! (disclosed, not faked); its teeth are the E2E frontend slice's real counter
//! and the catch test below.
...
mod tests {
    ...
    /// Catch: an error-log surface with a non-zero counter ⇒ the invariant
    /// fires.
    #[tokio::test]
