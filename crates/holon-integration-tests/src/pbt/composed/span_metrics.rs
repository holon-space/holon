...
//! `Invariant<CapMap, CapMap>` — its ref side is a `CapMap`, which can't hand back
//! a `ReferenceState`. So the composed path can't reuse that body. Instead this
//! module owns a ref-LESS budget cap: the harness hands the host the transition
//! ([`SutMetricsLifecycle::note_transition_start`]) and the post-transition oracle
//! ([`SutMetricsLifecycle::freeze_for_check`]), and the read cap
//! ([`ComposedBudget::budget_report`]) returns the decision computed from that
//! stored state via the SAME [`MetricsSut`] machinery `E2ESut` uses.
//!
...
use crate::pbt::invariants::bodies::sql_budget::{InvSqlBudget, SqlBudgetReport};
...
/// exactly ONE transition. `E2ESut` drives the identical lifecycle natively (its
/// proptest apply hook + `freeze_at_check_start`); this is the composed analog.
///
...
pub trait SutMetricsLifecycle {
    ...
    /// transition apply + settle, not the invariant bodies' own reads) and capture
    /// the post-transition oracle the budget's `expected_sql` inspects.
    fn freeze_for_check(&self, ref_state: &ReferenceState);
}

/// Composed span-metrics provider, hosting the same [`MetricsSut`] `E2ESut` uses.
...
/// Composed twin of [`InvSqlBudget`] (same id), dispatched over a `CapMap`: reads
/// the budget decision from the [`ComposedBudget`] cap and applies the identical
/// enforce/skip/ok decision. The budget is OFF by default (`HOLON_PERF_BUDGET`),
/// so a clean run is `Ok`; unenforced violations are `Skipped`; only enforced
/// violations `Fail`.
