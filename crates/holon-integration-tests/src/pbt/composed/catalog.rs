...
fn central_invariants() -> Vec<Box<dyn CapInvariant>> {
    let mut catalog: Vec<Box<dyn CapInvariant>> = vec![
        ...
        invariants::share_mount_carries_page_identity::wire(),
        invariants::overlay_placement_local::wire(),
        ...
        // No binding delivered rows short of a column its renderer REQUIRES.
        // WARN-level, so `inv-no-observed-errors` above is blind to it.
        #[cfg(feature = "otel-testing")]
