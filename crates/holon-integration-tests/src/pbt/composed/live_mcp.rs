...
//! REAL Holon app reached over its embedded MCP server ([`McpUserDriver`]) — the
//! iOS simulator, a desktop GPUI window, anything serving MCP at
...
//! [`WideE2ELiveMcpMachine`]) and the EXACT same composed invariant catalog as the
//! headless keystone; only the SUT axis changes — from an in-process
//! `compose_sut(full_headless)` `CapMap` to a `CapMap` whose providers speak MCP.
//!
...
use holon_api::{Block, EntityUri, Key, KeyChord, Value};
use holon_frontend::reactive_view_model::ReactiveViewModel;
use holon_frontend::user_driver::UserDriver;
use holon_pbt_core::capabilities::{
    CapRegion, SutBackend, SutBlockTreeWrite, SutEditorMirrorWrite, SutFocusWrite, SutLoroLog,
    SutOrgRead, SutOrgRender, SutQuiesce, SutSqlProjection,
};
...
use crate::pbt::composed::wide_e2e::{WideE2E, WideE2EMachine, page_root, wide_e2e_windowed_ref};
use crate::pbt::op_write_cap::IdResolver;
use crate::pbt::reference_state::ReferenceState;
use crate::pbt::sut_row_parsing::{BLOCK_RAW_SNAPSHOT_SQL, parse_block_rows};
...
/// `assets/default/index.org` the iOS app seeds, pinned to a fixed `#+ID:` so the
/// rebuilt vault's layout doc id is deterministic across resets.
...
/// Working-tree block ids that MUST be present in the rebuilt vault's `block_raw`
/// (the `reset_vault` self-check). Everything else the app boots (layout,
/// journals, `__default__`) is scaffold.
...
impl LiveMcp {
    ...
    /// The block_raw `content` for an already-resolved id (byte→keystroke source).
    ...
    /// Drive a block-reorder op through the app's chord-resolution path, exactly
    /// like `KeystrokeBlockTreeWriter::send_block_chord` — but with the fixed
    /// production binding (Alt+Up / Alt+Down; the live registry is in-process and
    /// unreadable over MCP). `send_key_chord`'s `root`/`tree` args are ignored by
    /// [`McpUserDriver`] (the app bubbles through its own tree), so a default VM
    /// is sound.
        ...
        let rows = self
            .rows(&format!(
                "SELECT id, parent_id, content, content_type, source_language, properties, \
                 tags, requires FROM block WHERE id = {}",
                sql_lit(id)
            ))
            .await;
...
impl LiveMcp {
    ...
    async fn list_org_aliases(&self) -> Vec<(String, String)> {
        ...
        assert!(
            !aliases.is_empty(),
            "no org-file aliases in the live Loro store — org caps would \
             silently compare empty snapshots"
        );
        aliases
    }
...
// they read an in-process `ReactiveEngine` (live editor text, the chord registry)
// which does not exist across a process boundary. The keystroke SEQUENCES are the
// same production ones, dispatched through the same `UserDriver` verbs; the byte→
// keystroke conversion reads live text over SQL and the settle is the server's
// `await_quiescence` (driven by the slice's `settle_after_apply`).
...
/// surface, so the transitions/invariants that need them DESELECT (disclosed via
/// the cap set), rather than being faked.
...
/// The live windowed cap set, captured ONCE (a throwaway MCP connect) before the
/// proptest strategy is built — mirrors `wide_e2e::WINDOWED_CAP_SET`.
...
/// `any_valid_wiring()`. That set auto-narrows `aggregate_transitions` to exactly
/// what the MCP surface can drive and is the same set the per-tick non-vacuity
...
/// The live-MCP slice: the production `E2ETransition` alphabet over a REAL Holon
/// app reached via [`McpUserDriver`], per-case reset via `reset_vault`.
pub struct LiveMcpE2E;

impl ComposedSlice for LiveMcpE2E {
    type Transition = E2ETransition;
    type Machine = WideE2ELiveMcpMachine;
    /// The driver is the whole handle: `settle_after_apply` awaits quiescence and
    /// refreshes the UI snapshot through it.
    ...
    /// Settle = the server's `await_quiescence` (budget exhaustion is an MCP tool
    /// error → fail loud), then refresh the UI snapshot so any gesture-observation
    /// read sees the post-transition frame.
    ...
    /// Same per-draw non-vacuity floor as the headless keystone — derived from the
    /// drawn cap set, so a narrower live cap set auto-drops the ids it has no caps
    /// for. Delegated to `WideE2E` (single source of truth).
    fn required_invariants(ref_state: &ReferenceState) -> Vec<InvariantId> {
        <WideE2E as ComposedSlice>::required_invariants(ref_state)
    }
}

/// Await the app's quiescence over MCP; a budget-exhaustion tool error propagates
/// as a test failure (fail loud, never swallow).
...
mod tests {
    ...
    /// sources of truth, so the live rung seeds the SAME tree the headless keystone
    /// and the iOS app boot.
    #[test]
    fn seed_wide_stays_aligned() {
        ...
        // 2. index.org IS the iOS app's default layout (assets/default/index.org,
        //    the `DEFAULT_INDEX_ORG` const in frontends/gpui/src/mobile.rs) PLUS a
        //    pinned `#+ID:` header so the rebuilt vault's layout doc id is
        //    deterministic across resets. That const is private to the gpui crate
        //    (not importable), so we compare against the SAME on-disk asset it
        //    `include_str!`s — fail-loud note: this is the asset copy, not the const.
        ...
        assert_eq!(
            ...
            "scripts/seed_wide/index.org body drifted from assets/default/index.org \
             (the DEFAULT_INDEX_ORG the iOS app boots)"
        );
    }
}
