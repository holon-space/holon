...
//! `pbt` feature). Lifted out of `subsystem_seed` so the windowed E4 increment
//! can seed a ref tree with the SAME fixed ids the spike uses — the ref↔SUT
//! identity mapping keystone — without un-gating the whole `#[cfg(test)]` spike
//! body.
//!
...
/// **fixed shared ids** and explicit sibling order (`c1` before `c2`) and
/// **no** `block_documents` entry — so they are non-seed and the block
/// invariants compare them against the identically-seeded SUT store.
