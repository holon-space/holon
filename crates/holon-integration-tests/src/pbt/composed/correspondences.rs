...
use holon_pbt_core::capabilities::{
    EntityUri, RefBackend, RefEditorMirror, RefLayout, SutEditorMirrorRead, SutOrgRead, SutRenderer,
};
...
// `holon_turso_testing::correspondences` and are folded into the composed catalog
// via `holon_turso_testing::pbt_contribution()`. What remains central below: the
// editor-mirror families, the `/org` block store, and the root-layout ghost-row
// check (renderer-observed).
