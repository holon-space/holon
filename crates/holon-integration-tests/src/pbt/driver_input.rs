...
//! ([`SutDriver`] focus-read, plus [`SutBlockInteract`] + [`SutArrowNavigate`] input
//! when a driver is installed) by forwarding to it — it NEVER re-implements input
//! (honesty gate). It is **not** GPUI-specific: the windowed builds wrap the live
//! window's `GpuiUserDriver`/`SimUserDriver` + a `GeometryProvider`; the headless
//! composed build (`with_input_headless`) wraps a `ReactiveEngineDriver` with no
//! geometry (the UI-adjacent VM rung, §8.11). The geometry is `Option`, so the type
//! itself carries no window dependency — hence this GPUI-independent home (moved off
...
use holon_pbt_core::capabilities::{CapRegion, EngineFocus, SutBlockInteract, SutDriver};
...
/// frontend [`ReactiveEngine`] focus (the same engine `GpuiFrontendEngineComponent`
/// renders from), compared against the per-frame window focus `SutLayout` reports.
///
...
/// click — the same gestures `E2ESut`'s `SutBlockInteract` impl funnels into its
/// own `Arc<dyn UserDriver>`. Every driving method is a thin forward to
/// `self.driver`; the harness owns all window pumping (the methods are pure
/// `&self` reads/dispatches), so a windowed `CapMap` registering these caps stops
/// the input transitions from cap-gating out (the value-level cap gate in
...
pub struct DriverInputComponent {
    ...
    /// input gesture forwards here; the component never synthesizes input itself.
    ...
    /// `GpuiWindowComponent::wait_for_bounds`): the harness settles the frame to a
    /// fixed point before reads, so a poll loop would only spin.
    geometry: Option<Box<dyn GeometryProvider>>,
    /// True for the `with_input_headless` build (the VM-rung driver in the headless
    /// composed `CapMap`). It gates OFF the `SutDriver` cap in `register`: headless,
    /// `engine_focused_block` reads the frontend `ReactiveEngine`'s global focus,
    /// which is honestly `None` for a non-editor page block (no editor mounts without
    /// a window), so claiming `SutDriver` would select the **windowed** focus
    /// invariants (`inv-focus-matches-ref`, `inv-window-focus-matches-engine-focus`)
    /// over a focus signal they were never meant to read — a faked cap. Headless
    /// focus coherence is already covered by `inv-navigation-focus`/`inv-focus-roots`
    /// (the `SutFocusWrite`/`SutSqlProjection` path). So the headless build provides
    /// only the gesture caps (`SutBlockInteract`/`SutArrowNavigate`).
    headless: bool,
    /// Synthetic-oracle→SUT-real id map (`Some` only for the headless composed build,
    /// which drives an id-minting backend: a split mints a fresh uuid that the harness
    /// reconciles onto the oracle's `block::split-N`). Gesture methods resolve every
    /// incoming id through it before driving — without this a gesture targeting a
    /// minted block drives a ghost. `None` for the windowed builds, whose fixed shared
...
impl DriverInputComponent {
    ...
    /// The **headless** input provider: wraps a headless production `UserDriver`
    /// (`ReactiveEngineDriver`) with **no geometry**. This is the UI-adjacent
    /// logic layer for the headless composed `CapMap` — input gestures drive the
    /// same production logic the real UI runs (`click_entity` resolves the bound
    /// click intent via `find_click_intent_in_region`, `send_raw_keystroke`
    /// edits through the `MutableText`/`InputState` mirror), just without a
    /// platform window. Without geometry there is no bounds frame to gate on, so
    /// the single-shot bounds precheck is skipped (the driver resolves targets
    /// itself — intent lookup / shadow-tree walk); see [`Self::require_bounds`].
    ...
    /// The installed production driver. Panics (fail-loud) if an input gesture is
    /// invoked on a focus-only build — the build mistake, not silent no-op.
    fn driver(&self) -> &Arc<dyn UserDriver> {
        self.driver.as_ref().expect(
            "DriverInputComponent: windowed input gesture invoked without an installed \
             UserDriver — build with `window_input_wide` (with_input), not `window_focus_wide`",
        )
    }

    /// Resolve an oracle-space id to its SUT-space id (identity when no resolver is
    /// installed — the windowed fixed-id builds). The id-minting headless composed
    /// build shares the runner's `IdResolver` so a gesture targeting a minted block
    /// (e.g. `block::split-N`) drives the real block, not a ghost.
    ...
    /// Single-shot bounds gate (mirror of `GpuiWindowComponent::wait_for_bounds`):
    /// the harness pumps to a fixed point before driving, so this never spins.
    ///
    ...
    /// shadow-tree walk), so the precheck is skipped. Windowed: assert the id is
    /// present in the settled frame before driving real input at its coords.
    ...
    /// Poll the window engine's focus until it lands on `id` or `timeout` elapses —
    /// the post-click barrier `SutDriver::wait_for_engine_focus` documents (GPUI's
    /// click dispatch is fire-and-forget). Runtime-agnostic awaits: the windowed
    /// per-tick path drives these via `futures::executor::block_on` against the
    /// harness's ambient runtime (same pattern as `resolve_watch`).
    ...
    /// Headless editor-readiness gate: poll until the block's live `MutableText`
    /// content cell resolves, so a following single-char keystroke (e.g. the slash
    /// menu's `/`) has somewhere to land. The windowed path gates this via
    /// `SutLayout::wait_for_window_focused_editor` (real geometry frame); headless has
    /// no such frame, so a gesture targeting a freshly-created block (e.g. a just-split
    /// block whose Loro `content_raw` is still landing) would otherwise race the
    /// `HeadlessEditorMirror`. No-op windowed (geometry present → its own gate).
...
/// [`UserDriver`] (`self.driver`), mirroring `E2ESut`'s `SutBlockInteract` bodies
/// (`sut_handle.rs`) minus the `RefCell` (this component owns the `Arc`) and the
/// `ref_state`-dependent post-actions (the harness reconcile/settle seam owns
/// those, per the `ref_state`-off-the-cap principle). The bounds precheck reads
/// `self.geometry` single-shot; root resolution uses the layout root the engine
/// renders from.
#[async_trait::async_trait(?Send)]
