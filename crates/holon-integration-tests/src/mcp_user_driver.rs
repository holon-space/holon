...
//!   `describe_ui` renders one block tree with no region/panel geometry, so
//!   region membership is not derivable. These panic with instructions.
//! - `click_intent_of` — the serialized `ViewModel` does not carry the resolved
//!   click intent (that lives in the reactive tree in-process). Panics.
...
//!    `reset_vault` op on the server, or `simctl terminate/launch` around each
//!    case (slow; batch via `--test-threads=1` + case budget).
//! 2. **Wiring actor**: add an `McpUserDriver` variant to the composed SUT's
//!    `any_valid_wiring()` axis so the keystone PBT can draw it, gated on a
//!    live-server env var (absent ⇒ the wiring is not drawn, not silently
//!    skipped).
//! 3. **Cap-narrowing**: invariants that read in-process handles (Loro docs,
//!    Turso conns) can't hold them across a process boundary. Follow the E3
//!    pattern: narrow the wiring's cap set to what the MCP surface honestly
//!    provides (`execute_query` / `execute_raw_sql` / `inspect_loro_blocks` /
//!    `diff_loro_sql`) and let the runner skip — disclosed, per cap —
//!    invariants whose caps the wiring lacks.
//! 4. **Region-aware observation**: extend `describe_ui` (or add a sibling
//!    tool) to report panel/region membership so the three region verbs above
//!    stop panicking.
...
impl McpUserDriver {
    ...
    fn with_snapshot<T>(&self, verb: &str, f: impl FnOnce(&ViewModel) -> T) -> T {
        ...
        let tree = guard.as_ref().unwrap_or_else(|| {
            panic!(
                "McpUserDriver::{verb} read before any UI snapshot was taken — call \
                 refresh_ui(root_block_id).await first (observation over MCP is an explicit async \
                 fetch, see module docs)"
            )
        });
        f(tree)
    }
...
impl UserDriver for McpUserDriver {
    ...
    async fn send_key_chord(
        ...
        if !extra_params.is_empty() {
            bail!(
                "McpUserDriver::send_key_chord got extra_params {:?}, but the MCP send_key_chord \
                 tool has no extra-params channel — refusing to drop them silently. Extend \
                 frontends/mcp SendKeyChordParams if a transition needs this.",
    ...
    async fn scroll_to_entity(&self, entity_id: &EntityUri) -> Result<()> {
        bail!(
            "McpUserDriver cannot scroll_to_entity({entity_id}): scrolling toward an off-viewport \
             target needs geometry the MCP surface doesn't expose. Use scroll_entity/scroll_at \
             with explicit deltas, or extend the MCP scroll tool with a scroll-into-view mode."
        )
    }
    ...
    async fn drop_entity(
        ...
        bail!(
            "McpUserDriver cannot drag {source_id} onto {target_id}: the MCP surface has no \
             drag/drop tool. Add one (routing through GpuiUserDriver::drop_entity) before \
             generating drag transitions for this rung."
        )
    }
    ...
    fn is_in_region(&self, entity_id: &EntityUri, region: holon_api::Region) -> bool {
        panic!(
            "McpUserDriver::is_in_region({entity_id}, {region:?}) is not honestly answerable: \
             describe_ui renders one block tree with no region/panel attribution. Extend \
             describe_ui with region info before using region-aware observation on this rung \
             (module docs, remaining work #4)."
        )
    }

    fn entities_in_region(&self, region: holon_api::Region) -> Vec<EntityUri> {
        panic!(
            "McpUserDriver::entities_in_region({region:?}) is not honestly answerable over the \
             current MCP surface (module docs, remaining work #4)."
        )
    }

    fn reachable_entities_in_region(&self, region: holon_api::Region) -> Vec<EntityUri> {
        panic!(
            "McpUserDriver::reachable_entities_in_region({region:?}) is not honestly answerable \
    ...
    fn click_intent_of(&self, entity_id: &EntityUri) -> Option<OperationIntent> {
        panic!(
            "McpUserDriver::click_intent_of({entity_id}) is not honestly answerable: the \
             serialized describe_ui ViewModel does not carry resolved click intents (they live in \
             the app's reactive tree). Generators on this rung must not condition on click \
             intents until describe_ui exposes them."
        )
    }
}
