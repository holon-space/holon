...
/// `cap_transition!` now lives in `holon-pbt-core` (shared with the per-subsystem
/// companion PBT crates). Re-exported at this crate root so the many
/// `crate::cap_transition!` call sites in `pbt::transitions::*` keep resolving.
