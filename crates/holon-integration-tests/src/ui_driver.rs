...
use crate::screenshot_overlay::Overlay;
use crate::screenshot_overlay::Phase;
...
/// - `XcapBackend`: window-level capture via xcap (default, works for any
///   frontend)
...
/// xcap-based window capture — finds the app window by title substring and
/// captures it.
///
...
pub trait UiDriver: Send + Sync {
    ...
    /// Capture a screenshot of the current state, optionally highlighting an
    /// element. Default implementation does nothing.
    ...
    /// Click on an element to focus it. Returns true if element was found and
    /// clicked.
...
/// `GeometryDriver::capture_screenshot`, read by inv-frontend-engine in the PBT
/// checker.
...
impl GeometryDriver {
    ...
    /// Share a `VisualState` so inv-frontend-engine can read the last
    /// screenshot analysis.
