...
//!   * installs a global panic hook that sleeps `n` seconds before any panic
...
//! - `PBT_PAUSE_AFTER_STEP=N` — sleep after the transition's invariants are
//!   checked.
//!
...
fn sleep_with_banner(header: &str, body: &str) {
    ...
    eprintln!(
        "\n═══════════════════════════════════════════════════════════════════\n[{header}] \
         {body}\nPID: {pid}    Sleeping: {secs}s    SIGINT aborts.\nConnect via the holon MCP \
         server (port {MCP_PAUSE_PORT}), attach a debugger, or open the test sqlite DB to inspect \
         live state.\n═══════════════════════════════════════════════════════════════════\n"
    );
