...
pub fn block_belongs_to_document(block: &Block, all_blocks: &[Block], doc_uri: &EntityUri) -> bool {
    if block.parent_id == *doc_uri {
        return true;
    }
    if let Some(parent) = all_blocks.iter().find(|b| block.parent_id == b.id) {
...
pub fn find_document_for_block(
    block_id: &EntityUri,
