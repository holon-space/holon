    ...
    fn smoke() {
        ...
        paint_overlay(
            ...
            &Overlay::fail(
                ...
                "inv-value-fn-provider-identity: block:7a44bf6f task_state expected `DONE` after \
                 toggle, but reference model has `TODO`. Reference state computed by \
                 ReferenceState::apply, observed state read from sut.snapshot().",
            ),
        );
