...
use crate::{
    apply_cdc_event_to_vec, assign_reference_sequences, block_belongs_to_document,
...
impl TestEnvironment {
    ...
    pub async fn query(
        ...
        let spec = self
            ...
            .await?;
        Ok(spec.data.into_iter().map(|r| r.data).collect())
    }

    /// Resolve a file-based document URI (e.g. "doc:doc_0.org") to the real
    /// UUID-based URI used by the system.
    pub async fn resolve_doc_uri(&self, file_uri: &str) -> Result<String> {
        let parsed = EntityUri::parse(file_uri)
            .map_err(|_| anyhow::anyhow!("Not a document URI: {}", file_uri))?;
        let path_part = parsed.id();
    ...
    pub async fn watch_ui_first_structure(
        ...
        let mut watch =
            holon::api::watch_ui(Arc::clone(engine), block_id.to_string(), None, false).await?;
    ...
    pub async fn initial_widget_with_stream(
        ...
        self.engine()
            .blocks()
            .render_block(holon_api::ROOT_LAYOUT_BLOCK_ID, None, true)
            .await
    }
    ...
    pub async fn initial_widget_fully_rendered(&self) -> Result<WidgetSpec> {
        ...
        for row in &root_spec.data {
            ...
            let source_language = row.data.get("source_language").and_then(|v| v.as_string());
                ...
                {
                    {
                        ...
                        let block_id = row.data.get("id").and_then(|v| v.as_string());
                        let parent_id = row.data.get("parent_id").and_then(|v| v.as_string());
                        let query_content = row.data.get("content").and_then(|v| v.as_string());
                            ...
                            (block_id, parent_id, query_content)
                        {
                            ...
                            match self.query_with_context(source, query_lang, parent_id).await {
                                Ok(nested_spec) => {
    ...
    pub async fn query_with_context(
        ...
        let context = QueryContext::for_block_with_path(
            holon_api::EntityUri::from_raw(context_block_id),
            None,
            block_path,
        );
    ...
    pub async fn navigate_focus(&mut self, region: Region, block_id: &str) -> Result<()> {
        let mut params = HashMap::new();
    ...
    pub async fn setup_watch(
        ...
        self.ui_model.insert(
            query_id.to_string(),
            widget_spec.data.into_iter().map(|r| r.data).collect(),
        );
