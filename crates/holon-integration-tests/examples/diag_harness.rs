...
//! vault exercises the per-file cadence. `TestEnvironmentBuilder` builds a fresh
//! empty Turso per run (no persisted `file.content_hash` rows), so every file is
//! ingested — the run is **cold by construction** (the warm-boot hash fast-path
//! cannot engage). The harness prints boot-to-last-block wall time; run under
//! `RUST_LOG=holon_latency=debug` and pipe to `scripts/measure_latency.py` for the
//! per-phase (`boot_parse`/`boot_write`/`boot_feed_wait`/`boot_feed_converge`) split.
//!
//! ```text
...
/// One file's org content with a stable `#+ID:` and stable per-block `:ID:`s, so
/// re-render produces identical bytes (no `#+ID:` writeback churn to skew timing).
...
fn main() -> anyhow::Result<()> {
    ...
    #[cfg(feature = "tokio-console")]
    {
        ...
        eprintln!(
            "[diag] tokio-console subscriber spawned (bind TOKIO_CONSOLE_BIND, default 127.0.0.1:6669)"
        );
    }
