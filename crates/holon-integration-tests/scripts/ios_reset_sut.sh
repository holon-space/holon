...
# 4. relaunch with MCP pinned; HOLON_MCP_ALLOW_RESET passes through so the
# live-PBT runner can enable in-process reset_vault on the relaunched app
SIMCTL_CHILD_MCP_SERVER_PORT="$PORT" \
  SIMCTL_CHILD_HOLON_MCP_ALLOW_RESET="${HOLON_MCP_ALLOW_RESET:-}" \
