...
impl EntityConfig {
    ...
    /// Convert to an `EntitySchema` for GQL graph registration.
    ///
    /// The `graph_label` is derived by PascalCasing the entity name
    /// (e.g., "todoist_task" → "TodoistTask").
    pub fn to_entity_schema(&self, entity_name: &str) -> holon_api::EntitySchema {
        use holon_api::FieldType;
        use holon_api::entity::{EntityFieldSchema, EntitySchema};

        EntitySchema {
            name: entity_name.to_string(),
            primary_key: self.id_column_or_default(),
            graph_label: Some(pascal_case(entity_name)),
            fields: self
                .schema
                .iter()
                .map(|f| EntityFieldSchema {
                    name: f.name.clone(),
                    field_type: FieldType::from_sql_type(&f.sql_type),
                    required: !f.nullable,
                    indexed: f.indexed,
                    edge_name: None,
                })
...
"#;
        let sidecar: McpSidecar = serde_yaml::from_str(yaml).unwrap();
        let schema = sidecar.entities["email"].to_entity_schema("email");
        assert_eq!(schema.name, "email");
        assert_eq!(schema.primary_key, "msg_id");
        assert_eq!(schema.graph_label.as_deref(), Some("Email"));
        assert_eq!(schema.fields.len(), 3);
        assert_eq!(schema.fields[0].name, "msg_id");
        assert!(schema.fields[0].required);
        assert!(!schema.fields[2].required); // nullable
    }
}
