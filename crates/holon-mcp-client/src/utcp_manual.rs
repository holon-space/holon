...
//! lives beside it under `holon:`
//! ([`crate::integration_config::HolonSection`]).
//!
...
/// one contributes no keys, so a manual that carried none exports
/// byte-identically.
...
/// How a tool is reached. The spec allows several `call_template_type`s
/// (`http`, `cli`, `mcp`, …); this build serves `http` and SKIPS a tool
/// declaring any other, disclosing the skip by name
/// ([`Self::unsupported_reason`]). One tool Holon cannot drive therefore costs
/// that tool, not the manual: a peer publishing an `mcp` tool beside its `http`
/// ones is still a peer we can talk to. Only a MALFORMED manual is refused
/// whole.
#[derive(Debug, Clone, PartialEq, Deserialize, Serialize)]
