...
//! added twice.
//!
//! This prints what [`ConnectionRoster::scan`] — the loader's own answer —
//! says, so there is one implementation and the script reads it.
...
name no connection (a symlink, a duplicate name, an unusable name) are reported
on stderr with the reason, and are NOT listed.
...
fn list(dir: &std::path::Path) {
    let roster = match ConnectionRoster::scan(dir) {
        Ok(roster) => roster,
        Err(e) => {
