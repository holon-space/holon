...
use holon_mcp_client::mcp_vtable::{McpForeignDataWrapper, VtableConfig};
use rmcp::model::{
    CallToolRequestParam, CallToolResult, Content, ReadResourceRequestParam, ReadResourceResult,
    ResourceContents,
};
use rmcp::service::ServiceError;
use turso_core::foreign::{ForeignDataWrapper, PushedConstraint};
use turso_core::{
    Connection as CoreConnection, Database, DatabaseOpts, MemoryIO, OpenFlags, StepResult, Value,
};
...
/// When the caller supplies the enumeration's owning param via WHERE, no fan-out
/// occurs — exactly one tool call with the user-provided value.
#[tokio::test(flavor = "multi_thread")]
