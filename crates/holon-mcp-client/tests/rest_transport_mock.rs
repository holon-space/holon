...
//! server (no network). Proves that a `utcp:` + `holon:` sidecar drives the
//! SAME connector read path (`SyncStrategy::fetch_records` over
//! `McpCallSurface`) as the MCP transports, producing Layer-1 replica records
//! from a plain JSON API.
//!
