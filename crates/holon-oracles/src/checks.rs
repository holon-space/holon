        ...
        while let Some(parent) = parents.get(current) {
            ...
            if !seen.insert(current) {
                failures.push(format!(
                    "[inv-no-parent-cycles] parent cycle detected walking up \
                     from {start}: revisited {current} (chain re-enters a node \
...
pub fn find_source_language_violations(rows: &[SourceLanguageRow]) -> Vec<String> {
    rows.iter()
        ...
        .map(|r| {
            format!(
                "[inv-source-language-iff-source] block {} violates the domain \
                 rule: is_source={} but source_language={:?} (source_language \
                 must be Some iff content_type == Source)",
