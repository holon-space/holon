...
//! - [`checks`] — pure check functions, the single shared implementation.
//!   The keystone PBT bodies delegate to these; the live runner feeds them
//!   from SQL snapshots. One implementation, no drift.
//! - [`status`] — process-global violation ledger + change notification.
//!   Global because the tracing subscriber (the latency oracle's source) is
