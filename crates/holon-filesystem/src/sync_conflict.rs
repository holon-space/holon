//! Foreign-file-syncer tripwire — fail-loud enforcement of Model.md invariant
//! 11 ("one consolidator per file replica").
//!
...
pub fn conflict_artifacts_error(root: &Path, offenders: &[PathBuf]) -> anyhow::Error {
    ...
    anyhow::anyhow!(
        "Model.md invariant 11 (one consolidator per file replica) violated: the vault at {root} \
         contains byte-level file-syncer conflict artifacts, which means it is under a byte \
         syncer (Syncthing / iCloud / Dropbox). That is out of contract — cross-device \
         convergence must travel through Loro/P2P, never a byte syncer, or foreign file writes \
         are re-ingested as duplicate-ID documents with wrong attribution and oscillating order. \
         Move the vault off the synced directory (or exclude it from the syncer) and remove these \
         artifacts:\n{list}",
        root = root.display(),
    )
}
...
mod tests {
    ...
    async fn startup_scan_errors_on_conflict_clean_vault_passes() {
        ...
        use crate::fs_port::RealFileSystem;
