...
impl Directory {
    pub fn new(id: String, name: String, parent_id: String, depth: i64) -> Self {
        Self {
            // ALLOW(entity_uri_from_raw): construction boundary — `Directory::new` takes raw
            // `String` ids from the org sync provider; parse once here.
...
impl holon_core::traits::OperationRegistry for Directory {
    fn all_operations() -> Vec<OperationDescriptor> {
        ...
        {
            use holon_core::traits::__operations_block_operations;
            use holon_core::traits::__operations_crud_operations;
            use holon_core::traits::__operations_move_operations;
            use holon_core::traits::__operations_rename_operations;
