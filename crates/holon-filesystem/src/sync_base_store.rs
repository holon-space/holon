...
impl BaseKey {
    /// The single global base used while there is one authority doc for the
    /// whole tree (the block→SQL projection's base). Coexists with
    /// per-`(peer, file)` keys used by domain-scoped consumers (the org
    /// reconciler).
    pub fn global() -> Self {
        Self {
...
///
/// `sidecar_path` is `Some` for persisted bases (the block→SQL projection, so
/// the base survives restart) and `None` for in-memory bases (the org
/// reconciler, which re-seeds per file from the consolidated store each
/// session).
...
impl SyncBaseStore {
    ...
    /// Build an in-memory (non-persisted) base store. Used by the org
    /// reconciler, which re-seeds each `(peer, file)` base from the
    /// consolidated store on first touch per session.
...
mod tests {
    ...
    /// (`.loro/holon_tree.sync_base.json`) written by pre-M3 code stored a
    /// block's junction-derived `tags`/`requires` as **siblings** of
    /// `block` (which itself omitted them). This is the exact shape every
    /// existing vault has on disk. It must still deserialize into a
    /// `SnapshotBlock` whose `Block` carries the edge fields — a format
    /// break would silently drop them on the next boot (BUG H1). The sample
    /// is checked in at `tests/fixtures/sync_base_legacy_sample.json`
