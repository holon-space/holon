...
"#,
    ...
    let m = marking(vec![
        ...
        ("d2", "document", vec![("who", Value::String("alice".into()))]),
    ]);
    let enabled = engine.enabled(&net, &m).unwrap();
...
"#,
    ...
    let m = marking(vec![
        ("t_done", "task", vec![("state", Value::String("done".into()))]),
        ("t_open", "task", vec![("state", Value::String("open".into()))]),
    ]);
    let engine = Engine::new();
    let enabled = engine.enabled(&net, &m).unwrap();
    assert_eq!(enabled.len(), 1);
...
"#,
    ...
    let mut m = marking(vec![
        ("doc1", "document", vec![("state", Value::String("open".into()))]),
        ("tmp1", "scratch", vec![("payload", Value::String("done".into()))]),
    ]);
    let engine = Engine::new();
    let enabled = engine.enabled(&net, &m).unwrap();
    assert_eq!(enabled.len(), 1);
    ...
    assert_eq!(event.created[0].attrs["origin"], Value::String("open".into()));
...
"#,
    ...
    assert!(event.changes.is_empty(), "no-op postcond must not record a change");
}
...
"#,
    ...
    assert!((ranked[0].delta_obj - 30.0).abs() < 1e-9, "slow delta_obj = {}", ranked[0].delta_obj);
    assert!((ranked[0].delta_per_minute - 3.0).abs() < 1e-9);
    assert!((ranked[1].delta_obj - 4.0).abs() < 1e-9, "quick delta_obj = {}", ranked[1].delta_obj);
...
"#,
    ...
    assert_eq!(result.constraint_violations, vec!["q.v > 100.0".to_string()]);
}
...
fn history_save_load_replay_roundtrip() {
    ...
    let history = History { events: vec![event] };
...
fn marking_save_load_roundtrip() {
    ...
    let m = marking(vec![
        ("q", "a", vec![("v", Value::Float(1.5)), ("s", Value::String("x".into()))]),
