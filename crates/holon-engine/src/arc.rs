...
pub enum PrecondSpec {
    /// Full spec including the `$` prefix — used verbatim as the placeholder map key.
    Placeholder(String),
    Comparison {
        op: CmpOp,
        rhs: String,
...
impl FromStr for PrecondSpec {
    ...
    fn from_str(spec: &str) -> Result<Self, String> {
        if let Some(name) = spec.strip_prefix('$') {
            if name.is_empty() {
                return Err(format!(
                    "invalid precondition spec '{spec}': '$' must be followed by a placeholder name"
                ));
            }
        ...
        for op in CmpOp::ALL {
            if let Some(rhs) = spec.strip_prefix(op.as_str()) {
                ...
                let compiled = CompiledExpr::compile(&engine, format!("x {} {}", op.as_str(), rhs))
                    .map_err(|e| {
                        format!(
                            "invalid precondition spec '{spec}': right-hand side does not compile: {e}"
                        )
                    })?;
        ...
        if spec.starts_with(['=', '<', '>', '!']) {
            let bad_op: String = spec
                ...
                .collect();
            return Err(format!(
                "invalid precondition spec '{spec}': '{bad_op}' is not a valid operator; \
                 valid operators are ==, !=, >=, <=, >, < (e.g. '== done'), \
                 '$name' for a placeholder, or a bare literal for exact match"
            ));
        }
...
impl FromStr for PostcondExpr {
    ...
    fn from_str(spec: &str) -> Result<Self, String> {
        if let Some(name) = spec.strip_prefix('$') {
            if name.is_empty() {
                return Err(format!(
                    "invalid postcondition expr '{spec}': '$' must be followed by a placeholder name"
                ));
            }
