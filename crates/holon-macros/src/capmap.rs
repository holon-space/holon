...
//! through the shared `Arc<dyn Cap>`; they get a fail-loud `unimplemented!` so a
//! composed slice that wrongly routes a drain through the map panics clearly
//! rather than silently returning empty.
//!
//! The `#[async_trait(?Send)]` wrapper is emitted **only when the trait has an
//! async method**. A fully-sync cap (e.g. the reference-side `RefBackend`, whose
//! methods return owned values and are already object-safe) gets a plain trait +
//! plain `impl … for CapMap`, so existing sync `impl Trait for ConcreteRef`s
//! don't need the attribute retro-fitted.
...
pub fn capmap_adapter_impl(trait_def: ItemTrait) -> TokenStream {
    ...
    for item in &trait_def.items {
        ...
        let body = if receiver_is_mut {
            let msg = format!(
                "`{trait_name}::{name}` takes &mut self — a drain/apply-phase op, \
                 not available on the shared CapMap (use the concrete SUT in apply)"
            );
