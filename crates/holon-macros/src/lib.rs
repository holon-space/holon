...
/// - One function `fn OPERATION_NAME_OP() -> OperationDescriptor` per async
///   method
/// - One function `fn TRAIT_NAME_operations() -> Vec<OperationDescriptor>`
...
fn extract_param_name(pat: &Pat) -> String {
    match pat {
        ...
        _ => {
            // Fallback: try to stringify the pattern // ALLOW(fallback): describes
...
/// Pass-through attribute for #[affects(...)] - allows Rust to accept the
/// attribute The actual parsing is done by extract_affected_fields() in the
...
/// Pass-through attribute for #[triggered_by(...)] - allows Rust to accept the
/// attribute The actual parsing is done by extract_param_mappings() in the
/// operations_trait macro.
///
/// This attribute declares that an operation is triggered by the availability
/// of a contextual parameter. When that parameter is present (e.g., from a
/// gesture like drag-drop), only operations that declare they're triggered by
...
/// Pass-through attribute for #[enum_from(...)] - allows Rust to accept the
/// attribute The actual parsing is done by the operations_trait macro.
///
...
/// single function. Useful for operations that aren't part of a trait.
///
/// Usage:
/// ```rust
/// #[operation]
