...
struct RenderSignature {
    ...
    /// True if first param is `node` — pass the full ViewModel instead of
    /// destructuring.
    pass_node: bool,
}

/// Read a builder source file and extract the `pub fn render(...)` parameter
/// names.
#[allow(dead_code)]
...
enum RegistrationMode {
    /// `register: Type` — generate `register_all(interp: &mut
    /// RenderInterpreter<Type, Ext>)`
    Register(syn::Type),
    /// `dispatch: Type` — generate `dispatch_build(name, args, ctx) ->
...
struct BuilderRegistryInput {
    ...
    /// If true, generate `all_widget_metas()` collecting WIDGET_META from each
    /// builder
    widget_metas: bool,
    ...
    ///   - `__name`  — substituted with a `&'static str` literal of the
    ///     variant's snake_case name
    ///
    /// Example: `transform: crate::tag(__name, __inner).into_any_element()`
    /// expands per-arm to `crate::tag("column", column::render(gap, children,
    /// ctx)).into_any_element()`.
    ///
    /// When omitted, the raw render result is used directly (identity
    /// transform).
...
/// idents with the per-arm substitutions. Recurses into `Group`s so
/// placeholders inside nested parens/brackets/braces are spliced too.
            ...
            match ident.to_string().as_str() {
                ...
                other => {
                    return Err(syn::Error::new(
                        ident.span(),
                        format!(
                            "expected `skip`, `register`, `dispatch`, `node_dispatch`, `ext`, \
                             `context`, `node_type`, `kind_type`, `transform`, `empty`, or \
                             `widget_metas`, found `{other}`"
                        ),
                    ));
                }
            }
