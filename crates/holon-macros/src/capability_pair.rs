...
//! ```
//!
//! Trait names follow the convention `Sut{Stem}` / `Ref{Stem}` — so a reader can
//! find a cap's counterpart by name. Only when a side genuinely needs a divergent
//! name does the pair opt out, via the trait-level attribute:
//!
...
//!   transformed body's compare step (`&Vec<T>` deref-coerces to `&[T]`). This is
//!   the common case: these are differential invariants over *different storage
//!   representations*, so agreement is "equal modulo known-benign differences".
...
pub fn capability_pair_impl(mut decl: ItemTrait) -> TokenStream {
    ...
    //    `Sut{Stem}`/`Ref{Stem}` name on either side (the divergent-name exception).
