...
//! side), hosts each on `CapMap` (by reusing
//! [`crate::capmap::capmap_adapter_impl`]), and — for methods marked
//! `#[compare]` — auto-derives a `BridgedInvariant` constructor that asserts
//! the two sides are equal.
//!
//! ```ignore
//! capability_pair! {
//!     /// docs shared by both generated traits
//!     pub trait ViewSelection {            // → SutViewSelection + RefViewSelection
...
//! ```
//!
//! Trait names follow the convention `Sut{Stem}` / `Ref{Stem}` — so a reader
//! can find a cap's counterpart by name. Only when a side genuinely needs a
//! divergent name does the pair opt out, via the trait-level attribute:
//!
//! - `#[pair(sut_name = .., ref_name = ..)]` — override either generated trait
//!   name. Reserve it for a real local-clarity exception; its presence should
//!   flag "these two are named differently *on purpose*", not be routine.
//!
//! `#[compare]` accepts three optional keys:
//!
...
//!   transformed body's compare step (`&Vec<T>` deref-coerces to `&[T]`). This
//!   is the common case: these are differential invariants over *different
//!   storage representations*, so agreement is "equal modulo known-benign
//!   differences".
...
//!   externally-referenced id instead of the derived
//!   `inv-pair-<stem>-<method>`.
//!
...
//! the Correspondence registry
//! (`holon-integration-tests/src/pbt/correspondence.rs`), not this macro.
...
///
/// - `with`: an optional custom comparator path (see
///   [`build_compare_invariant`] for its contract). Absent → plain `==`
///   equality.
...
pub fn capability_pair_impl(mut decl: ItemTrait) -> TokenStream {
    ...
    // 1. Trait-level `#[pair(sut_name = .., ref_name = ..)]`: override the default
    //    `Sut{Stem}`/`Ref{Stem}` name on either side (the divergent-name
    //    exception).
            ...
            let r = attr.parse_nested_meta(|meta| {
                if meta.path.is_ident("sut_name") {
                    ...
                    return Err(meta.error(
                        "capability_pair: unknown `#[pair(..)]` key (expected `sut_name` / \
                         `ref_name`)",
                    ));
                }
                Ok(())
...
fn extract_role(f: &TraitItemFn) -> syn::Result<(Role, Vec<Attribute>)> {
    ...
    match role {
        ...
        None => Err(syn::Error::new_spanned(
            &f.sig,
            "capability_pair: each method needs exactly one of `#[compare]` / `#[ref_only]` / \
             `#[sut_only]`",
        )),
    }
}
...
fn build_compare_invariant(
    ...
    let ctor_doc = format!(
        "Auto-derived `#[compare]` invariant `{id_str}`: asserts `{sut_name}::{sut_method}` (SUT) \
         is {how} `{ref_name}::{ref_method}` (reference). Register with one line in \
