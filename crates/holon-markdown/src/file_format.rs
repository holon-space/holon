...
impl FileFormatAdapter for MarkdownFormatAdapter {
    ...
    fn doc_id_from_content(&self, _: &str) -> Option<String> {
        unimplemented!(
            "MarkdownFormatAdapter::doc_id_from_content — markdown vault write-sync is not wired; \
             define the frontmatter id convention before enabling it"
        )
    }

    fn build_block_params(&self, _: &Block, _: &EntityUri, _: &EntityUri) -> StorageEntity {
        unimplemented!(
            "MarkdownFormatAdapter::build_block_params — markdown vault write-sync is not wired; \
             define the op-param mapping before enabling it"
        )
    }

    fn content_differs(&self, _: &Block, _: &Block) -> bool {
        unimplemented!(
            "MarkdownFormatAdapter::content_differs — markdown vault write-sync is not wired; \
             define content-equivalence before enabling it"
        )
    }

    fn sync_document_metadata(&self, _: &Block, _: &mut Block) -> bool {
        unimplemented!(
            "MarkdownFormatAdapter::sync_document_metadata — markdown vault write-sync is not \
             wired; markdown has no #+TODO:-style header metadata to reconcile"
        )
    }
}
