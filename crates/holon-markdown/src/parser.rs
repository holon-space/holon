...
//! ## Block model
//!
//! - **Document block**: file-as-entity. `content` is empty; metadata (title,
//!   tags) lives in properties. Frontmatter `extra` keys go through verbatim
//!   under a `frontmatter` JSON property so unknown keys round-trip.
//! - **Heading blocks**: one per ATX heading. `level` matches heading depth.
//!   Parent is the nearest enclosing heading of strictly lower depth, or the
//!   document itself for top-level headings. `content` is the heading text on
//!   the first line followed by the verbatim body markdown that appears between
//!   this heading and the next (excluding code fences, which are split out into
//!   source children).
//! - **Source children**: each fenced code block becomes a
//!   `ContentType::Source` child of the surrounding heading (same as org's
//!   `#+BEGIN_SRC`).
//! - **Preamble block**: content above the first heading is attached to the
//!   document block's `content` field — there is no separate "preamble" child
//!   block. This matches org's behavior (top-level paragraphs without a
//!   headline live on the document).
//!
//! ## Block IDs
//!
...
use crate::dialect::DailyNoteConfig;
use crate::dialect::MarkdownDialect;
use crate::dialect::TaskKeywords;
use crate::frontmatter::Frontmatter;
use crate::frontmatter::{self};
use crate::inline::extract_comments;
use crate::inline::extract_highlights;
use crate::inline::extract_inline_tags;
...
pub fn parse_markdown_file(
    ...
    // Use the file stem (no extension) as the page title default so the //
    // ALLOW(fallback): default-value comment markdown and org adapters agree on
    // the name shape.
    let file_name = path
        .file_stem()
        .and_then(|n| n.to_str())
        ...
        for (src_index, src) in source_blocks.into_iter().enumerate() {
            ...
            let mut src_block = Block {
                ...
                source_language: src
                    .language
                    .as_deref()
                    .and_then(|l| l.parse::<SourceLanguage>().ok()), /* ALLOW(ok): unknown
                                                                      * language → None is
                                                                      * intentional */
                created_at: now,
                updated_at: now,
