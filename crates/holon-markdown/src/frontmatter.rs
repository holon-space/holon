...
///
/// We do this manually rather than relying on `markdown::ParseOptions {
/// frontmatter: true }` because we need the raw YAML text to round-trip
/// unrecognized keys without normalization, and we need to control the body
/// offset so `Position` info from the AST aligns with our source text.
