...
use holon_markdown::DailyNoteConfig;
use holon_markdown::MarkdownDialect;
use holon_markdown::MarkdownFormatAdapter;
use holon_markdown::ParseResult;
use holon_markdown::dialect::TaskKeywords;
use holon_markdown::dialect::TaskMarker;
use holon_markdown::parse_markdown_file;
...
fn frontmatter_aliases_toggle_moves_storage_but_both_round_trip() {
    ...
    assert!(
        prop(&off.document, "frontmatter_extra")
            .unwrap()
            .contains("aliases")
    );
}
