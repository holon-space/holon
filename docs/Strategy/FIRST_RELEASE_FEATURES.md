...
Implementation: `holon-obsidian` crate following the `FileSyncController` pattern. Parse Markdown + YAML frontmatter → Block entities. Watch folder. Bidirectional: write-back Markdown when blocks are mutated in Holon.
