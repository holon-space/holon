...
| `render_block` | `block_domain.rs` | Block rendering (query compile + watch + profile) |
...
2. Spans nest: `watch_ui` → `do_render` → `render_block` → `query_and_watch` → `ensure_view` + `query`
