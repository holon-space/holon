...
#### B3. `turso_event_bus.rs` — REMOVED

The EventBus was decommissioned: `turso_event_bus.rs`, the `events` table, and
`sql/events/*.sql` are deleted. The inline SQL that was here (event subscription
matview, view-exists check, mark-processed `UPDATE events`) no longer exists.
