...
|------|---------------------|-----------|
| EventBus integration | (resolved) EventBus removed | QueryableCache uses direct broadcast channels; blocks use LiveData<Block> |
...
- [x] **P3** ~~Resolve EventBus vs broadcast channel architecture drift~~ DONE: the EventBus was fully removed (`TursoEventBus`, `events` table, block event plumbing all deleted). ARCHITECTURE.md now documents the real wiring in the "Sync Wiring (no EventBus)" section: direct broadcast→cache feeds for external sources, and `LiveData<Block>` for reactive block consumers.
