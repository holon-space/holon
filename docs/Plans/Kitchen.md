...
* **Parses the GARMIN endpoint and is NOT WIRED to a live poll** — the same staging Inc A landed with. The phone API (P36) is the production target; that read swap, its rotating-token auth, the live wiring and the `/commit` write leg are **the next lane**, pending a token-refresh handshake capture (R10). Everything below the response parsing — the `(name, cat)` key, the snapshot type, the reconciler and its intents — is shared by both shapes, so the swap is confined to `CompleteSnapshot::from_response` and the vocabulary source. Noted in `crates/holon-kitchen/src/shopping.rs` and `assets/integrations/shopping.yaml`.
...
* **Red-first, four reds.** `lane-logs/red-capability-token-leak.*.log` — a whole-URL secret printed verbatim in a 500 (P26 layer 1). `lane-logs/red3-body-echo-inversion.*.log` — the rotating token surviving in the echoed BODY with the structural layer removed, restored byte-for-byte (sha `b1f15bee…`) (P26 layer 2). `lane-logs/red4-rotating-teeth.*.log` — the ROTATING, never-registered `!<token>` leaking verbatim from both the error and the echoed body (the shape the production peer actually uses; restored sha `f16f4dd7…`). `lane-logs/red1-shopping-feature-absent.*.log` — every rung as `unresolved import holon_kitchen::shopping`. Green at the frozen rev: **533/533**; `just keystone-smoke` 4/4.
...
#### C2 — write leg + bidirectional reconcile (**`/commit` contract CAPTURED; blocked on the token handshake, R10**)
...
* **R10 (NEW, OPEN — gates the NEXT lane, not C1). How is each `!<token>` obtained?** The capture recorded requests, not the handshake, so nothing can mint one and the phone read-swap + auth + live wiring + C2 all wait on it. Martin needs another live-app session. Redaction does NOT wait on it: the structural `!`-segment scrub protects a token it was never told about, and is landed and red-proven now.
...
| P26 layer 2 (rotating token) | `grep -n "redact_marked_segments(&self.redact_registered" crates/holon-mcp-client/src/redaction.rs` — if `redact` no longer calls it, a rotating token leaks through an echoed BODY |
