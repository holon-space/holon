...
**Holon:** Org journal files synced via FileSyncController. No automatic daily page creation or journal-first navigation.
...
**Holon:** Org-mode files as source of truth, synced bidirectionally to Turso (SQLite) cache via FileSyncController. Loro CRDT documents for collaboration. Holon treats files as an external system, not as the storage format.
