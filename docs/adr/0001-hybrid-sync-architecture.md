...
Partially superseded. The two-model split (owned Loro data vs. cached third-party data) survives, but most Decision-section mechanics were never built or were replaced: `DataSource<T>` shipped as a read-only trait (no CRUD / `ReconciliationEngine` / per-provider `SyncStrategy` of the shape described here), and third-party sync was rebuilt as generic MCP + YAML sidecars (`holon-mcp-client`, `assets/integrations/*.yaml`). Turso's role widened from third-party shadow cache to a projection of owned data (matviews + CDC + consolidator). See ADR 0004 / 0006 and the MCP sidecar integrations.
