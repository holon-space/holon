...
Creation goes through the OperationProvider, not direct SQL. The write lands in `block_raw`; Turso CDC then updates the block matview, and the `LiveData<Block>` feed drives the `block_link` indexer and other reactive consumers.
