...
- **`crates/holon-api/src/entity_profile.rs`** (data model + resolution; parsing and `ProfileResolver` stay in `crates/holon/src/entity_profile.rs`): Per-entity, per-row render + operation resolution
  - `EntityProfile` struct: `entity_name`, `variants: Vec<StoredVariant>` (includes the conditionless default), `computed_fields: Vec<CompiledComputedField>`, `virtual_child`
  - `RenderProfile` (resolved output, `render_types.rs`): `name`, `render: RenderExpr`, `operations: Vec<OperationDescriptor>`, `variants`
  - `StoredVariant`: `name`, `priority`, `condition_source` (Rhai), `data_condition`/`ui_condition` split, `profile`
  - `CompiledComputedField`: `(name, CompiledExpr)` — pre-compiled Rhai expression
  - `parse_entity_profile()` (holon): parses YAML, validates Rhai compilation, topo-sorts computed fields
  - `resolve()`: pushes all row columns into Rhai scope, evaluates computed fields, matches variants by condition
...
| **No change needed** | FileSyncController, LoroBackend, QueryableCache (direct broadcast ingest), TodoistSyncProvider, CDC, Flutter render interpreter, all frontends | — | — |
