...
> For the philosophical foundation and product vision, see [Vision/LongTerm.md](Vision/LongTerm.md).
...
#### 1.0 Unified Item Types and Entity Identity
...
**Entity Identity (the Zeroth Principle)**: Beyond unifying item _types_ across systems, Holon must unify item _identities_. The same person appears as a Todoist assignee, a JIRA reporter, a calendar attendee, and an email sender — these are not four data points but four references to one entity. Establishing canonical identity is the precondition for all cross-system intelligence: without it, the system cannot form edges between entities, and without edges, the knowledge graph cannot compound.

Entity resolution works in layers:

- **Convention-based**: Identically-named items across systems are automatically linked (same as Project mapping below)
- **Attribute-based**: Matching email addresses, usernames, or other identifiers across systems
- **AI-proposed, human-confirmed**: The Integrator proposes entity matches; the user confirms or rejects. This confirmation-driven approach preserves human judgment while scaling beyond what manual linking can achieve. See [Vision/AI.md](Vision/AI.md) §The Integrator.

The Petri Net model (see [Vision/PetriNet.md](Vision/PetriNet.md)) already defines canonical token types — Person, Organization, Document, Monetary, Knowledge, Resource — each with a single place. Entity resolution is what populates these places from cross-system data.
...
P.A.R.A. serves as a **navigation and filing metaphor** — it determines where things appear in the outliner. The primary organizational principle is the **entity graph**: typed entities (Person, Organization, Project, Document) with confirmed edges between them. P.A.R.A. folders are projections over this graph, not the graph itself. A project page aggregates all entities linked to that project, regardless of which P.A.R.A. category they'd individually fall into.
...
**HTML Embedding**: Full HTML embed support is a first-class capability thanks to the Dioxus web frontend. The browser provides native `<iframe>` sandboxing, `postMessage` communication, and CSP enforcement — all required for [MCP Apps](https://github.com/modelcontextprotocol/ext-apps) interactive UI hosting (see [Architecture/Integrations.md](Architecture/Integrations.md)).

- Sandboxed iframes for third-party MCP App views (kanban, charts, dashboards)
- `web-sys` bindings for postMessage transport and iframe lifecycle
- Browser-native CSP enforcement per `UIResourceMeta.csp` declarations
- Structured data rendering as fallback when no MCP App UI is available
...
See [Vision/AI.md](Vision/AI.md) for detailed AI architecture.
...
**Target platforms**: Web (all browsers), Windows, macOS, Linux, iOS, Android.

**Current frontends**:

- **Dioxus (Web)**: Primary frontend — Rust compiled to WASM, runs in any modern browser. Enables MCP Apps hosting with native iframe/postMessage/CSP support. Progressive Web App (PWA) for mobile.
- **TUI**: Terminal interface for keyboard-driven workflows

**Future**: Native desktop/mobile wrappers via Tauri (WebView) or Capacitor for OS-level features (keychain, notifications, file access). The web-first approach ensures all features work across platforms from day one.
...
**Goal**: Prove hybrid architecture and entity resolution
...
- Entity resolution: Todoist assignees/collaborators → canonical Person tokens
- Bulk import path: initial sync imports all Todoist projects/tasks/labels at once, creating the entity graph in minutes rather than through gradual manual entry
...
- Entity resolution produces correct cross-system identity with minimal manual confirmation

### Phase 3: Multiple Integrations

**Goal**: Validate type unification and cross-system entity identity scale
...
- Cross-system entity resolution: same person across Todoist, JIRA, Linear, Calendar resolved to one canonical Person token
- Confirmation stream in Orient mode: AI-proposed entity matches and relationship edges for rapid human review
...
- Entity graph compounds: each integration makes the graph denser, not just wider
...
- [Vision/LongTerm.md](Vision/LongTerm.md) - Philosophical foundation and product vision
- [Vision/AI.md](Vision/AI.md) - AI feature specifications and development path
- [Vision/PetriNet.md](Vision/PetriNet.md) - Petri-Net foundations, Digital Twins, WSJF sorting
- [Architecture/Principles.md](Architecture/Principles.md) - Foundational architectural decisions
