...
in the vault rather than per lost registration. The disclosure has since moved to the write-back path, so it fires on a real write attempt rather than on feed traffic.
