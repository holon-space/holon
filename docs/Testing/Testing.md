...
A single Rust-native PBT test validates that all frontends (Flutter, GPUI, Blinc, Dioxus) correctly render and interact with the shared backend.
...
The `RenderInterpreter` has an optional **annotator** (`set_annotator()`) that tags widgets with entity IDs from `ctx.row().get("id")`. Dispatch-mode frontends (Blinc) annotate in their `build()` wrapper.
