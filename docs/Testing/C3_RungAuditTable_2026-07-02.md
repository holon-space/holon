...
- The deferred base registers ALL the NON-gesture caps via
  `HeadlessFrontendComponent::register_non_gesture` (`frontend_slice/components.rs`),
  but registers NONE of the gesture-write family. The whole gesture rung —
  `SutDriver`/`SutBlockInteract`/`SutArrowNavigate` (driver) PLUS
  `SutBlockTreeWrite`/`SutFocusWrite`/`SutEditorMirrorWrite`/`SutMutate` (writes) —
  is gated by `DriverPlacement` and left ABSENT in the base. The pre-window state is
  honestly capless: the generated alphabet auto-narrows the gesture transitions out
  until a window exists. No `caps.replace` overlay, no register-then-override.

## The table

**Insert-only realization (2026-07-02, updated):** the gesture-write family is enumerated
in ONE place — `HeadlessFrontendComponent::register_gesture_writes(caps, driver)` (an inherent routine on
`HeadlessFrontendComponent`, `frontend_slice/components.rs`). It is `insert`ed exactly once
by whoever owns the driver rung: the composed builder's `DriverPlacement::HeadlessReactive`
arm passes the component's own `ReactiveEngineDriver`; the windowed `overlay_windowed_caps`
(`window_slice/builders.rs`) passes the live window's `GpuiUserDriver`/`SimUserDriver`. Both
go through the SAME bodies — `SutBlockTreeWrite` via `keystroke_writer_with(driver)`;
`SutFocusWrite`/`SutEditorMirrorWrite`/`SutMutate` via `DriverBoundFrontendWrite`
...
|---|---|---|---|---|---|---|---|
| 1 | SplitBlock | `SutBlockTreeWrite` (`transitions/split_block.rs:269-273`, apply `:296-298`) | `KeystrokeBlockTreeWriter::apply_split_block` `op_write_cap.rs:268-307` (focus + home + N×right + enter over `comp.driver()` = headless `ReactiveEngineDriver`; installed by `register_gesture_writes` (`composed/builder.rs`, `HeadlessReactive` arm)) | ReactiveEngine | Gpui | **OK** (2026-07-02) | Windowed sibling = same keystrokes through the window driver: `register_gesture_writes(driver)` INSERTS it in `overlay_windowed_caps` (`window_slice/builders.rs`). |
...
| 16 | NavigateFocus (sidebar click) | `SutFocusWrite` (`navigate_focus.rs:41-44`, apply `:238-247`) | `HeadlessFrontendComponent::apply_navigate_focus` `frontend_slice/components.rs:1118-1158` — clicks the LeftSidebar entry via the component's INTERNAL `ReactiveEngineDriver` (`:1122`, `:1143`) | ReactiveEngine — base; **Gpui after overlay** | Gpui (window sidebar click) | **OK** (2026-07-02) | Windowed sibling landed: `DriverBoundFrontendWrite::apply_navigate_focus` → `apply_navigate_focus_via(window_driver)` (`frontend_slice/components.rs`; body `frontend_slice/components.rs:1150`) clicks the sidebar entry through the window driver; overlay INSERT via `register_gesture_writes` at `window_slice/builders.rs`. |
| 17 | FocusEditableText | `SutFocusWrite` (`focus_editable_text.rs:52-55`, apply `:205-206`) | `HeadlessFrontendComponent::apply_focus_editable_text` `frontend_slice/components.rs:1160-1165` (`click_entity(main, id)` via internal headless driver) | ReactiveEngine | Gpui (window click on the editable) | **OK** (2026-07-02) | `DriverBoundFrontendWrite::apply_focus_editable_text` → `apply_focus_editable_text_via(window_driver)` (`frontend_slice/components.rs:1185`); overlay INSERT via `register_gesture_writes` at `window_slice/builders.rs`. |
| 18 | ToggleState / cycle_task_state | `SutMutate` (local cap, `toggle_state.rs:149-153`, apply `:310-313`) | `HeadlessFrontendComponent::toggle_state` `frontend_slice/components.rs:1545-1571` — loops `engine.execute_operation("cycle_task_state")` click_count times | ReactiveEngine (widget-click via shim `toggle_state_via`) | Gpui (click the `state_toggle` widget N times) | **OK** (2026-07-02) | `DriverBoundFrontendWrite::toggle_state` → `toggle_state_via(window_driver)` (`frontend_slice/components.rs:1637`) clicks the `state_toggle` widget `click_count` times through the window driver (click_count math shared with the headless direct-dispatch path via `toggle_click_count`); overlay INSERT via `register_gesture_writes` at `window_slice/builders.rs`. ⚠ Rebound + cap-set-present; not yet DRIVEN end-to-end by an existing windowed test (4b). |
