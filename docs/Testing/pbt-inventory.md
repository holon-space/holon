...
| inv-viewmodel-tree-virtual-slots | internal-consistency — R:RefBlockTree only LOCATES the Main focus roots; nothing from ref is compared as an expected value, the assertions are structural (slot order, no editable/rendered text in the title subtree) | RefBlockTree | SutRenderer | — | virtual-slot-misorder + page-title-wrong-variant — creation slot not last child; focused page title on the default (rendered_text) or editing (editable_text) variant instead of the page_title h1 variant |
...
| trigger_slash_command | input-pipeline `SutBlockInteract::trigger_slash_command`: click + home + type '/' + command chars + Enter, every step a real UserDriver gesture. | RefBlockTree RefFocusRoots RefLayout RefLayoutInteract RefLayoutMutate RefLifecycle | SutBlockInteract SutDriver SutLayout | yes |
