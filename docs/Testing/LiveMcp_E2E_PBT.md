...
- **Per-case reset**: `reset_vault` with the `include_str!`-embedded
  `scripts/seed_wide/*.org` (`index.org` is generated from `index.org.header` +
  `assets/default/index.org` by `scripts/seed_wide/gen-index.sh`).
  ~0.85 s/reset, retire cap 20/launch, RSS grows
...
```
# app on a booted sim, MCP on 8521, reset enabled (the repo-default seed dir is
# generated by scripts/seed_wide/gen-index.sh):
