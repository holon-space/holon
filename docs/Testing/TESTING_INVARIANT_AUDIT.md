...
| Invariant | Min-SUT |
|---|---|
| `inv-displayed-text/widget` | `{editor-state, viewmodel, frontend-bounds}` |
| `inv-displayed-text/viewmodel` | `{editor-state, viewmodel}` (Warn, rollout) |
...
4. **`inv-displayed-text/widget` and `inv-org-render-fixed-point` straddle three subsystems each** — these will stay in the wide PBTs and possibly the T1 PBTs whose SUT happens to span the set. Acceptable: they're invariants whose *value proposition* is precisely the cross-subsystem check. The `inv-displayed-text/viewmodel` arm splits the render-axis text check to the ViewModel layer so a failure localises (VM-correct-but-paint-stale vs bad VM projection).
