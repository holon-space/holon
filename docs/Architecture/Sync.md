...
When Loro is disabled (SqlOnly mode), `LwwTextCellBacking` substitutes the Loro-backed text cell with last-write-wins semantics for direct cell writes. There is no P2P sync in this mode; conflicts can only arise from OrgMode file changes racing with UI operations. For that specific race, the org reconciler no longer resolves by whole-value LWW: when a file edit and a UI edit concurrently change the **same block's text content** (both diverge from the diff base held in the `BaseStore`), the file-sync controller performs a base-3-way TEXT merge via a **transient** `LoroText` — `(base, disk, current-store)` → merged text — and ingests the merged result (spec 0008 §3.1, Model.md merge-fidelity ladder). The transient doc is the merge *function* only: nothing is stored or cached, and Full (Loro-store) mode is untouched because the live CRDT already merges there. Every such merge is disclosed with a `tracing::info!` naming the block. Non-conflict cases (only disk changed, or both sides converged to the same text) are unchanged — no merge, disk content ingested as before. Structural conflicts (parent/order) are out of scope: content only.
