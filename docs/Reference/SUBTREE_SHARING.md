...
**A structural edit merged against a concurrent one converges** — decision D70,
fixed upstream by loro `ddc47ecc` and taken through the fork rebase and the pin
bump. Pinned by
`structure_merged_against_a_concurrent_op_converges_on_a_shallow_share`, and the
share PBT drives concurrent structural authors on both peers again.
