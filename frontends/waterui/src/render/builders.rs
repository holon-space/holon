...
fn build_selectable(args: &ResolvedArgs, ctx: &RenderContext) -> AnyView {
    // Render the child template; selectable behavior not yet wired
    if let Some(tmpl) = args
        .get_template("item_template")
        .or(args.get_template("item"))
    {
    ...
    match (content_type.as_str(), is_query_lang) {
        ("source", true) => {
            let block_id = match ctx.row().get("id").and_then(|v| v.as_string()) {
                ...
                None => {
                    return AnyView::new(
                        text("[render_entity: no id]")
                            .size(12.0)
                            .foreground(Color::srgb_hex("#FF0000")),
                    )
                }
            };
            render_entity_by_id(&block_id, ctx)
        }
...
fn render_entity_by_id(block_id: &str, ctx: &RenderContext) -> AnyView {
    ...
    match result {
        ...
        Err(e) => {
            tracing::warn!("render_entity({bid}) failed: {e}");
            AnyView::new(
                text(format!("render_entity error: {e}"))
                    .size(12.0)
                    .foreground(Color::srgb_hex("#FF0000")),
            )
        }
...
fn sorted_rows(
    rows: &[holon_api::widget_spec::DataRow],
    sort_key: Option<&str>,
