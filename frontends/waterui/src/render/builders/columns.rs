use holon_api::render_eval::{
    self as render_eval, has_drawer_rows, sort_key_column, sorted_rows, ScreenLayoutPartition,
};
...
fn build_screen_layout(ba: &BA, tmpl: &holon_api::render_types::RenderExpr, rows: &[std::sync::Arc<holon_api::widget_spec::DataRow>]) -> AnyView {
    if rows.is_empty() {
        return (ba.interpret)(tmpl, &ba.ctx.with_row(Default::default()));
    }
    ...
    let main_content = if partition.main.len() == 1 {
        ...
        let views: Vec<AnyView> = partition
            .main
            .into_iter()
            .map(|r| AnyView::new(r.widget.max_width(f32::MAX)))
            .collect();
