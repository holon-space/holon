...
//!   * `FrontendSession::watch_ui(id, None, true)`            → signature is now
//!     `watch_ui(&EntityUri) -> WatchHandle`.
//!   * `holon_frontend::frontend_module::FrontendInjectorExt` / `add_frontend`
//!                                                            → the module was
//!     deleted; frontend DI wiring moved to the `holon-app` crate
//!     (`holon_app::FrontendInjectorExt::add_frontend`).
//!   * `RenderInterpreter::interpret(expr, ctx)`              → now takes a third
//!     `services: &dyn BuilderServices` argument.
//!
