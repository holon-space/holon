...
//! render walk and stores it in `state.last_registry:
//! Arc<Mutex<RenderRegistry>>` (see `app_main`). [`TuiGeometry`] wraps that
//! same `Arc` so PBT consumers (drivers, inv-frontend-engine, the readiness
//! gate) get the latest paint via the standard
