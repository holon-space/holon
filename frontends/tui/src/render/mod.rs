...
fn render_collection_vertical(
    ...
    //
    // The registration uses `widget_type = "render_entity"` to match GPUI's
    // `frontends/gpui/src/render/builders/render_entity.rs` semantics: this
    // rect IS the rendering of a particular block entity, not a wrapper
    // around one. (GPUI's `live_block` builder deliberately does NOT track,
    // so `widget_type == "live_block"` never appears in GPUI's
    // BoundsRegistry — the PBT invariants in
    ...
    // conflation that surfaces as a vm-data-tracked-as-content false positive
    // whenever the VM emits only such rows.
        ...
        let block_focused = if register_as_blocks {
            ...
            if let Some(row_id) = row_id {
                ...
                ctx.registry.selectables.push(SelectableRegion {
                    ...
                    rows: 0,
                    cols: inner_width,
                    widget_type: "render_entity".into(),
                    displayed_text,
                });
