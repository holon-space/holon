...
//!    `tokio::sync::mpsc::Sender<InputEvent>` plumbed into the renderer task.
//!    Every PBT verb translates into a sequence of `InputEvent`s pushed through
//!    that channel; the renderer's third `tokio::select!` arm calls the real
//!    `AppMain::app_handle_input_event` for each event. **This bypasses the
//!    pump entirely** — the pump's `dispatch_interaction` is never called for
//!    PBT-emitted events.
//!
//! 2. **MCP path (engine fast-path)** — external MCP clients still send
//!    `InteractionCommand`s carrying a oneshot response. The pump task drains
//!    them and dispatches via `dispatch_interaction`. Today this only handles
//!    `MouseClick` (resolved to a click via `navigation::editor_focus` engine
//!    dispatch); other event kinds return `handled: false` with a `detail`
//!    explaining the v1 limit. MCP clients aren't on the input pipeline because
//!    (a) the message shape carries a oneshot the renderer arm has no place to
//!    wire up, and (b) MCP semantics are coarser-grained than per-keystroke
//!    events. Folding both paths into the renderer would force the MCP pump to
//!    learn the `InputEvent` shape and lose the response_tx; keep them
//!    separate.
//!
...
/// 2. Set [`DebugServices::interaction_tx`] (so MCP tools can send events) and
///    [`DebugServices::user_driver`] (so the PBT harness can fetch a
///    [`crate::user_driver::TuiUserDriver`] via the same shared
///    `Arc<DebugServices>`).
/// 3. Spawn a tokio task on `runtime` that drains the MCP channel and
///    dispatches each event via [`dispatch_interaction`] (engine fast-path).
///
...
pub async fn dispatch_interaction(
    ...
    match event {
        InteractionEvent::MouseClick { position, .. } => {
            ...
            // ALLOW(entity_uri_from_raw): boundary — entity_id from TUI geometry hit-test
            // (a String)
        ...
        _ => InteractionResponse {
            handled: false,
            detail: Some(
                "TUI MCP-path interaction pump only handles MouseClick — see crate::input_pump \
