...
/// Blinc-specific render context wrapping the shared RenderContext with Blinc
/// extensions.
#[derive(Clone)]
...
impl RenderContext {
    ...
    pub fn with_operations(&self, operations: Vec<OperationWiring>) -> Self {
        Self {
            ctx: self.ctx.with_operations(operations),
            ext: self.ext.clone(),
        }
    }
    ...
    pub fn indented(&self) -> Self {
        Self {
            ctx: self.ctx.indented(),
            ext: self.ext.clone(),
        }
    }
}

pub fn new_render_context(services: Arc<dyn BuilderServices>) -> RenderContext {
    RenderContext {
        ctx: holon_frontend::RenderContext::new(services),
