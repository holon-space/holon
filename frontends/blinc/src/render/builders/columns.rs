...
use holon_api::render_eval::has_drawer_rows;
use holon_api::render_eval::partition_screen_columns;
use holon_api::render_eval::sort_key_column;
use holon_api::render_eval::sorted_rows;
...
fn build_screen_layout(tmpl: &RenderExpr, rows: &[Arc<DataRow>], ctx: &RenderContext) -> Div {
    if rows.is_empty() {
        let child_ctx = ctx.with_row(Arc::new(Default::default()));
        return interpret(tmpl, &child_ctx);
    }

    let partition = partition_screen_columns(rows, |resolved_row| {
        let row_ctx = ctx.with_row(Arc::new(resolved_row.clone()));
    ...
    if let Some(region) = partition.left_sidebar {
        if let (Some(ref state), Some(ref bid)) = (&ctx.ext.left_sidebar_block_id, &region.block_id)
        {
