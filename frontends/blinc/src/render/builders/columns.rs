...
/// Screen layout: partitions columns into left sidebar, right sidebar, and main content.
/// First drawer → left, last drawer → right, everything else → main.
fn build_screen_layout(args: &ResolvedArgs, ctx: &RenderContext) -> Div {
    ...
    // Left sidebar — check widget_states map, then fall back to ext.sidebar_open
...
/// Resolve open state: widget_states map first, then ext State<bool> fallback, then default true.
...
/// Resolve sidebar width from widget_states, falling back to SIDEBAR_WIDTH.
