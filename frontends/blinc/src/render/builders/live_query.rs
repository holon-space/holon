...
///
/// Supports `prql:`, `gql:`, and `sql:` DSL arg keys (these are render DSL
/// syntax, not source_language values).
pub fn build(args: &ResolvedArgs, ctx: &RenderContext) -> Div {
    // Resolve the query to SQL regardless of input language.
    // PRQL is special-cased: passed raw to build_prql which handles compilation +
    // context injection.
...
pub fn build_prql(prql: String, context_id: Option<String>, ctx: &RenderContext) -> Div {
    ...
    match result {
        Ok(_stream) => {
            // Initial data arrives via the stream's first batch.
            // For synchronous rendering, start with empty data; CDC will fill it.
