//! Blinc GeometryProvider — wraps `query(id).bounds()` from blinc_layout
//! selector API.
//!
//! Uses the global `query()` function which accesses `BlincContextState`
//! singleton. This means `element_info()` only works when called from the Blinc
//! event loop thread.

use std::sync::Arc;

use blinc_layout::prelude::ElementHandle;
use blinc_layout::prelude::ElementRegistry;
