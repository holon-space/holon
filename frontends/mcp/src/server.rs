...
pub struct DebugServices {
    ...
    /// Frontend-supplied builder that boots a fresh, seeded SUT WITHOUT starting
    /// a second MCP server (the existing one is reused via the swappable
    /// [`LiveMcpBackend`] cell). Lives in the gpui crate; installed here so the
    /// `reset_vault` tool stays decoupled from gpui. Runs on the same tokio
    /// runtime the MCP server runs on.
    pub reset_builder: std::sync::OnceLock<ResetBuilderFn>,
    /// Live, swappable convergence/mirror handles the debug PBT tools
    /// (`await_quiescence`, `debug_pbt_snapshot`) read. Populated at boot by the
    /// frontend and SWAPPED in place by `reset_vault` — so a per-case rebind
    /// points these at the fresh session's Loro sync controller / org idle
    /// signal / CDC-driven `BlockQuerySource` instead of the retired one. A plain
    /// boot-time `OnceLock` (like [`Self::loro_doc_store`]) would go stale after
    /// a reset and silently answer against the retired engine; this cell fails
    /// that failure mode by being swappable alongside [`LiveMcpBackend`].
    pub live_debug: LiveDebugHandles,
}

/// Live, swappable [`DebugHandlesCell`] shared across every session's server (a
/// cloned `Arc`), so a `reset_vault` swap is visible everywhere. `std::sync::RwLock`
/// (not tokio's) so the debug tools can read it without an `.await`.
pub type LiveDebugHandles = Arc<std::sync::RwLock<DebugHandlesCell>>;

/// The swappable convergence/mirror handles behind [`DebugServices::live_debug`].
/// Each is `None` when the running config doesn't wire that substrate (Loro off,
/// no org file-sync). The debug tools distinguish "not wired" from "should be
/// wired but unreachable" and fail loud on the latter rather than skipping.
...
impl HolonMcpServer {
    ...
    /// The shared service layer over the live engine. Cheap: `HolonService` is a
    /// thin `Arc<BackendEngine>` wrapper, rebuilt per call from the live cell.
