...
//!
//! This module sets up OpenTelemetry tracing and logging, similar to the main
//! application. It supports both OTLP (OpenTelemetry Protocol) and stdout
//! exporters.
...
///
/// This function only sets up the OpenTelemetry providers. The layers should be
/// added to the tracing subscriber in the calling code.
pub fn init_opentelemetry() -> Result<()> {
    ...
    if exporter_type.contains("otlp") {
        ...
        tracing::debug!("Initializing OpenTelemetry OTLP exporters:");
        tracing::debug!("  Traces endpoint: {}", traces_endpoint);
        tracing::debug!("  Logs endpoint: {}", logs_endpoint);
        ...
        // Note: Can't use tracing::info! here because subscriber isn't initialized yet
        tracing::debug!("Initializing OpenTelemetry stdout exporters");
...
///
/// This function creates the tracing layers that bridge tracing to
/// OpenTelemetry. It should be called after `init_opentelemetry()`.
///
