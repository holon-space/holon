...
impl HolonMcpServer {
                    ...
                    return Err(rmcp::ErrorData::invalid_params(
                        format!(
                            "Key chord matched operation '{}.{}', but it requires UI-context \
                             ...
                             the real input pipeline instead: use `type_text` / `send_raw_keystroke` \
                             to send the keystroke, which lets the focused editor supply {:?}.",
    ...
    #[tool(
        description = "Insert text through the soft-keyboard `insertText:` path instead of hardware \
                       KeyDown events. Unlike `type_text` (which sends per-character KeyDown \
                       keystrokes through the GPUI keymap), this commits the whole string straight \
                       into the focused editor's input handler — the same path iOS UIKit uses for \
                       the on-screen keyboard. A soft Return is sent as \"\\n\" and becomes an \
                       `enter` action (split_block), not a literal newline. Use this to exercise \
                       soft-keyboard-only behavior that a KeyDown-driven test cannot reach."
    )]
