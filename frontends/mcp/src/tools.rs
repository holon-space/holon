    ...
    match context_id {
        Some(id) => {
            ...
            Some(QueryContext::for_block_with_path(
                &uri,
                context_parent_id.map(|s| {
                    EntityUri::parse(s).expect("context_parent_id is not a valid EntityUri")
                }),
                path,
            ))
        }
...
impl HolonMcpServer {
    ...
    async fn render_org_from_blocks(
        ...
        let doc_uri =
            EntityUri::parse(&params.doc_id).unwrap_or_else(|_| EntityUri::doc(&params.doc_id));
    ...
    async fn describe_ui(
        ...
        let (widget_spec, _stream) = self
            .engine
            .blocks()
            .render_block(&block_uri, &None, false)
            .await
            .map_err(|e| {
                rmcp::ErrorData::internal_error(
        ...
        let data_rows = widget_spec.data.clone();
...
fn capture_window_as_png(window_title: Option<&str>) -> Result<Vec<u8>, String> {
    ...
    let window = if let Some(title) = window_title {
        ...
        windows.iter().find(|w| w.pid().ok() == Some(our_pid))
    };
    ...
    let window = window.ok_or_else(|| {
        let available: Vec<String> = windows
            ...
            .filter_map(|w| {
                ...
                let app = w.app_name().ok().unwrap_or_default();
                let pid = w.pid().ok().unwrap_or(0);
                Some(format!("{title:?} (app={app:?}, pid={pid})"))
            })
            .collect();
        format!(
            "No window found (our pid={our_pid}, searched for {:?}). Available: {available:?}",
            window_title.unwrap_or("(own process)")
        )
    })?;
