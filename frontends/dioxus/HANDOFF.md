...
- Operations module with `dispatch_operation`, clickable action parsing
...
| `render_block` | Full — query language dispatch + source display |
| `block_ref` | Full — recursive block rendering |
| `clickable` | Full — action dispatch on click |
...
| `src/render/context.rs` | `RenderContext` with data rows, operations, session, depth tracking, `is_screen_layout` flag |
...
`build_block_ref_by_id` and `build_prql_query` currently call `render_block()` / `query_and_watch()` synchronously (via `std::thread::scope`) and discard the returned CDC stream. This means nested queries show initial data but don't update live.
