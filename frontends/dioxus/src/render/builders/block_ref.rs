use super::prelude::*;
use holon_frontend::render_interpreter::{shared_block_ref_build, BuilderArgs};

pub fn build(ba: BA<'_>) -> Element {
    let block_id = match ba.ctx.row().get("id").and_then(|v| v.as_string()) {
        Some(id) => id.to_string(),
        None => {
            return rsx! { span { font_size: "12px", color: "var(--error)", "[block_ref: no id in row]" } }
        }
    };
    let ref_args =
    ...
    match shared_block_ref_build(&ref_ba) {
        Ok(widget) => widget,
