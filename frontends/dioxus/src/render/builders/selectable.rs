...
use super::dispatch::{click_modifiers, dispatch_intent};
use super::prelude::*;
...
/// Click-to-operation wrapper. Mirrors `gpui/src/render/builders/selectable.rs`:
/// pre-resolves every modifier-bound intent on the node into a
/// `HashMap<ClickModifiers, OperationIntent>` and dispatches the match on
/// mouse-down. A modifier click `stop_propagation`s so an outer row click
/// doesn't also fire (LogSeq-style "pin to sidebar without focusing").
#[component]
