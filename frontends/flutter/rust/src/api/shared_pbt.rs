...
//!
//! Two modes:
//! 1. **Monolithic** (`run_shared_pbt`): FFI-only, Dart callback executes
//!    operations directly.
//! 2. **Phased** (`pbt_setup`/`pbt_step`/`pbt_teardown`): re-exported from
//!    shared crate, but Flutter wraps `pbt_setup` and `pbt_teardown` to
//!    install/clear the PBT engine as GLOBAL_SESSION.

use std::sync::Arc;

use flutter_rust_bridge::DartFnFuture;
use holon_integration_tests::UserDriver;
use holon_integration_tests::pbt::E2ESut;
use holon_integration_tests::pbt::E2ETransition;
use holon_integration_tests::pbt::VariantRef;
use holon_integration_tests::pbt::phased::create_initial_ref_state;
use holon_integration_tests::pbt::phased::create_runner;
use holon_integration_tests::pbt::phased::create_runtime;
...
use crate::api::ffi_bridge::clear_pbt_engine;
use crate::api::ffi_bridge::set_pbt_engine;
use crate::api::flutter_mutation_driver::ApplyMutationCallback;
use crate::api::flutter_mutation_driver::FlutterUserDriver;
...
/// Run the full PBT state machine with Flutter providing UI mutations via
/// callback.
pub async fn run_shared_pbt(
    apply_mutation_cb: impl Fn(String, String, String) -> DartFnFuture<()> + Send + Sync + 'static,
    num_steps: u32,
) -> anyhow::Result<String> {
    let driver: Box<dyn UserDriver> = Box::new(FlutterUserDriver::new(
        Arc::new(apply_mutation_cb) as ApplyMutationCallback
    ));
...
/// Flutter-specific pbt_setup that also installs the PBT engine as
/// GLOBAL_SESSION.
pub async fn pbt_setup(num_steps: u32) -> anyhow::Result<String> {
    ...
    sut.driver = Some(Box::new(holon_integration_tests::DirectUserDriver::new(
