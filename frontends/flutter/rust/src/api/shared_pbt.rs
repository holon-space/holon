...
use crate::api::flutter_mutation_driver::{ApplyMutationCallback, FlutterUserDriver};
...
pub async fn run_shared_pbt(
    ...
    let driver: Box<dyn UserDriver> = Box::new(FlutterUserDriver::new(Arc::new(
        apply_mutation_cb,
    )
...
pub async fn pbt_setup(num_steps: u32) -> anyhow::Result<String> {
    ...
    sut.driver = Some(Box::new(
        holon_integration_tests::DirectUserDriver::new(sut.ctx.engine().clone()),
    ));
