...
///
/// Resolves EntityProfiles for Created/Updated rows before forwarding.
/// Uses the shared `resolve_batch_profiles` from `ui_watcher` for profile resolution.
fn spawn_stream_forwarder(
    mut stream: RowChangeStream,
    sink: MapChangeSink,
    profile_resolver: std::sync::Arc<dyn holon::entity_profile::ProfileResolving>,
    profile_ctx: holon::entity_profile::ProfileContext,
) {
    tokio::spawn(async move {
        ...
        while let Some(batch_with_metadata) = stream.next().await {
            ...
            let map_changes = holon::api::ui_watcher::resolve_batch_profiles(
...
pub async fn watch_ui(
    ...
    let watch = session
        .watch_ui(block_id.clone(), preferred_variant, is_root)
        .await?;
...
pub async fn query_and_watch(
    ...
    let profile_ctx = holon::entity_profile::ProfileContext {
        preferred_variant,
        view_width: None,
    };
    ...
    let context = if let Some(id) = context_block_id {
        ...
        let mut ctx = QueryContext::for_block_with_path(
            holon_api::EntityUri::from_raw(&id),
            None,
            block_path,
        );
        ctx.profile_context = Some(profile_ctx.clone());
        Some(ctx)
    } else {
        let mut ctx = QueryContext::root();
        ctx.profile_context = Some(profile_ctx.clone());
        Some(ctx)
    };
...
pub async fn render_block(
    ...
    let profile_ctx = holon::entity_profile::ProfileContext {
        preferred_variant: preferred_variant.clone(),
        view_width: None,
    };
    ...
    let (widget_spec, stream) = session
        .engine()
        .blocks()
        .render_block(&block_id, preferred_variant, is_root)
        .await?;
