...
//!
//! This module provides a minimal FFI surface exposing only FrontendSession and
//! essential types. Low-level holon_prql_render types (Expr, ModuleDef,
//! Lineage) are hidden as implementation details.
//!
//! ## Architecture: FrontendSession as the Primary API
//!
//! All query and operation methods are accessed through `FrontendSession`, not
//! `BackendEngine`. This design guarantees that:
//!
//! 1. **No initialization race conditions**: `FrontendSession::new()` completes
//!    all schema initialization (including materialized views like
//!    `block_with_path`) before returning. Since all query methods are on
//!    `FrontendSession`, they can only be called after init.
//!
//! 2. **Identical code paths**: Flutter and E2E tests use the exact same API
//!    surface (`FrontendSession`), eliminating bugs that only appear in
//!    production.
//!
...
///
/// If trace_context is provided, creates a child span. Otherwise creates a new
/// root span.
...
async fn init_opentelemetry() -> anyhow::Result<()> {
    ...
    if exporter_type.contains("otlp") {
        ...
        // Create OTLP trace exporter using builder - use with_http() to set HTTP
        // protocol
        ...
        // Filter to only include actual log events (info!, debug!, warn!, error!), not
        // span lifecycle events
        use opentelemetry_appender_tracing::layer::OpenTelemetryTracingBridge;
        use tracing_subscriber::filter::FilterFn;
        use tracing_subscriber::filter::Filtered;

        // Only process events (not spans) - span lifecycle events are handled by
        // telemetry_layer
        ...
        tracing::debug!(
            "[FFI] OpenTelemetry tracing and logging initialized with stdout exporters"
        );
    }
...
///
/// This is the main entry point for Flutter. It creates a `FrontendSession`
/// which:
...
///   - `MCP_SERVER_PORT` - Port for MCP HTTP server (optional, starts server if
///     provided)
///
/// # Returns
/// An opaque handle to the session. The actual `FrontendSession` is stored
/// globally and accessed by subsequent FFI calls. This return value is kept
/// alive by Flutter to prevent the Rust Bridge from disposing the underlying
/// resources.
pub async fn init_render_engine(
    ...
    // Initialize OpenTelemetry (includes tracing subscriber with OpenTelemetry
    // bridge)
...
///
/// This is returned from `init_render_engine` and should be kept alive by
/// Flutter to prevent the Rust Bridge from disposing the underlying session.
...
/// Sink for UiEvent stream (required by FRB — can't use type alias for
/// StreamSink<T>)
///
/// flutter_rust_bridge:non_opaque
...
/// forwarded to a Flutter `StreamSink` in a spawned task. The abort guard is
/// kept alive here so the actor pipeline behind `watch_ui` is cancelled when
/// the FFI handle is dropped, instead of leaking onto the runtime.
#[frb(opaque)]
pub struct FfiWatchHandle {
    command_tx: tokio::sync::mpsc::Sender<holon_api::WatcherCommand>,
    _aborts: holon_api::streaming::ActorAbortGuard,
}

/// Watch a block's UI with automatic error recovery and structural hot-swap.
///
/// Returns a `FfiWatchHandle` for variant switching. UiEvents (Structure +
/// Data) are sent to the provided sink. Unlike `render_entity`, errors become
/// `UiEvent::Structure` events with inline error RenderExprs — the stream
/// stays open and recovers when the block is fixed.
pub async fn watch_ui(block_id: String, sink: UiEventSink) -> anyhow::Result<FfiWatchHandle> {
    let session = get_session()?;

    let block_uri = holon_api::EntityUri::parse(&block_id)
        .map_err(|e| anyhow::anyhow!("Invalid block_id '{}': {}", block_id, e))?;
    let watch = session.watch_ui(&block_uri).await?;

    let (mut output_rx, command_tx, aborts) = watch.into_parts();
...
/// Compile one of the supported query languages, execute it, and set up CDC
/// streaming.
///
...
pub async fn query_and_watch(
    ...
    spawn_stream_forwarder(stream, sink, session.engine().profile_resolver().clone());
...
///
/// Given a block ID, finds its query source child, compiles and executes the
/// query, parses any render sibling into a RenderExpr, and returns the
/// RenderExpr. Initial data and CDC deltas are forwarded to the sink.
...
///
/// Previously found a block with `source_language = "render"` that shares the
/// same parent_id. TODO: Render sibling lookup was removed with PRQL render
/// directive removal. This stub remains for FFI compatibility.
...
///
/// Executes the inverse operation from the undo stack and pushes it to the redo
/// stack. Returns true if an operation was undone, false if the undo stack is
/// empty.
...
///
/// Executes the inverse of the last undone operation and pushes it back to the
/// undo stack. Returns true if an operation was redone, false if the redo stack
/// is empty.
...
///
/// The frontend captures the OAuth redirect callback URL (via
/// flutter_web_auth_2) and extracts the `code` and `state` query parameters,
/// then passes them here. Rust exchanges the code for a token and connects to
/// the MCP server.
///
/// # Parameters
/// * `provider_name` - The provider identifier (MCP server URI) from
///   `NeedsConsent`
...
/// Interpret a RenderExpr with data rows into a ViewModel tree and return it as
/// JSON.
///
/// This is the same interpretation that GPUI does inline
/// (shadow_interp.interpret). Flutter calls this after receiving a
/// UiEvent::Structure to get the pre-built widget tree, avoiding the need for
/// Dart-side RenderInterpreter.
///
...
pub fn interpret_render_expr(
    ...
    let rows: Vec<std::sync::Arc<HashMap<String, Value>>> =
...
// `run_shared_pbt` is in shared_pbt.rs (separate file required for FRB
// DartFnFuture codegen)

// ──── PBT Engine routing ────
// The shared PBT creates its own BackendEngine (with its own temp dir,
// database, etc.). The Dart callback calls `pbt_execute_operation` which routes
// to this engine instead of the production GLOBAL_SESSION.
