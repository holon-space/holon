...
use holon_api::{OperationDescriptor, ProviderAuthStatus, QueryLanguage, Value, WidgetSpec};
...
fn spawn_stream_forwarder(
    ...
    tokio::spawn(async move {
        ...
        while let Some(batch_with_metadata) = stream.next().await {
            ...
            let map_changes = holon::api::ui_watcher::enrich_batch(
...
pub async fn init_render_engine(
    ...
    let mut frontend_config = FrontendConfig::new(ui_info).with_db_path(db_path.into());
    ...
    if let Some(port) = mcp_port {
        ...
        let mcp_debug = std::sync::Arc::new(holon_mcp::server::DebugServices::default());
        tokio::spawn(async move {
...
pub async fn query_and_watch(
    ...
    let (widget_spec, stream) = session
        .engine()
...
pub async fn render_block(
    ...
    let (widget_spec, stream) = session
        .engine()
        .blocks()
        .render_block(&block_uri, &preferred_variant, is_root)
        .await?;
...
pub fn interpret_widget_spec(widget_spec: WidgetSpec) -> anyhow::Result<String> {
    let session = get_session()?;
    let rt = tokio::runtime::Handle::current();
    let render_ctx = holon_frontend::RenderContext::new(session, rt);
    let render_ctx = render_ctx.with_data_rows(widget_spec.data.clone());
    let shadow_interp = holon_frontend::create_shadow_interpreter();
    let view_model = shadow_interp.interpret(&widget_spec.render_expr, &render_ctx);
...
pub fn interpret_render_expr(
    ...
    let rt = tokio::runtime::Handle::current();
    let render_ctx = holon_frontend::RenderContext::new(session, rt);
    let render_ctx = render_ctx.with_data_rows(rows);
    let shadow_interp = holon_frontend::create_shadow_interpreter();
    let view_model = shadow_interp.interpret(&render_expr, &render_ctx);
