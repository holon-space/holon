//! Flutter-specific UserDriver that delegates UI mutations to Dart via
//! DartFnFuture.
//!
//! The Dart callback receives (entity, op, params_json) and uses WidgetTester
//! to drive the mutation through the Flutter UI. Phase 1: Dart calls
//! executeOperation directly. Phase 2: actual WidgetTester interactions (tap,
//! enterText, drag).
...
///
/// The Dart side receives the mutation as JSON-serialized params and dispatches
/// it through the Flutter UI (or calls executeOperation directly in Phase 1).
...
impl UserDriver for FlutterUserDriver {
    ...
    async fn drop_entity(
        ...
        anyhow::bail!(
            "FlutterUserDriver does not yet implement drop_entity — extend apply_mutation_cb to \
             dispatch a flutter_test drag gesture"
        )
    }
}
