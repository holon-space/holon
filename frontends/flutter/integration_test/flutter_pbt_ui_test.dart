...
Future<void> captureDiagnostics(
  ...
  try {
    final renderTree = RendererBinding.instance.renderViews.first
        .toStringDeep();
...
void main() {
  ...
  testWidgets('PBT - UI-driven state machine', (tester) async {
        ...
        overrides: [
          ...
          queryResultByPrqlProvider.overrideWith((ref, prqlQuery) async {
            ...
            return (
              ...
              changeStream: const Stream<BatchMapChangeWithMetadata>.empty(),
            );
          }),
    ...
    while (true) {
      final step = await tester.runAsync(() => shared_pbt.pbtStep());
      if (step!.done) break;
      stepNum++;
