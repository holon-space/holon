# Handoff: Implement `render_entity` Primitive with Live Query Support

## Goal

Add a `render_entity(this)` render primitive that automatically dispatches based on block type. For query blocks (PRQL source), it executes the embedded query with its own CDC stream and renders the results inline.
...
We want a polymorphic `render_entity` that does the right thing based on the block's `content_type` and `source_language`:
```prql
render (list item_template:(row (render_entity this)))
```
...
```
render_entity(this) dispatches based on content_type/source_language:
...
2. **Modify**: `lib/render/render_interpreter.dart`
   ...
   - Add `case 'render_entity':` in switch
...
- A PRQL block executed via `render_entity` may itself define multiple views
