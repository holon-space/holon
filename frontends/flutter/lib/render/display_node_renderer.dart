...
class DisplayRenderContext {
  ...
  final Future<void> Function(String entityName, String opName, Map<String, dynamic> params)?
      onOperation;
...
Widget renderNode(ViewModel node, DisplayRenderContext ctx) {
  return switch (node.widget) {
    ...
    'render_block' => _renderContentWrapper(node, ctx),
...
Widget _renderBadge(ViewModel node, DisplayRenderContext ctx) {
  ...
  return Container(
    ...
    child: Text(label, style: TextStyle(fontSize: 12, color: ctx.colors.textSecondary)),
  );
}

Widget _renderIcon(ViewModel node, DisplayRenderContext ctx) {
  // Map icon names to Flutter IconData when needed
  return Icon(Icons.circle, size: node.getDouble('size', 16.0), color: ctx.colors.textSecondary);
}
...
Widget _renderSpacer(ViewModel node) {
  ...
  if (w > 0 || h > 0) return SizedBox(width: w > 0 ? w : null, height: h > 0 ? h : null);
  return const SizedBox.shrink();
}

Widget _renderEditableText(ViewModel node, DisplayRenderContext ctx) {
  final content = node.getString('content');
  return Text(content, style: TextStyle(fontSize: 16, color: ctx.colors.textPrimary));
}
    ...
    children: [
      if (title.isNotEmpty)
        Padding(
          ...
          child: Text(title, style: TextStyle(fontWeight: FontWeight.bold, color: ctx.colors.textPrimary)),
        ),
      if (children != null)
        ...children.items.map((c) => renderNode(c, ctx)),
    ],
  );
}
...
Widget _renderColumns(ViewModel node, DisplayRenderContext ctx) {
  ...
  final mainContent = mainWidgets.isEmpty
      ...
      : mainWidgets.length == 1
          ? mainWidgets.first
          : Row(crossAxisAlignment: CrossAxisAlignment.stretch, children: mainWidgets);
...
Widget _renderSourceBlock(ViewModel node, DisplayRenderContext ctx) {
  ...
  return Container(
    ...
    child: SelectableText(
      content,
      style: TextStyle(fontFamily: 'monospace', fontSize: 13, color: ctx.colors.textPrimary),
    ),
  );
}
...
Widget _renderBlockOperations(ViewModel node, DisplayRenderContext ctx) {
  ...
  return GestureDetector(
    ...
    child: MouseRegion(
      ...
      child: Text('[...]', style: TextStyle(fontSize: 12, color: ctx.colors.textSecondary)),
    ),
  );
}
...
Widget _renderTableRow(ViewModel node, DisplayRenderContext ctx) {
  ...
  final cells = data.entries
      ...
      .map((e) => Expanded(
            child: Text(e.value?.toString() ?? '', style: TextStyle(color: ctx.colors.textPrimary)),
          ))
      .toList();
...
Widget _renderPieMenu(ViewModel node, DisplayRenderContext ctx) {
  ...
  if (fieldsStr == 'this' || fieldsStr == '*' || fieldsStr == 'this.*') {
    fieldList = node.operations
        .expand((op) => op.affectedFields)
        .toList();
  } else {
...
List<Widget> _intersperse(List<Widget> widgets, double gap, {required bool isHorizontal}) {
        ...
        builder: (_, isLeftOpen, _) {
          if (rightDrawerState != null) {
            return ValueListenableBuilder<bool>(
              ...
              builder: (_, isRightOpen, _) => _buildStack(isLeftOpen, isRightOpen),
            );
          }
