...
import '../src/rust/third_party/holon_api/streaming.dart'
    show UiEvent, UiEvent_Data, UiEvent_Structure;
...
class _BlockRefWidgetState extends ConsumerState<BlockRefWidget> {
  ...
  void dispose() {
    log.warn(
      '[BlockRefWidget] dispose() called for ${widget.blockId} — WatchHandle will be dropped',
    );
  ...
  Future<void> _startWatching() async {
    ...
    try {
      ...
      log.error(
        '[BlockRefWidget] Failed to start watch_ui for ${widget.blockId}',
        error: e,
      );
      if (mounted) {
        setState(() {
  ...
  Future<void> _interpretAndRender(
    UiEvent_Structure event,
    int newGeneration,
  ) async {
    try {
      final json = await ffi.interpretWidgetSpec(widgetSpec: event.widgetSpec);
      if (!mounted) return;
      ...
      log.error(
        '[BlockRefWidget] interpretWidgetSpec failed for ${widget.blockId}',
        error: e,
      );
      if (!mounted) return;
      setState(() {
