...
class _SearchSelectOverlayState extends ConsumerState<SearchSelectOverlay> {
  ...
  List<ResolvedRow> _filterResults(SearchSelectOverlayState state) {
    String? _field(ResolvedRow row, String name) {
    ...
    if (_searchQuery.isEmpty) {
      return state.rowCache.values
          .where((row) => _field(row, 'id') != state.draggedItem?.id)
          .take(10)
          .toList();
    }
    ...
    return state.rowCache.values
        .where((row) {
          if (_field(row, 'id') == sourceId) return false;
          final content = _field(row, 'content') ?? '';
                      ...
                      child: Container(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final nodeId = idVal != null ? valueToDynamic(idVal)?.toString() ?? '' : '';
                            final contentVal = node.data['content'];
                            final nameVal = node.data['name'];
                            final content =
                                (contentVal != null ? valueToDynamic(contentVal)?.toString() : null) ??
                                (nameVal != null ? valueToDynamic(nameVal)?.toString() : null) ??
                                nodeId;
