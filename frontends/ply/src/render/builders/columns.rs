...
fn build_screen_layout(tmpl: &RenderExpr, rows: &[Arc<DataRow>], ctx: &RenderContext) -> PlyWidget {
    if rows.is_empty() {
    ...
    let partition = partition_screen_columns(rows, |row| {
        let row_ctx = ctx.with_row(Arc::new(row.clone()));
        interpret(tmpl, &row_ctx)
    });
    ...
    let left_open = left_sidebar.as_ref().map(|r| {
        ...
        let ws = ctx.widget_state(key);
        (ws.open, ws.width.unwrap_or(SIDEBAR_WIDTH))
    });
