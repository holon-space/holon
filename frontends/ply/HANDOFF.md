...
Wired `selectable`, `state_toggle`, and `block_operations` builders using ply's `ui.element().on_press()` callback API. Each clones `Arc<Session>` and `runtime_handle` into the `on_press` closure for `dispatch_operation()`. `pie_menu` still renders child-only (no radial menu in ply).
