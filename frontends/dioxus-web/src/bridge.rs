...
impl WorkerBridge {
    /// Spawn the worker at `worker_url` and wait for it to emit `{ kind:
    /// 'ready' }`.
    pub async fn spawn(worker_url: &str) -> Result<Self, String> {
        ...
        match ready_result {
            ...
            futures::future::Either::Right(_) => {
                return Err(format!(
                    "worker did not emit `ready` within {READY_TIMEOUT_MS}ms — likely failed to \
                     instantiate (check wasm URL, imports, or the worker console for link errors)"
                ));
            }
        }
