//! Vendored Turso OPFS browser shim.
//!
//! Source: https://github.com/tursodatabase/turso (fork: nightscape/turso, branch: holon)
//! Upstream commit: 84b440e70eae8f0943e57f0535007e164cb9e294
//! Original path: bindings/javascript/src/browser.rs
//! Copied: 2026-04-14 (Phase 1 spike, see
//! ~/.claude/plans/nifty-bouncing-ladybug.md H1)
//!
//! Why vendored rather than depending on `turso_node`:
//!   - `turso_node` depends on a Turso-internal `workspace-hack` crate via a
//!     path import that is not addressable from holon's workspace.
//!   - Even if the dep resolved, it drags in the full Node binding surface
//!     (Database/Statement napi glue for Node APIs we never use).
//!   - The OPFS bridge is a small, load-bearing slice of that crate. Vendoring
//!     keeps our compile surface minimal.
//!
//! Re-sync procedure: diff this file against the upstream commit above. When
//! upgrading Turso, pull the latest version, eyeball the diff in browser.rs,
//! and copy relevant changes here. Update the commit hash.
//!
//! This module is gated behind the `browser` cargo feature. Phase 1 compiles
//! without it; Phase 2 enables it once Turso deps are wired in.
...
use turso_core::Clock;
use turso_core::Completion;
use turso_core::File;
use turso_core::IO;
use turso_core::MonotonicInstant;
use turso_core::WallClockInstant;
