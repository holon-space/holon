...
//!   - `WindowTextSystem::shape_text` with per-run styling (Bold/Italic via
//!     TextRun.font + color)
//!   - `WrappedLine::position_for_index` (caret offset → pixel point)
//!   - `WrappedLine::closest_index_for_position` (mouse hit-testing → caret
//!     offset)
...
/// State for the spike. In Phase 3 this lives behind a `LoroText` + Loro
/// cursor.
...
/// EditorViewModel. The trait *existing* on our type is the proof that
