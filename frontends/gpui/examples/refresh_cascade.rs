...
//!
//! Each panel tests a different nesting pattern to find what breaks
//! uniform_list. Run with: cargo run --example refresh_cascade -p holon-gpui
...
fn panel_holon_chain(list_entity: Entity<ListEntity>) -> AnyElement {
    // Outer: panel() wrapper from the example (flex_1 + size_full + flex_col +
    // border)
    div()
        .flex_1()
        .size_full()
