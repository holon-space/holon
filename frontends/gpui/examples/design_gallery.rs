...
fn main() {
    ...
    app.run(move |cx| {
        ...
        cx.spawn(async move |cx| {
            ...
            holon_gpui::setup_interaction_pump(&debug_for_window, window_handle.into(), &cx);
...
struct GalleryView {
    ...
    focus: holon_gpui::entity_view_registry::FocusRegistry,
...
impl GalleryView {
    ...
    fn mode_switcher(&self, cx: &mut Context<Self>) -> Div {
        let modes = [Mode::Capture, Mode::Orient, Mode::Flow, Mode::Chat];
        let mut row = div()
            .flex()
            .gap(px(2.0))
            .bg(c(0x16161400))
...
impl GalleryView {
    fn new_with_services(
        ...
        let focus = holon_gpui::entity_view_registry::FocusRegistry::new();
        Self {
            mode: Mode::Chat,
