...
gpui::actions!(holon, [ViewModeTree, ViewModeTable]);
...
/// (BlockRefView, LiveQueryView) each have their own independent streams.
...
struct AppModel {
    ...
    /// The reactive root tree. BlockRef nodes are placeholders.
    ...
    root_block_refs: std::collections::HashMap<String, Entity<views::ReactiveShell>>,
...
fn root_reactive_view(rvm: &ReactiveViewModel) -> Option<Arc<holon_frontend::ReactiveView>> {
    match &rvm.kind {
        holon_frontend::reactive_view_model::ReactiveViewKind::Reactive { view } => {
            Some(view.clone())
        }
        _ => None,
    }
...
impl AppModel {
    ...
    fn rebuild(&mut self, cx: &mut gpui::Context<Self>) {
        ...
        self.root_vm = self.engine.snapshot_reactive(&root_uri);
        ...
        self.reconcile_root_block_refs(cx);

        self.view_model =
            resolved_view_model(&self.root_vm, &self.engine, &self.root_block_refs, cx);
        let shadow_index = holon_frontend::shadow_index::IncrementalShadowIndex::build(
            root_uri.as_str(),
            &self.root_vm,
        );
        self.nav.set_shadow_index(shadow_index, &self.focus);
    }
    ...
    /// Walk the root reactive tree to find BlockRef nodes and create/GC their entities.
    fn reconcile_root_block_refs(&mut self, cx: &mut gpui::Context<Self>) {
        let mut needed = std::collections::HashSet::new();
        collect_root_block_refs(&self.root_vm, &mut needed);

        for block_id in &needed {
            if !self.root_block_refs.contains_key(block_id) {
                ...
                self.root_block_refs.insert(block_id.clone(), entity);
            }
        }
...
///
/// Each BlockRef is resolved by reading its BlockRefView entity's current
...
fn resolved_view_model(
    ...
    root_vm.snapshot_resolved(&|block_id| resolve_block(block_id, root_block_refs, services, cx))
}

/// Resolve a single block_ref by reading its BlockRefView's reactive tree.
/// Recurses for nested block_refs via snapshot_resolved.
fn resolve_block(
    ...
    if let Some(entity) = root_block_refs.get(&key) {
...
/// Walk a reactive tree to collect all BlockRef block_ids at any depth.
/// Stops at BlockRef nodes (they manage their own subtrees).
fn collect_root_block_refs(node: &ReactiveViewModel, ids: &mut std::collections::HashSet<String>) {
    ...
    match &node.kind {
        ...
        _ => {
            views::reactive_shell::for_each_child(node, |child| {
                collect_root_block_refs(child, ids)
            });
        }
    }
}
...
impl Render for HolonApp {
    ...
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // Editor reconciliation is now handled by each BlockRefView in its render().
        // Shadow index is built in the signal callback.

        let local = {
            let root_refs = self.app_model.read(cx).root_block_refs.clone();
            let mut l = LocalEntityScope::new().with_cache(self.entity_cache.clone());
            // Pre-populate the entity cache with root block_ref entities.
            // This way the block_ref builder finds them in get_or_create and
            // doesn't call watch_live + cx.new() during the render pass.
            for (bid, entity) in &root_refs {
                let key = format!("block-ref-{bid}");
                l.entity_cache
                    .write()
                    .unwrap()
                    .entry(key)
        ...
        // Render from the reactive tree — node_dispatch matches on ReactiveViewKind
        let root = {
            let model = self.app_model.read(cx);
        ...
        let content = {
            ...
            div()
                ...
                .on_action({
                    let engine = engine.clone();
                    move |_: &ViewModeTree, _window, cx: &mut App| {
                        set_focused_view_mode(&engine, "tree_view", cx);
                    }
                })
                .on_action({
                    let engine = engine.clone();
                    move |_: &ViewModeTable, _window, cx: &mut App| {
                        set_focused_view_mode(&engine, "table_view", cx);
                    }
                })
                .child(root)
        };
...
pub fn launch_holon_window_with_engine(
    ...
    let bounds_registry = BoundsRegistry::new();
    let mut nav = NavigationState::with_shadow_index(debug.shadow_index.clone());
...
fn launch_holon_window_impl(
    ...
    cx.bind_keys([
        KeyBinding::new("cmd-shift-l", ViewModeTree, None),
        KeyBinding::new("cmd-shift-t", ViewModeTable, None),
    ]);
            ...
            loop {
                ...
                match select(Box::pin(next_fut), Box::pin(timeout)).await {
                    Either::Left((Some(rvm), _)) => {
                        if !matches!(
                            &rvm.kind,
                            holon_frontend::reactive_view_model::ReactiveViewKind::Loading
                        ) {
                            eprintln!(
    ...
    let window_result = cx.open_window(window_options, |window, cx| {
        ...
        let app_model = cx.new(|cx| {
            let mut model = AppModel {
                ...
                root_block_refs: std::collections::HashMap::new(),
                pending_cursor: None,
                root_view: initial_root_view,
            };
            // Initial reconciliation — create root BlockRefView entities.
            // Each BlockRefView manages its own child entities (editors, live queries).
            model.reconcile_root_block_refs(cx);
                ...
                .for_each(|rvm| {
                    ...
                    let _ = cx.update_window(wh, |_, window, cx| {
                        app_model.update(cx, |m, cx| {
                            ...
                            // Reconcile root BlockRefView entities.
                            // Each BlockRefView manages its own child entities.
                            m.reconcile_root_block_refs(cx);

                            // Resolve view_model + shadow index.
                            m.view_model =
                                resolved_view_model(&m.root_vm, &m.engine, &m.root_block_refs, cx);
                            let root_uri = holon_api::root_layout_block_uri();
                            let shadow_index =
                                holon_frontend::shadow_index::IncrementalShadowIndex::build(
                                    root_uri.as_str(),
                                    &m.root_vm,
                                );
                            m.nav.set_shadow_index(shadow_index, &m.focus);
...
/// Set the view mode globally (all view_mode_switcher widgets react).
fn set_focused_view_mode(engine: &ReactiveEngine, mode: &str, _cx: &App) {
    let sentinel = holon_api::EntityUri::from_raw("sentinel:view_mode");
    engine.ui_state().set_view_mode(&sentinel, mode.to_string());
}
...
fn handle_cross_block_nav(
    ...
    let Some(focused_id) = focus.focused_editor_row_id(window, cx) else {
        tracing::debug!(
            "cross_block_nav: no focused editor found (editors={}, shadow_entities={})",
            focus.editor_inputs.len(),
            nav.shadow_entity_count()
        );
        return;
    };
    ...
    match nav.bubble_input(&focused_id, &input) {
        ...
        None => {
            tracing::debug!(
                "cross_block_nav: bubble_input returned None for focused_id={focused_id}, direction={direction:?} (shadow_entities={})",
                nav.shadow_entity_count()
            );
        }
    }
