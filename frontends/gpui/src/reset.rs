...
//! is `GpuiModule`-only). The reset pump then `rebind`s the live window onto
//! the fresh SUT. See `docs/Testing/Phase1_OptionA_RebindReset_Plan.md`.
//!
...
//! seed files onto `org_root` on disk (no `InMemoryFileSystem`); (b) it lives
//! in the gpui crate for colocation with `rebind` + `render_supported_widgets`.
...
/// given fresh, seeded paths, WITHOUT starting an MCP server (that is
/// `GpuiModule`-only, so the already-running server is reused — see plan C2 for
/// the live-cell swap).
///
...
/// back into it, so in-place reuse lets the retired engine corrupt the new
/// seed.
