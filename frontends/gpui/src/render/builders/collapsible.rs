...
pub fn render(node: &holon_frontend::reactive_view_model::ReactiveViewModel, ctx: &GpuiRenderContext) -> AnyElement {
    let (header_text, icon_text, children) = match &node.kind {
        holon_frontend::reactive_view_model::ReactiveViewKind::Collapsible { header, icon, children } => {
            (header.clone(), icon.clone(), children)
        }
        _ => return div().child("[collapsible: wrong kind]").into_any_element(),
    };

    let detail_text: String = children.iter().filter_map(|child| {
        if let holon_frontend::reactive_view_model::ReactiveViewKind::Text { content, .. } = &child.kind {
            Some(content.clone())
        } else {
            None
        }
    ...
    let cache_key = format!("collapsible:{header_text}");
