...
impl gpui::Render for CollapsibleView {
    fn render(&mut self, _: &mut gpui::Window, cx: &mut gpui::Context<Self>) -> impl IntoElement {
        ...
        let header_row = div()
            ...
            .child(div().text_size(px(12.0)).child(self.icon_text.clone()))
            .child(
                div()
                    .flex_1()
                    .text_size(px(12.0))
...
pub fn render(
    ...
    let detail_text: String = children
        .iter()
        .filter_map(|child| child.prop_str("content").map(|s| s.to_string()))
        .collect::<Vec<_>>()
        .join("\n");

    let cache_key =
        crate::entity_view_registry::CacheKey::Ephemeral(format!("collapsible:{header_text}"));
