...
pub fn render(node: &ReactiveViewModel, ctx: &GpuiRenderContext) -> AnyElement {
    ...
    let key = crate::entity_view_registry::CacheKey::Ephemeral(format!("pie-open:{menu_id}"));
    let toggle: gpui::Entity<ToggleState> = ctx.local.get_or_create_typed(key, || {
    ...
    let trigger = div().id(hashed_id(&menu_id)).child(child_el).on_mouse_down(
    ...
    for (i, op) in ops.iter().enumerate() {
        ...
        let intent_template = row_id
            .as_ref()
            .map(|id| OperationIntent::for_row(op, id, entity_name.as_ref()));
