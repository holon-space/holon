...
pub fn render(node: &ReactiveViewModel, ctx: &GpuiRenderContext) -> AnyElement {
    ...
    let ReactiveViewKind::RenderBlock { slot } = &node.kind else {
        unreachable!()
    };

    // Check for a pre-created RenderBlockView entity (from CollectionView).
    if let Some(row_id) = node.row_id() {
        if let Some(entity) = ctx.local.render_blocks.get(&row_id) {
    ...
    let inner = div()
        ...
        .into_any_element();
    crate::geometry::tracked(el_id, inner, &ctx.bounds_registry, "render_block", Some(&id.to_string()), true).into_any_element()
}
