use super::prelude::*;
use gpui::{
    FontStyle, FontWeight, HighlightStyle, SharedString, StrikethroughStyle, StyledText,
    UnderlineStyle,
};
...
pub fn render(node: &ReactiveViewModel, ctx: &GpuiRenderContext) -> Div {
    ...
    match marks {
        Some(ref m) if !m.is_empty() => {
            ...
            el.child(StyledText::new(SharedString::from(content)).with_highlights(highlights))
        }
        _ => el.child(content),
    }
}
