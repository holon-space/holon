...
pub fn render(node: &ReactiveViewModel, ctx: &GpuiRenderContext) -> Div {
    ...
    let (on, current, intent) = match binding {
        StateToggleBinding::Bool => {
            ...
            (on, String::new(), intent)
        }
        StateToggleBinding::Words => {
            let current = node.prop_str("current").unwrap_or_default();
            // Non-task block: no task_state, so there is no checkbox to show.
            // Collapse the placeholder to zero width instead of reserving a
            // full icon box — otherwise every ordinary block carries a wide
            // empty gutter before its text (dogfood PHASE 3 bug 8). A bool
            // binding has no such empty: `false` is a state to paint.
            if current.is_empty() {
