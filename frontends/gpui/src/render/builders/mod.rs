...
///
/// `Reactive` nodes are rendered via a `ReactiveShell` entity, cached in `EntityCache`.
...
pub fn render(
    ...
    match &node.kind {
        ReactiveViewKind::Empty => return div().into_any_element(),
        ReactiveViewKind::Reactive { view } => {
            ...
            if matches!(
                ...
                let gap = match view.layout() {
                    Some(holon_frontend::CollectionVariant::Columns { gap }) => gap,
                    _ => 4.0,
                };
                let items: Vec<std::sync::Arc<holon_frontend::ReactiveViewModel>> =
                    view.items.lock_ref().iter().cloned().collect();
                return columns::render(&gap, &items, ctx).into_any_element();
            }
            ...
            // `BoundsRegistry`. Without this, a `Reactive { view }` with 0
            ...
            return tag(
                ctx,
                "reactive_shell",
                scrollable_list_wrapper(
                    gpui::AnyView::from(entity),
                    prelude::hashed_id(&shell_key),
                ),
            );
        }
        _ => {}
