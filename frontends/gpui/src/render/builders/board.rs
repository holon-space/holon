...
fn resolve_row_op_entity(
    ...
    let Some(op) = profile.operations.iter().find(|o| o.name == op_name) else {
        tracing::warn!(
            "board {context}: {op_name} op not found on profile for row_id={row_id}"
        );
        return None;
    };
...
///
/// Anchor choice:
/// - a previous card exists → "place after prev" under prev's parent
///   (the exact positional meaning of the drop);
/// - dropped at the top of the lane → "first child" of the next card's
///   parent. When the lane is a filtered subset of a larger sibling set
///   this can overshoot to the very front of that sibling set (still
///   before `next`, so the lane order is correct).
    ...
    let (parent_id, after_block_id) = match (prev, next) {
        (Some(prev), _) => {
            let (Some(parent), Some(prev_id)) = (prev.parent_id.as_deref(), prev.row_id.as_deref())
                ...
                tracing::warn!(
                    "board {context}: reorder anchor lacks parent_id/row_id — \
                     not persisting position of row_id={row_id}"
                );
                return;
            };
        ...
        (None, Some(next)) => {
            let Some(parent) = next.parent_id.as_deref() else {
                tracing::warn!(
                    "board {context}: top-of-lane anchor lacks parent_id — \
                     not persisting position of row_id={row_id}"
                );
                return;
            };
