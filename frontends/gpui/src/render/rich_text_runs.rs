...
//! expose. (See Phase 0.2 spike at
//! `frontends/gpui/examples/rich_input_spike.rs` for the proof-of-concept paint
//! pass this output feeds.)
//!
//! # Algorithm
//!
...
//! 3. Convert scalar offsets to byte offsets at the segment level so the `len:
//!    usize` field on `TextRun` is bytes (per gpui's contract).
