...
//!
//! Builds on Phase 0.2's bare-GPUI spike
//! (`frontends/gpui/examples/rich_input_spike.rs`), Phase 2's mark vocabulary,
//! and Phase 3 primitives:
//! - `crate::render::rich_text_runs::marks_to_text_runs` for the paint pass
//! - `holon_frontend::rich_text_selection::RichTextSelection` for
//!   caret/selection state
//! - Phase 3.1a Loro cursor helpers (consumed by the embedding view, not
//!   directly here)
//!
...
//! - Replacing the existing `EditorView` — that's a separate integration that
//!   wires the new editor into `editable_text` builder paths
