...
///
/// Owns per-row entities (editors, live_blocks, live_queries).
...
impl RenderEntityView {
    ...
    pub fn set_content(&mut self, new: Arc<ReactiveViewModel>, cx: &mut Context<Self>) {
        // Merge fresh tree into existing, preserving node identity where
        // widget types match. Expand states and GPUI entity caches survive.
