...
fn matrix() -> Vec<(&'static str, Container, &'static str, Content)> {
    vec![
        ...
        ("column", column, "badge", || badge("NEW")),
        ("column", column, "icon", || icon("info")),
...
fn containment_col_of_text_and_badge(cx: &mut TestAppContext) {
    let fixture = column(vec![text("first"), text("second"), badge("LBL")]);
    let snap = render_fixture(cx, Arc::new(fixture));
...
fn invariant_fires_on_zero_spacer(cx: &mut TestAppContext) {
    let fixture = row(vec![ReactiveViewModel::from_widget(
        "spacer",
        props(&[
            ("width", holon_api::Value::Float(0.0)),
            ("height", holon_api::Value::Float(0.0)),
        ]),
    )]);
    let snap = render_fixture(cx, Arc::new(fixture));
...
fn snapshot_captures_each_widget(cx: &mut TestAppContext) {
    ...
    let col_count = snap.of_type("column").count();
...
fn fake(
    ...
    (
        ...
        ElementInfo {
            ...
            width: w,
            height: h,
            widget_type: std::sync::Arc::from(widget),
            entity_id: None,
            has_content: false,
...
fn containment_passes_on_valid_tree() {
    let snap = fake_snapshot(vec![
        fake("root#0", "column", 0.0, 0.0, 100.0, 200.0, None),
...
fn containment_fires_when_child_escapes_parent() {
    let snap = fake_snapshot(vec![
        fake("root#0", "column", 0.0, 0.0, 100.0, 100.0, None),
...
fn sibling_overlap_ignores_unrelated_subtrees() {
    ...
    let snap = fake_snapshot(vec![
        fake("root#0", "column", 0.0, 0.0, 200.0, 100.0, None),
