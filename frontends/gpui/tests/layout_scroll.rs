...
//!    `ScrollableListView` wrapped in the production `scrollable_list_wrapper`
//!    chain, then either assert on `UniformListScrollHandle::last_item_size`
//!    (structural) or dispatch a real wheel event through `add_window_view` +
//!    `simulate_wheel_at` (interaction). Catches regressions in the outer
//!    wrapper chain.
//!
//! 2. **Real-reactive tests** — render an actual `ReactiveViewModel:: Reactive
//!    { view }` through `builders::render` via `ReactiveFixtureView` +
//!    `TestServices`, then drive the inner `ReactiveShell`'s `ListState` either
//!    with a programmatic `scroll_by` ("viewport invariants" section) or a
//!    wheel event through the full hit-testing pipeline.
//!
...
fn production_chain_overflows_viewport(cx: &mut TestAppContext) {
    ...
    assert!(
        ...
        "production chain should produce contents ({:?}) > viewport ({:?}), got contents={:?} \
         viewport={:?}. Cascade regression class.",
    ...
    assert!(
        ...
        "viewport height should be ~{VIEWPORT_H}px but was {:?}. Content height leaked into the \
         viewport — cascade regression.",
        sizes.item.height
    );
}
...
fn wheel_event_scrolls_production_list(cx: &mut TestAppContext) {
    ...
    assert!(
        max_offset.y > px(0.0),
        "list has no scrollable distance — max_offset={max_offset:?}. Content may have leaked \
         into viewport (cascade regression)."
    );
    ...
    assert!(
        after.y < before.y,
        "scroll wheel should decrease offset.y (content moves up) but before={before:?} \
         after={after:?}. list_bounds={list_bounds:?}, max_offset={max_offset:?}. Regression \
         class: hit-testing broke, `overflow_hidden` missing, or scroll hitbox not registered."
    );
}
...
fn wheel_event_does_not_scroll_when_content_fits(cx: &mut TestAppContext) {
    ...
    assert_eq!(
        before, after,
        "with only 3 items (no scroll needed), wheel should leave offset unchanged but \
         before={before:?} after={after:?}. Negative control broken: the positive test may be \
         vacuous."
    );
}
...
fn production_chain_wheel_event_scrolls_real_reactive(cx: &mut TestAppContext) {
    ...
    assert!(
        ...
        "wheel event did not advance logical_scroll_top past {before:?} — got {after:?}. The \
         `ReactiveShell` + `gpui::list` scroll path is broken: wheel events don't move the \
         viewport. Historically this manifested as snap-to-top; the `Infer`→`Auto` sizing fix \
         cleared that, so check `reactive_shell.rs` for any regressions."
    );
}
...
fn columns_child_wheel_event_scrolls_real_reactive(cx: &mut TestAppContext) {
    ...
    let shell = entity
        ...
        .expect(
            "inner ReactiveShell entity was not created — columns::render may have taken a branch \
             that bypasses get_or_create_reactive_shell",
        );
    let list_state = shell.read_with(vcx, |s, _| s.list_state_handle());
    let before = list_state.logical_scroll_top();
    ...
    assert!(
        after.item_ix > before.item_ix || after.offset_in_item > before.offset_in_item,
        "columns-wrapped wheel event did not advance logical_scroll_top (before={before:?}, \
         after={after:?}). The list panel is nested inside `columns::render`'s `flex_1 relative → \
         absolute size_full flex_col` wrapper. If this is the only broken scroll scenario, the \
         fix is most likely in how that wrapper propagates definite height through to the \
         `scrollable_list_wrapper` + `ReactiveShell` chain."
    );
}
...
fn root_list_viewport_is_scrollable(cx: &mut TestAppContext) {
    ...
    assert!(
        ...
        "scroll_by(240) should land in the mid-range (1..=20) but got {after:?}. item_ix 0 → \
         viewport collapsed to content height (Infer sizing trap, `scroll_max = 0`). item_ix ~200 \
         → items were never measured so `cursor.seek_forward(Height)` walked off the end. Check \
         that `ReactiveShell` uses `ListSizingBehavior::Auto` + a definite-height ancestor chain \
         (`h_full` / `size_full`, not `flex_grow`)."
    );
}
...
fn view_mode_switcher_child_viewport_is_scrollable(cx: &mut TestAppContext) {
    ...
    let shell = entity
        .read_with(vcx, |fv, _| fv.reactive_shell(&inner_list))
        .expect(
            "ReactiveShell for the inner list was not created under the view_mode_switcher — \
             get_or_create_reactive_shell key mismatch or the switcher's slot rendering skipped \
             the shell",
        );
    let list_state = shell.read_with(vcx, |s, _| s.list_state_handle());
    ...
    assert!(
        (1..=20).contains(&after.item_ix),
        "view_mode_switcher slot collapsed the inner list to zero height — `scroll_by(240)` \
         should have landed in 1..=20 but got {after:?}. The switcher wraps its slot in \
         `div().size_full().flex_1().flex_col()` with a small switcher bar + the slot content \
         stacked as flex children; nothing gives the content child a definite height, so the \
         ReactiveShell's `h_full()` collapses. Fix in `builders/view_mode_switcher.rs`: wrap \
         `child_el` in a `flex_1 min_h_0` div so the list inherits a definite viewport."
    );
}
...
fn columns_child_viewport_is_scrollable(cx: &mut TestAppContext) {
    ...
    assert!(
        (1..=20).contains(&after.item_ix),
        "columns-wrapped `scroll_by(240)` should land in 1..=20 but got {after:?}. If item_ix is \
         0, `columns::render` is handing the inner panel a non-definite height (regression in the \
