...
use holon_frontend::reactive_view_model::{
    CollectionVariant, ReactiveSlot, ReactiveViewKind, ReactiveViewModel,
};
...
/// Wrap a `ReactiveView` into the root `ReactiveViewModel::Reactive` node
/// that `builders::render` dispatches on.
fn reactive_root(view: Arc<ReactiveView>) -> Arc<ReactiveViewModel> {
    Arc::new(ReactiveViewModel::from_kind(ReactiveViewKind::Reactive {
        view,
    }))
}
...
fn production_chain_wheel_event_scrolls_real_reactive(cx: &mut TestAppContext) {
    ...
    let view = build_text_list(200, CollectionVariant::List { gap: 0.0 });
...
fn columns_child_wheel_event_scrolls_real_reactive(cx: &mut TestAppContext) {
    ...
    let inner_list = build_text_list(200, CollectionVariant::List { gap: 0.0 });
    let main_list = ReactiveViewModel::from_kind(ReactiveViewKind::Reactive {
        view: inner_list.clone(),
    });
    let columns_view = Arc::new(ReactiveView::new_static_with_layout(
        vec![main_list],
        CollectionVariant::Columns { gap: 4.0 },
    ));
...
fn view_mode_switcher_child_viewport_is_scrollable(cx: &mut TestAppContext) {
    ...
    let inner_list = build_text_list(200, CollectionVariant::List { gap: 0.0 });
    let slot_vm = ReactiveViewModel::from_kind(ReactiveViewKind::Reactive {
        view: inner_list.clone(),
    });
    let switcher = Arc::new(ReactiveViewModel::from_kind(
        ReactiveViewKind::ViewModeSwitcher {
            entity_uri: EntityUri::from_raw("block:test-main-panel"),
            modes: r#"[{"name":"list","icon":"list"}]"#.to_string(),
            slot: ReactiveSlot::new(slot_vm),
        },
    ));
