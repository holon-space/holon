...
//!
//! Spike A (`list_scroll_spike.rs`) proved that
//! `ListState::scroll_to_reveal_item` drives the test scheduler past a paint
//! that records bounds keyed by `entity_id` in `BoundsRegistry`, using a
//! synthetic fixture that wraps each
...
fn cache_lookup_yields_real_reactive_shell(cx: &mut TestAppContext) {
    ...
    let shell = entity
        ...
        .expect(
            "ReactiveShell entity not in cache — production render path didn't reach \
             get_or_create_reactive_shell for our collection",
        );
...
fn initial_paint_records_first_item_entity_id(cx: &mut TestAppContext) {
    ...
    assert!(
        ...
        "item 0 missing from BoundsRegistry after initial paint — entity_id wiring in production \
         text builder is broken. entity_ids in registry: {:?}",
        bounds
            .all_elements()
            .iter()
...
fn production_shell_scroll_to_reveal_item_brings_far_row_into_bounds(cx: &mut TestAppContext) {
    ...
    assert!(
        visible_after,
        "post-scroll: {target} missing from BoundsRegistry. visible_before={visible_before}, \
         bounds_for_item({TARGET_IX})={bounds_for_target:?}. entity_ids present: {registry_ids:?}"
    );
}
...
fn virtualized_list_bounds_stay_viewport_sized(cx: &mut TestAppContext) {
    ...
    assert!(
        ...
        "last of {BIG_ITEM_COUNT} rows has bounds without scrolling — the collection is being \
         rendered eagerly, virtualization regressed"
    );
    assert!(
        tracked_rows.len() < BIG_ITEM_COUNT / 2,
        "{} of {BIG_ITEM_COUNT} rows have bounds — far more than a viewport's worth; \
         virtualization regressed",
        tracked_rows.len()
    );
}
