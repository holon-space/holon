...
// ── Targeted: A→B→A live_block cycle terminates ──────────────────────
//
// Regression guard for the `LiveBlockAncestors` chain that replaced the
// synchronous `RECONCILING` thread-local. Two blocks reference each other
// through `live_block` widgets: A's tree is `live_block(B)` and B's tree
// is `live_block(A)`. Without cycle prevention, rendering would recurse
// forever (each `ReactiveShell::new_for_block` creates a fresh
// `entity_cache`, so the parent-scoped cache can't deduplicate across the
// cycle).
//
// The chain plumbing prevents this at the builder layer: A's render frame
// has ancestors=[A], so it allows creating B with ancestors=[A]; B's render
// frame has ancestors=[A, B], so the recursive `live_block(A)` is refused
// before the cache lookup. Total ReactiveShell entities created: 2.
//
// Failure mode: if cycle prevention regresses, this test wedges the
// scheduler until OOM or `gpui::TestScheduler` hits its deterministic
// step limit. The wall-clock budget below catches the scheduler-bounded
// case (and a panic catches OOM).

#[gpui::test]
fn live_block_self_reference_no_infinite_construction(cx: &mut TestAppContext) {
    use std::time::Instant;

    use holon_api::EntityUri;
    use holon_frontend::reactive_view_model::ReactiveViewModel;
    use holon_layout_testing::registry::BlockTreeThunk;

    let a_uri = EntityUri::block("cycle-a").to_string();
    let b_uri = EntityUri::block("cycle-b").to_string();

    let b_uri_for_a = b_uri.clone();
    let a_thunk: BlockTreeThunk = std::sync::Arc::new(move || {
        ReactiveViewModel::live_block(EntityUri::from_raw(&b_uri_for_a))
    });
    let a_uri_for_b = a_uri.clone();
    let b_thunk: BlockTreeThunk = std::sync::Arc::new(move || {
        ReactiveViewModel::live_block(EntityUri::from_raw(&a_uri_for_b))
    });

    let blocks = vec![
        (a_uri.clone(), vec![("default".to_string(), a_thunk)], 0),
        (b_uri.clone(), vec![("default".to_string(), b_thunk)], 0),
    ];

    let root_vm = std::sync::Arc::new(ReactiveViewModel::live_block(EntityUri::from_raw(&a_uri)));
    ...
    assert!(
        ...
        "cyclic live_block construction took {:?} — cycle prevention may have regressed",
        elapsed,
    );

    let snap = session.snapshot();
    let dump = snap.structural_dump();
    eprintln!("A→B→A cycle dump:\n{dump}");
    assert!(
        !snap.entries.is_empty(),
        "no widgets rendered for A→B→A cycle — fixture is broken",
    );

    // Cycle was contained: at most one live_block widget per ancestor link
    // is constructed. We don't pin an exact count (intermediate `tag()`
    // wrappers and the empty cycle-cut div make that brittle), but we do
    // assert no runaway accumulation.
    let live_block_count = snap
        .entries
        .iter()
        .filter(|(id, _)| id.starts_with("live_block#"))
        .count();
