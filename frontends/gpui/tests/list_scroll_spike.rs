...
use gpui::{
    div, list, prelude::*, px, Context, IntoElement, ListAlignment, ListState, Render,
    TestAppContext, Window,
};

use holon_frontend::geometry::GeometryProvider;
use holon_gpui::geometry::{BoundsRegistry, TransparentTracker};
...
fn initial_paint_records_visible_items(cx: &mut TestAppContext) {
    ...
    assert!(
        ...
        "item:0 should be in BoundsRegistry after initial paint — \
         TransparentTracker.prepaint isn't recording. \
         all_elements()={:?}",
        bounds
            .all_elements()
            .iter()
...
fn scroll_to_reveal_item_makes_far_item_visible_in_bounds(cx: &mut TestAppContext) {
    ...
    assert!(
        ...
        "after scroll_to_reveal_item({target}): expected {} in BoundsRegistry. \
         ...
         entity_ids in registry after scroll: {all_ids:?}",
...
fn bounds_for_item_returns_some_after_scroll(cx: &mut TestAppContext) {
    ...
    assert!(
        item_bounds.is_some(),
        "ListState::bounds_for_item({target}) is None — \
         scroll_to_reveal_item didn't reach the prepaint that records bounds. \
         logical_scroll_top={:?}",
