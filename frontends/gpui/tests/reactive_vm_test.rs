...
fn main() {
    ...
    run("children_share_parent_data_mutable", |cx| {
        ...
        assert_eq!(
            ...
            "child.data.get() must return parent's new value SYNCHRONOUSLY. Got '{child_content}' \
             — Mutables are independent, not shared."
        );
    });
    ...
    run("data_change_updates_gpui_rendered_output", |cx| {
        ...
        assert!(
            ...
            "GPUI rendered output after data change: '{}' (expected 'Updated!'). The display \
             Mutable updates but GPUI doesn't re-render the children.",
            after
        );
    });
    ...
    run("child_render_called_after_data_change", |cx| {
        ...
        for (i, (b, a)) in before.iter().zip(after.iter()).enumerate() {
            assert!(
                a > b,
                "child[{i}] render_count did not increase after data change (before={b}, \
                 after={a}). GPUI did not re-render the child even though its cx.notify() was \
    ...
    run("render_uses_live_data_not_stale_display", |cx| {
        ...
        assert!(
            ...
            "tree_snapshot (= render path) must use live data, not stale display. Got '{}' — \
             render() would show stale content for one frame.",
            leaves[0]
        );
        assert!(
    ...
    run("named_args_swapped_order_preserves_children", |cx| {
        // Start: toggle(#{header: text(col("content")), content:
        // badge(col("priority"))})
        let initial = expr(
            "toggle",
            vec![
        ...
        // After swap: new order is [content, header] → entities [old_content,
        // old_header]
        assert_eq!(
    ...
    run("expand_content_lazy_via_interpreter", |cx| {
        ...
        assert!(
            ...
            "expand should trigger interpretation: before={count_after_create} \
             after={count_after_expand}"
        );
    });
    ...
    run("concurrent_data_and_expr_mutation", |cx| {
        ...
        assert!(
            ...
            "after concurrent mutation: display should use new expr (badge) + new data (low). \
             Got: {}",
            snap1.display
        );
    });
