//! E0c-(b) make-or-break: prove the Holon widget tree yields **real,
//! deterministic layout geometry** under gpui `TestPlatform` — no on-screen
//! window.
//!
...
//! `frontend_slice` can't provide, because there are no element bounds without
//! a layout/paint pass. The question: can a `TestPlatform` window produce that
//! geometry **deterministically** (the occlusion/blur flakiness that drove the
//! real-window→TestPlatform migration was a *real-window* property;
//! TestPlatform's fake dispatcher + fake clock should be reproducible)?
//!
//! `test_platform_smoke.rs` already proves a single boot yields non-empty
//! bounds. This test strengthens that to the actual risk:
//!   1. **Real** — each boot yields non-degenerate geometry (≥1 element with
//!      real width/height) and no leftover `"loading"` placeholders.
//!   2. **Deterministic** — N independent boots settle to byte-identical
//!      geometry (same element set, same widget types, same pixel bounds).
//!      Independent boots (fresh backend + window each) is a *stronger*
//!      determinism claim than one window rebound across ticks (no
//!      shared-window state to mask drift).
//!
//! PASS ⇒ the windowed frontend is "just another component"; E4
//! (`GpuiWindowComponent`) is mechanical. FAIL ⇒ keep a real-window residue
//! (the slim-residue fallback).
//!
//! Reuses the production launcher (`launch_holon_window_rebindable`) and the
//! proven cross-runtime fixed-point settle — the exact path E4's component will
//! stand up.
...
/// Independent boots to compare. ≥2 proves determinism; 3 guards against a
/// lucky pair. Each boot is a full backend + window stand-up + settle, so this
/// is the slow part — keep it small.
const BOOTS: usize = 3;
...
/// runs real tokio time between gpui pump cycles so backend→frontend signals
/// deliver through the channel bridge, advances the fake clock for timer-driven
/// code, and promotes staged bounds. Returns once the element count is stable
/// and no `"loading"` placeholders remain. Panics loudly if it never settles.
...
/// One element's geometry as a canonical tuple: (widget_type, x, y, w, h),
/// pixel bounds rounded to whole pixels. The make-or-break is *gross geometry*
...
/// `entity_id`. Independent boots seed a fresh vault and mint **fresh random
/// block UUIDs**, so entity ids legitimately differ boot-to-boot while the
/// layout is pixel-identical; in real PBT use the ids come from a seeded
/// `ReferenceState` (fixed ids) and are orthogonal to this geometry question.
type Geom = (String, i32, i32, i32, i32);

/// A settled snapshot: the multiset of element geometries (sorted so record
/// order in `all_elements()` is irrelevant) plus structural counts that must
/// also match.
struct Snap {
...
/// Boot a fresh backend + TestPlatform window, settle to a fixed point, and
/// return the canonical geometry snapshot. Each call is fully independent (own
/// backend, own window). The `TestApp` is intentionally leaked at the end:
/// shutdown clears windows but detached pump tasks still hold entity handles
/// and gpui's leak detector runs before the dispatcher drops them.
...
/// Symmetric-difference of two geometry multisets, for actionable failure
/// output.
...
fn test_platform_geometry_is_real_and_deterministic() {
    ...
    for (i, snap) in snaps.iter().enumerate() {
        ...
        assert!(
            non_degenerate >= 1,
            "boot {i}: every element is degenerate (w/h ≤ 1px) — a zero-size or occluded window \
             records collapsed rects."
        );
        eprintln!(
            "[determinism] boot {i}: {} elements, {non_degenerate} non-degenerate, {} distinct \
             entities",
            snap.geom.len(),
