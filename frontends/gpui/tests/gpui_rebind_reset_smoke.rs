...
//! settles, then builds SUT#2 (a DISTINCT seed) and rebinds the SAME window
//! onto it, asserting the rendered tree switches from SUT#1's content to
//! SUT#2's.
//!
...
/// Materialize seed .org files into a fresh temp org root + return fresh
/// db/config paths.
fn materialize_seed(
    files: &[(&str, &str)],
) -> (
