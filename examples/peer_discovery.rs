...
async fn main() -> Result<()> {
    ...
    if args.len() < 2 {
        println!("Usage:");
        println!(
            "  cargo run --example peer_discovery --features iroh-sync <doc_id> [peer_node_id]"
        );
        println!();
        println!("Examples:");
        ...
        println!(
            "  cargo run --example peer_discovery --features iroh-sync project-notes \
             <node_id_from_first_peer>"
        );
        println!();
