...
impl<'tcx> LateLintPass<'tcx> for ErrorToOptionViaOkQuestion {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>) {
        ...
        span_lint_and_help(
            ...
            None,
            "change the enclosing fn's return type to `Result<_, _>` and propagate the error with \
             `?`, or wrap the error in a richer enum that this fn's caller can distinguish from a \
             real `None`. CLAUDE.md: \"DO NOT return null or None in case of an error\".",
        );
    }
}
