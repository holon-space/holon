...
//! `BlockWire` DTO). The lint stays as a standing guard so the
//! `#[serde(skip)]`- on-a-`Deserialize`-type pattern can't be re-introduced
//! elsewhere.
    ...
    fn check_item(&mut self, cx: &LateContext<'tcx>, item: &'tcx Item<'tcx>) {
        ...
        for field in variant.fields() {
            ...
            span_lint_and_help(
                ...
                None,
                "the type derives `Deserialize` and this field has `#[serde(skip)]` / \
                 `#[serde(skip_deserializing)]`, so callers receive defaulted data without \
                 warning. Either remove the derived `Deserialize` (force a hand-rolled \
                 constructor that hydrates the field explicitly), or, if the field really is \
                 runtime-only state, document the intent and add \
                 `#[allow(serde_skip_default_on_deserialize)]`. See devlog/MEMORY: Block has two \
                 deserializers, May 2026.",
            );
        }
    }
}
