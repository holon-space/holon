...
//! and `r.ok()` whose result flows directly to `None`/`?` in an
//! Option-returning position. Codifies the CLAUDE.md rule "DO NOT return None
//! in case of an error" with type information that ast-grep can't supply on its
//! own.
//!
...
impl<'tcx> LateLintPass<'tcx> for ResultToNone {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>) {
        ...
        span_lint_and_help(
            ...
            None,
            "either change the return type to `Result<_, _>` and propagate the error, or wrap it \
             in a richer enum variant. CLAUDE.md: \"DO NOT return null or None in case of an \
             error\".",
        );
    }
}
