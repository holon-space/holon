    ...
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>) {
        ...
        span_lint_and_help(
            ...
            "reference it via the typed constant in \
             `crates/holon/src/storage/block_table_names.rs` \
             (`BLOCK_WRITE_TABLE`, `BLOCK_READ_TABLE`, …) so a future rename \
             or matview-promotion is a compile error, not a silent runtime \
             write-rejection. See devlog/MEMORY: LoroSyncController May 2026.",
        );
    }
}
