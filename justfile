...
#
# CANON normalises both halves of that hash: the package set (`--workspace`) and
# the feature set (CANON_FEATURES). Narrow what RUNS with `--test <name>` or
# nextest's `-E`, never with `-p`: `-p` narrows the feature resolve too, which is
# the churn. `build` and `test` stay off CANON — a developer build is not a gate
# and must not link the pbt instrumentation.
#
