...
#
# Three of the four `tags` clauses are `move`: org's structured probe drives
# them through the real render -> parse leg. `mint_new` stays `still` and
# always will — `certify_tags` has no arm for it, so nothing can contradict
# either value (see `TagMinting`); it is marked, and movement would mean a
# probe appeared that the vocabulary does not have.
#
...
, tif]|]|move
attach_existing: carried|attach_existing: unsupported|move
detach_existing: carried|detach_existing: unsupported|move
unknown_reference: minted|unknown_reference: refused|move
