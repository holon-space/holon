...
## UPDATE 2026-07-07 (session 4): ROOT-CAUSED + FIXED (antijoin `read_r_count` mishandled `SeekResult::TryAdvance`)

### Gate red-rate before
`test_ivm_block_matview_replay_auto_reproduces_duplicate`: **8/14 red (~57%)**.
Trace mode confirmed the first dup-producing statement is ALWAYS a `block_raw`
UPSERT (a left-side δL update) on an already-tag-matched block.

### Trace evidence (the confirmed mechanism — NOT what session 3 hypothesized)
Instrumented `AntijoinOperator` (op=8 = the `block_tags_agg` LEFT JOIN). For a
victim block that was Page-matched, on the δL-update pass the tags-antijoin
**L-term read `c_pre = 0`** and therefore emitted `+null_pad(new_row)` with
`c_post==0` → the stale `tags=[]` ghost beside the matched `tags=[Page]` row.

Reconstructing R_COUNT[victim] over the whole stream showed R_COUNT was persisted
**stably = 1** (last `PersistRCount` write when the tag was first added; never
rewritten afterwards), yet the L-term READ returned 0 at some statements and 1 at
others *for the same key in the same process*. So this was **not** a maintenance
drift (session-3's "retraction dropped on 0→1 edge" hypothesis is FALSE) — it was
an intermittent **read** failure.

Instrumenting `read_r_count`'s seek branch pinned it exactly: the R_COUNT index
seek (`SeekOp::GE { eq_only: true }`) returned **`SeekResult::TryAdvance`**, and
`read_r_count` only accepted `SeekResult::Found`, so it treated the present entry
as absent and returned 0. `TryAdvance` occurs when the matching entry sits just
past the current leaf-page boundary (the cursor is parked at the boundary, not on
the record) — which is why it is hash-order / btree-shape dependent, matching the
"victim varies per process" symptom. (The `SeekResult` doc claiming TryAdvance
only happens with `eq_only:false` is stale.)

### Fix (minimal, at the confirmed leak)
`core/incremental/antijoin_operator.rs` `read_r_count`: on `TryAdvance`, advance
the cursor (`next()`), check `has_record()`, and re-verify the landed entry's
`(storage_id, join_key_hash)` before reading the weight — mirroring the existing
`join_operator.rs::read_next_join_row` and `aggregate_operator.rs` TryAdvance
handling. No consolidate change (session-3 falsified that class), no harness
dedup. Single-function change.

### Verification (all GREEN)
- `test_ivm_block_matview_replay_auto_reproduces_duplicate`: **16/16 green**
  (was 8/14 red). Un-ignored.
- `test_ivm_block_matview_replay_no_duplicate` (REPLAY_MODE=txn): **14/14 green**.
  Un-ignored.
- Turso `query_processing::test_ivm* / test_left* / test_matview*`: 96 passed,
  0 failed. Core `turso_core incremental` unit tests: 193 passed, 0 failed.
  (4 pre-existing failures in `test_transactions` rollback + `test_write_path`
  concurrent-commit — confirmed identical with the fix neutralized; no matview
  code path, unrelated.)
- Holon `matview_duplicate_row_repro`: 8/8 green.
- Forced-weight keystone (`WriteOrgFile:100,SetEdgeField:100`,
  `inv-org-render-fixed-point:skip`, PROPTEST_CASES=6): **PASS (284s)** — count
  divergence cleared, no new divergence surfaced.
